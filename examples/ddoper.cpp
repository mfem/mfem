...
InjectionOperator::InjectionOperator(MPI_Comm comm,
                                     ParFiniteElementSpace *subdomainSpace, FiniteElementSpace *interfaceSpace,
                                     int *a,
                                     std::vector<int> const& gdofmap, std::vector<int> const& gflip) : id(a)
{
   ...
   */
   ...
   const int sdtsize = (subdomainSpace == NULL) ? 0 :
                       subdomainSpace->GetTrueVSize();
   const int iftsize = (interfaceSpace == NULL) ? 0 :
                       interfaceSpace->GetTrueVSize();
   ...
   m_numLocalTrueSDmappedFromProc.assign(m_nprocs,
                                         0);  // reset in the following loop
...
#ifdef USE_SIGN_FLIP
            m_alltrueSDflip[os[ifp] + m_numLocalTrueSDmappedFromProc[ifp]] =
               gflip[alliftos[ifp] + i];
#endif
            m_numLocalTrueSDmappedFromProc[ifp]++;
...
double test2_rho_exact_0(const Vector &x)
{
   ...
   return -pi * pi * ((sin(pi * x(2)) * sin(pi * x(0))) + (sin(pi * x(0)) * sin(
                                                              pi * x(2))));
}
                                      ...
                                      set<int> const& tdofsbdry1, const int nprocs, const int rank)
{
   ...
   {
      int i = 0;
      for (std::set<int>::const_iterator it = tdofsbdry0.begin();
           it != tdofsbdry0.end(); ++it, ++i)
      {
         tdofsbdryvec0[i] = *it;
      }
      ...
      i = 0;
      for (std::set<int>::const_iterator it = tdofsbdry1.begin();
           it != tdofsbdry1.end(); ++it, ++i)
      {
         tdofsbdryvec1[i] = *it;
      }
   ...
   MPI_Allgather(&tsize1, 1, MPI_INT, alltsize1.data(), 1, MPI_INT,
                 MPI_COMM_WORLD);

   std::vector<int> alltbdrysize1(nprocs);
   MPI_Allgather(&tbdrysize1, 1, MPI_INT, alltbdrysize1.data(), 1, MPI_INT,
                 MPI_COMM_WORLD);
   ...
   MPI_Allgatherv(tdofsbdryvec1.data(), tbdrysize1, MPI_INT,
                  alltdofsbdryvec1.data(), alltbdrysize1.data(), rdspl.data(), MPI_INT,
                  MPI_COMM_WORLD);
   ...
   file << myosf0 << " " << myosf0 + nf0 - 1 << " " << myosf1 << " " << myosf1 +
        nf1 - 1 << endl;
...
SparseMatrix* GatherHypreParMatrix(HypreParMatrix *A)
{
   ...
   MPI_Gatherv(nnz, nrows, MPI_INT, allnnz, allnrows, dspl, MPI_INT, 0,
               A->GetComm());
   ...
   MPI_Gatherv(csr->j, totalnnz, MPI_INT, allj, alltotalnnz, dspl, MPI_INT, 0,
               A->GetComm());
   MPI_Gatherv(csr->data, totalnnz, MPI_DOUBLE, alldata, alltotalnnz, dspl,
               MPI_DOUBLE, 0, A->GetComm());
...
void GetStarts(MPI_Comm comm, const int rank, const int nprocs, const int n,
               std::vector<HYPRE_Int> & starts, int& g)
{
   ...
   */
   ...
   if (adding && haveA)
   {
      ...
      hypre_ParCSRMatrix * parcsr_op = (hypre_ParCSRMatrix *)
                                       const_cast<HypreParMatrix&>(*Acopy);
   ...
   int *ifullsdt = (ifullsize > 0) ? new int[ifullsize] :
                   NULL;  // interface full to subdomain true DOF map, well-defined as the max of inj over all processes.
   ...
   int *ifgtsdt = new
   int[alligsize];  // interface global true to subdomain true DOF's.
   int *maxifgtsdt = new
   int[alligsize];  // interface global true to subdomain true DOF's.
   ...
   for (int i=0; i<ifullsize; ++i)
   {
      ...
      ifgtsdt[gtdof] = (inj[i] >= 0) ? inj[i] + ossdt :
                       inj[i];  // could be -1 if not defined
      //ifullsdt[i] = inj[i];  // could be -1 if not defined
   }

   bool allMaxGlobalTDofFound = false;

   MPI_Allreduce(&maxGlobalTDofFound, &allMaxGlobalTDofFound, 1, MPI_C_BOOL,
                 MPI_LOR, ifcomm);

   MFEM_VERIFY(gtdofInRange && allMaxGlobalTDofFound, "");

   MPI_Allreduce(ifgtsdt, maxifgtsdt, alligsize, MPI_INT, MPI_MAX,
                 ifcomm);  // TODO: can this be done in place?

   int *iftfull = (iftsize > 0) ? new int[iftsize] : NULL;
   int *iftfull2 = (iftsize2 > 0) ? new int[iftsize2] : NULL;
   int *iftsdt = (iftsize > 0) ? new int[iftsize] :
                 NULL;  // interface true to subdomain true DOF map
   ...
   if (globalI != NULL && A != NULL &&
       adding) // Add values from globalI to the local rows of csr_op
   {
      ...
      for (int i=0; i<alligsize; ++i)
      {
         ...
         if (ginj[i] >= ossdt &&
             ginj[i] < ossdtNext)  // if subdomain true DOF is on this process
         {
   ...
   //if (A != NULL)
   {
      ...
      else if (!adding && globalI != NULL)
      {
         ...
         const bool mixed = (ifespace2 !=
                             NULL);  // interface matrix I is in the mixed space (ifespace, ifespace2)
         ...
         const int numLocRows = injRows ? ((A == NULL) ? 0 : A->Height()) : ((
                                                                                I == NULL) ? 0 : I->Height());
         ...
         if (injRows) // Inject the rows from interface DOF's to subdomain DOF's.
         {
            ...
            for (int i=0; i<alligsize; ++i)
            {
                   ...
                   ginj[i] < ossdtNext)  // if subdomain true DOF is on this process
               {
                  ...
                  for (int k=globalI->GetI()[i]; k<globalI->GetI()[i+1];
                       ++k, ++cnt, rowCount[ltsd]++)
                  {
                     // Copy values in row i of globalI to local row ltsd of S.
                     SJ[SI[ltsd]+rowCount[ltsd]] =
                        globalI->GetJ()[k];  // Column index in globalI is already global in ifespace or ifespace2.
         ...
         else
         {
            ...
            for (int i=0; i<iftsize; ++i)
            {
               ...
               for (int k=globalI->GetI()[gtdof]; k<globalI->GetI()[gtdof+1];
                    ++k, ++cnt, rowCount[i]++)
               {
                  ...
                  SJ[SI[i]+rowCount[i]] =
                     ginj[globalI->GetJ()[k]];  // Column index in globalI is already global in ifespace or ifespace2. Map to global injected DOF.
         ...
         if (injRows)
         {
            ...
            S = new HypreParMatrix(ifcomm, numLocRows, glob_nrows, globalI->Width(), SI, SJ,
                                   Sdata, (HYPRE_Int*) rowStarts.data(), (HYPRE_Int*) colStarts.data());
         }
         else
         {
            ...
            S = new HypreParMatrix(ifcomm, numLocRows, globalI->Height(), glob_ncols, SI,
                                   SJ, Sdata, (HYPRE_Int*) rowStarts.data(), (HYPRE_Int*) colStarts.data());
         }
#else
         ...
         SparseMatrix *globalI_tr = mixed ? Transpose(*globalI) :
                                    NULL;  // not the most efficient implementation, to avoid premature optimization.
         SparseMatrix *globalImat = mixed ? globalI_tr :
                                    globalI;  // In the non-mixed case, globalI is assumed to be symmetric, so no transpose is needed.
         ...
         for (int i=0; i<numLocRows;
              ++i)  // Loop over interface true DOF's, which correspond to local rows of S.
         {
#ifdef SERIAL_INTERFACES
            const HYPRE_Int gtdof = mixed ? iftfull2[i] : iftfull[i];
#else
            const HYPRE_Int gtdof = mixed ? ifespace2->GetGlobalTDofNumber(
                                       iftfull2[i]) : ifespace->GetGlobalTDofNumber(iftfull[i]);
#endif
            SI[i+1] = SI[i] + globalImat->GetI()[gtdof+1] -
                      globalImat->GetI()[gtdof];  // local offsets to rows
         }
              ...
              ++i)  // Loop over interface true DOF's, which correspond to local rows of S.
         {
            ...
            const HYPRE_Int gtdof = mixed ? ifespace2->GetGlobalTDofNumber(
                                       iftfull2[i]) : ifespace->GetGlobalTDofNumber(iftfull[i]);
#endif
            for (int k=globalImat->GetI()[gtdof]; k<globalImat->GetI()[gtdof+1];
                 ++k, ++cnt)  // Copy values in row gtdof of globalImat to values in local row i of S.
            {
         ...
         MPI_Barrier(ifcomm);
         S = new HypreParMatrix(ifcomm, numLocRows, globalI->Width(),
                                A->GetGlobalNumRows(), SI, SJ, Sdata, I->ColPart(), A->RowPart());
#endif
...
// Row and column offsets are assumed to be the same, for each process.
// Array offsets stores process-local offsets with respect to the blocks. Process offsets are not included.
HypreParMatrix* blockgmg::CreateHypreParMatrixFromBlocks2(MPI_Comm comm,
                                                          Array<int> const& offsets, Array2D<HypreParMatrix*> const& blocks,
                                                          Array2D<SparseMatrix*> const& blocksSp,
                                               ...
                                               const Array2D<double> & coefficient)
{
   ...
   Operator *op = new STRUMPACKRowLocMatrix(comm, num_loc_rows, first_loc_row,
                                            glob_nrows, glob_ncols, opI, opJ, data);
...
void FindBoundaryTrueDOFs(ParFiniteElementSpace *pfespace, set<int>& tdofsBdry)
{
   ...
   for (int be = 0; be < pmesh->GetNBE(); ++be)
   {
      ...
      for (int i=0; i<dofs.Size(); ++i)
      {
         ...
         const int ldof = pfespace->GetLocalTDofNumber(
                             dof_i);  // If the DOF is owned by the current processor, return its local tdof number, otherwise -1.
         if (ldof >= 0)
         {
...
bool FacesCoincideGeometrically(ParMesh *volumeMesh, const int face,
                                ParMesh *surfaceMesh, const int elem, const bool surface)
{
   Array<int> faceVert;
...
void PrintMeshBoundingBox(ParMesh *mesh)
{
   ...
   cout << mesh->GetMyRank() << ": sd box [" << vmin[0] << ", " << vmin[1] << ", "
        << vmin[2] << "] to [" << vmax[0] << ", " << vmax[1] << ", " << vmax[2] << "]"
        << endl;
}
...
int FindDofByPointValue(std::vector<double> const& facebv, const int dim,
                        const int facendofs, const int facenip, const int face, const int dof,
...
void SetInterfaceToSurfaceDOFMap(MPI_Comm ifsdcomm,
                                 ...
                                 std::set<int> const& pmeshEdgesInInterface,
                                 std::set<int> const& pmeshVerticesInInterface,
                                 const FiniteElementCollection *fec,
                                 std::vector<int>& dofmap, //std::vector<int>& fdofmap,
                                 std::vector<int>& gdofmap, std::set<int>& flippedSDDofs,
                                 std::vector<int>& gflip)
{
   const int ifSize = (ifespace == NULL) ? 0 :
                      ifespace->GetVSize();  // Full DOF size
   const int iftSize = (ifespace == NULL) ? 0 :
                       ifespace->GetTrueVSize();  // True DOF size, used for global gdofmap
...
#ifdef SERIAL_INTERFACES
   Mesh *ifMesh = (ifespace == NULL) ? NULL :
                  ifespace->GetMesh();  // Interface mesh
#else
   ParMesh *ifMesh = (ifespace == NULL) ? NULL :
                     ifespace->GetParMesh();  // Interface mesh
#endif

   ParMesh *sdMesh = (fespace == NULL) ? NULL :
                     fespace->GetParMesh();  // Subdomain mesh
   ...
   for (int elId=0; elId<pmesh->GetNE(); ++elId)
   {
      if (pmesh->GetAttribute(elId) == sdAttribute)
      {
         ...
         for (int j=0; j<elFaces.Size(); ++j)
         {
            ...
            std::set<int>::const_iterator itneg = pmeshFacesInInterface.find(
                                                     -1 - elFaces[j]);
   ...
   */
   ...
   for (int elId=0; elId<sdNE; ++elId)
   {
      ...
      const int pmeshElemId = sdMesh->GetAttribute(elId) -
                              1;  // 1 was added to ensure a positive attribute.
      ...
      if (it != pmeshElemsByInterface.end())  // if pmeshElemId neighbors the interface
      {
         ...
         for (int j=0; j<pVertex.Size(); ++j)
         {
            ...
            for (int k=0; k<sdVertex.Size(); ++k)
            {
               ...
               for (int l=0; l<3; ++l)
               {
                  if (fabs(pmesh->GetVertex(pVertex[j])[l] - sdMesh->GetVertex(
                              sdVertex[k])[l]) > vertexTol)
                  {
                     vertexMatch = false;
                  }
               }
         ...
         for (int j=0; j<pEdge.Size(); ++j)
         {
            ...
            for (int l=0; l<3; ++l)
            {
               mp[l] = 0.5 * (pmesh->GetVertex(pmeshEdgeVert[0])[l] + pmesh->GetVertex(
                                 pmeshEdgeVert[1])[l]);
            }
            ...
            for (int k=0; k<sdEdge.Size(); ++k)
            {
               ...
               for (int l=0; l<3; ++l)
               {
                  if (fabs(mp[l] - (0.5 * (sdMesh->GetVertex(sdMeshEdgeVert[0])[l] +
                                           sdMesh->GetVertex(sdMeshEdgeVert[1])[l]))) > vertexTol)
                  {
                     kjmatch = false;
                  }
               }
   ...
   int numEdgesPerFace = 0;
   {
      ...
      ifemp.assign(3 * numEdgesPerFace * pmeshFacesInInterface.size(),
                   std::numeric_limits<double>::max());
   }

   std::map<int, int> pmeshEdgeToEdgeInInterface;

   for (std::set<int>::const_iterator it = pmeshEdgesInInterface.begin();
        it != pmeshEdgesInInterface.end(); ++it, ++i)
   {
   ...
   */
   for (std::vector<int>::const_iterator it = pmeshFacesInInterfaceOrdered.begin();
        it != pmeshFacesInInterfaceOrdered.end(); ++it, ++i)
#else
   for (std::set<int>::const_iterator it = pmeshFacesInInterface.begin();
        it != pmeshFacesInInterface.end(); ++it, ++i)
#endif
   {
      const int drefit = *it;
...

      {
         ...
         osf = (nv*numVerticesOnFace) +
               (ne*numEdgesOnFace);  // offset of face DOF's, which come after vertex and edge DOF's in the DOF arrays ifdofs and sddofs.
      ...
      if (faceInInterface && nf > 0)
      {
         ...
         */
...

         {
            ...
            {
               std::map<int, int>::const_iterator it = pmeshEdgeToEdgeInInterface.find(
                                                          edge[j]);
               ...
               for (int l=0; l<3; ++l)
               {
                  ifemp[(3*((numEdgesPerFace * i) + j)) + l] = 0.5 * (pmesh->GetVertex(
                                                                         pmeshEdgeVert[0])[l] + pmesh->GetVertex(pmeshEdgeVert[1])[l]);
               }
            }
         }
#else
         ...
         MFEM_VERIFY(pdofs.Size() == ifdofs.Size() &&
                     pdofs.Size() == osf + nf, "hack only works for second order");

         const HYPRE_Int ifgtdof = ifespace->GetGlobalTDofNumber(
                                      ifdofs[osf]);  // Just use the first face DOF on the face, as a global identification for the face.
         MFEM_VERIFY(ifespace->GetGlobalTDofNumber(ifdofs[osf]) + 1 ==
                     ifespace->GetGlobalTDofNumber(ifdofs[osf+1]), "");
   ...
   //if (nfpos)
   {
      ...
      MPI_Gather(&interfaceFaceOffset, 1, MPI_INT, (int*) allifos.data(), 1, MPI_INT,
                 0, ifsdcomm);
      ...
      MPI_Gatherv((int*) ifgi.data(), nif, MPI_INT, (int*) allifgi.data(),
                  (int*) allnif.data(), (int*) dspl.data(), MPI_INT, 0, ifsdcomm);
      ...
      if (myid == 0)
      {
         ...
         for (int p=0; p<nprocs; ++p)
         {
            ...
            {
               const int ifMeshElem = allifos[p] +
                                      i;  // The corresponding element index in ifMesh.
      ...
      MPI_Gatherv((int*) ifeli.data(), numEdgesPerFace * nif, MPI_INT,
                  (int*) allifeli.data(), (int*) allnif.data(), (int*) dspl.data(), MPI_INT, 0,
                  ifsdcomm);
      ...
      MPI_Gatherv((double*) ifemp.data(), 3 * numEdgesPerFace * nif, MPI_DOUBLE,
                  (double*) allifemp.data(), (int*) allnif.data(),
            ...
            for (int i=0; i<allnif[p]; ++i)  // Loop over pmesh faces in interface on proc p
            {
               ...
               for (int j=0; j<numEdgesPerFace; ++j)
               {
                  ...
                  for (int k=0; k<numEdgesPerFace; ++k)
                  {
                     ...
                     {
#ifdef USE_IFMESH3D_VERT
                        const double ifMeshEdgeMidpoint_l = 0.5 * (ifMesh3D->GetVertex(
                                                                      ifmeshEdgeVert[0])[l] + ifMesh3D->GetVertex(ifmeshEdgeVert[1])[l]);
#else
                        const double ifMeshEdgeMidpoint_l = 0.5 * (ifMesh->GetVertex(
                                                                      ifmeshEdgeVert[0])[l] + ifMesh->GetVertex(ifmeshEdgeVert[1])[l]);
#endif

                        if (fabs(ifMeshEdgeMidpoint_l - allifemp[(3*numEdgesPerFace * (dspl[p] + i)) +
                                                                 (3*j) + l]) > vertexTol)
                        {
                           pointsEqual = false;
                        }
                  ...
                  allietoifedge[dsple[p] + allifeli[(numEdgesPerFace * (dspl[p] + i)) + j]] =
                     ifMeshEdge;
               }
            }
         }
...

   {
      ...
      MPI_Scatterv((double*) vals.data(), (int*) scnt.data(), (int*) sdspl.data(),
                   MPI_DOUBLE, (double*) ifbv.data(), nrecv, MPI_DOUBLE, 0, ifsdcomm);
   }
#endif

   MPI_Allreduce((int*) ifpface.data(), (int*) maxifpface.data(), ifgSize, MPI_INT,
                 MPI_MAX, ifsdcomm);
   ...
   for (i=0; i<ifgSize; ++i)
   {
      const HYPRE_Int pgtdof =
         maxifpface[i]; // minimum of fespaceGlobal->GetGlobalTDofNumber() among face DOF's on its face.
      if (pgtdof >= 0)
      {
...
#endif
   {
      ...
      if (ite ==
          pmeshFaceToElem.end())  // This process does not have an element in this subdomain neighboring the face.
      {
         continue;
      }
      ...
      if (nf > 0)
      {
         ...
         const int osf = (nv*numVerticesOnFace) +
                         (ne*numEdgesOnFace);  // offset of face DOF's, which come after vertex and edge DOF's in the DOF arrays ifdofs and sddofs.
         ...
         if (faceInInterface)
         {
            ifespace->GetElementDofs(i, ifdofs);
            MFEM_VERIFY(ifdofs.Size() == sddofs.Size(), "");
            MFEM_VERIFY(ifespace->GetOrder(i) == 2,
                        "fswitch hack only works for second order");
         }
...
#else
            const int fswitch2 = (pmeshFaceOri == 0) ? (1 - (2*(d - osf))) :
                                 0; // ((d - osf) + 1) % 2;  // TODO: this is valid only for second order!
#endif
...
#else
            const int sddof_d = (sddofs[d + fswitch] >= 0) ? sddofs[d + fswitch] : -1 -
                                sddofs[d + fswitch];
            const int sdtdof = fespace->GetLocalTDofNumber(sddof_d);
#endif

            const int sddof_d2 = (sddofs[d + fswitch2] >= 0) ? sddofs[d + fswitch2] : -1 -
                                 sddofs[d + fswitch2];
            //const int sdtdof = fespace->GetLocalTDofNumber(sddofs[d]);
            ...
            */
...
#endif
            {
               ...
               else
               {
                  ...
                  int flip = 0;
                  const int faceDofIndex = FindDofByPointValue(ifbv, sdMesh->SpaceDimension(),
                                                               ifndofs, ifnip, i, sddofs[d], sdMeshElem, fespace, normal, flip);
                  ...
                  */
                  ...
                  if (!(gdofmap[pfaceToIFGDOF[i] + faceDofIndex - osf] == sdtdof2 + sdtos ||
                        gdofmap[pfaceToIFGDOF[i] + faceDofIndex - osf] == -1))
                  {
                     cout << "BUG" << endl;
                  }

                  MFEM_VERIFY(gdofmap[pfaceToIFGDOF[i] + faceDofIndex - osf] == sdtdof2 + sdtos ||
                              gdofmap[pfaceToIFGDOF[i] + faceDofIndex - osf] == -1, "");

                  gdofmap[pfaceToIFGDOF[i] + faceDofIndex - osf] = sdtdof2 + sdtos;

#ifdef USE_SIGN_FLIP
                  MFEM_VERIFY(gflip[pfaceToIFGDOF[i] + faceDofIndex - osf] == flip ||
                              gflip[pfaceToIFGDOF[i] + faceDofIndex - osf] == -1, "");
                  gflip[pfaceToIFGDOF[i] + faceDofIndex - osf] = flip;
#endif
#else
                  if (!(gdofmap[pfaceToIFGDOF[i] + d - osf] == sdtdof2 + sdtos ||
                        gdofmap[pfaceToIFGDOF[i] + d - osf] == -1))
                  {
                     cout << "BUG" << endl;
                  }
                  MFEM_VERIFY(gdofmap[pfaceToIFGDOF[i] + d - osf] == sdtdof2 + sdtos ||
                              gdofmap[pfaceToIFGDOF[i] + d - osf] == -1, "");
        ...
        it != pmeshEdgesInInterface.end(); ++it, ++i)
   {
...

      {
         ...
         const HYPRE_Int gtdof = fespaceGlobal->GetGlobalTDofNumber(
                                    pdofs[0]);  // Just use the first DOF on the edge, as a global identification for the edge.
         pgdofToSet[gtdof] = i;
      ...
      {
         std::map<int, int>::iterator ite = pmeshEdgeToAnyFaceInInterface.find(
                                               pmeshEdge);
         ...
         for (int k=0; k<3; ++k)
         {
            pmeshEdgeMidpoint[k] = 0.5 * (pmesh->GetVertex(pmeshEdgeVert[0])[k] +
                                          pmesh->GetVertex(pmeshEdgeVert[1])[k]);
         }
         ...
         for (int j=0; j<ifEdge.Size(); ++j)
         {
            ...
            {
#ifdef USE_IFMESH3D_VERT
               const double ifMeshEdgeMidpoint_k = 0.5 * (ifMesh3D->GetVertex(
                                                             ifmeshEdgeVert[0])[k] + ifMesh3D->GetVertex(ifmeshEdgeVert[1])[k]);
#else
               const double ifMeshEdgeMidpoint_k = 0.5 * (ifMesh->GetVertex(
                                                             ifmeshEdgeVert[0])[k] + ifMesh->GetVertex(ifmeshEdgeVert[1])[k]);
#endif
            ...
            if (pointsEqual)
            {
               ...
               for (int k=0; k<2; ++k)
               {
                  ...
                  for (int m=0; m<2; ++m)
                  {
                     ...
                     {
#ifdef USE_IFMESH3D_VERT
                        if (fabs(pmesh->GetVertex(pmeshEdgeVert[k])[l] - ifMesh3D->GetVertex(
                                    ifmeshEdgeVert[m])[l]) > vertexTol)
#else
                        if (fabs(pmesh->GetVertex(pmeshEdgeVert[k])[l] - ifMesh->GetVertex(
                                    ifmeshEdgeVert[m])[l]) > vertexTol)
#endif
                           vertexMatch = false;
                     }
                     ...
                     if (vertexMatch)
                     {
                        ...
                        {
                           MFEM_VERIFY(itv->first == pmeshEdgeVert[k] &&
                                       itv->second == ifmeshEdgeVert[m], "");
                        }
...
#ifdef SERIAL_INTERFACES
            const HYPRE_Int ifgtdof =
               ifdofs[0];  // Just use the first DOF on the edge, as a global identification for the edge.
#else
            const HYPRE_Int ifgtdof = ifespace->GetGlobalTDofNumber(
                                         ifdofs[0]);  // Just use the first DOF on the edge, as a global identification for the edge.
#endif
   ...
   // Set ifpedge by using gathered iegi values.
   {
      ...
      MPI_Gatherv((int*) iegi.data(), nie, MPI_INT, (int*) alliegi.data(),
                  (int*) allnie.data(), (int*) dsple.data(), MPI_INT, 0, ifsdcomm);
      ...
      if (myid == 0)
      {
         ...
         for (int p=0; p<nprocs; ++p)
         {
            ...
            {
               const int ifMeshEdge = allietoifedge[dsple[p] +
                                                    i];  // The corresponding edge index in ifMesh.

               Array<int> ifdofs;
               ifespace->GetEdgeDofs(ifMeshEdge, ifdofs);

               const HYPRE_Int ifgtdof =
                  ifdofs[ose];  // Just use the first edge DOF on the edge, as a global identification for the edge.
   ...
   MPI_Allreduce((int*) ifpedge.data(), (int*) maxifpedge.data(), ifgSize, MPI_INT,
                 MPI_MAX, ifsdcomm);

   for (i=0; i<ifgSize; ++i)
   {
      const HYPRE_Int pgtdof =
         maxifpedge[i]; // fespaceGlobal->GetGlobalTDofNumber() for the first DOF on its edge.
      if (pgtdof >= 0)
      {
        ...
        it != pmeshEdgesInInterface.end(); ++it, ++i)
   {
      ...
      const int ifMeshEdge =
         pToIFedge[i];  // The local edge of ifMesh that coincides with pmesh edge pmeshEdge.
      ...
      if (ne > 0)
      {
         ...
         for (int d=0; d<sddofs.Size(); ++d)
         {
            ...
            if (sdtdof >= 0)  // if this is a true DOF of fespace.
            {
               ...
               {
                  // Set global gdofmap
                  MFEM_VERIFY(gdofmap[pedgeToIFGDOF[i] + d] == -1 ||
   ...
   for (std::set<int>::const_iterator it = pmeshVerticesInInterface.begin();
        it != pmeshVerticesInInterface.end(); ++it)
   {
...

   {
      ...
      MPI_Allreduce((int*) ifpedge.data(), (int*) gdofmap.data(), ifgSize, MPI_INT,
                    MPI_MAX, ifsdcomm);
...
#ifdef USE_SIGN_FLIP
      ifpedge = gflip;
      MPI_Allreduce((int*) ifpedge.data(), (int*) gflip.data(), ifgSize, MPI_INT,
                    MPI_MAX, ifsdcomm);
                                    ...
                                    ParFiniteElementSpace *fespaceDomain, const Vector & ssd, Vector & s)
{
   ...
   {
      // The sdMesh element attribute is set as the local index of the corresponding pmesh element, which is unique since SD elements do not overlap.
      const int domainElemId = sdMesh->GetAttribute(elId) -
                               1;  // 1 was added to ensure a positive attribute.
      ...
      for (int i=0; i<dofs.Size(); ++i)
      {
         ...
         const int ldof = fespaceDomain->GetLocalTDofNumber(
                             dof_i);  // If the DOF is owned by the current processor, return its local tdof number, otherwise -1.
         ...
         if (ldof >= 0)
         {
            const double flip = ((dofs[i] >= 0 && sddofs[i] < 0) || (dofs[i] < 0 &&
                                                                     sddofs[i] >= 0)) ? -1.0 : 1.0;
                                    ...
                                    ParFiniteElementSpace *fespaceDomain, const Vector & s, Vector & ssd)
{
   ...
   for (int elId=0; elId<sdMesh->GetNE(); ++elId)
   {
      ...
      for (int i=0; i<dofs.Size(); ++i)
      {
         const int dof_i = dofs[i] >= 0 ? dofs[i] : -1 - dofs[i];
         const int sddof_i = sddofs[i] >= 0 ? sddofs[i] : -1 - sddofs[i];
         const int lsddof = fespaceSD->GetLocalTDofNumber(
                               sddof_i);  // If the DOF is owned by the current processor, return its local tdof number, otherwise -1.
...
void GatherSetInt(MPI_Comm comm, std::vector<int> const& a,
                  std::set<int>& allset)
{
   int rank, nprocs;
   ...
   MPI_Allgatherv(a.data(), s, MPI_INT, recv.data(), rcnt.data(), rdspl.data(),
                  MPI_INT, comm);
...
void CompareOperators(Operator *A, Operator *B, MPI_Comm comm)
{
   ...
   for (int j=0; j<n; ++j)
   {
      ...
      {
         cout << "WARNING: large operator diff " << err << " relative to " << nrm << ", "
              << j << " of " << n << endl;
      }
   }
}
...
void TestCGMG(HypreParMatrix * Re, HypreParMatrix * Im,
              std::vector<HypreParMatrix*> P)
{
   ...
   ComplexGMGSolver *cgmg = new ComplexGMGSolver(AZ, P,
                                                 ComplexGMGSolver::CoarseSolver::STRUMPACK);
   cgmg->SetTheta(0.5);
   cgmg->SetSmootherType(
      HypreSmoother::Jacobi);  // Some options: Jacobi, l1Jacobi, l1GS, GS
...
bool GetOutwardNormalVector(ParFiniteElementSpace *fespace,
                            InjectionOperator *injop, Vector& n)
{
   ...
   for (int i=0; i<nbe; ++i)
   {
      ...
      if (onInterface)
      {
         ...
         const IntegrationRule *ir = &IntRules.Get(be.GetGeomType(),
                                                   2);  // order is arbitrary here
         const IntegrationPoint &ip = ir->IntPoint(0);
...
DDMInterfaceOperator::DDMInterfaceOperator(const int numSubdomains_,
                                           const int numInterfaces_, ParMesh *pmesh_,
   ...
   fecbdry(orderND_, spaceDim-1), fecbdryH1(orderND_, spaceDim-1),
   localInterfaces(localInterfaces_), interfaceLocalIndex(interfaceLocalIndex_),
   ...
   alpha(1.0), beta(1.0), gamma(1.0)  // TODO: set these to the right values
{
   ...
   bf_ifNDH1grad = numInterfaces > 0 ? new MixedBilinearForm*[numInterfaces] :
                   NULL;
#else
   ifespace = numInterfaces > 0 ? new ParFiniteElementSpace*[numInterfaces] : NULL;
   iH1fespace = numInterfaces > 0 ? new ParFiniteElementSpace*[numInterfaces] :
                NULL;
   ...
   for (int i=0; i<numInterfaces; ++i)
   {
      ...
      else
      {
         ...
         ifespace[i] = new FiniteElementSpace(smeshIF2D[i],
                                              &fecbdry);  // Nedelec space for f_{m,j} when interface i is the j-th interface of subdomain m.
         iH1fespace[i] = new FiniteElementSpace(smeshIF2D[i],
                                                &fecbdryH1);  // H^1 space \rho_{m,j} when interface i is the j-th interface of subdomain m.
#else
         ifespace[i] = new FiniteElementSpace(smeshIF[i],
                                              &fecbdry);  // Nedelec space for f_{m,j} when interface i is the j-th interface of subdomain m.
         iH1fespace[i] = new FiniteElementSpace(smeshIF[i],
                                                &fecbdryH1);  // H^1 space \rho_{m,j} when interface i is the j-th interface of subdomain m.
#endif
...
#else
         ifespace[i] = new ParFiniteElementSpace(pmeshIF[i],
                                                 &fecbdry);  // Nedelec space for f_{m,j} when interface i is the j-th interface of subdomain m.
         iH1fespace[i] = new ParFiniteElementSpace(pmeshIF[i],
                                                   &fecbdryH1);  // H^1 space \rho_{m,j} when interface i is the j-th interface of subdomain m.
      ...
      if (ifli >= 0)
      {
         subdomainLocalInterfaces[(*localInterfaces)[ifli].FirstSubdomain()].push_back(
            i);
         subdomainLocalInterfaces[(*localInterfaces)[ifli].SecondSubdomain()].push_back(
            i);

         MFEM_VERIFY(globalInterfaceIndex[ifli] == i ||
                     globalInterfaceIndex[ifli] == -1, "");

         globalInterfaceIndex[ifli] = i;

         interfaceSDs[i] = (numSubdomains * (*localInterfaces)[ifli].FirstSubdomain())
                           + (*localInterfaces)[ifli].SecondSubdomain();
      }
   }

   {
      ...
      MPI_Allreduce(localInterfaceSDs.data(), interfaceSDs.data(), numInterfaces,
                    MPI_INT, MPI_MAX, MPI_COMM_WORLD);  // TODO: can this be done in place?
   }
   ...
   */
   ...
   for (int m=0; m<numSubdomains; ++m)  // TODO: make this scalable
   {
      ...
      GatherSetInt(MPI_COMM_WORLD, subdomainLocalInterfaces[m],
                   allGlobalSubdomainInterfaces[m]);
   }
   ...
   for (int m=0; m<numSubdomains; ++m)
   {
      ...
      InterfaceToSurfaceInjectionGlobalData[m].resize(
         subdomainLocalInterfaces[m].size());
      ...
      {
         fespace[m] = new ParFiniteElementSpace(pmeshSD[m],
                                                &fec);  // Nedelec space for u_m
      ...
      if (sd_nonempty[m])
      {
         ...
         for (int i=0; i<subdomainLocalInterfaces[m].size(); ++i)
         {
            ...
            MFEM_VERIFY((ifespace[interfaceIndex] == NULL) == (iH1fespace[interfaceIndex] ==
                                                               NULL), "");
            ...
            */
            ...
            block_trueOffsets[m+1] +=
               ifNDtrue[interfaceIndex]; // ifespace[interfaceIndex]->GetTrueVSize();
            block_trueOffsets[m+1] +=
               ifH1true[interfaceIndex]; // iH1fespace[interfaceIndex]->GetTrueVSize();

            if (m == -10)
            {
               cout << "SD 0 has interface " << interfaceIndex << " with ND, H1: " <<
                    ifNDtrue[interfaceIndex] << ", " << ifH1true[interfaceIndex] << endl;
            }

#ifdef SDFOSLS
            block_trueOffsets_FOSLS[m+1] += ifNDtrue[interfaceIndex] +
                                            ifH1true[interfaceIndex];
#endif
      ...
      */
      ...
      {
         MFEM_VERIFY(interfaceIndex > lastIF,
                     "");  // Verify ascending order of global indices.
         ...
         if (sd_nonempty[m])
         {
            ...
            const std::set<int> &ifFaces = (ifli >= 0) ? (*localInterfaces)[ifli].facesSet :
                                           facesEmpty;
            const std::set<int> &ifEdges = (ifli >= 0) ? (*localInterfaces)[ifli].edges :
                                           edgesEmpty;
            const std::set<int> &ifVertices = (ifli >= 0) ?
                                              (*localInterfaces)[ifli].vertices : verticesEmpty;

            std::vector<int> dofmapEmpty;

            std::vector<int> &dofmap = (i >= 0) ? InterfaceToSurfaceInjectionData[m][i] :
                                       dofmapEmpty;
            ...
            */
            ...
            const std::vector<int> &ifFacesOrdered = (ifli >= 0) ?
                                                     (*localInterfaces)[ifli].faces : ifFacesOrderedEmpty;
            ...
            SetInterfaceToSurfaceDOFMap(sd_com[m], ifespace[interfaceIndex],
                                        ...
                                        GlobalInterfaceToSurfaceInjectionGlobalData[m][interfaceIndex], flippedSDDofs,
                                        gflip[m][interfaceIndex]);

            if (i >= 0)
            {
               InterfaceToSurfaceInjectionGlobalData[m][i] =
                  GlobalInterfaceToSurfaceInjectionGlobalData[m][interfaceIndex];   // TODO: don't keep 2 copies.
            }
            ...
            */
            ...
            InjectionOperator *injOp = new InjectionOperator(sd_com[m], fespace[m],
            ...
            //#ifndef SERIAL_INTERFACES
            {
               ...
               const int ifsize = (ifespace[interfaceIndex] != NULL) ?
                                  ifespace[interfaceIndex]->GetTrueVSize() : 0;
               ...
               for (int j=0; j<ifsize; ++j)
               {
                  ...
                  {
                     cout << m_rank << ": DISCREPANCY A sd " << m << ", entry " << j << ", inj " <<
                          injIF[j] << ", if " << vif[j] << endl;
                  }
               }
            }
            //#endif
   ...
   if (m_rank == 0)
   {
      cout << m_rank << ": DDM constructor SD loop 1 timing " << chronoSD.RealTime()
           << endl;
   }
   ...
   for (int m=0; m<numSubdomains; ++m)
   {
      ...
      {
         FindBoundaryTrueDOFs(fespace[m],
                              tdofsBdry[m]);  // Determine all true DOF's of fespace[m] on the boundary of pmeshSD[m], representing u_m^s.
         ...
         tdofsBdryInjection[m] = new SetInjectionOperator(fespace[m]->GetTrueVSize(),
                                                          &(tdofsBdry[m]));
      ...
      GMRESSolver *gmres = new GMRESSolver(
         MPI_COMM_WORLD);  // TODO: this communicator is not necessarily the same as the pmeshIF communicators. Does GMRES actually use the communicator?
      ...
      CreateSubdomainDiagHypreBlocks(m, DsdRe_HypreBlocks[m], DsdRe_SparseBlocks[m],
                                     DsdRe_HypreBlockCoef[m]);
#else
      CreateSubdomainAuxiliaryHypreBlocks(m, DsdRe_HypreBlocks[m],
                                          DsdRe_SparseBlocks[m], DsdRe_HypreBlockCoef[m]);
#endif
#endif
#endif
...
#ifdef SD_ITERATIVE
#ifdef SD_ITERATIVE_FULL
      CreateSubdomainDiagHypreBlocks(m, DsdIm_HypreBlocks[m], DsdIm_SparseBlocks[m],
                                     DsdIm_HypreBlockCoef[m]);
#else
      CreateSubdomainAuxiliaryHypreBlocks(m, DsdIm_HypreBlocks[m],
                                          DsdIm_SparseBlocks[m], DsdIm_HypreBlockCoef[m]);
#endif
#endif
#endif
   ...
   if (m_rank == 0)
   {
      cout << m_rank << ": DDM constructor SD loop 2 timing " << chronoSD.RealTime()
           << endl;
   }
...
#ifdef SDFOSLS
   globalInterfaceOpRe = new BlockOperator(block_trueOffsets_FOSLS,
                                           block_trueOffsets);
   globalInterfaceOpIm = new BlockOperator(block_trueOffsets_FOSLS,
                                           block_trueOffsets);
#else
   ...
   */
   ...
   globalInterfaceOp = new BlockOperator(block_trueOffsets_FOSLS2,
                                         block_trueOffsets2);
   ...
   if (m_rank == -10)
   {
      cout << "block_trueOffsets2 print, tdofsBdry[0].size() " << tdofsBdry[0].size()
           << endl;
   ...
   */
   ...
   gpwd_size =
      12*numInterfaces*Nphi;  // 2 subdomains per interface, 2 complex parts, 3 fields (u, f, rho).
   ...
   for (int i=0; i<numInterfaces; ++i)
   {
      ...
      MFEM_VERIFY(0 <= sd0 && 0 <= sd1 && sd0 < numSubdomains &&
                  sd1 < numSubdomains, "");

      if (ili >= 0)
      {
         MFEM_VERIFY(globalInterfaceIndex[ili] == i, "");
         MFEM_VERIFY(sd0 == (*localInterfaces)[ili].FirstSubdomain() &&
      ...
      if (ili >= 0)
      {
         if (!(*localInterfaces)[ili].IsEmpty())
         {
            ...
            MFEM_VERIFY((*localInterfaces)[ili].GetOwningRank() == m_rank ||
                        (*localInterfaces)[ili].GetSharingRank() == m_rank, "");

            int otherSubdomainRoot = -1;

            if ((sd_nonempty[sd0] || sd_nonempty[sd1]) && !(sd_nonempty[sd0] &&
                                                            sd_nonempty[sd1]))
            {
               const int otherRank = ((*localInterfaces)[ili].GetOwningRank() == m_rank) ?
                                     (*localInterfaces)[ili].GetSharingRank() :
                                     (*localInterfaces)[ili].GetOwningRank();
               ...
               {
                  MPI_Recv(&otherSubdomainRoot, 1, MPI_INT, otherRank, i, MPI_COMM_WORLD,
                           MPI_STATUS_IGNORE);
               }
            ...
            if (sd_nonempty[sd])
            {
               ...
               if (sd_root[sd] == m_rank)
               {
                  ...
                  for (int p=0; p<sdnprocs; ++p)
                  {
                     if (allsdata[2*p] >= 0)
                     {
                        MFEM_VERIFY(otherSubdomainRoot == -1 || (otherSubdomainRoot == allsdata[2*p] &&
      ...
      if (ifNDtrue[i] > 0 && !(sd_nonempty[sd0] && sd_nonempty[sd1]))
      {
         ...
         MFEM_VERIFY((*localInterfaces)[ili].GetOwningRank() == m_rank ||
                     (*localInterfaces)[ili].GetSharingRank() == m_rank, "");
         MFEM_VERIFY((*localInterfaces)[ili].GetOwningRank() !=
                     (*localInterfaces)[ili].GetSharingRank(), "");
         ...
         SIsender[i] = new SerialInterfaceCommunicator((2*ifNDtrue[i]) + ifH1true[i],
                                                       true, otherRank, i);
         SIreceiver[i] = new SerialInterfaceCommunicator((2*ifNDtrue[i]) + ifH1true[i],
                                                         false, otherRank, i);
      }
      else
      {
      ...
      // relative to the outward normal of sd0.
      {
         ...
         {
            if (!(*localInterfaces)[ili].IsEmpty() &&
                m_rank == (*localInterfaces)[ili].GetOwningRank())
            {
               if_root = true;
            }
         }
         ...
         for (int j=0; j<2; ++j)
         {
            ...
            if (sd_nonempty[sd])
            {
               const bool nset = GetOutwardNormalVector(fespace[sd],
                                                        InterfaceToSurfaceInjection[sd][i], nsd);
               ...
               MPI_Gather(nsd.GetData(), 3, MPI_DOUBLE, allnsd.data(), 3, MPI_DOUBLE, 0,
                          sd_com[sd]);
               ...
               if (sd_root[sd] == m_rank)
               {
                  ...
                  if ((*localInterfaces)[ili].GetOwningRank() != m_rank)
                  {
                     ...
                     MPI_Send(nsd.GetData(), 3, MPI_DOUBLE, (*localInterfaces)[ili].GetOwningRank(),
                              8000 + i, MPI_COMM_WORLD);
                  }
               }
            }  // if sd nonempty
            ...
            if (if_root)
            {
               if (sd_root[sd] != m_rank)
               {
                  ...
                  double r[3];
                  MPI_Recv(r, 3, MPI_DOUBLE, (*localInterfaces)[ili].GetSharingRank(), 8000 + i,
         ...
         // Communicate outwardFromSD[i] from (*localInterfaces)[ili].GetOwningRank() to (*localInterfaces)[ili].GetSharingRank()
         {
            ...
            {
               if (!(*localInterfaces)[ili].IsEmpty() &&
                   m_rank == (*localInterfaces)[ili].GetSharingRank() &&
                   m_rank != (*localInterfaces)[ili].GetOwningRank())
               {
                  int r[2];
                  MPI_Recv(r, 2, MPI_INT, (*localInterfaces)[ili].GetOwningRank(), 9000 + i,
                           MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                  for (int l=0; l<2; ++l)
                  {
      ...
      if (sd_nonempty[sd0] || sd_nonempty[sd1])
      {
         ...
         globalInterfaceOpRe->SetBlock(sd0, sd1, CreateInterfaceOperator(sd0, sd1, ili,
                                                                         i, 0, partialConstructor));
#ifndef ELIMINATE_REDUNDANT_VARS
         globalInterfaceOpRe->SetBlock(sd1, sd0, CreateInterfaceOperator(sd1, sd0, ili,
                                                                         i, 1, partialConstructor));
#endif
         ...
         */
         ...
         globalInterfaceOpIm->SetBlock(sd0, sd1, CreateInterfaceOperator(sd0, sd1, ili,
                                                                         i, 0, partialConstructor));
#ifndef ELIMINATE_REDUNDANT_VARS
         globalInterfaceOpIm->SetBlock(sd1, sd0, CreateInterfaceOperator(sd1, sd0, ili,
                                                                         i, 1, partialConstructor));
#endif
      }
...

      {
         ...
         MPI_Gather(&isroot, 1, MPI_INT, allisroot.data(), 1, MPI_INT, 0,
                    MPI_COMM_WORLD);
         if (m_rank == 0)
         {
            ifroot[i] = -1;
      ...
      rowTrueOffsetsComplexIF[i][1] = block_trueOffsets[sd0+1] -
                                      block_trueOffsets[sd0];
      rowTrueOffsetsComplexIF[i][2] = block_trueOffsets[sd0+1] -
                                      block_trueOffsets[sd0];

      colTrueOffsetsComplexIF[i][1] = block_trueOffsets[sd1+1] -
                                      block_trueOffsets[sd1];
      colTrueOffsetsComplexIF[i][2] = block_trueOffsets[sd1+1] -
                                      block_trueOffsets[sd1];
      ...
      rowTrueOffsetsComplexIF_FOSLS[i][1] = block_trueOffsets_FOSLS[sd0+1] -
                                            block_trueOffsets_FOSLS[sd0];
      rowTrueOffsetsComplexIF_FOSLS[i][2] = block_trueOffsets_FOSLS[sd0+1] -
                                            block_trueOffsets_FOSLS[sd0];

      colTrueOffsetsComplexIF_FOSLS[i][1] = block_trueOffsets_FOSLS[sd1+1] -
                                            block_trueOffsets_FOSLS[sd1];
      colTrueOffsetsComplexIF_FOSLS[i][2] = block_trueOffsets_FOSLS[sd1+1] -
                                            block_trueOffsets_FOSLS[sd1];
      ...
      {
#ifdef SDFOSLS
         BlockOperator *complexBlock01 = new BlockOperator(
            rowTrueOffsetsComplexIF_FOSLS[i], colTrueOffsetsComplexIF[i]);
#else
         BlockOperator *complexBlock01 = new BlockOperator(rowTrueOffsetsComplexIF[i],
                                                           colTrueOffsetsComplexIF[i]);
#endif
         complexBlock01->SetBlock(0, 0, &(globalInterfaceOpRe->GetBlock(sd0, sd1)));
         complexBlock01->SetBlock(0, 1, &(globalInterfaceOpIm->GetBlock(sd0, sd1)),
                                  -1.0);
...
#ifndef ELIMINATE_REDUNDANT_VARS
#ifdef SDFOSLS
         BlockOperator *complexBlock10 = new BlockOperator(
            colTrueOffsetsComplexIF_FOSLS[i], rowTrueOffsetsComplexIF[i]);
#else
         BlockOperator *complexBlock10 = new BlockOperator(colTrueOffsetsComplexIF[i],
                                                           rowTrueOffsetsComplexIF[i]);
#endif
         complexBlock10->SetBlock(0, 0, &(globalInterfaceOpRe->GetBlock(sd1, sd0)));
         complexBlock10->SetBlock(0, 1, &(globalInterfaceOpIm->GetBlock(sd1, sd0)),
                                  -1.0);
      ...
      if (ifNDtrue[i] > 0)
      {
         ...
         if (cgp_u[i] == NULL)
         {
            cgp_u[i] = new ComplexGlobalProjection(Nphi, osRe, osIm, ifespace[i]->GetComm(),
                                                   &(eTE_Re[i]), &(eTE_Im[i]), &(eTE_M[i]));
         }
         ...
         if (cgp_f[i] == NULL)
         {
            cgp_f[i] = new ComplexGlobalProjection(Nphi, osRe, osIm, ifespace[i]->GetComm(),
                                                   &(bcr_Re[i]), &(bcr_Im[i]), &(bcr_M[i]));
         }
         ...
         if (cgp_rho[i] == NULL)
         {
            cgp_rho[i] = new ComplexGlobalProjection(Nphi, osRe, osIm,
                                                     iH1fespace[i]->GetComm(), &(rhoc_Re[i]), &(rhoc_Im[i]), &(rhoc_M[i]));
         }
      }
      ...
      if (sd_nonempty[sd0]) // && ifNDtrue[i] > 0)
      {
         ...
         BlockOperator *complexIFMR = new BlockOperator(ifmrOffsetsComplex[i],
                                                        rowTrueOffsetsComplexIF[i]);
   ...
   if (m_rank == 0)
   {
      cout << m_rank << ": DDM constructor IF loop timing " << chronoSD.RealTime() <<
           endl;
   }
   ...
   */
   ...
   for (int m=0; m<numSubdomains; ++m)
   {
      //if (Asd[m] != NULL)
      {
         ...
         injComplexSD[m] = new BlockOperator(rowTrueOffsetsComplexSD[m],
                                             colTrueOffsetsComplexSD[m]);
         ...
         injComplexSD2[m] = new BlockOperator(rowTrueOffsetsComplexSD2[m],
                                              colTrueOffsetsComplexSD2[m]);
         ...
         */
         ...
         if (locSizeSD > 0)
         {
            ...
            const int numBlocks = (2*subdomainLocalInterfaces[m].size()) +
                                  1;  // 1 for the subdomain, 2 for each interface (f_{mn} and \rho_{mn}).
            ...
            for (auto interfaceIndex :
                 allGlobalSubdomainInterfaces[m])  // loop over ordered global interfaces for consistency in parallel
            {
               int k = -1;
               for (int i=0; i<subdomainLocalInterfaces[m].size(); ++i)
               {
...
#ifndef SDFOSLS
            HypreParMatrix *HypreAsdComplexRe = CreateHypreParMatrixFromBlocks(sd_com[m],
                                                                               trueOffsetsSD[m], AsdRe_HypreBlocks[m],
                                                                               //#ifdef SERIAL_INTERFACES
                                                                               AsdRe_SparseBlocks[m],
                                                                               //#endif
                                                                               AsdRe_HypreBlockCoef[m], blockGI, blockProcOffsets, all_block_num_loc_rows);

            HypreParMatrix *HypreAsdComplexIm = CreateHypreParMatrixFromBlocks(sd_com[m],
                                                                               trueOffsetsSD[m], AsdIm_HypreBlocks[m],
                                                                               //#ifdef SERIAL_INTERFACES
                                                                               AsdIm_SparseBlocks[m],
                                                                               //#endif
...
#ifdef SD_ITERATIVE
#ifdef SD_ITERATIVE_FULL
            HypreParMatrix *HypreDsdComplexRe = CreateHypreParMatrixFromBlocks(sd_com[m],
                                                                               trueOffsetsSD[m], DsdRe_HypreBlocks[m], DsdRe_SparseBlocks[m],
                                                                               DsdRe_HypreBlockCoef[m]); // , blockGI, blockProcOffsets, all_block_num_loc_rows);

            HypreParMatrix *HypreDsdComplexIm = CreateHypreParMatrixFromBlocks(sd_com[m],
                                                                               trueOffsetsSD[m], DsdIm_HypreBlocks[m], DsdIm_SparseBlocks[m],
...
#ifndef SDFOSLS_PA
            HypreParMatrix *HypreDsdComplexRe = CreateHypreParMatrixFromBlocks(sd_com[m],
                                                                               trueOffsetsAuxSD[m], DsdRe_HypreBlocks[m], DsdRe_SparseBlocks[m],
                                                                               DsdRe_HypreBlockCoef[m], blockGI, blockProcOffsetsAux,
                                                                               all_block_num_loc_rows_Aux);

            HypreParMatrix *HypreDsdComplexIm = CreateHypreParMatrixFromBlocks(sd_com[m],
                                                                               trueOffsetsAuxSD[m], DsdIm_HypreBlocks[m], DsdIm_SparseBlocks[m],
                                                                               DsdIm_HypreBlockCoef[m], blockGI, blockProcOffsetsAux,
                                                                               all_block_num_loc_rows_Aux);
#endif
#endif
#endif
            ...
            {
               ComplexHypreParMatrix tmpComplex(HypreAsdComplexRe, HypreAsdComplexIm, false,
                                                false);
...
#ifndef SDFOSLS_PA
            {
               ComplexHypreParMatrix tmpDiagComplex(HypreDsdComplexRe, HypreDsdComplexIm,
                                                    false, false);
            ...
            */
...
#ifdef SD_ITERATIVE
            {
...
#ifdef SD_ITERATIVE_GMG_PA
#ifndef SDFOSLS
               ComplexOperator *AsdPA = new ComplexOperator(AsdPARe[m], AsdPAIm[m], false,
                                                            false);
...
#ifdef SD_ITERATIVE_FULL
               STRUMPACKSolver *sdprec = CreateStrumpackSolver(new STRUMPACKRowLocMatrix(*
                                                                                         (HypreDsdComplex[m])), sd_com[m]);
#else
#ifdef SD_ITERATIVE_COMPLEX
#ifdef SD_ITERATIVE_GMG
               {
...
#ifdef TEST_SD_CMG
                     cgmg = new ComplexGMGPASolver(sd_com[m], oppa_sdND[m].Ptr(), NULL, diagSDND,
                                                   sd_ess_tdof_list[m], sdP[m], (*sdcRe)[m], (*sdcIm)[m]); //, (m == 0));
                     if (m == -10)
                     {
...
#else
#ifndef SDFOSLS_PA
                     cgmg = new ComplexGMGPASolver(sd_com[m], &(AsdPARe[m]->GetBlock(0,0)),
                                                   &(AsdPAIm[m]->GetBlock(0,0)), diagSDND, sd_ess_tdof_list[m], sdP[m],
                                                   (*sdcRe)[m], (*sdcIm)[m]); //, (m == 0));
#endif
#endif
                  }
                  ...
                  ComplexHypreParMatrix *AZ = new ComplexHypreParMatrix(sdND[m], NULL, false,
                                                                        false);
#else
#ifndef SDFOSLS
                  ComplexHypreParMatrix *AZ = new ComplexHypreParMatrix(AsdRe_HypreBlocks[m](0,0),
                                                                        AsdIm_HypreBlocks[m](0,0), false, false);
#endif
#endif
#ifndef SDFOSLS
#ifdef MFEM_USE_STRUMPACK
                  ComplexGMGSolver *cgmg = new ComplexGMGSolver(AZ, sdP[m],
                                                                ComplexGMGSolver::CoarseSolver::STRUMPACK); //, (m == 0));
#else
                  ComplexGMGSolver *cgmg = new ComplexGMGSolver(AZ, sdP[m],
                                                                ComplexGMGSolver::CoarseSolver::UMFPACK); //, (m == 0));
#endif
                  cgmg->SetSmootherType(
                     HypreSmoother::Jacobi);  // Some options: Jacobi, l1Jacobi, l1GS, GS
#endif
...
#else  // not GMG
               {
                  ComplexHypreParMatrix tmpSDComplex(AsdRe_HypreBlocks[m](0,0),
                                                     AsdIm_HypreBlocks[m](0,0), false, false);
                  HypreParMatrix *HypreSDComplex = tmpSDComplex.GetSystemMatrix();

#ifdef MFEM_USE_STRUMPACK
                  sdNDinv[m] = CreateStrumpackSolver(new STRUMPACKRowLocMatrix(*(HypreSDComplex)),
                                                     sd_com[m]);
               ...
               sdNDinv[m] = CreateStrumpackSolver(new STRUMPACKRowLocMatrix(*(sdND[m])),
                                                  sd_com[m]);
               ...
               */
#ifdef SD_ITERATIVE_GMG_PA
               DDAuxSolver *auxInv = new DDAuxSolver(&(allGlobalSubdomainInterfaces[m]),
                                                     oppa_ifNDmass, alphaIm, &ifH1true);
#else
               DDAuxSolver *auxInv = new DDAuxSolver(&(allGlobalSubdomainInterfaces[m]),
                                                     ifNDmassSp, alphaIm, &ifH1true);
#endif
#else
#ifdef SDFOSLS_PA
               IdentityOperator *auxInv = new IdentityOperator(
                  2*trueOffsetsAuxSD[m][trueOffsetsAuxSD[m].Size() - 1]);
#else
#ifdef MFEM_USE_STRUMPACK
               STRUMPACKSolver *auxInv = CreateStrumpackSolver(new STRUMPACKRowLocMatrix(*
                                                                                         (HypreDsdComplex[m])), sd_com[m]);
#else
...
#ifndef SDFOSLS
#ifdef SD_ITERATIVE_GMG_PA
               Solver *sdprec = new BlockSubdomainPreconditioner(AsdPA->Height(), sdNDinv[m],
                                                                 sdImag[m], auxInv);
#else
               Solver *sdprec = new BlockSubdomainPreconditioner(HypreAsdComplex[m]->Height(),
                                                                 sdNDinv[m], sdImag[m], auxInv);
#endif
#endif
#endif
               ...
               if (true)
               {
...

                  {
                     ...
                     cfosls[m] = new FOSLSSolver(sd_com[m], fespace[m], ess_tdof_list_empty, xsd,
                                                 sdP[m], sqrt(k2));
#else
...
#ifdef IFFOSLS
#ifdef IFFOSLS_ESS
                     cfosls[m] = new FOSLSSolver(sd_com[m], fespace[m], sd_ess_tdof_list[m], xsd,
                                                 sdP[m], sqrt(k2));
#else
                     Array<int> ess_tdof_list_empty;
                     cfosls[m] = new FOSLSSolver(sd_com[m], fespace[m], ess_tdof_list_empty, xsd,
...
#ifdef IFFOSLS_H
                     invAsdComplex[m] = new LowerBlockTriangularSubdomainSolver(HypreAsdComplex[m],
                                                                                cfosls[m], auxInv, tdofsBdryInjectionTranspose[m],
...
#else
               invAsdComplex[m] =
                  NULL;  // TODO: in the SOTC case, the system is not lower block triangular, due to the (v,rho) block.
#endif
#else
               invAsdComplex[m] = gmres;
#endif
...
#else
            invAsdComplex[m] = CreateStrumpackSolver(new STRUMPACKRowLocMatrix(*
                                                                               (HypreAsdComplex[m])), sd_com[m]);
         ...
         */
         ...
         HypreAsdComplex[m] = new HypreParMatrix(MPI_COMM_WORLD, SpAsdComplex[m]->Size(),
                                                 SpAsdComplexRowSizes[m], SpAsdComplex[m]);  // constructor with 4 arguments, v1

         //CompareOperators(HypreAsdComplex[m], AsdComplex[m], sd_com[m]);

         invAsdComplex[m] = CreateStrumpackSolver(new STRUMPACKRowLocMatrix(*
                                                                            (HypreAsdComplex[m])), MPI_COMM_WORLD);
            ...
            for (int j=0; j<300; ++j)
            {
               ...
               */
               ...
               if (sumdnrm2 > 1.0e-18 || j % 100 == 0)
                  cout << m_rank << ": j " << j << " of " << Nm << ", norm of rej " << sqrt(
                          sumnrm2) << ", norm of diff " << sqrt(sumdnrm2)
                       << ", local rej norm " << locrnrm << ", local Aej norm " << Anrm <<
                       ", local op*ej norm " << opnrm << ", local (op-H)*ej norm "
         ...
         {
#ifdef SDFOSLS
            globalSubdomainOp->SetBlock(m, m,
                                        new TripleProductOperator(new TransposeOperator(injComplexSD[m]),
                                                                  invAsdComplex[m], injComplexSD2[m],
                                                                  false, false, false));
#else
            globalSubdomainOp->SetBlock(m, m,
                                        new TripleProductOperator(new TransposeOperator(injComplexSD[m]),
                                                                  invAsdComplex[m], injComplexSD[m],
                                                                  false, false, false));
#endif
         }
#else // if not DDMCOMPLEX
         globalSubdomainOp->SetBlock(m, m,
                                     new TripleProductOperator(new TransposeOperator(injSD[m]), invAsd[m], injSD[m],
                                                               false, false, false));
#endif // DDMCOMPLEX
   ...
   if (m_rank == 0)
   {
      cout << m_rank << ": DDM constructor SD loop 3 timing " << chronoSD.RealTime()
           << endl;
   }
   ...
   globalOffdiag = new ProductOperator(globalSubdomainOp, globalInterfaceOp, false,
                                       false);
   globalOp = new SumOperator(globalOffdiag, new IdentityOperator(height), false,
                              false, 1.0, 1.0);
   ...
   if (false)  // Test ProjectionMult by checking whether P^2 = P.
   {
      ...
      for (int i=0; i<height; ++i)
      {
         ...
         */
         ...
         MPI_Bcast(gpwd_u.GetData(), 12*Nphi*numInterfaces, MPI_DOUBLE, 0,
                   MPI_COMM_WORLD);
         ...
         for (int ri=0; ri<2; ++ri)  // loop over real and imaginary parts
         {
            ...
            for (int l=0; l<numInterfaces; ++l)
            {
               ...
               for (int s=0; s<2; ++s)  // loop over sides of the interface
               {
                  ...
                  const int osri = (ri == 0) ? 0 : (block_trueOffsets2[sd+1] -
                                                    block_trueOffsets2[sd]) / 2;
                     ...
                     for (int b=0; b<Nphi; ++b)
                     {
                        ...
                        if (tdofsBdryInjection[sd] != NULL)
                        {
                           ...
                           for (int q=0; q<uS.Size(); ++q)
                           {
                              ...
                              gpwd_w[block_trueOffsets2[sd] + osri + q] += gpwd_u[(12*Nphi*l) + (2*Nphi*s) +
                                                                                  (Nphi*ri) + b] * uS[q];
                  ...
                  for (int b=0; b<Nphi; ++b)
                  {
                     ...
                     const double fcoef = gpwd_u[(12*Nphi*l) + (4*Nphi) + (2*Nphi*s) +
                                                 (Nphi*ri) + b];
                     for (int q=0; q<ifNDtrue[l]; ++q)
                     {
                        if (ri == 0)
                        {
                           gpwd_w[block_trueOffsets2[sd] + osri + tdofsBdry[sd].size() + ossdif[sd] + q] +=
                              fcoef * bcr_Re[l][b][q];
                        }
                        else
                        {
                        ...
                        if (std::isnan(gpwd_w[block_trueOffsets2[sd] + osri + tdofsBdry[sd].size() +
                                              ossdif[sd] + q]))
                        {
                           cout << "nan" << endl;
                        }
                  ...
                  for (int b=0; b<Nphi; ++b)
                  {
                     ...
                     const double rhocoef = gpwd_u[(12*Nphi*l) + (8*Nphi) + (2*Nphi*s) +
                                                   (Nphi*ri) + b];
                     for (int q=0; q<ifH1true[l]; ++q)
                     {
                        ...
                        {
                           gpwd_w[block_trueOffsets2[sd] + osri + tdofsBdry[sd].size() + ossdif[sd] + q] +=
                              rhocoef * rhoc_Re[l][b][q];
                        }
                        else
                        {
                           gpwd_w[block_trueOffsets2[sd] + osri + tdofsBdry[sd].size() + ossdif[sd] + q] +=
                              rhocoef * rhoc_Im[l][b][q];
                        }
         ...
         if (m_rank == 0)
         {
            ...
            cout << "ProjectionMult test " << i << " nrm diff " << nrmd << " relative to "
                 << std::max(nrmPu, nrmP2u)
                 << ", rel " << nrmd / std::max(nrmPu,
                                                nrmP2u) << ", nrm2 lifted " << nrm2lift << endl;
         }
      }
   }
...
void DDMInterfaceOperator::PrintTiming(const int myid) const
{
   for (int i=0; i<numSubdomains; ++i)
   {
      ...
      LowerBlockTriangularSubdomainSolver *lbts =
         dynamic_cast<LowerBlockTriangularSubdomainSolver*>(invAsdComplex[i]);
      if (lbts != NULL)
      {
         cout << myid << ": LBTS sdInv timing on SD " << i << ": " << lbts->time_sdInv <<
              endl;
      }
   }
}
...
void DDMInterfaceOperator::GaussSeidelPreconditionerMult(const Vector & x,
                                                         Vector & y) const
{
   Vector u, v, w;
   ...
   for (int i=0; i<numSubdomains; ++i)
   {
      ...
      MFEM_VERIFY((globalSubdomainOp->IsZeroBlock(i,
                                                  i) == 0) == (block_trueOffsets2[i] == block_trueOffsets2[i+1]), "");
...
void DDMInterfaceOperator::GetReducedSource(ParFiniteElementSpace
                                            *fespaceGlobal, Vector & sourceGlobalRe, Vector & sourceGlobalIm,
                                            Vector & sourceReduced, std::vector<int> const& sdOrder) const
{
   ...
   //for (int m=0; m<numSubdomains; ++m)
   for (std::vector<int>::const_iterator it = sdOrder.begin(); it != sdOrder.end();
        ++it)
   {
      const int m = *it;
      ...
      if (sd_nonempty[m])
      {
         ...
         cout << rank << ": Setting real subdomain DOFs, sd " << m << ", size " << sdsize
              << endl;

         ySD[m] = new Vector(
            invAsdComplex[m]->Width());  // Size of [u_m f_m \rho_m], real and imaginary parts
         rhsSD[m] = new Vector(
            invAsdComplex[m]->Height());  // Size of [u_m f_m \rho_m], real and imaginary parts
      ...
      if (sd_nonempty[m])
      {
         cout << rank << ": ySD norm: " << ySD[m]->Norml2() << ", wSD norm " <<
              wSD.Norml2() << " (sd " << m << ")" << endl;
         ...
         cout << rank << ": Done applying invAsdComplex[" << m << "], norm of wSD: " <<
              wSD.Norml2() << endl;
         ...
         */
#else
         SetSubdomainDofsFromDomainDofs(fespace[m], fespaceGlobal, sourceGlobalRe,
                                        sourceSD);
...
void DDMInterfaceOperator::CheckContinuityOfUS(const Vector & solReduced,
                                               const bool imag)
{
   for (int i=0; i<numInterfaces; ++i)
   {
      ...
      cout << m_rank << ": CheckContinuityOfUS for interface " << i << " sd " << sd0
           << ", " << sd1 << endl;
      ...
      const int offsetImag0 = imag ? (block_trueOffsets2[sd0+1] -
                                      block_trueOffsets2[sd0]) / 2 : 0;
      const int offsetImag1 = imag ? (block_trueOffsets2[sd1+1] -
                                      block_trueOffsets2[sd1]) / 2 : 0;
      ...
      for (int j=0; j<subdomainLocalInterfaces[sd0].size(); ++j)
      {
         ...
         {
            osf0 += ifNDtrue[subdomainLocalInterfaces[sd0][j]] +
                    ifH1true[subdomainLocalInterfaces[sd0][j]];
         }
      }
      ...
      for (int j=0; j<subdomainLocalInterfaces[sd1].size(); ++j)
      {
         ...
         {
            osf1 += ifNDtrue[subdomainLocalInterfaces[sd1][j]] +
                    ifH1true[subdomainLocalInterfaces[sd1][j]];
         }
      }
      ...
      cout << " part: " << err << " relative to " << nrm << "; for f: " << f0.Norml2()
           << " relative to " << fnrm << endl;
   }
}

void DDMInterfaceOperator::RecoverDomainSolution(ParFiniteElementSpace
                                                 *fespaceGlobal, const Vector & solReduced,
                                                 Vector const& femSol, Vector & solDomain)
{
   ...
   for (int m=0; m<numSubdomains; ++m)
   {
      ...
      if (ySD[m] != NULL)
      {
         ...
         */
         wSD.SetOffset(1.0, w, block_trueOffsets_FOSLS2[m], 0,
                       block_trueOffsets_FOSLS2[m+1] - block_trueOffsets_FOSLS2[m]);
...

         {
...
#ifdef DEBUG_RECONSTRUCTION
               maxdsrcIm = std::max(maxdsrcIm,
                                    fabs(dsourcered[block_trueOffsets2[m] + imsize + i]));
            ...
            {
               cout << "Subdomain " << m <<
                    " global Norml2 of imaginary part of C_ij times reduced solution: " << sqrt(
                       sumNormIm2) << ", full reduced " << sqrt(sumNorm2) << endl;
               cout << "Subdomain " << m << " global Norml2 of real part of reduced solution: "
                    << sqrt(sumNormSolRe2) << ", imaginary " << sqrt(sumNormSolIm2) << endl;
            }
         }
   ...
   for (int m=0; m<numSubdomains; ++m)
   {
      ...
      maxeSD = 0.0;
      MPI_Allreduce(eSD.GetData(), maxeSD.GetData(), 3, MPI_DOUBLE, MPI_MAX,
                    MPI_COMM_WORLD);
...

   {
      ...
      cout << "Reduced residual norm " << DDuReReduced.Norml2() << " relative to " <<
           std::max(ddnrm, srcnrm) << ", max entry "
           << dmax << " at " << imax << endl;
   }
   ...
   cout << m_rank << ": CfReReduced error " << CfReReduced.Norml2() <<
        " relative to " << std::max(Cfnrm, Cunrm) << endl;
   ...
   for (int m=0; m<numSubdomains; ++m)
   {
      ...
      cout << m_rank << ": FEMSOL res SD " << m << " Re " << diffRe << " relative to "
           << rhsSD[m]->Norml2()
           << ", Im " << diffIm << endl;
   }
#endif
   ...
   for (int sd=0; sd<numSubdomains; ++sd)
   {
      ...
      cout << "REDUCED error on subdomain " << sd << ", real part " << sqrt(
              nrmerr) << " relative to " << sqrt(nrm) << endl;
      ...
      for (int i=0; i<subdomainLocalInterfaces[sd].size(); ++i)
      {
         ...
         if (ifespace[interfaceIndex] != NULL && ifNDtrue[interfaceIndex] > 0)
         {
            ...
            cout << "REDUCED error on subdomain " << sd << " interface " << interfaceIndex
                 << ", real part " << sqrt(nrmerr)
      ...
      cout << "REDUCED error on subdomain " << sd << ", imag part " << sqrt(
              nrmerr) << " relative to " << sqrt(nrm) << endl;
                                                   ...
                                                   const bool fullAssembly)
{
   ...
   bf_ifNDH1grad[interfaceIndex] = new MixedBilinearForm(
      iH1fespace[interfaceIndex], ifespace[interfaceIndex]);
   ...
   */
   BilinearForm *ND_FS = new BilinearForm(
      ifespace[interfaceIndex]);  // TODO: make this a class member and delete at the end.
   BilinearForm *H1stiff = new BilinearForm(
      iH1fespace[interfaceIndex]);  // TODO: make this a class member and delete at the end.
#endif
#else
   ParBilinearForm *NDmass = new ParBilinearForm(
      ifespace[interfaceIndex]);  // TODO: make this a class member and delete at the end.
   ParBilinearForm *NDcurlcurl = new ParBilinearForm(
      ifespace[interfaceIndex]);  // TODO: make this a class member and delete at the end.
   ParBilinearForm *ND_FS = new ParBilinearForm(
      ifespace[interfaceIndex]);  // TODO: make this a class member and delete at the end.
   ParBilinearForm *H1mass = new ParBilinearForm(
      iH1fespace[interfaceIndex]);  // TODO: make this a class member and delete at the end.
   ParBilinearForm *H1stiff = new ParBilinearForm(
      iH1fespace[interfaceIndex]);  // TODO: make this a class member and delete at the end.
   ParMixedBilinearForm *NDH1grad = new ParMixedBilinearForm(
      iH1fespace[interfaceIndex],
      ifespace[interfaceIndex]);  // TODO: make this a class member and delete at the end.
#endif

   // TODO: use variable wavenumber as coefficient?

   bf_ifNDmass[interfaceIndex]->AddDomainIntegrator(new VectorFEMassIntegrator(
                                                       one));
   bf_ifNDmass[interfaceIndex]->Assemble();

#ifdef SDFOSLS
   bf_ifNDtang[interfaceIndex]->AddDomainIntegrator(new
                                                    VectorFEBoundaryTangentIntegrator(1.0));
   ...
   bf_ifNDcurlcurl[interfaceIndex]->AddDomainIntegrator(new CurlCurlIntegrator(
                                                           one));
...
#ifdef SERIAL_INTERFACES
   {
      ...
      if (!fullAssembly)
      {
         ...
         */
         ...
         bf_ifNDmass[interfaceIndex]->FormLinearSystem(
            if_ND_ess_tdof_list[interfaceIndex], xif, b, oppa_ifNDmass[interfaceIndex], ifX,
            ifB);
         bf_ifNDcurlcurl[interfaceIndex]->FormLinearSystem(
            if_ND_ess_tdof_list[interfaceIndex], xif, b, oppa_ifNDcurlcurl[interfaceIndex],
            ifX, ifB);
      }
      else
#endif
      {
         ...
         bf_ifNDmass[interfaceIndex]->FormSystemMatrix(ND_ess_tdof_list,
                                                       *(ifNDmassSp[interfaceIndex]));
         bf_ifNDcurlcurl[interfaceIndex]->FormSystemMatrix(ND_ess_tdof_list,
                                                           *(ifNDcurlcurlSp[interfaceIndex]));
         //ND_FS->FormSystemMatrix(ND_ess_tdof_list, *(ifND_FS[interfaceIndex]));

#ifdef SDFOSLS
         ifNDtangSp[interfaceIndex] = new SparseMatrix();
#ifdef ZERO_IFND_BC
         bf_ifNDtang[interfaceIndex]->FormSystemMatrix(ND_ess_tdof_list,
                                                       *(ifNDtangSp[interfaceIndex]));  // TODO: fix the BC.
#else
         Array<int> ess_tdof_list_empty;
         bf_ifNDtang[interfaceIndex]->FormSystemMatrix(ess_tdof_list_empty,
                                                       *(ifNDtangSp[interfaceIndex]));
#endif
#endif
   ...
   Operator *ifNDmassStrumpack = new STRUMPACKRowLocMatrix(*
                                                           (ifNDmass[interfaceIndex]));
   ifNDmassInv[interfaceIndex] = CreateStrumpackSolver(ifNDmassStrumpack,
                                                       ifespace[interfaceIndex]->GetComm());

   ifH1mass[interfaceIndex] = new HypreParMatrix();
#endif

   cout << myid << ": interface " << interfaceIndex << ", ND true size " <<
        ifespace[interfaceIndex]->GetTrueVSize() << ", mass height "
#ifdef SERIAL_INTERFACES
#ifdef SD_ITERATIVE_GMG_PA
        << bf_ifNDmass[interfaceIndex]->Height() << ", width " <<
        bf_ifNDmass[interfaceIndex]->Width() << ", ND V size "
#else
        << ifNDmassSp[interfaceIndex]->Height() << ", width " <<
        ifNDmassSp[interfaceIndex]->Width() << ", ND V size "
#endif
#else
        << ifNDmass[interfaceIndex]->Height() << ", width " <<
        ifNDmass[interfaceIndex]->Width() << ", ND V size "
#endif
...

   {
      ...
      */
...
#ifdef SD_ITERATIVE_GMG_PA
      {
         ...
         Vector ifB, ifX;
         bf_ifH1mass[interfaceIndex]->FormLinearSystem(
            if_H1_ess_tdof_list[interfaceIndex], xif, b, oppa_ifH1mass[interfaceIndex], ifX,
            ifB);
      }
#else
      bf_ifH1mass[interfaceIndex]->FormSystemMatrix(H1_ess_tdof_list,
                                                    *(ifH1massSp[interfaceIndex]));
      ...
      ifH1massInv[interfaceIndex] = CreateStrumpackSolver(ifH1massStrumpack,
                                                          iH1fespace[interfaceIndex]->GetComm());
#endif
   }

   // Mixed interface operator
   bf_ifNDH1grad[interfaceIndex]->AddDomainIntegrator(new
                                                      MixedVectorGradientIntegrator(one));
   ...
   */
   ...
   cout << myid << ": interface " << interfaceIndex << ", ND true size " <<
        ifespace[interfaceIndex]->GetTrueVSize()
        << ", H1 true size " << iH1fespace[interfaceIndex]->GetTrueVSize()
#ifdef SERIAL_INTERFACES
#ifdef SD_ITERATIVE_GMG_PA
        << ", NDH1 height " << bf_ifNDH1grad[interfaceIndex]->Height() << ", width " <<
        bf_ifNDH1grad[interfaceIndex]->Width() << endl;
#else
        << ", NDH1 height " << ifNDH1gradSp[interfaceIndex]->Height() << ", width " <<
        ifNDH1gradSp[interfaceIndex]->Width() << endl;
#endif
#else
        << ", NDH1 height " << ifNDH1grad[interfaceIndex]->Height() << ", width " <<
        ifNDH1grad[interfaceIndex]->Width() << endl;
#endif
}

#ifdef SDFOSLS
#ifdef SERIAL_INTERFACES
Operator* DDMInterfaceOperator::CreateCij_FOSLS(const int localInterfaceIndex,
                                                const int orientation, const bool fullAssembly)
{
   const int sd0 = (orientation == 0) ?
                   (*localInterfaces)[localInterfaceIndex].FirstSubdomain() :
                   (*localInterfaces)[localInterfaceIndex].SecondSubdomain();
   const int sd1 = (orientation == 0) ?
                   (*localInterfaces)[localInterfaceIndex].SecondSubdomain() :
                   (*localInterfaces)[localInterfaceIndex].FirstSubdomain();
   ...
   if (orientation == 0)
   {
      rowTrueOffsetsIF[localInterfaceIndex].resize(
         2);  // TODO: this does not seem to depend on orientation, so it could be just one array.
      colTrueOffsetsIF[localInterfaceIndex].resize(2);
   }

   rowTrueOffsetsIF[localInterfaceIndex][orientation].SetSize(
      4);  // Number of blocks + 1
   colTrueOffsetsIF[localInterfaceIndex][orientation].SetSize(
      4);  // Number of blocks + 1

   rowTrueOffsetsIF[localInterfaceIndex][orientation][0] = 0;
   colTrueOffsetsIF[localInterfaceIndex][orientation][0] = 0;

   rowTrueOffsetsIF[localInterfaceIndex][orientation][1] =
      ifNDtrue[interfaceIndex];
   colTrueOffsetsIF[localInterfaceIndex][orientation][1] =
      ifNDtrue[interfaceIndex];

   rowTrueOffsetsIF[localInterfaceIndex][orientation][2] =
      ifNDtrue[interfaceIndex];
   colTrueOffsetsIF[localInterfaceIndex][orientation][2] =
      ifNDtrue[interfaceIndex];

   rowTrueOffsetsIF[localInterfaceIndex][orientation][3] =
      ifNDtrue[interfaceIndex];
   colTrueOffsetsIF[localInterfaceIndex][orientation][3] =
      ifH1true[interfaceIndex];

   rowTrueOffsetsIF[localInterfaceIndex][orientation].PartialSum();
   colTrueOffsetsIF[localInterfaceIndex][orientation].PartialSum();

   BlockOperator *op = new BlockOperator(
      rowTrueOffsetsIF[localInterfaceIndex][orientation],
      colTrueOffsetsIF[localInterfaceIndex][orientation]);

   const double tangSign = (outwardFromSD[interfaceIndex][1 - orientation] == 1) ?
                           1.0 : -1.0;
   ...
   if (realPart)
   {
      ...
      op->SetBlock(1, 1, ifNDtangSp[interfaceIndex],
                   -tangSign);  // (f, n x R) = -(n x f, R)
...
Operator* DDMInterfaceOperator::CreateCij(const int localInterfaceIndex,
                                          const int orientation)
{
   ...
   */
   ...
   {
      op->SetBlock(0, 0, new SumOperator(new SumOperator(ifNDmass[interfaceIndex],
                                                         ifNDcurlcurl[interfaceIndex], false, false, alpha, beta),
...
#ifdef SERIAL_INTERFACES
#ifdef SD_ITERATIVE_GMG_PA
      op->SetBlock(0, 0, new SumOperator(oppa_ifNDmass[interfaceIndex].Ptr(),
                                         oppa_ifNDcurlcurl[interfaceIndex].Ptr(), false, false, alpha, beta));
#else
      op->SetBlock(0, 0, new SumOperator(ifNDmassSp[interfaceIndex],
                                         ifNDcurlcurlSp[interfaceIndex], false, false, alpha, beta));
#endif
#else
      op->SetBlock(0, 0, new SumOperator(ifNDmass[interfaceIndex],
                                         ifNDcurlcurl[interfaceIndex], false, false, alpha, beta));
#endif
   }
#endif
...
#ifdef SERIAL_INTERFACES
#ifdef SD_ITERATIVE_GMG_PA
            op->SetBlock(1, 0, new SumOperator(oppa_ifNDmass[interfaceIndex].Ptr(),
                                               oppa_ifNDcurlcurl[interfaceIndex].Ptr(), false, false, -1.0, -betaOverAlpha));
#else
            op->SetBlock(1, 0, new SumOperator(ifNDmassSp[interfaceIndex],
                                               ifNDcurlcurlSp[interfaceIndex], false, false, -1.0, -betaOverAlpha));
#endif
#else
            op->SetBlock(1, 0, new SumOperator(ifNDmass[interfaceIndex],
                                               ifNDcurlcurl[interfaceIndex], false, false, -1.0, -betaOverAlpha));
#endif
         }
         else
         {
...
#ifdef SD_ITERATIVE_GMG_PA
      op->SetBlock(1, 0, new SumOperator(oppa_ifNDmass[interfaceIndex].Ptr(),
                                         oppa_ifNDcurlcurl[interfaceIndex].Ptr(), false, false, -1.0, -beta / alpha));
#else
      op->SetBlock(1, 0, new SumOperator(ifNDmassSp[interfaceIndex],
                                         ifNDcurlcurlSp[interfaceIndex], false, false, -1.0, -beta / alpha));
#endif
#else
      op->SetBlock(1, 0, new SumOperator(ifNDmass[interfaceIndex],
                                         ifNDcurlcurl[interfaceIndex], false, false, -1.0, -beta / alpha));
#endif
#endif
   ...
   if (realPart && orientation == 1)
   {
      op->SetBlock(1, 1, new IdentityOperator(
                      ifespace[interfaceIndex]->GetTrueVSize()));
   }
   else
#endif
...
void DDMInterfaceOperator::ProjectionMult(const Vector & x)
{
   ...
   for (int i=0; i<numInterfaces; ++i)
   {
      ...
      {
         MPI_Recv(s.GetData(), 12*Nphi, MPI_DOUBLE, ifroot[i], i, MPI_COMM_WORLD,
                  MPI_STATUS_IGNORE);
      }
...
void DDMInterfaceOperator::ProjectionMultTranspose(Vector &
                                                   x)  // x is output here
{
   MFEM_VERIFY(x.Size() == height, "");
   ...
   for (int i=0; i<numInterfaces; ++i)
   {
      ...
      if (isroot && m_rank != 0)
      {
         MPI_Recv(s.GetData(), 12*Nphi, MPI_DOUBLE, 0, i, MPI_COMM_WORLD,
                  MPI_STATUS_IGNORE);
      }
...
void ComplexGlobalProjection::MultTranspose(const Vector & x, Vector & y) const
{
   ...
   for (int j=0; j<z.Size(); ++j)
   {
      ...
      for (int i=0; i<N; ++i)
      {
         ...
         y[osIm + j] += (w[i] * (*b_Im)[i][j]) + (w[N+i] *
                                                  (*b_Re)[i][j]);  // Imaginary part
      }
   }
}
...
Operator* DDMInterfaceOperator::CreateInterfaceMassRestriction(const int sd,
                                                               const int localInterfaceIndex, const int interfaceIndex, const bool firstSD)
{
   ...
   BlockOperator *rightInjection = new BlockOperator(
      rowTrueOffsetsIFR[interfaceIndex][orientation],
      colTrueOffsetsIFR[interfaceIndex][orientation]);

   if (tdofsBdryInjection[sd] == NULL)
   {
      if (InterfaceToSurfaceInjection[sd][interfaceIndex] != NULL)
         rightInjection->SetBlock(0, 0,
                                  new ProductOperator(new TransposeOperator(
                                                         InterfaceToSurfaceInjection[sd][interfaceIndex]),
   ...
   rightInjection->SetBlock(1, 1,
                            NewIdentityOrEmptyOperator(ifNDtrue[interfaceIndex] +
                                                       ifH1true[interfaceIndex]));

   BlockOperator *blockInjectionRight = new BlockOperator(
      rowTrueOffsetsIFBR[interfaceIndex][orientation],
      colTrueOffsetsIFBR[interfaceIndex][orientation]);

   blockInjectionRight->SetBlock(0, 0,
                                 NewIdentityOrEmptyOperator(tdofsBdry[sd].size()));
   blockInjectionRight->SetBlock(1, 2,
                                 NewIdentityOrEmptyOperator(ifNDtrue[interfaceIndex] +
                                                            ifH1true[interfaceIndex]));

   BlockOperator *M = new BlockOperator(
      rowTrueOffsetsIFR[interfaceIndex][orientation]);
   ...
   if (M1[interfaceIndex] == NULL)
   {
      ...
      for (int i=0; i<Nphi; ++i)
      {
         ...
         if (ifNDtrue[interfaceIndex] > 0)
         {
            ...
            NormalizeVector(iH1fespace[interfaceIndex]->GetComm(),
                            rhoc_Re[interfaceIndex][i]);
      ...
      if (ifNDtrue[interfaceIndex] > 0)
      {
         ...
         for (int i=0; i<Nphi; ++i)
         {
            for (int j=0; j<Nphi; ++j)
            {
               ...
               double ijRe = InnerProduct(ifespace[interfaceIndex]->GetComm(), u,
                                          eTE_Re[interfaceIndex][j]); // Re part
               double ijIm = InnerProduct(ifespace[interfaceIndex]->GetComm(), u,
                                          eTE_Im[interfaceIndex][j]); // Im part

               ifNDmass[interfaceIndex]->Mult(eTE_Im[interfaceIndex][i], u);

               ijRe -= InnerProduct(ifespace[interfaceIndex]->GetComm(), u,
                                    eTE_Im[interfaceIndex][j]); // Re part
               ijIm += InnerProduct(ifespace[interfaceIndex]->GetComm(), u,
                                    eTE_Im[interfaceIndex][j]); // Im part (symmetry of M implies this is unnecessary).
               ...
               ijRe = InnerProduct(ifespace[interfaceIndex]->GetComm(), u,
                                   bcr_Re[interfaceIndex][j]); // Re part
               ijIm = InnerProduct(ifespace[interfaceIndex]->GetComm(), u,
                                   bcr_Im[interfaceIndex][j]); // Im part

               ifNDmass[interfaceIndex]->Mult(bcr_Im[interfaceIndex][i], u);

               ijRe -= InnerProduct(ifespace[interfaceIndex]->GetComm(), u,
                                    bcr_Im[interfaceIndex][j]); // Re part
               ijIm += InnerProduct(ifespace[interfaceIndex]->GetComm(), u,
                                    bcr_Im[interfaceIndex][j]); // Im part (symmetry of M implies this is unnecessary).
               ...
               ijRe = InnerProduct(iH1fespace[interfaceIndex]->GetComm(), v,
                                   rhoc_Re[interfaceIndex][j]); // Re part
               ijIm = InnerProduct(iH1fespace[interfaceIndex]->GetComm(), v,
                                   rhoc_Im[interfaceIndex][j]); // Im part

               ifH1mass[interfaceIndex]->Mult(rhoc_Im[interfaceIndex][i], v);

               ijRe -= InnerProduct(iH1fespace[interfaceIndex]->GetComm(), v,
                                    rhoc_Im[interfaceIndex][j]); // Re part
               ijIm += InnerProduct(iH1fespace[interfaceIndex]->GetComm(), v,
                                    rhoc_Im[interfaceIndex][j]); // Im part (symmetry of M implies this is unnecessary).
   ...
   TripleProductOperator *MR = new TripleProductOperator(M, rightInjection,
                                                         blockInjectionRight, false, false, false);

   return MR;
}
#endif

Operator* DDMInterfaceOperator::CreateInterfaceOperator(const int sd0,
                                                        const int sd1, const int localInterfaceIndex,
                                                        const int interfaceIndex, const int orientation, const bool fullAssembly)
{
   ...
   */
   ...
   for (int i=0; i<subdomainLocalInterfaces[sd0].size(); ++i)
   {
      ...
      if (sd0if == -1)
      {
         sd0os += ifNDtrue[subdomainLocalInterfaces[sd0][i]] +
                  ifH1true[subdomainLocalInterfaces[sd0][i]];
         //sd0os += ifespace[subdomainLocalInterfaces[sd0][i]]->GetTrueVSize() + iH1fespace[subdomainLocalInterfaces[sd0][i]]->GetTrueVSize();
      }
      else
      {
         sd0osComp += ifNDtrue[subdomainLocalInterfaces[sd0][i]] +
                      ifH1true[subdomainLocalInterfaces[sd0][i]];
   ...
   for (int i=0; i<subdomainLocalInterfaces[sd1].size(); ++i)
   {
      ...
      if (sd1if == -1)
      {
         sd1os += ifNDtrue[subdomainLocalInterfaces[sd1][i]] +
                  ifH1true[subdomainLocalInterfaces[sd1][i]];
         //sd1os += ifespace[subdomainLocalInterfaces[sd1][i]]->GetTrueVSize() + iH1fespace[subdomainLocalInterfaces[sd1][i]]->GetTrueVSize();
      }
      else
      {
         sd1osComp += ifNDtrue[subdomainLocalInterfaces[sd1][i]] +
                      ifH1true[subdomainLocalInterfaces[sd1][i]];
   ...
   */
...
#ifdef SDFOSLS
   Operator *Cij = (ifNDtrue[interfaceIndex] > 0) ? CreateCij_FOSLS(
                      localInterfaceIndex, orientation, fullAssembly) : new EmptyOperator();
   //Operator *Cij = (ifNDtrue[interfaceIndex] > 0) ? CreateCij(localInterfaceIndex, orientation) : new EmptyOperator();  // TODO
#else
   Operator *Cij = (ifNDtrue[interfaceIndex] > 0) ? CreateCij(localInterfaceIndex,
                                                              orientation) : new EmptyOperator();
#endif
   ...
   const int numBlocks =
      2;  // 1 for the subdomain surface, 1 for the interface (f_{mn} and \rho_{mn}).
   rowTrueOffsetsIFR[interfaceIndex][orientation].SetSize(numBlocks +
                                                          1);  // Number of blocks + 1
   colTrueOffsetsIFR[interfaceIndex][orientation].SetSize(numBlocks +
                                                          1);  // Number of blocks + 1

   rowTrueOffsetsIFR[interfaceIndex][orientation] = 0;
   rowTrueOffsetsIFR[interfaceIndex][orientation][1] = ifNDtrue[interfaceIndex];
   rowTrueOffsetsIFR[interfaceIndex][orientation][2] = ifNDtrue[interfaceIndex] +
                                                       ifH1true[interfaceIndex];
   /*
   ...
   */
   ...
   colTrueOffsetsIFR[interfaceIndex][orientation][2] = ifNDtrue[interfaceIndex] +
                                                       ifH1true[interfaceIndex];
   /*
   ...
   */
   ...
   BlockOperator *rightInjectionBO = new BlockOperator(
      rowTrueOffsetsIFR[interfaceIndex][orientation],
      colTrueOffsetsIFR[interfaceIndex][orientation]);
   ...
   if (tdofsBdryInjection[sd1] == NULL)
   {
      if (InterfaceToSurfaceInjection[sd1][interfaceIndex] != NULL)
         rightInjectionBO->SetBlock(0, 0,
                                    new ProductOperator(new TransposeOperator(
                                                           InterfaceToSurfaceInjection[sd1][interfaceIndex]),
                                                        new EmptyOperator(), false, false));
   }
   else
   {
   ...
   if (ifNDtrue[interfaceIndex] > 0 && !(sd_nonempty[sd0] && sd_nonempty[sd1]))
   {
      if (sd_nonempty[sd1])
      {
         rightInjection = new ProductOperator(SIsender[interfaceIndex], rightInjectionBO,
                                              false, false);
      }
      else
      {
         rightInjection = new ProductOperator(SIreceiver[interfaceIndex],
                                              rightInjectionBO, false, false);
      }
   }
#endif
   ...
   rowTrueOffsetsIFL[interfaceIndex][orientation].SetSize(numBlocks +
                                                          1);  // Number of blocks + 1
   colTrueOffsetsIFL[interfaceIndex][orientation].SetSize(numBlocks +
                                                          1);  // Number of blocks + 1

   rowTrueOffsetsIFL[interfaceIndex][orientation] = 0;
   rowTrueOffsetsIFL[interfaceIndex][orientation][1] = tdofsBdry[sd0].size();
   rowTrueOffsetsIFL[interfaceIndex][orientation][2] =
      ifNDtrue[interfaceIndex]; // + iH1fespace[interfaceIndex]->GetTrueVSize();
   ...
   */
...
#ifdef SERIAL_INTERFACES
#ifdef SDFOSLS
   rowTrueOffsetsIFL2[interfaceIndex][orientation].SetSize(
      3 + 1);  // Number of blocks + 1
   colTrueOffsetsIFL2[interfaceIndex][orientation].SetSize(
      3 + 1);  // Number of blocks + 1
   ...
   BlockOperator *leftInjection = new BlockOperator(
      rowTrueOffsetsIFL2[interfaceIndex][orientation],
      colTrueOffsetsIFL2[interfaceIndex][orientation]);
#else
   BlockOperator *leftInjection = new BlockOperator(
      rowTrueOffsetsIFL[interfaceIndex][orientation],
      colTrueOffsetsIFL[interfaceIndex][orientation]);
#endif
   ...
   if (tdofsBdryInjectionTranspose[sd0] == NULL)
   {
      ...
      {
         leftInjection->SetBlock(0, 0, new ProductOperator(new EmptyOperator(),
                                                           InterfaceToSurfaceInjection[sd0][interfaceIndex], false, false));
#if defined SERIAL_INTERFACES && defined SDFOSLS
         leftInjection->SetBlock(1, 1, new ProductOperator(new EmptyOperator(),
                                                           InterfaceToSurfaceInjection[sd0][interfaceIndex], false, false));
#endif
      }
   }
   ...
   {
      leftInjection->SetBlock(0, 0,
                              new ProductOperator(tdofsBdryInjectionTranspose[sd0],
                                                  InterfaceToSurfaceInjection[sd0][interfaceIndex], false, false));
#if defined SERIAL_INTERFACES && defined SDFOSLS
      leftInjection->SetBlock(1, 1,
                              new ProductOperator(tdofsBdryInjectionTranspose[sd0],
                                                  InterfaceToSurfaceInjection[sd0][interfaceIndex], false, false));
#endif
   }
   ...
   TripleProductOperator *CijS = new TripleProductOperator(leftInjection, Cij,
                                                           rightInjection, false, false, false);
   ...
   rowTrueOffsetsIFBL[interfaceIndex][orientation].SetSize(
      5 + 1);  // Number of blocks + 1
   colTrueOffsetsIFBL[interfaceIndex][orientation].SetSize(
      3 + 1);  // Number of blocks + 1
   ...
   rowTrueOffsetsIFBL[interfaceIndex][orientation].SetSize(
      4 + 1);  // Number of blocks + 1
   colTrueOffsetsIFBL[interfaceIndex][orientation].SetSize(
      2 + 1);  // Number of blocks + 1
   ...
   BlockOperator *blockInjectionLeft = new BlockOperator(
      rowTrueOffsetsIFBL[interfaceIndex][orientation],
      colTrueOffsetsIFBL[interfaceIndex][orientation]);

   blockInjectionLeft->SetBlock(0, 0,
                                NewIdentityOrEmptyOperator(tdofsBdry[sd0].size()));
#if defined SERIAL_INTERFACES && defined SDFOSLS
   blockInjectionLeft->SetBlock(1, 1,
                                NewIdentityOrEmptyOperator(tdofsBdry[sd0].size()));
#endif
   ...
   {
#if defined SERIAL_INTERFACES && defined SDFOSLS
      blockInjectionLeft->SetBlock(3, 2,
                                   NewIdentityOrEmptyOperator(ifNDtrue[interfaceIndex]));
#else
      blockInjectionLeft->SetBlock(2, 1,
                                   NewIdentityOrEmptyOperator(ifNDtrue[interfaceIndex]));
   ...
   rowTrueOffsetsIFBR[interfaceIndex][orientation].SetSize(
      2 + 1);  // Number of blocks + 1
   colTrueOffsetsIFBR[interfaceIndex][orientation].SetSize(
      4 + 1);  // Number of blocks + 1

   rowTrueOffsetsIFBR[interfaceIndex][orientation] = 0;
   rowTrueOffsetsIFBR[interfaceIndex][orientation][1] = tdofsBdry[sd1].size();
   rowTrueOffsetsIFBR[interfaceIndex][orientation][2] = ifNDtrue[interfaceIndex] +
                                                        ifH1true[interfaceIndex];
   ...
   colTrueOffsetsIFBR[interfaceIndex][orientation][3] = ifNDtrue[interfaceIndex] +
                                                        ifH1true[interfaceIndex];
   ...
   BlockOperator *blockInjectionRightBO = new BlockOperator(
      rowTrueOffsetsIFBR[interfaceIndex][orientation],
      colTrueOffsetsIFBR[interfaceIndex][orientation]);

   blockInjectionRightBO->SetBlock(0, 0,
                                   NewIdentityOrEmptyOperator(tdofsBdry[sd1].size()));

#ifdef SERIAL_INTERFACES
   if (sd_nonempty[sd1])
#endif
   {
      blockInjectionRightBO->SetBlock(1, 2,
                                      NewIdentityOrEmptyOperator(ifNDtrue[interfaceIndex] +
                                                                 ifH1true[interfaceIndex]));
   ...
   if (orientation == 1 && realPart)
   {
      ...
      rowTrueOffsetsIFRR[localInterfaceIndex][1] = tdofsBdry[sd0].size() + sd0os +
                                                   ifNDtrue[interfaceIndex];
      rowTrueOffsetsIFRR[localInterfaceIndex][2] = ifH1true[interfaceIndex];
      rowTrueOffsetsIFRR[localInterfaceIndex][3] = sd0osComp -
                                                   ifH1true[interfaceIndex];
      /*
      ...
      */
      ...
      colTrueOffsetsIFRR[localInterfaceIndex][1] = tdofsBdry[sd1].size() + sd1os +
                                                   ifNDtrue[interfaceIndex];
      ...
      */
      ...
      BlockOperator *Irhorho = new BlockOperator(
         rowTrueOffsetsIFRR[localInterfaceIndex],
         colTrueOffsetsIFRR[localInterfaceIndex]);
      ...
      return new SumOperator(new TripleProductOperator(blockInjectionLeft, CijS,
                                                       blockInjectionRight, false, false, false), Irhorho, false, false, 1.0, 1.0);
   }
   else
   {
      return new TripleProductOperator(blockInjectionLeft, CijS, blockInjectionRight,
                                       false, false, false);
   }
#else
   return new TripleProductOperator(blockInjectionLeft, CijS, blockInjectionRight,
                                                   ...
                                                   const bool fullAssembly)
{
...
#endif
   {
      bf_sdND[subdomain] = new ParBilinearForm(
         fespace[subdomain]);  // TODO: make this a class member and delete at the end.
...

   {
      ...
      for (set<int>::const_iterator it = tdofsBdry[subdomain].begin();
           it != tdofsBdry[subdomain].end(); ++it)
      {
         true_ess_dofs[*it] = 1;
      }

      MFEM_VERIFY(InterfaceToSurfaceInjectionData[subdomain].size() ==
                  subdomainLocalInterfaces[subdomain].size(), "");
      MFEM_VERIFY(InterfaceToSurfaceInjectionGlobalData[subdomain].size() ==
                  subdomainLocalInterfaces[subdomain].size(), "");
      ...
      for (int i=0; i<subdomainLocalInterfaces[subdomain].size(); ++i)
      {
         ...
         for (int j=0; j<InterfaceToSurfaceInjectionGlobalData[subdomain][i].size(); ++j)
         {
            ...
            {
               true_ess_dofs[InterfaceToSurfaceInjectionGlobalData[subdomain][i][j] - sdtos] =
                  0;
            }
         }
      ...
      for (int i=0; i<numBdryFaces; ++i)
      {
         ...
         {
            const int dof_d = dofs[d] >= 0 ? dofs[d] : -1 - dofs[d];
            const int ldof = fespace[subdomain]->GetLocalTDofNumber(
                                dof_d);  // If the DOF is owned by the current processor, return its local tdof number, otherwise -1.
            if (ldof >= 0)
            {
      ...
      for (int i=0; i<numBdryFaces; ++i)
      {
         ...
         belem->SetAttribute(essBdryFace[i] +
                             1);  // 1 means face is interior to the domain; 2 means face is on the exterior boundary.
...

      {
...
#endif
         {
            ...
            bf_sdND[subdomain]->FormLinearSystem(empty_tdof_list, xsd, *b, Aptr, sdX, *(srcSD[subdomain]));
               ...
               */
            {
               bf_sdND[subdomain]->FormLinearSystem(ess_tdof_list, xsd, *b, Aptr, sdX,
                                                    *(srcSD[subdomain]));
            }
         ...
         {
            cout << m_rank << ": sdND RHS size init " << srcSD[subdomain]->Size() <<
                 ", tvsize " << fespace[subdomain]->GetTrueVSize() << endl;
            sd_ess_tdof_list[subdomain] = ess_tdof_list;  // deep copy
            //sd_ess_tdof_list[subdomain] = dbg_tdof_list;  // deep copy
#ifdef SD_ITERATIVE_GMG_PA_GALERKIN_FA
            *(srcSD[subdomain]) = 0.0;
#else
            bfpa_sdND[subdomain]->FormLinearSystem(sd_ess_tdof_list[subdomain], xsd, *b,
                                                   oppa_sdND[subdomain], sdX, *(srcSD[subdomain]));
#endif

            cout << m_rank << ": sdND RHS size computed " << srcSD[subdomain]->Size() <<
                 ", norm " << srcSD[subdomain]->Norml2() << ", sd " << subdomain << endl;
         }
#endif
...
#ifdef MFEM_USE_STRUMPACK
Solver* DDMInterfaceOperator::CreateSubdomainPreconditionerStrumpack(
   const int subdomain)
{
   ...
   else
   {
      ...
      sdNDinv[subdomain] = CreateStrumpackSolver(A_subdomain,
                                                 fespace[subdomain]->GetComm());
   }

   BlockDiagonalPreconditioner *op = new BlockDiagonalPreconditioner(
      trueOffsetsSD[subdomain]);
   ...
   for (int i=0; i<subdomainLocalInterfaces[subdomain].size(); ++i)
   {
      ...
      if (subdomain == 1)
      {
         op->SetDiagonalBlock((2*i) + 1,
                              new IdentityOperator(ifespace[interfaceIndex]->GetTrueVSize()));
      }
      else
#endif
      {
         ScaledOperator *A_FF_solver = new ScaledOperator(ifNDmassInv[interfaceIndex],
                                                          alpha);
      ...
      if (subdomain == 1)
      {
         op->SetDiagonalBlock((2*i) + 2,
                              new IdentityOperator(iH1fespace[interfaceIndex]->GetTrueVSize()));
      }
      else
#endif
      {
...
{
   const int numBlocks =
      (2*subdomainLocalInterfaces[subdomain].size());  // 2 for each interface (f_{mn} and \rho_{mn}).
...
{
   const int numBlocks = (2*subdomainLocalInterfaces[subdomain].size()) +
                         1;  // 1 for the subdomain, 2 for each interface (f_{mn} and \rho_{mn}).
   ...
   trueOffsetsSD[subdomain][1] = (!sdNull) ? fespace[subdomain]->GetTrueVSize() :
                                 0;
   ...
   if (debugging && subdomain == 1)
   {
      ...
      for (int j=0; j<sdMesh->GetNEdges(); ++j)
      {
         ...
         MFEM_VERIFY(dofs.Size() == 1,
                     ""); // only implemented for testing order 1 edge elements
         ...
         if (sdtdof >= 0)
         {
            ...
            for (int l=0; l<3; ++l)
            {
               const double mp_l = 0.5 * (sdMesh->GetVertex(sdMeshEdgeVert[0])[l] +
                                          sdMesh->GetVertex(sdMeshEdgeVert[1])[l]);
               if (dofcrd[os + l] > -largeOffset + 1.0 &&
                   fabs(dofcrd[os + l] - mp_l) > vertexTol)
               {
                  uniqueness = false;
               }
      ...
      for (int i=0; i<subdomainLocalInterfaces[subdomain].size(); ++i)
      {
         ...
         for (int j=0; j<ifMesh->GetNEdges(); ++j)
         {
            ...
            if (iftdof >= 0)
            {
               ...
               for (int l=0; l<3; ++l)
               {
                  const double mp_l = (0.5 * (ifMesh->GetVertex(edgeVert[0])[l] +
                                              ifMesh->GetVertex(edgeVert[1])[l])) + ifshift;
                  if (dofcrd[os + l] > -largeOffset + 1.0 &&
                      fabs(dofcrd[os + l] - mp_l) > vertexTol)
                  {
                     uniqueness = false;
         ...
         for (int j=0; j<ifMesh->GetNV(); ++j)
         {
            ...
            MFEM_VERIFY(dofs.Size() == 1,
                        ""); // only implemented for testing order 1 nodal elements
            ...
            if (iftdof >= 0)
            {
               ...
               {
                  if (dofcrd[os + l] > -largeOffset + 1.0 &&
                      fabs(dofcrd[os + l] - ifMesh->GetVertex(j)[l]) > vertexTol)
                  {
                     uniqueness = false;
                  }
...
#ifdef SD_ITERATIVE
Operator* DDMInterfaceOperator::CreateSubdomainImaginaryPart(
   const int subdomain)
{
   Operator *A = NULL;
   ...
   for (int i=0; i<subdomainLocalInterfaces[subdomain].size(); ++i)
   {
      ...
      if (A == NULL)
      {
         if (ifNDtrue[interfaceIndex] > 0)
         {
            A = new TripleProductOperator(
               InterfaceToSurfaceInjection[subdomain][interfaceIndex],
#ifdef SERIAL_INTERFACES
      ...
      else
      {
         if (ifNDtrue[interfaceIndex] > 0)
         {
            A = new SumOperator(A, new TripleProductOperator(
                                   InterfaceToSurfaceInjection[subdomain][interfaceIndex],
#ifdef SERIAL_INTERFACES
                                   new SumOperator(ifNDmassSp[interfaceIndex], ifNDcurlcurlSp[interfaceIndex],
#else
...
#ifdef SD_ITERATIVE_GMG_PA
BlockOperator* DDMInterfaceOperator::CreateSubdomainOperatorPA(
   const int subdomain)
{
   ...
   for (auto interfaceIndex : allGlobalSubdomainInterfaces[subdomain])
   {
      ...
      {
#ifdef SERIAL_INTERFACES
         ifsum = new SumOperator(oppa_ifNDmass[interfaceIndex].Ptr(),
                                 oppa_ifNDcurlcurl[interfaceIndex].Ptr(), false, false, -alpha, -beta);
#else
         ifsum = new SumOperator(ifNDmass[interfaceIndex], ifNDcurlcurl[interfaceIndex],
                                 false, false, -alpha, -beta);
#endif
      }

      Operator *ifop = new TripleProductOperator(
         InterfaceToSurfaceInjection[subdomain][interfaceIndex], ifsum,
         new TransposeOperator(InterfaceToSurfaceInjection[subdomain][interfaceIndex]),
         false, false, false);
...
#ifdef SERIAL_INTERFACES
      op->SetBlock(0, (2*i) + 2,
                   new ProductOperator(InterfaceToSurfaceInjection[subdomain][interfaceIndex],
                                       bf_ifNDH1grad[interfaceIndex], false, false), -gamma);
      op->SetBlock((2*i) + 1, (2*i) + 2, bf_ifNDH1grad[interfaceIndex],
                   gammaOverAlpha);

      if (realPart)
      {
         op->SetBlock((2*i) + 1, 0,
                      new ProductOperator(new SumOperator(oppa_ifNDmass[interfaceIndex].Ptr(),
                                                          oppa_ifNDcurlcurl[interfaceIndex].Ptr(), false, false, 1.0, betaOverAlpha),
                                          new TransposeOperator(InterfaceToSurfaceInjection[subdomain][interfaceIndex]),
                                          false, false));

#ifndef ROBIN_TC
         op->SetBlock((2*i) + 2, (2*i) + 1,
                      new TransposeOperator(bf_ifNDH1grad[interfaceIndex]));
#endif
      ...
      {
         op->SetBlock((2*i) + 1, 0,
                      new ProductOperator(oppa_ifNDcurlcurl[interfaceIndex].Ptr(),
                                          new TransposeOperator(InterfaceToSurfaceInjection[subdomain][interfaceIndex]),
                                          false, false),  betaOverAlpha);
      }

      op->SetBlock((2*i) + 1, (2*i) + 1, oppa_ifNDmass[interfaceIndex].Ptr(),
                   alphaInverse);
#else
      ...
      if (sdND[subdomain] != NULL)
      {
         ...
         if (subdomain == -1)
         {
            ...
            if (sdNDPen[subdomain] == NULL)
            {
               ...
               MPI_Allgather(&num_loc_rows, 1, MPI_INT, all_num_loc_rows, 1, MPI_INT,
                             fespace[subdomain]->GetComm());
   ...
   for (int i=0; i<subdomainLocalInterfaces[subdomain].size(); ++i)
   {
      ...
      if (realPart)
      {
         if (preconditionerMode)
         {
            ...
            {
#ifdef SERIAL_INTERFACES
               sumMassCC = Add(-alpha, *(ifNDmassSp[interfaceIndex]), -beta,
                               *(ifNDcurlcurlSp[interfaceIndex]));
#else
            ...
            {
               A_SS[subdomain] = AddSubdomainMatrixAndInterfaceMatrix(sd_com[subdomain],
                                                                      sdND[subdomain], sumMassCC, InterfaceToSurfaceInjectionData[subdomain][i],
            ...
            else
            {
               ...
               A_SS[subdomain] = AddSubdomainMatrixAndInterfaceMatrix(sd_com[subdomain],
                                                                      previousSum, sumMassCC, InterfaceToSurfaceInjectionData[subdomain][i],
                                                                      InterfaceToSurfaceInjectionGlobalData[subdomain][i],
#ifdef USE_SIGN_FLIP
                                                                      gflip[subdomain][interfaceIndex], true, true,
#endif
         ...
         else  // not preconditionerMode
         {
            ...
            if (A_SS_op == NULL)
            {
               ...
               else
               {
                  ...
                  A_SS_op = (sdND[subdomain] == NULL) ? ifop : new SumOperator(sdND[subdomain],
                                                                               ifop, false, false, 1.0, 1.0);
               }
            }
            else
            {
      ...
      else  // if not real part
      {
         ...
         if (A_SS_op == NULL)
         {
            if (ifNDtrue[interfaceIndex] > 0)
            {
               A_SS_op = new TripleProductOperator(
                  InterfaceToSurfaceInjection[subdomain][interfaceIndex],
#ifdef SERIAL_INTERFACES
                  new SumOperator(ifNDmassSp[interfaceIndex], ifNDcurlcurlSp[interfaceIndex],
#else
      ...
      if (ifNDmass[interfaceIndex] != NULL)
      {
         op->SetBlock(0, (2*i) + 1,
                      new ProductOperator(InterfaceToSurfaceInjection[subdomain][interfaceIndex],
                                          ifNDmass[interfaceIndex], false, false));
      }
#else
      ...
      op->SetBlock(0, (2*i) + 1,
                   new TripleProductOperator(new ProductOperator(
                                                InterfaceToSurfaceInjection[subdomain][interfaceIndex],
                                                ifNDH1grad[interfaceIndex],
                                                false, false),
                                             ifH1massInv[interfaceIndex], ifNDH1gradT[interfaceIndex], false, false, false),
                   gamma);
#else
#ifdef RL_VARFORMOP
      ...
      op->SetBlock(0, (2*i) + 2,
                   new ProductOperator(InterfaceToSurfaceInjection[subdomain][interfaceIndex],
                                       ifNDH1gradSp[interfaceIndex], false, false), -gamma);
#else
      op->SetBlock(0, (2*i) + 2,
                   new ProductOperator(InterfaceToSurfaceInjection[subdomain][interfaceIndex],
                                       ifNDH1grad[interfaceIndex], false, false), -gamma);
#endif
#endif
#endif
      ...
      {
#ifdef SERIAL_INTERFACES
         op->SetBlock((2*i) + 1, (2*i) + 2, ifNDH1gradSp[interfaceIndex],
                      gammaOverAlpha);
#else
...
#endif
      {
...
#ifdef SERIAL_INTERFACES
            op->SetBlock((2*i) + 1, 0,
                         new ProductOperator(new SumOperator(ifNDmassSp[interfaceIndex],
                                                             ifNDcurlcurlSp[interfaceIndex], false, false, 1.0, betaOverAlpha),
                                             new TransposeOperator(InterfaceToSurfaceInjection[subdomain][interfaceIndex]),
                                             false, false));
#else
            op->SetBlock((2*i) + 1, 0,
                         new ProductOperator(new SumOperator(ifNDmass[interfaceIndex],
                                                             ifNDcurlcurl[interfaceIndex], false, false, 1.0, betaOverAlpha),
                                             new TransposeOperator(InterfaceToSurfaceInjection[subdomain][interfaceIndex]),
                                             false, false));
#endif
         ...
         {
#ifdef SERIAL_INTERFACES
            op->SetBlock((2*i) + 1, 0, new ProductOperator(ifNDcurlcurlSp[interfaceIndex],
                                                           new TransposeOperator(InterfaceToSurfaceInjection[subdomain][interfaceIndex]),
                                                           false, false),  betaOverAlpha);
#else
            op->SetBlock((2*i) + 1, 0, new ProductOperator(ifNDcurlcurl[interfaceIndex],
                                                           new TransposeOperator(InterfaceToSurfaceInjection[subdomain][interfaceIndex]),
                                                           false, false),  betaOverAlpha);
#endif
         }
...
#ifdef SD_ITERATIVE
void DDMInterfaceOperator::CreateSubdomainAuxiliaryHypreBlocks(
   const int subdomain, Array2D<HypreParMatrix*>& block,
   Array2D<SparseMatrix*>& blockSp, Array2D<double>& blockCoefficient)
{
   ...
   const int locSizeSD = trueOffsetsSD[subdomain][trueOffsetsSD[subdomain].Size()
                                                  -1];
   if (trueOffsetsSD[subdomain][trueOffsetsSD[subdomain].Size()-1] <
       1)  // equivalent to checking sd_nonempty. TODO: replace with sd_nonempty.
   {
...
void DDMInterfaceOperator::CreateSubdomainDiagHypreBlocks(const int subdomain,
                                                          ...
                                                          Array2D<SparseMatrix*>& blockSp, Array2D<double>& blockCoefficient)
{
   ...
   for (auto interfaceIndex : allGlobalSubdomainInterfaces[subdomain])
   {
      ...
      std::vector<int> &dofmap = (i >= 0) ?
                                 InterfaceToSurfaceInjectionData[subdomain][i] : dofmapEmpty;
      ...
      SparseMatrix *ifSum = (ifNDtrue[interfaceIndex] == 0) ? NULL : Add(-alpha,
                                                                         *(ifNDmassSp[interfaceIndex]), -beta, *(ifNDcurlcurlSp[interfaceIndex]));
#else
      HypreParMatrix *ifSum = (ifNDtrue[interfaceIndex] == 0) ? NULL : Add(-alpha,
                                                                           *(ifNDmass[interfaceIndex]), -beta, *(ifNDcurlcurl[interfaceIndex]));
#endif
      ...
      if (A_SS_op == NULL)
      {
         A_SS_op = AddSubdomainMatrixAndInterfaceMatrix(sd_com[subdomain],
                                                        sdND[subdomain], ifSum, //InterfaceToSurfaceInjectionData[subdomain][i],
                                                        //InterfaceToSurfaceInjectionGlobalData[subdomain][i],
                                                        dofmap,
      ...
      else
      {
         ...
         A_SS_op = AddSubdomainMatrixAndInterfaceMatrix(sd_com[subdomain], previousSum,
                                                        ifSum, //InterfaceToSurfaceInjectionData[subdomain][i],
                                                        //InterfaceToSurfaceInjectionGlobalData[subdomain][i],
                                                        dofmap,
                                                        GlobalInterfaceToSurfaceInjectionGlobalData[subdomain][interfaceIndex],
#ifdef USE_SIGN_FLIP
...
void DDMInterfaceOperator::CreateSubdomainHypreBlocks(const int subdomain,
                                                      Array2D<HypreParMatrix*>& block,
                                                      //#ifdef SERIAL_INTERFACES
                                                      ...
                                                      Array2D<double>& blockCoefficient)
{
   ...
   for (auto interfaceIndex : allGlobalSubdomainInterfaces[subdomain])
   {
      ...
      //if (ifNDmass[interfaceIndex] != NULL)
      {
         HypreParMatrix* injectedM = AddSubdomainMatrixAndInterfaceMatrix(
                                        sd_com[subdomain], sdND[subdomain], ifNDmass[interfaceIndex],
                                        dofmap,
                                        GlobalInterfaceToSurfaceInjectionGlobalData[subdomain][interfaceIndex],
#ifdef USE_SIGN_FLIP
...
#ifdef SERIAL_INTERFACES
      HypreParMatrix* injectedTr = AddSubdomainMatrixAndInterfaceMatrix(
                                      sd_com[subdomain], sdND[subdomain], ifNDH1gradSp[interfaceIndex],
#else
      HypreParMatrix* injectedTr = AddSubdomainMatrixAndInterfaceMatrix(
                                      sd_com[subdomain], sdND[subdomain], ifNDH1grad[interfaceIndex],
#endif
...
#else
         block(0, (2*i) + 2) =
            injectedTr->Transpose();  // not the most efficient implementation, to avoid premature optimization.
         delete injectedTr;
#endif
...
#endif
      {
         ...
         SparseMatrix *ifSumF = (ifNDtrue[interfaceIndex] == 0) ? NULL : Add(1.0,
                                                                             *(ifNDmassSp[interfaceIndex]), betaOverAlpha,
                                                                             *(ifNDcurlcurlSp[interfaceIndex]));
#else
         HypreParMatrix *ifSumF = (ifNDtrue[interfaceIndex] == 0) ? NULL : Add(1.0,
                                                                               *(ifNDmass[interfaceIndex]), betaOverAlpha, *(ifNDcurlcurl[interfaceIndex]));
#endif
         HypreParMatrix *matrixSum = AddSubdomainMatrixAndInterfaceMatrix(
                                        sd_com[subdomain], sdND[subdomain], ifSumF,
                                        //InterfaceToSurfaceInjectionData[subdomain][i],
                                        //InterfaceToSurfaceInjectionGlobalData[subdomain][i],
                                        dofmap,
                                        GlobalInterfaceToSurfaceInjectionGlobalData[subdomain][interfaceIndex],
...
#ifdef SERIAL_INTERFACES
         HypreParMatrix *matrixSum = AddSubdomainMatrixAndInterfaceMatrix(
                                        sd_com[subdomain], sdND[subdomain], ifNDcurlcurlSp[interfaceIndex],
#else
         HypreParMatrix *matrixSum = AddSubdomainMatrixAndInterfaceMatrix(
                                        sd_com[subdomain], sdND[subdomain], ifNDcurlcurl[interfaceIndex],
#endif
                                        //InterfaceToSurfaceInjectionData[subdomain][i],
                                        //InterfaceToSurfaceInjectionGlobalData[subdomain][i],
                                        dofmap,
...
#ifdef MFEM_USE_STRUMPACK
Operator* DDMInterfaceOperator::CreateSubdomainOperatorStrumpack(
   const int subdomain)
{
   const int numBlocks = (2*subdomainLocalInterfaces[subdomain].size()) +
                         1;  // 1 for the subdomain, 2 for each interface (f_{mn} and \rho_{mn}).
   ...
   */
   ...
   return CreateStrumpackMatrixFromHypreBlocks(fespace[subdomain]->GetComm(),
                                               trueOffsetsSD[subdomain],
   ...
   */
   ...
   for (int m=0; m<numSubdomains; ++m)
   {
      if (sd_nonempty[m])
      {
         ...
         cout << m_rank << ": subdomain " << m << " full projected error " << enorm <<
              " relative to " << ynorm << endl;
      }
   }
}

int DDMInterfaceOperator::JustComputeF(const int sd, Vector const& u,
                                       Vector const& rhs, Vector& uFullSD, Vector& Cf)
{
   ...
   for (int i=0; i<subdomainLocalInterfaces[sd].size(); ++i)
   {
      ...
      if (ifespace[interfaceIndex] != NULL && ifNDtrue[interfaceIndex] > 0)
      {
         ...
         cout << m_rank << ": JustComputeF subdomain " << sd << " interface " <<
...
#ifdef DEBUG_RECONSTRUCTION
void DDMInterfaceOperator::ComputeF(const int sd, Vector const& u,
                                    Vector const& ufull, Vector const& rhs)
{
   Vector Au(fespace[sd]->GetTrueVSize());
   ...
   *(dsolSD[sd]) = 0.0;
...

   {
      ...
      for (int j=0; j<tdofsBdryInjection[sd]->Width(); ++j)
      {
         ...
         err += (dcopysol[os + j] - dsolred[os + j]) * (dcopysol[os + j] - dsolred[os +
                                                                                   j]);
      }

      cout << "Subdomain " << sd <<
           " error in real part of u_S after local full solution and restriction: " <<
           sqrt(err) << " relative to " << sqrt(nrm) << endl;
   }
   ...
   for (int i=0; i<subdomainLocalInterfaces[sd].size(); ++i)
   {
      ...
      if (ifespace[interfaceIndex] != NULL && ifNDtrue[interfaceIndex] > 0)
      {
         ...
         cout << m_rank << ": ComputeF subdomain " << sd << " interface " <<
              interfaceIndex << " f error " << err << " -f error " << errm
              << " relative to " << fL2 << endl;
         ...
         for (int j=0; j<ifNDtrue[interfaceIndex]; ++j)
         {
            ...
            ferr += (ufull[oss + j] - dcopysol[os + osf_i + j]) * (ufull[oss + j] -
                                                                   dcopysol[os + osf_i + j]);

            (*(dsolSD[sd]))[oss + j] = fS[j];
         }

         cout << "Subdomain " << sd << ", interface " << interfaceIndex <<
              " error in real part of f after local inversion " << sqrt(ferr)
   ...
   cout << "SERIAL ComputeF subdomain " << sd << " reconstructed real u error " <<
        sqrt(err) << " relative to " << sqrt(nrm) << endl;
   ...
   for (int i=0; i<subdomainLocalInterfaces[sd].size(); ++i)
   {
      ...
      if (ifespace[interfaceIndex] != NULL && ifNDtrue[interfaceIndex] > 0)
      {
         ...
         cout << "SERIAL ComputeF subdomain " << sd << " interface " << interfaceIndex <<
              " reconstructed real f error " << sqrt(err)
              << " relative to " << sqrt(nrm) << endl;
      }
   }
}
...
void DDMInterfaceOperator::CopySDMatrices(std::vector<HypreParMatrix*>& Re,
