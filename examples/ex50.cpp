...
int main(int argc, char *argv[])
{
   ...
   const char *mesh_file = "../data/punch-test.msh";
   int ref_levels = 0;
   int order = 2;
   ...
   const int nb = mesh->bdr_attributes.Max();

   Array<int> bdr1(nb), bdr2(nb), bdr3(nb), bdr45(nb);
   bdr1 = 0; bdr2 = 0; bdr3 = 0; bdr45 = 0;

   // attributes are 1-based, marker indices are 0-based
   bdr1[0]  = 1;      // face attr 1
   bdr2[1]  = 1;      // face attr 2
   bdr3[2]  = 1;      // face attr 3
   bdr45[3] = 1;      // face attr 4
   bdr45[4] = 1;      // face attr 5

   Array<int> ess_x, ess_y, ess_z;
   Array<int> comp(1);

   // ux=0 on face 2 and 4,5
   comp[0] = 0; // x-component
   R_space.GetEssentialTrueDofs(bdr2, ess_x, comp);
   {
     Array<int> tmp;
     R_space.GetEssentialTrueDofs(bdr45, tmp, comp);
     ess_x.Append(tmp);
   }

   // uy=0 on face 3 and 4,5
   comp[0] = 1; // y-component
   R_space.GetEssentialTrueDofs(bdr3, ess_y, comp);
   {
     Array<int> tmp;
     R_space.GetEssentialTrueDofs(bdr45, tmp, comp);
     ess_y.Append(tmp);
   ...
   // uz=0 on face 1  (NOTE: do NOT constrain z on face 4 if you apply traction in z there)
   comp[0] = 2; // z-component
   R_space.GetEssentialTrueDofs(bdr1, ess_z, comp);

   // Union them into one ess_tdof_list_u (remove duplicates)
   Array<int> ess_tdof_list_u;
   ess_tdof_list_u.Append(ess_x);
   ess_tdof_list_u.Append(ess_y);
   ess_tdof_list_u.Append(ess_z);
