...
void LBBSolver::Mult(const Vector &b, Vector &x) const
{
   ...
   {
      mfem::out << "LBB iteration " << std::setw(2) << 0
                << " : ||r|| = " << norm << "...\n";
   }
   ...
   for (it = 0; true; it++)
   {
      ...
      // LBB - construct descent direction
      ...
      *ykArray[last_saved_id] = yk;
      ...
      {
         rho(i) = 1.0/Dot((*ykArray[i]),(*ykArray[i]));
         alpha(i) = rho(i)*Dot((*ykArray[i]),c);
      ...
      if (it > m-1)
      {
         ...
         {
            rho(i) = 1./Dot((*ykArray[i]), (*ykArray[i]));
            alpha(i) = rho(i)*Dot((*ykArray[i]),c);
            add(c, -alpha(i), (*ykArray[i]), c);
         }
      }
   ...
   {
      mfem::out << "LBB: Number of iterations: " << final_iter << '\n'
                << "   ||r|| = " << final_norm << '\n';
   }
   if (print_options.summary || (!converged && print_options.warnings))
   {
      mfem::out << "LBB: No convergence!\n";
   }
}
}
