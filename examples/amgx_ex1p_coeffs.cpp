...
int main(int argc, char *argv[])
{
   ...
   ParBilinearForm *a0 = new ParBilinearForm(fespace);
   if (pa) { a->SetAssemblyLevel(AssemblyLevel::PARTIAL); }
   ConstantCoefficient ten(200.0);
   a->AddDomainIntegrator(new DiffusionIntegrator(one));
   a0->AddDomainIntegrator(new DiffusionIntegrator(ten));
   ...
   a0->FormLinearSystem(ess_tdof_list, x, *b, A0, X0, B0);
   ...
   //     be viewed later using GLVis: "glvis -np <np> -m mesh -g sol".
   {
      ...
      mesh_name << "mesh_coeffs." << setfill('0') << setw(6) << myid;
      sol_name << "sol_coeffs." << setfill('0') << setw(6) << myid;
