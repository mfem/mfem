...
void RationalApproximation_AAA(const Vector &val, const Vector &pt,
                               Array<fptype> &z, Array<fptype> &f, Vector &w,
                               fptype tol, int max_order)
{
...
void ComputePolesAndZeros(const Vector &z, const Vector &f, const Vector &w,
                          Array<fptype> & poles, Array<fptype> & zeros, fptype &scale)
{
   // Initialization
   poles.SetSize(0);
   zeros.SetSize(0);
...
void PartialFractionExpansion(fptype scale, Array<fptype> & poles,
                              Array<fptype> & zeros, Array<fptype> & coeffs)
{
   ...
   {
      fptype tmp_numer=1.0;
      for (int j=0; j<zsize; j++)
      {
         tmp_numer *= poles[i]-zeros[j];
      }

      fptype tmp_denom=1.0;
...
/** ComputePartialFractionApproximation: compute a rational approximation (RA)
   ...
   See pg. A1501 of Nakatsukasa et al. [1]. */
void ComputePartialFractionApproximation(fptype & alpha,
                                         Array<fptype> & coeffs, Array<fptype> & poles,
                                         fptype lmax = 1000.,
                                         fptype tol=1e-10, int npoints = 1000,
                                         int max_order = 100)
{
   ...
   const fptype eps = std::numeric_limits<fptype>::epsilon();

   if (abs(alpha - 0.33) < eps)
   {
      coeffs = Array<fptype> ({1.821898e+03, 9.101221e+01, 2.650611e+01,
                               1.174937e+01, 6.140444e+00, 3.441713e+00,
                               1.985735e+00, 1.162634e+00, 6.891560e-01,
                               4.111574e-01, 2.298736e-01});
      poles = Array<fptype> ({-4.155583e+04, -2.956285e+03, -8.331715e+02,
   ...
   {
      coeffs = Array<fptype>({2.919591e-02, 1.419750e-02, 1.065798e-02,
                              9.395094e-03, 8.915329e-03, 8.822991e-03,
                              9.058247e-03, 9.814521e-03, 1.180396e-02,
                              1.834554e-02, 9.840482e-01});
      poles = Array<fptype> ({-1.069683e+04, -1.769370e+03, -5.718374e+02,
   ...
   else
   {
      ...
      coeffs = Array<fptype>({2.290262e+02, 2.641819e+01, 1.005566e+01,
                              5.390411e+00, 3.340725e+00, 2.211205e+00,
                              1.508883e+00, 1.049474e+00, 7.462709e-01,
                              5.482686e-01, 4.232510e-01, 3.578967e-01});
      poles = Array<fptype>({-3.168211e+04, -3.236077e+03, -9.868287e+02,
   ...
   Vector x(npoints);
   Vector val(npoints);
   fptype dx = lmax / (fptype)(npoints-1);
