...
int main(int argc, char *argv[])
{
   ...
   if (myid == 0) 
   {
      ...
      if (size != 0) 
          cout << "patch no, my vertices = " << patch_global_dof_ids[i] << "," ; myvertex_contr[i].Print(); cout << " and " ; vertex_contr[i].Print() ; cout << endl;;
      }
   }
   ...
   if (myid == 1) 
   {
      ...
      int size = myvertex_contr[i].Size() + vertex_contr[i].Size();
      if (size != 0) 
          cout << "patch no, my vertices = " << patch_global_dof_ids[i] << "," ; myvertex_contr[i].Print(); cout << " and " ; vertex_contr[i].Print();
      }
   }
   MPI_Barrier(MPI_COMM_WORLD);
   if (myid == 2) 
