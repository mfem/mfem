...
int main(int argc, char *argv[])
{
   ...
   b_E->Update(fespace, b.GetBlock(0), 0);
   ...
   b_H->Update(fespace, b.GetBlock(1), 0);
   ...
   //
   BilinearForm *a_EE = new BilinearForm(fespace);
   ...
   MixedBilinearForm *a_EH = new MixedBilinearForm(fespace, fespace);
   ...
   BilinearForm *a_HH = new BilinearForm(fespace);
   ...
   BlkSchwarzSmoother * M = new BlkSchwarzSmoother(cmesh,ref_levels, fespace, S);
   ...
   E_gf->MakeRef(fespace, x.GetBlock(0), 0);
   H_gf->MakeRef(fespace, x.GetBlock(1), 0);
   ...
   GridFunction *E_exgf = new GridFunction(fespace);
   ...
   if (visualization)
   {
      ...
      {
         sol_sock << "solution\n"
                  << *mesh << *E_gf << "keys rRljc\n"
                  << flush;
         ex_sock << "solution\n"
                 << *mesh << *E_exgf << "keys rRljc\n"
                 << flush;
      }
      else
      {
