...
void CreateInterfaceMeshes(const int numInterfaces, const int numSubdomains,
                           const int myid, const int num_procs, SubdomainParMeshGenerator & sdMeshGen,
                           std::vector<int> & interfaceGlobalToLocalMap,
                           std::vector<int> const& interfaceGI, ParMesh **pmeshSD,
...
void TestProlongation(const int rank, HypreParMatrix *P,
                      ParFiniteElementSpace & cspace, ParFiniteElementSpace & fspace,
                      ParGridFunction const& xc)
{
   ...
   cout << rank << ": norm of xf " << txf.Norml2() << ", norm of Px " << tPx_norm
        << ", norm of diff " << tPx.Norml2() << endl;
}
...
void TestHypreRectangularSerial()
{
   ...
   HypreParMatrix *A = new HypreParMatrix(MPI_COMM_WORLD, num_loc_rows,
                                          num_loc_rows, num_loc_cols, I_nnz, J_col, diag.GetData(), rowStarts2,
                                          colStarts2);
...
void TestHypreIdentity(MPI_Comm comm)
{
   ...
   HypreParMatrix *A = new HypreParMatrix(comm, num_loc_rows, size, size, I_nnz,
                                          J_col, diag.GetData(), rowStarts2, rowStarts2);
                          ...
                          const int ifId, const int myid)
{
   ...
   if (ifId >= 0)
   {
      cout << myid << ": interface " << ifId << " VISIT TEST: true V size " <<
           fespace.GetTrueVSize() << ", V size " << fespace.GetVSize() << endl;
   }
...
void PrintDenseMatrixOfOperator(Operator const& op, const int nprocs,
                                const int rank)
{
...
#ifdef TEST_GMG
void TestGlobalGMG(ParMesh *pmesh, ParFiniteElementSpace *fespace,
                   std::vector<HypreParMatrix*> const& P)
{
   ...
   */
   ...
   gmg->SetTheta(0.5);
   gmg->SetSmootherType(
      HypreSmoother::Jacobi);  // Some options: Jacobi, l1Jacobi, l1GS, GS
...
void MapElementDofsByValue(const int dim, ParFiniteElementSpace *fespaceA,
                           const int elemA, ParFiniteElementSpace *fespaceB, const int elemB,
                           std::vector<int>& dofmap, std::vector<int>& flip)
{
   ...
   MFEM_VERIFY(feA->GetOrder() == feB->GetOrder() &&
               feA->GetGeomType() == feB->GetGeomType(), "");
...
HypreParMatrix * CreateFESpaceMapForReorderedMeshes(ParMesh *pmeshA,
                                                    ParFiniteElementSpace *fespaceA, ParMesh *pmeshB, FiniteElementCollection * fec)
{
   ...
   MPI_Comm comm =
      pmeshA->GetComm();  // Note that A and B should have the same comm.
   ...
   MPI_Allgather(&num_loc_cols, 1, MPI_INT, all_num_loc_cols.data(), 1, MPI_INT,
                 comm);
   int glob_ncols = 0;
   int first_loc_col = 0;
   for (int i=0; i<nprocs; ++i)
   ...
   for (int elemB=0; elemB<pmeshB->GetNE(); ++elemB)
   {
      ...
      MFEM_VERIFY(dofsA.Size() == elemMap.size() &&
                  dofsB.Size() == elemMap.size(), "");

      for (int i=0; i<elemMap.size(); ++i)
      {
         const int dofB = (dofsB[elemMap[i]] >= 0) ? dofsB[elemMap[i]] : -1 -
                          dofsB[elemMap[i]];
   ...
   HypreParMatrix *fesmap = new HypreParMatrix(comm, ntdof, glob_nrows, glob_ncols,
                                               (int*) opI.data(), (HYPRE_Int*) opJ.data(), (double*) data.data(),
...
int main(int argc, char *argv[])
{
   ...
   */
   ...
   {
      MFEM_VERIFY(numSubdomains < 10000,
                  "SubdomainInterface::SetGlobalIndex will overflow");
      return 3;
   }
   ...
   if (myid == 0)
   {
      cout << "Subdomain partition " << nxyzSubdomains[0] << ", " << nxyzSubdomains[1]
           << ", " << nxyzSubdomains[2] << endl;
   }
      ...
      for (int i=0; i<nxyzSubdomains[2]; ++i)
      {
         for (int j=0; j<nxyzSubdomains[1]; ++j)
         {
            for (int l=0; l<nxyzSubdomains[0]; ++l)
            {
               ...
               const int sdid = (i * nxyzSubdomains[1] * nxyzSubdomains[0]) +
                                (j * nxyzSubdomains[0]) + l;
   ...
   if (geometricPartition)
   {
      ...
      if (myid == 0)
      {
         cout << "Parallel partition " << nxyzGlobal[0] << ", " << nxyzGlobal[1] << ", "
              << nxyzGlobal[2] << endl;
      }
...

   {
...
#ifdef SD_ITERATIVE_GMG_PA
      {
         ...
         const int numInterfaces_C = sdInterfaceGen_C.GlobalToLocalInterfaceMap(
                                        interfaces_C, interfaceGlobalToLocalMap_C, interfaceGI_C);

         cout << myid << ": created " << numSubdomains << " COARSE subdomains with " <<
              numInterfaces_C <<  " interfaces" << endl;
         ...
         Mesh **smeshInterfaces_C = (numInterfaces_C > 0 ) ? new Mesh*[numInterfaces_C] :
                                    NULL;
#else
         ParMesh **pmeshInterfaces_C = (numInterfaces_C > 0 ) ? new
                                       ParMesh*[numInterfaces_C] : NULL;
#endif

         std::vector<int> interfaceFaceOffset_C;

         CreateInterfaceMeshes(numInterfaces_C, numSubdomains, myid, num_procs,
                               sdMeshGen_C,
                               interfaceGlobalToLocalMap_C, interfaceGI_C, pmeshSD_C, interfaceFaceOffset_C,
                               interfaces_C,
#ifdef SERIAL_INTERFACES
                               smeshInterfaces_C
#else
   ...
   */
   ...
   const int numInterfaces = sdInterfaceGen.GlobalToLocalInterfaceMap(interfaces,
                                                                      interfaceGlobalToLocalMap, interfaceGI);

   cout << myid << ": created " << numSubdomains << " subdomains with " <<
        numInterfaces <<  " interfaces" << endl;
   ...
   for (int sd=0; sd<numSubdomains; ++sd)
   {
      ...
      if (pmeshSDcoarse[sd] != NULL)
      {
         ...
         HypreParMatrix *fesMapSD = CreateFESpaceMapForReorderedMeshes(pmeshSDcoarse[sd],
                                                                       sdfespace[sd], pmeshSD[sd], fec);
...
#else
   ParMesh **pmeshInterfaces = (numInterfaces > 0 ) ? new ParMesh*[numInterfaces] :
                               NULL;
#endif
   ...
   CreateInterfaceMeshes(numInterfaces, numSubdomains, myid, num_procs, sdMeshGen,
                         interfaceGlobalToLocalMap, interfaceGI, pmeshSD, interfaceFaceOffset,
                         interfaces,
#ifdef SERIAL_INTERFACES
                         smeshInterfaces
#else
   ...
   */
   ...
   if (testSubdomains)
   {
      ...
      if (printInterfaceVertices)
      {
         ...
         {
            cout << myid << ": Interface " << interfaces[i].GetGlobalIndex() << " has " <<
                 interfaces[i].NumVertices() << endl;
   ...
   */
   ...
   if (myid == 0)
   {
      cout << myid << ": DDMInterfaceOperator constructor timing " <<
           chronoDDC.RealTime() << endl;
   }
   ...
   */
   ...
   cout << myid << ": fespace size " << fespace->GetVSize() << ", true size " <<
   ...
   if (solveDD)
   {
      ...
      */
      ...
      if (myid == 0)
      {
         cout << myid << ": Solver and recovery only timing " << chronoSolver.RealTime()
              << endl;
      }
   ...
   if (myid == 0)
   {
      cout << myid << ": Total DDM timing (setup, solver, recovery) " <<
   ...
   */
...

   {
      ...
      if (myid == 0)
      {
         cout << myid << ": Real FEM system residual norm " << sqrt(
                 sumRnrm2) << ", B norm " << sqrt(sumBnrm2) << endl;
      }
   }
   ...
   */
...
#ifdef SERIAL_INTERFACES
   {
      ...
      for (int sd=0; sd<numSubdomains; ++sd)
      {
         if (pmeshSD[sd] != NULL)
         {
            ...
            MPI_Gather(&nif, 1, MPI_INT, allnif.data(), 1, MPI_INT, 0,
                       pmeshSD[sd]->GetComm());
            ...
            MPI_Gatherv(sbuf.data(), nif, MPI_INT, recv.data(), allnif.data(), rdspl.data(),
                        MPI_INT, 0, pmeshSD[sd]->GetComm());
               ...
               for (int j=0; j<sumcnt/2; ++j)
               {
                  ...
                  if (interfaceGlobalToLocalMap[ifid] < 0)
                  {
                     ...
                     interfaces.push_back(SubdomainInterface(sd0, sd1, myid,
                                                             true));  // Create empty interface

                     interfaces[interfaceGlobalToLocalMap[ifid]].SetOwningRank(owningRank);

                     const int ifgi = interfaces[interfaceGlobalToLocalMap[ifid]].SetGlobalIndex(
                                         numSubdomains);
                     MFEM_VERIFY(ifgi == interfaceGI[ifid], "");
                  }
               }
   ...
   for (int i=0; i<numInterfaces; ++i)
   {
      ...
      if (iloc >= 0)
      {
         ...
         smeshInterfaces[i] = sdMeshGen.CreateSerialInterfaceMesh(
                                 interfaceFaceOffset[iloc], interfaces[iloc], mustBuild);
         //interfaceFaceOffset[iloc] = 0;
#else
         pmeshInterfaces[i] = sdMeshGen.CreateParallelInterfaceMesh(interfaces[iloc]);
#endif
      ...
      else
      {
         ...
         // be to use the appropriate communicator or point-to-point communication, which would make the interface
         // mesh generation more parallel.
         const int sd0 = interfaceGI[i] /
                         numSubdomains;  // globalIndex = (numSubdomains * sd0) + sd1;
         const int sd1 = interfaceGI[i] - (numSubdomains *
                                           sd0);  // globalIndex = (numSubdomains * sd0) + sd1;
         SubdomainInterface emptyInterface(sd0, sd1, myid, true);
         emptyInterface.SetGlobalIndex(numSubdomains);
#ifdef SERIAL_INTERFACES
         int elemOffset = 0;
         Mesh* ifmesh = sdMeshGen.CreateSerialInterfaceMesh(elemOffset, emptyInterface,
                                                            1);
         MFEM_VERIFY(ifmesh == NULL, "");
         smeshInterfaces[i] = NULL;
#else
...

   {
      ...
      MPI_Alltoall(numOwnedByRank.data(), 1, MPI_INT, numSharedByRank.data(), 1,
      ...
      for (int r=0; r<num_procs; ++r)
      {
         if (numOwnedByRank[r] > 0 && r != myid)
         {
            ...
            for (int i=0; i<numInterfaces; ++i)
            {
               if (interfaceGlobalToLocalMap[i] >= 0)
               {
                  if ((!interfaces[interfaceGlobalToLocalMap[i]].IsEmpty()) &&
                      interfaces[interfaceGlobalToLocalMap[i]].GetOwningRank() == r)
                  {
                     ifid[cnt] = i;
                     cnt++;
                  }
         ...
         if (numSharedByRank[r] > 0 && r != myid)
         {
            ...
            MPI_Recv(ifid.data(), numSharedByRank[r], MPI_INT, r, r, MPI_COMM_WORLD,
                     MPI_STATUS_IGNORE);
      ...
      for (int r=0; r<num_procs; ++r)
      {
         if (numSharedByRank[r] > 0 && r != myid)
         {
            ...
            MPI_Send(ifos.data(), numSharedByRank[r], MPI_INT, r, num_procs + myid,
                     MPI_COMM_WORLD);
         }
         ...
         if (numOwnedByRank[r] > 0 && r != myid)
         {
            ...
            MPI_Recv(ifos.data(), numOwnedByRank[r], MPI_INT, r, num_procs + r,
