...
double dF_exact(const Vector &p);
void f_exact(const Vector &, Vector &);
double freq = 1.0, kappa, alpha = 1.0, beta = 1.0;
...
int main(int argc, char *argv[])
{
   ...
   args.AddOption(&freq, "-f", "--frequency", "Set the frequency for the exact"
                  " solution.");
   args.AddOption(&alpha, "-a", "--alpha", "Diffusion coefficient.");
   args.AddOption(&beta, "-b", "--beta", "Mass coefficient.");
   ...
   FunctionCoefficient dF(dF_exact);
   dx.ProjectCoefficient(dF);
   cout << "\n|| dF_h - dF ||_{L^2} = " << dx.ComputeL2Error(dF) << '\n' << endl;

   DivergenceGridFunctionCoefficient dF_gfc(&x);
   dx.ProjectCoefficient(dF_gfc);
   cout << "\n|| dF_h - dF ||_{L^2} = " << dx.ComputeL2Error(dF) << '\n' << endl;

   DiscreteLinearOperator div(fespace, &dfes);
   div.AddDomainInterpolator(new DivergenceInterpolator);
   div.Assemble();
   div.Finalize();
   if (false)
   {
      cout << "\nDiscrete Divergence:" << endl;
      div.SpMat().Print(cout, 6);
      cout << endl;
   }
   ...
   //    integrators.
   Coefficient *alphaCoef = new ConstantCoefficient(alpha);
   Coefficient *betaCoef  = new ConstantCoefficient(beta);
   BilinearForm *a = new BilinearForm(fespace);
   if (pa) { a->SetAssemblyLevel(AssemblyLevel::PARTIAL); }
   a->AddDomainIntegrator(new DivDivIntegrator(*alphaCoef));
   a->AddDomainIntegrator(new VectorFEMassIntegrator(*betaCoef));
...
// The divergence of the exact solution (for non-surface meshes)
double dF_exact(const Vector &p)
{
   int dim = p.Size();
   ...
   return -kappa*(2.0*sin(kappa*x)*sin(kappa*y)+sin(kappa*z));
}
...
void f_exact(const Vector &p, Vector &f)
{
   ...
   double temp1 = beta + alpha*kappa*kappa;
   double temp2 = beta + 2.*alpha*kappa*kappa;

   f(0) = temp2*cos(kappa*x)*sin(kappa*y);
   f(1) = temp2*cos(kappa*y)*sin(kappa*x);
   if (dim == 3)
   {
      f(2) = temp1*cos(kappa*z);
   }
}
