...
Array<int> ComputeHdivLORPermutation(
   FiniteElementSpace &fes_ho,
   FiniteElementSpace &fes_lor)
{
   // Given an index `i` of a LOR dof, `perm[i]` is the index of the
   ...
   for (int ilor=0; ilor<mesh_lor.GetNE(); ++ilor)
   {
      ...
      int p = fes_ho.GetOrder(iho);
      int p1 = p+1;
      int ndof_per_dim = (dim == 2) ? p*p1 : p*p*p1;
      ...
      auto set_perm = [&](int off_lor, int off_ho, int n1)
      {
         for (int i1=0; i1<2; ++i1)
         {
            int i;
            i = dofmap_lor[off_lor + i1];
            int s1 = i < 0 ? -1 : 1;
            int idof_lor = vdof_lor[absdof(i)];
            i = dofmap_ho[off_ho + i1*n1];
            int s2 = i < 0 ? -1 : 1;
            int idof_ho = vdof_ho[absdof(i)];
            int s3 = idof_lor < 0 ? -1 : 1;
            int s4 = idof_ho < 0 ? -1 : 1;
...
Array<int> ComputeHcurlLORPermutation(
   FiniteElementSpace &fes_ho,
   FiniteElementSpace &fes_lor)
{
   // Given an index `i` of a LOR dof, `perm[i]` is the index of the
   ...
   for (int ilor=0; ilor<mesh_lor.GetNE(); ++ilor)
   {
      ...
      int p = fes_ho.GetOrder(iho);
      int p1 = p+1;
      int ndof_per_dim = (dim == 2) ? p*p1 : p*p1*p1;
...
void WriteMatlab(const char *fname, Operator &op)
{
...
int main(int argc, char *argv[])
{
   ...
   // Debug printing:
   // WriteMatlab("A_ho.txt", *A_ho);
   // WriteMatlab("A_lor.txt", *A_lor);
   // {
