...
void BasisEvalDerivs(const Vector xi, Vector& N, DenseMatrix& dNdxi, DenseMatrix& dN2dxi)
{
    N[0] = 0.25*(1-xi[0])*(1-xi[1]);
    N[1] = 0.25*(1+xi[0])*(1-xi[1]);
    N[2] = 0.25*(1+xi[0])*(1+xi[1]);
    ...
    dN2dxi.SetSize(3,4); dN2dxi = 0.0;  // first row dxi2, second detadxi, third deta2
    ...
    dN2dxi(1,0) = 0.25; dN2dxi(1,1) = -0.25; dN2dxi(1,2) = 0.25; dN2dxi(1,3) = -0.25;
}
...
// dphidxi 2*4
// coords 4*3
void ComputeNormal(const DenseMatrix& dphidxi, const DenseMatrix& coords, Vector& normal, double& nnorm)
{	
...
void ComputeGapHessian(const Vector x_s, const Vector xi, const DenseMatrix m_coords,
                       DenseMatrix& dg2dx)
{
    Vector m_N(4);
    DenseMatrix m_dN(2,4);
    ...
    Vector dxidxs_row1(3); dxidxs_row1 = 0.0; Vector dxidxs_row2(3); dxidxs_row2 = 0.0;
    Vector mdx2_row1(3); mdx2_row1 = 0.0; Vector mdx2_row2(3); mdx2_row2 = 0.0;  Vector mdx2_row3(3); mdx2_row3 = 0.0;
...
void NodeSegConPairs(const Vector x1, const Vector xi2, const DenseMatrix coords2,
...
// m_conn: (npoints*4) 
void Assemble_Contact(const int m, const int npoints, const int ndofs, const Vector x_s, 
                      const Vector xi, const DenseMatrix coordsm, const Array<int> s_conn, 
		      const Array<int> m_conn, Vector& g, SparseMatrix& M, std::vector<SparseMatrix>& dM)
{
    int n = ndofs;
    int ndim = 3; 
