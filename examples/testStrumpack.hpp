...
void TestStrumpackConstructor()
{
  ...
  for (int i=0; i<num_loc_rows; ++i)
    {
      ...
      if ((first_loc_row + i) == 0 || (first_loc_row + i) == glob_nrows-1)  // if first or last row
	nnz_i = 2;
  ...
  Operator *op = new STRUMPACKRowLocMatrix(MPI_COMM_WORLD, num_loc_rows, first_loc_row, glob_nrows, glob_ncols, opI, opJ, data);
