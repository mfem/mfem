...
void PrintNonzerosOfInterfaceOperator(Operator const& op, const int tsize0,
                                      const int tsize1, set<int> const& tdofsbdry0,
...
HypreParMatrix* AddSubdomainMatrixAndInterfaceMatrix(MPI_Comm ifcomm,
                                                     HypreParMatrix *A,
...
#endif
                                                     const bool adding=true, const double cI=0.0, const double cA=1.0,
                                                     const bool injRows=true);
...
Operator* CreateStrumpackMatrixFromHypreBlocks(MPI_Comm comm,
                                               const Array<int> & offsets, const Array2D<HypreParMatrix*> & blocks,
...
bool FacesCoincideGeometrically(ParMesh *volumeMesh, const int face,
                                ParMesh *surfaceMesh, const int elem);
...
void SetDomainDofsFromSubdomainDofs(ParFiniteElementSpace *fespaceSD,
                                    ParFiniteElementSpace *fespaceDomain, const Vector & ssd, Vector & s);
void SetSubdomainDofsFromDomainDofs(ParFiniteElementSpace *fespaceSD,
                                    ParFiniteElementSpace *fespaceDomain, const Vector & s, Vector & ssd);
...
public:
   SetInjectionOperator(const int height, std::set<int> *a) : Operator(height,
                                                                          a->size()), id(a)
   {
...
private:
   ...
   std::vector<int>
   m_iftToSDrank;  // map from interface true DOF to rank owning the corresponding subdomain true DOF.
...
public:
   ...
   InjectionOperator(MPI_Comm comm, ParFiniteElementSpace *subdomainSpace,
                     FiniteElementSpace *interfaceSpace, int *a,
   ...
   */
   ...
   virtual void Mult(const Vector & x, Vector & y) const
   {
      ...
      MPI_Alltoallv(m_send.data(), m_scnt.data(), m_sdspl.data(), MPI_DOUBLE,
                    (double*) m_recv.data(), m_rcnt.data(), m_rdspl.data(), MPI_DOUBLE, m_comm);
   ...
   virtual void MultTranspose(const Vector &x, Vector &y) const
   {
      ...
      for (int i=0; i<m_recv.size(); ++i)
      {
         ...
         m_recv[i] = (m_alltrueSDflip[i] == 1) ? -x_host[m_alltrueSD[i]] :
                     x_host[m_alltrueSD[i]];
#else
      ...
      MPI_Alltoallv(m_recv.data(), m_rcnt.data(), m_rdspl.data(), MPI_DOUBLE,
                    m_send.data(), m_scnt.data(), m_sdspl.data(), MPI_DOUBLE, m_comm);
...
public:
   SerialInterfaceCommunicator(const int n_, const bool isSender_,
                               const int otherRank_, const int id_)
   ...
   virtual void Mult(const Vector & x, Vector & y) const
   {
      ...
      {
         auto y_host = y.HostWrite();
         MPI_Recv(y_host, n, MPI_DOUBLE, otherRank, id, MPI_COMM_WORLD,
                  MPI_STATUS_IGNORE);
...
{
public:
   ComplexGlobalProjection(const int N_, const int osRe_, const int osIm_,
                           MPI_Comm comm_,
                           std::vector<Vector> *b_Re_, std::vector<Vector> *b_Im_, DenseMatrix *Minv_)
      : N(N_), osRe(osRe_), osIm(osIm_), comm(comm_), b_Re(b_Re_), b_Im(b_Im_),
        Minv(Minv_)
   {
...
{
public:
   BlockSubdomainPreconditioner(const int size, Operator *invSD_,
                                Operator *sdImag_, Operator *invAuxComplex_)
...
public:
   DDAuxSolver(std::set<int> *allGlobalSubdomainInterfaces_,
      ...
      : allGlobalSubdomainInterfaces(allGlobalSubdomainInterfaces_), M(M_),
        aIm(1.0 / alphaIm_), ifH1true(ifH1true_)
   {
      int os = 0;
...
{
public:
   DDMInterfaceOperator(const int numSubdomains_, const int numInterfaces_,
                        ParMesh *pmesh_, ParFiniteElementSpace *fespaceGlobal, ParMesh **pmeshSD_,
#ifdef SERIAL_INTERFACES
                        Mesh **smeshIF_, std::vector<int> const& interfaceFaceOffset,
#else
                        ParMesh **pmeshIF_,
#endif
                        const int orderND_, const int spaceDim,
                        std::vector<SubdomainInterface> *localInterfaces_,
   ...
   void CopySDMatrices(std::vector<HypreParMatrix*>& Re,
   ...
   void FullSystemAnalyticTest()
   {
      ...
      cout << m_rank << ": Cxif norm " << Cxif_norm << ", Axfull norm " <<
           Axfull.Norml2() << endl;
      ...
      cout << m_rank << ": Cxif u_s component norm " << Cxif_us <<
           ", f and rho component norm " << Cxif_norm - Cxif_us << endl;
      ...
      for (int m=0; m<numSubdomains; ++m)
      {
         ...
         if (pmeshSD[m] != NULL)
         {
            ...
            for (set<int>::const_iterator it = tdofsBdry[m].begin();
                 it != tdofsBdry[m].end(); ++it)
            {
         ...
         for (int i=0; i<subdomainLocalInterfaces[m].size(); ++i)
         {
            ...
            {
               dfull_rho[osFull + osm + i] = Axfull[osFull + osm + i] - yfull[osFull + osm +
                                                                              i];
      ...
      cout << m_rank << ": dfull norm " << dfull.Norml2() << ", y norm " <<
           yfull.Norml2() << ", (A+C)x norm " << Axfull.Norml2() << endl;
      cout << m_rank << ": dfull_u norm " << dfull_u.Norml2() << ", (A+C)x_u norm " <<
           Axfull_u.Norml2() << endl;
      cout << m_rank << ": dfull_us norm " << dfull_us.Norml2() << endl;
      cout << m_rank << ": dfull_f norm " << dfull_f.Norml2() << ", (A+C)x_f norm " <<
           Axfull_f.Norml2() << endl;
      cout << m_rank << ": dfull_rho norm " << dfull_rho.Norml2() <<
           ", (A+C)x_rho norm " << Axfull_rho.Norml2() << endl;
   }
#endif

   void GetReducedSource(ParFiniteElementSpace *fespaceGlobal,
                         Vector & sourceGlobalRe, Vector & sourceGlobalIm, Vector & sourceReduced,
                         std::vector<int> const& sdOrder) const;

#ifdef DEBUG_RECONSTRUCTION
   void ComputeF(const int sd, Vector const& u, Vector const& ufull,
                 Vector const& rhs);
#endif
   ...
   int JustComputeF(const int sd, Vector const& u, Vector const& rhs,
                    Vector& uFullSD, Vector& Cf);
   ...
   void PrintInterfaceError(const int sd, Vector const& u)
   {
      ...
      int osf_i =
         tdofsBdry[sd].size();  // offset for f variable in ifespace on this interface, within the real part for this subdomain.
      for (int i=0; i<subdomainLocalInterfaces[sd].size(); ++i)
      {
         ...
         if (ifespace[interfaceIndex] != NULL && ifNDtrue[interfaceIndex] > 0)
         {
            ...
            cout << m_rank << ": PrintInterfaceError subdomain " << sd << " interface " <<
                 interfaceIndex << " f error " << err << " -f error " << errm
   ...
   void PrintSubdomainInteriorError(const int sd, Vector const& u)
   {
      ...
      cout << m_rank << ": Interior SD " << sd << " u error " << err <<
           " relative to " << normX << endl;
   }
                            ...
                            Vector const& rhs)
   {
...
#ifdef IFFOSLS
      for (int i=fespace[sd]->GetTrueVSize() + block_ComplexOffsetsSD[sd][1];
           i < u.Size(); ++i)
      {
      ...
      */
      ...
      //if (m_rank == 0)
      {
         cout << m_rank << ": sd " << sd << " || E_h - E ||_{L^2} Re = " << errRe <<
              endl;
         cout << m_rank << ": sd " << sd << " || E_h - E ||_{L^2} Im = " << errIm <<
              endl;
         cout << m_rank << ": sd " << sd << " || E_h ||_{L^2} Re = " << normXRe << endl;
         cout << m_rank << ": sd " << sd << " || E ||_{L^2} Re = " << normE << endl;
         cout << m_rank << ": sd " << sd << " rel err Re " << relErrRe <<
              endl; // ", max " << relErrReMax << endl;
         cout << m_rank << ": sd " << sd << " rel err tot " << relErrTot <<
              endl; // ", max " << relErrTotMax << endl;
...
#ifdef IFFOSLS
         cout << m_rank << ": sd " << sd << " || f,rho ||_{l2}^2 Re = " << normIFRe <<
              endl;
         cout << m_rank << ": sd " << sd << " || f,rho ||_{l2}^2 Im = " << normIFIm <<
              endl;
#endif
   ...
   void RecoverDomainSolution(ParFiniteElementSpace *fespaceGlobal,
                              const Vector & solReduced, Vector const& femSol,
                              Vector & solDomain);
...
private:
   ...
   SparseMatrix **ifNDmassSp, **ifNDcurlcurlSp, **ifNDH1gradSp, **ifNDH1gradTSp,
                **ifH1massSp;
   ...
   HypreParMatrix **ifNDmass, **ifNDcurlcurl, **ifNDH1grad, **ifNDH1gradT,
                  **ifH1mass, **ifND_FS;
#endif
   ...
   std::vector<Array<int> > rowTrueOffsetsComplexIF_FOSLS,
       colTrueOffsetsComplexIF_FOSLS;
#endif
   ...
   std::vector<std::vector<Vector> > eTE_Re, eTE_Im, bcr_Re, bcr_Im, rhoc_Re,
       rhoc_Im;
   ...
   Operator* CreateInterfaceMassRestriction(const int sd,
                                            const int localInterfaceIndex, const int interfaceIndex, const bool firstSD);
   ...
   InterfaceToSurfaceInjectionGlobalData;

   std::vector<std::vector<std::vector<int> > >
   GlobalInterfaceToSurfaceInjectionGlobalData;
   ...
   void SetParameters()
   {
      ...
      if (m_rank == 0)
      {
         cout << "Set parameters with k = " << k << ", h = " << h << ", kh " << k*h <<
              ", cTE " << cTE << ", cTM " << cTM << ", FEM order " << feOrder << endl;
#ifdef ROBIN_TC
   ...
   void SetToRealParameters()
   {
      ...
      betaOverAlpha = ((alphaRe * betaRe) + (alphaIm * betaIm)) / ((
                                                                      alphaRe * alphaRe) + (alphaIm * alphaIm));
      gammaOverAlpha = ((alphaRe * gammaRe) + (alphaIm * gammaIm)) / ((
                                                                         alphaRe * alphaRe) + (alphaIm * alphaIm));
   }
   ...
   void SetToImaginaryParameters()
   {
      ...
      betaOverAlpha = ((alphaRe * betaIm) - (alphaIm * betaRe)) / ((
                                                                      alphaRe * alphaRe) + (alphaIm * alphaIm));
      gammaOverAlpha = ((alphaRe * gammaIm) - (alphaIm * gammaRe)) / ((
                                                                         alphaRe * alphaRe) + (alphaIm * alphaIm));
   }
   ...
   Operator* CreateInterfaceOperator(const int sd0, const int sd1,
                                     const int localInterfaceIndex, const int interfaceIndex, const int orientation,
                                     const bool fullAssembly);
...
#ifdef HYPRE_PARALLEL_ASDCOMPLEX
   void CreateSubdomainHypreBlocks(const int subdomain,
                                   Array2D<HypreParMatrix*>& block,
                                   //#ifdef SERIAL_INTERFACES
                                   Array2D<SparseMatrix*>& blockSp,
                                   //#endif
                                   Array2D<double>& blockCoefficient);
#ifdef SD_ITERATIVE
   void CreateSubdomainAuxiliaryHypreBlocks(const int subdomain,
                                            Array2D<HypreParMatrix*>& block,
                                            Array2D<SparseMatrix*>& blockSp, Array2D<double>& blockCoefficient);

   void CreateSubdomainDiagHypreBlocks(const int subdomain,
                                       Array2D<HypreParMatrix*>& block,
                                       Array2D<SparseMatrix*>& blockSp, Array2D<double>& blockCoefficient);
#endif
#endif
...
{
public:
   FOSLSSolver(MPI_Comm comm, ParFiniteElementSpace *fespace_,
        ...
        omega(omega_)
   {
      ...
      MFEM_VERIFY(pmesh->bdr_attributes.Max() == 1 ||
                  pmesh->bdr_attributes.Max() == 2, "");
      Array<int> bdr_marker(pmesh->bdr_attributes.Max());
      bdr_marker[0] =
         1;  // add boundary integrators only in the interior of the domain
      ...
      bdr_marker_ext[0] =
         0;  // add boundary integrators only on the exterior boundary
      if (pmesh->bdr_attributes.Max() == 2)
      {
         bdr_marker_ext[1] = 1;
      }
      ...
      bM_curl->FormRectangularSystemMatrix(ess_tdof_list_empty, ess_tdof_list_empty,
                                           M_curl);
      ...
      */
      ...
      a_mix1->AddDomainIntegrator(new MixedVectorCurlIntegrator(
                                     coeffT)); // Not supported for PA!
      ...
      if (!pa)
      {
         ...
         a_mix1->FormRectangularSystemMatrix(ess_tdof_list_empty, ess_tdof_list_empty,
                                             A_mix1_ptr);
      ...
      if (!pa)
      {
         ...
         a_mix2->FormRectangularSystemMatrix(ess_tdof_list_E, ess_tdof_list_empty,
                                             A_mix2_ptr);
      ...
      */
      ///* 1234
      LS_Maxwellop->SetBlock(1, 0, pa ? (Operator*) a_mix2 : (Operator*) A_mix2,
                             -1.0); // no bc
      //LS_Maxwellop->SetBlock(3, 0, A_tang_Ecol, -omega);
      //LS_Maxwellop->SetBlock(0, 1, pa ? a_mix2_tr : (Operator*) A_mix1_E, -1.0);  // no bc
      LS_Maxwellop->SetBlock(0, 1, pa ? a_mix1 : (Operator*) A_mix1_E,
                             -1.0);  // no bc
      LS_Maxwellop->SetBlock(1, 1, A_HH.Ptr());
      //LS_Maxwellop->SetBlock(2, 1, A_tang_Erow, omega);  // other rotation
      //LS_Maxwellop->SetBlock(1, 2, A_tang_Ecol, omega);
      //LS_Maxwellop->SetBlock(2, 2, A_EE);
      LS_Maxwellop->SetBlock(2, 2, A_EE.Ptr());
      LS_Maxwellop->SetBlock(3, 2, pa ? (Operator*) a_mix2 : (Operator*) A_mix2,
                             -1.0);  // no bc
      //LS_Maxwellop->SetBlock(0, 3, A_tang_Erow, -omega);  // other rotation
      //LS_Maxwellop->SetBlock(2, 3, pa ? a_mix2_tr : (Operator*) A_mix1_E, -1.0);  // no bc
      LS_Maxwellop->SetBlock(2, 3, pa ? a_mix1 : (Operator*) A_mix1_E,
                             -1.0);  // no bc
      LS_Maxwellop->SetBlock(3, 3, A_HH.Ptr());
      //*/
      /*
      ...
      */
      ...
      LSH = blockgmg::CreateHypreParMatrixFromBlocks2(comm, block_trueOffsets, blockA,
                                                      Asp,
   ...
   {
      MFEM_VERIFY(LS_Maxwellop->NumRowBlocks() == 4 &&
                  LS_Maxwellop->NumColBlocks() == 4, "");
      const int n = 4;
      A.SetSize(n,n);
   ...
   void PrintTimings(const int myid)
   {
      ...
      cout << myid << ": FOSLSSolver a_EE subdomain integrator timing " << t1 <<
           ", boundary integrator timing " << t2 << endl;

      blockgmg::BlockMGPASolver* bmgpa = dynamic_cast<blockgmg::BlockMGPASolver*>
                                         (precLS);
      t1 = 0.0;
      ...
      cout << myid << ": FOSLSSOLVER BlockMGPASolver Mult invAc timing " << t1 <<
           endl;

      t1 = 0.0;
      if (bmgpa) { t1 = bmgpa->timeMultPresmooth; }

      cout << myid << ": FOSLSSOLVER BlockMGPASolver Mult presmooth timing " << t1 <<
           endl;

      t1 = 0.0;
      if (bmgpa) { t1 = bmgpa->timeMultResidual; }

      cout << myid << ": FOSLSSOLVER BlockMGPASolver Mult residual timing " << t1 <<
           endl;
...
public:
#ifdef IFFOSLS_H
   LowerBlockTriangularSubdomainSolver(HypreParMatrix *fullMat_,
                                       FOSLSSolver *sdInv_, Operator *auxInv_,
                                       Operator *tdofsBdryInjectionTranspose_,
                                       std::set<int> *allGlobalSubdomainInterfaces_,
                                       std::vector<InjectionOperator*> *InterfaceToSurfaceInjection_,
                                       std::vector<int> *ifNDtrue_,
                                       std::vector<int> *ifH1true_)
      : fullMat(fullMat_), sdInv(sdInv_), auxInv(auxInv_),
        allGlobalSubdomainInterfaces(allGlobalSubdomainInterfaces_),
        tdofsBdryInjectionTranspose(tdofsBdryInjectionTranspose_),
        InterfaceToSurfaceInjection(InterfaceToSurfaceInjection_),
   ...
   virtual void Mult(const Vector & x, Vector & y) const
   {
...

      {
         ...
         */
         ...
         for (auto interfaceIndex : *allGlobalSubdomainInterfaces)
         {
            ...
            MFEM_VERIFY((*ifNDtrue)[interfaceIndex] ==
                        (*InterfaceToSurfaceInjection)[interfaceIndex]->Width(), "");
      ...
      tmp = 0.0;
      fullMat->Mult(y,
                    tmp);  // Multiply solution for (0) block to get action of (1,0) block.

      for (int i=0; i<nAuxHalf; ++i)
      {
         xAux[i] = x[nSD + i] - tmp[nSDhalf + i];  // real part
         xAux[nAuxHalf + i] = x[widthHalf + nSD + i] - tmp[nSD + nAuxHalf +
                                                           i];  // imaginary part
      }
