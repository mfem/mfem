...
int main(int argc, char *argv[])
{
   ...
   for (int k = 1; k <= max_outer_iter; k++)
   {
      ...
      for (int j = 0; j <= max_newton_iter; j++)
      {
         ...
         FlatVectorCoefficient nalpha_vstress_u1_curr_coeff(stress_u1_curr_coeff,
                                                            -alpha);
         StressGridFunctionCoefficient stress_u2_curr_coeff(lambda2, mu2, &u2_current);
         FlatVectorCoefficient nalpha_vstress_u2_curr_coeff(stress_u2_curr_coeff,
                                                            -alpha);
         ...
         b1->AddDomainIntegrator(new VectorDomainLFStrainIntegrator(
                                    nalpha_vstress_u1_curr_coeff));
         ...
         b2->AddDomainIntegrator(new VectorDomainLFStrainIntegrator(
                                    nalpha_vstress_u2_curr_coeff));
