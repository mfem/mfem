...
int main(int argc, char *argv[])
{
   ...
   if (pa)
   {
      ...
      Vector tx(darcyOp->Width());
      Vector ty(darcyOp->Height());
      tx.UseDevice(true);
      ty.UseDevice(true);
      tx = 1.0;
      ...
      const int s1 = 0;
      //const int s1 = W_space->TrueVSize()/2;
      const int s2 = W_space->TrueVSize() - s1;

      Array<int> block11_trueOffsets(3); // number of variables + 1
      block11_trueOffsets[0] = 0;
      block11_trueOffsets[1] = s1;
      block11_trueOffsets[2] = s2;
      ...
      darcyOp->Mult(tx, ty);
      //Operator *globalOp = new SumOperator(darcyOp, new IdentityOperator(darcyOp->Height()), false, false, 1.0, 1.0);
      //Operator *globalOp = new ProductOperator(darcyOp, new IdentityOperator(darcyOp->Height()), false, false);
      //globalOp->Mult(tx, ty);

      cout << "ty norm " << ty.Norml2() << endl;
   }
   else
   {
