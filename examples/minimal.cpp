...
void AddToComponent(GridFunction &phi, const GridFunction &phi_i, int d, double alpha=1.0)
{
   const FiniteElementSpace *fes = phi_i.FESpace();
   // ASSUME phi IS ORDERED byNODES!
   int ndof = fes->GetNDofs();
...
int main(int argc, char *argv[])
{
   ...
   for (int i=0; i<sdim; ++i) { phi_i[i] = new GridFunction(fespace); }
   ...
   // UMFPackSolver M_solv;
   // M_solv.SetOperator(*M);
