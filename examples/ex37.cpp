...
int main(int argc, char *argv[])
{
   ...
   int problem = 0;
   const char *mesh_file = "../data/elasticity_compliance.mesh";
   int ref_levels = 2;
   int order = 1;
   ...
   VectorFunctionCoefficient volume_source(dim, [&center, &force, radius](const Vector &x, Vector &y){
      Vector xx(x);
      xx -= center;
      const double r = xx.Norml2();
      if (r < radius)
      {
         y = force;
      }
   ...
   FiniteElementSpace fes_H1_Qk2_vec(&mesh, new H1_FECollection(order + 2, dim,
                                                            mfem::BasisType::GaussLobatto), dim);
   FiniteElementSpace fes_H1_Qk1_vec(&mesh, new H1_FECollection(order + 1, dim,
                                                            mfem::BasisType::GaussLobatto), dim);
   FiniteElementSpace fes_H1_Qk0_vec(&mesh, new H1_FECollection(order + 0, dim,
                                                            mfem::BasisType::GaussLobatto), dim);
   ...
   VectorGradientGridFunctionCoefficient Du(&u);
   GradientGridFunctionCoefficient Df_rho(&f_rho);
   GradientGridFunctionCoefficient Df_lam(&f_lam);

   FrobeniusNormCoefficient squared_normDu(Du);

   ProductCoefficient simp_lame(lame_first, simp_cf);
   ProductCoefficient simp_shear(shear_modulus, simp_cf);
   ...
   ScalarMatrixProductCoefficient neg_simp_Du(neg_simp, Du);
   ScalarVectorProductCoefficient neg_eps_Df_rho(-epsilon, Df_rho);
   ScalarVectorProductCoefficient neg_eps_Df_lam(-epsilon, Df_lam);
   ScalarMatrixProductCoefficient dsimp_Du(dsimp_cf, Du);
   ScalarMatrixProductCoefficient dsimp_Du_times2(dsimp_times2, Du);
   ...
   newtonSystem.GetDiagBlock(Vars::u)->AddDomainIntegrator(
      // A += (r(ρ̃^i)∇δu, ∇v)
      new ElasticityIntegrator(simp_lame, simp_shear)
   );
   newtonSystem.GetLinearForm(Vars::u)->AddDomainIntegrator(
      new VectorDomainLFIntegrator(volume_source)
   );
