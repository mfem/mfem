...
public:
   SubdomainInterface(const int sd0_, const int sd1_, const int rank,
                      const bool empty_=false) : sd0(sd0_), sd1(sd1_), myRank(rank), empty(empty_)
   {
      MFEM_VERIFY(sd0 < sd1, "");
      globalIndex = -1;
      owningRank = -1;
   ...
   std::set<int> vertices, edges,
       facesSet;  // local pmesh vertex, edges, and face indices
   ...
   void ConstructFacesVector(ParFiniteElementSpace const& face_fes)
   {
      ...
      for (auto it = gi.begin(); it != gi.end();
           ++it)  // Loop over faces in order of global index
      {
         const int gid = *it;
   ...
   int NumFaces() const
   {
      ...
      int n=0;
      for (std::set<int>::const_iterator it = facesSet.begin(); it != facesSet.end();
           ++it)
      {
         if ((*it) >= 0) { n++; }
      }
   ...
   {
      for (std::set<int>::const_iterator it = vertices.begin(); it != vertices.end();
           ++it)
      {
...
public:
   ...
   void CreateInterfaces(std::vector<SubdomainInterface>& interfaces)
   {
      ...
      ParDiscreteLinearOperator vert_elem_oper(&vert_fes,
                                               &elem_fes); // maps vert_fes to elem_fes
   ...
   int GlobalToLocalInterfaceMap(std::vector<SubdomainInterface>& localInterfaces,
                                 ...
                                 std::vector<int>& globalIndices)
   {
      ...
      if (sumcts > 0)
      {
         ...
         int giPrev = -1;
         int cnt = 0;
         for (std::set<int>::const_iterator it = allGI.begin(); it != allGI.end();
              ++it, ++cnt)
         {
            const int gi = *it;
...
private:
   ...
   {
      int numElements =
         0;  // Number of elements on this process (in pmesh) in the subdomain (i.e. with the given attribute).
      for (int i=0; i<pmesh->GetNE(); ++i)
      {
   ...
   void GatherSubdomainOrInterfaceMeshData(const int attribute,
                                           ...
                                           int& subdomainNumElements, int& elemOffset, const SubdomainInterface *interface)
   {
      ...
      for (int i = 0; i < num_procs; ++i)
      {
         ...
         {
            elemOffset =
               subdomainNumElements;   // For interfaces, this is the correct value only for processes owning faces on the interface.
         }
      ...
      vector<int> my_element_vgid(std::max(numElVert*numLocalElements,
                                           1));  // vertex global indices, for each element
      ...
      {
         int prev = -1;
         for (std::vector<int>::const_iterator it = interface->faces.begin();
              it != interface->faces.end(); ++it)
         {
            if ((*it) < 0)
            {
               continue;
   ...
   Mesh* BuildSerialMesh(const int attribute, const int subdomainNumElements,
                         const SubdomainInterface *interface)
   {
      ...
      const int elGeom = (mode == SubdomainMesh) ? pmesh->GetElementBaseGeometry(
                            0) : pmesh->GetFaceBaseGeometry(0);
      idx = 0;
      int ielem = 0;
      for (int p=0; p<num_procs; ++p)
      {
         for (int i=0; i<procNumElems[p]; ++i, ++ielem)
         {
            ...
            if (mode == SubdomainMesh)
            {
               sel->SetAttribute(element_pmeshId[ielem] +
                                 1);   // Add 1 to ensure positive attribute, then subtract 1 later.
            }
            else
            {
   ...
   Mesh* CreateSerialSubdomainOrInterfaceMesh(int& elemOffset, const int attribute,
                                              const SubdomainInterface *interface, const int mustBuild=0)
   {
      int numLocalElements = (interface == NULL) ? NumberOfLocalElementsForSubdomain(
                                attribute) : interface->NumFaces();

      int subdomainNumElements = 0;
      GatherSubdomainOrInterfaceMeshData(attribute, numLocalElements,
                                         subdomainNumElements, elemOffset, interface);
...
public:
      ...
      numSubdomains(numSubdomains_), pmesh(pmesh_), d(pmesh_->Dimension())
   {
      ...
      h1_coll = new H1_FECollection(1,
                                    d);  // Must be first order, to get a bijection between vertices and DOF's.
      H1_space = new ParFiniteElementSpace(pmesh_,
   ...
   ParMesh** CreateParallelSubdomainMeshes()
   {
      ...
      MPI_Allreduce(procTouchesSubdomain.data(), numProcsPerSubdomain.data(),
                    numSubdomains, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
      MPI_Allreduce(&numSubdomainsOnProc, &maxNumSubdomainsOnProc, 1, MPI_INT,
      ...
      MFEM_VERIFY(maxNumSubdomainsOnProc == 1 ||
                  maxNumProcsPerSubdomain == 1, "");  // Case (3) not allowed.

      MPI_Comm sd_com;
      int color = -1;

      if (maxNumSubdomainsOnProc ==
          1)  // Case (1): one or more processes per subdomain.
      {
   ...
   Mesh* CreateSerialInterfaceMesh(int& elemOffset, SubdomainInterface& interface,
                                   const int mustBuild=0)
   {
      mode = InterfaceMesh;
      ...
      return CreateSerialSubdomainOrInterfaceMesh(elemOffset,
                                                  interface.GetGlobalIndex(), &interface, mustBuild);
   }
