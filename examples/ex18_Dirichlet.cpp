...
VectorFunctionCoefficient EulerInitialCondition(
    const int problem, const double specific_heat_ratio,
...
int main(int argc, char *argv[]) {
  ...
  DGHyperbolicConservationLaws euler =
      getEulerSystem(&vfes, numericalFlux, specific_heat_ratio, IntOrderOffset);
  ...
  switch (problem) {
    case 1:  // fast moving vortex
      return VectorFunctionCoefficient(
          4, [specific_heat_ratio, gas_constant](const Vector &x, Vector &y) {
    ...
    case 5:
      return VectorFunctionCoefficient(4, [](const Vector &x, Vector &y) {
        ...
        const double velocity_y = abs(x(1)) < 0.25
                                      ? 0.01 * sin(M_PI * x(0) / L)
                                      : 0.01 * sin(M_PI * x(0) / L);
