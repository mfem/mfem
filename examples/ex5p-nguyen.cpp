...
int main(int argc, char *argv[])
{
   ...
   if (hybridization)
   {
      ...
      chrono.Stop();
      if (verbose) { std::cout << "Hybridization init took " << chrono.RealTime() << "s.\n"; }
   }
   else if (reduction)
   {
      chrono.Clear();
      ...
      chrono.Stop();
      if (verbose) { std::cout << "Reduction init took " << chrono.RealTime() << "s.\n"; }
   }

   if (pa) { darcy->SetAssemblyLevel(AssemblyLevel::PARTIAL); }

   // 10. Define the BlockStructure of the problem, i.e. define the array of
   ...
   // 11. Allocate memory (x, rhs) for the analytical solution and the right hand
