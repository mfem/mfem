...
int main(int argc, char *argv[])
{
   ...
   else
   {
      ...
      {
         HypreParMatrix &Mum = *mVarf->ParallelAssembleInternalMatrix();
         Vector Md;
         Mum.GetDiag(Md);
         ...
         if (mpVarf)
         {
            HypreParMatrix &Mpm = *mpVarf->ParallelAssembleInternalMatrix();
            HypreParMatrix *Snew = ParAdd(&Mpm, S);
            delete S;
            S = Snew;
         }

         invM = new HypreDiagScale(Mum);
