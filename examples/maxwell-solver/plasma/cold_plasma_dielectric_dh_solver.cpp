     ...
     visit_dc_(NULL)
{
   ...
   */
   ...
   if ( myid_ == 0 )
   {
      std::cout << "Adding CurlCurl domain integrator with epsInvCoef_" << std::endl;
      std::cout << "Adding VectorFEMass domain integrator with massCoef_" << std::endl;
   }
   ...
   if (kReCoef_ || kImCoef_)
   {
      ...
      if ( myid_ == 0 )
      {
         std::cout << "Adding VectorFEMass domain integrator with kekCoef_" << std::endl;
         std::cout << "Adding MixedVectorCurlIntegrator domain integrator with keCoef_" << std::endl;
         std::cout << "Adding MixedVectorWeakCurlIntegrator domain integrator with ekCoef_" << std::endl;
      }
   ...
   if ( abcCoef_ )
   {
      ...
      if ( myid_ == 0 )
      {
         std::cout << "Adding VectorFEMass boundary integrator with abcCoef_" << std::endl;
      }
...
CPDSolverDH::Solve()
{
   ...
   *h_ = 0.0;
   ...
   int direct_sol = 2; // choice between SuperLU (0) and MUMPS (1) DST (2)
   Solver * AInv = nullptr;
   ...
   {
      ParDST * S = nullptr;
      Array2D<double> lengths(3,2); lengths = 0.;
      double omega = 1.0;
      int nrlayers = 2;
      MatrixComplexCoefficient * c1 = new MatrixComplexCoefficient(
         epsInvReCoef_,epsInvReCoef_);
      ComplexCoefficient * c2 = new ComplexCoefficient(massCoef_,nullptr);
      int nx = 1;
      int ny = 1;
      int nz = 1;
      ParDST::BCType bct = ParDST::BCType::NEUMANN;
      S = new ParDST(a1_,lengths, omega_, nrlayers, 
                  nullptr, c2, c1, nullptr,
                  nx, ny, nz, bct);        
      AInv = new GMRESSolver(MPI_COMM_WORLD);
      dynamic_cast<GMRESSolver *>(AInv)->SetPreconditioner(*S);
      dynamic_cast<GMRESSolver *>(AInv)->SetOperator(*A1);
      dynamic_cast<GMRESSolver *>(AInv)->SetRelTol(1e-8);
      dynamic_cast<GMRESSolver *>(AInv)->SetMaxIter(20);
      dynamic_cast<GMRESSolver *>(AInv)->SetPrintLevel(1);                    
   }
