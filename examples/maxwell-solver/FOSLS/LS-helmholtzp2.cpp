...
int main(int argc, char *argv[])
{
   ...
   // |   |            p             |           u           |   RHS    | 
   // -------------------------------------------------------------------
   // | q | (gradp,gradq) + w^4(p,q) | w^2(divu,q)-(u, gradq) | w^2(f,q) |
   // |   |                          |                        |          |
   // | v | w^2(p,divv) - (gradp,v)   | (divu,divv) + (u,v)   | (f,divv) |
   ...
   b_q.AddDomainIntegrator(new DomainLFIntegrator(omega2_f));
   // (f, div v)
   ...
   // (grad p, grad q) + \omega^4 (p,q)
   a_qp.AddDomainIntegrator(new DiffusionIntegrator(one));
   a_qp.AddDomainIntegrator(new MassIntegrator(omeg4));
...
#else   
   // w^2(divu,q)
   a_qu.AddDomainIntegrator(new MixedScalarDivergenceIntegrator(omeg2));
#endif
   // -(u, gradq)
   a_qu.AddDomainIntegrator(new MixedVectorWeakDivergenceIntegrator(one));
   // w^2(p,divv) - (gradp,v)
...
#else
   // w^2(p,divv)
   a_vp.AddDomainIntegrator(new MixedScalarWeakGradientIntegrator(negomeg2));
#endif
   // - (gradp,v)
   a_vp.AddDomainIntegrator(new MixedVectorGradientIntegrator(negone));

   ParBilinearForm a_vu(RTfespace);
   a_vu.AddDomainIntegrator(new DivDivIntegrator(one));
   a_vu.AddDomainIntegrator(new VectorFEMassIntegrator(one));
