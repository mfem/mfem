 ...
 */
fptype proj(ParGridFunction &psi, fptype target_volume, fptype tol=1e-12,
            int max_its=10)
{
   ...
   for (int k=0; k<max_its; k++) // Newton iteration
   {
      ...
      fptype f = int_sigmoid_psi.Sum();
      MPI_Allreduce(MPI_IN_PLACE, &f, 1, MPITypeMap<fptype>::mpi_type,
                    MPI_SUM, MPI_COMM_WORLD);
      f -= target_volume;

      int_der_sigmoid_psi.Assemble(); // Recompute df(c) with updated ψ
      fptype df = int_der_sigmoid_psi.Sum();
      MPI_Allreduce(MPI_IN_PLACE, &df, 1, MPITypeMap<fptype>::mpi_type,
                    MPI_SUM, MPI_COMM_WORLD);
   ...
   fptype material_volume = int_sigmoid_psi.Sum();
...
int main(int argc, char *argv[])
{
   ...
   for (int k = 1; k <= max_it; k++)
   {
      ...
      MPI_Allreduce(MPI_IN_PLACE, &compliance, 1, MPITypeMap<fptype>::mpi_type,
                    MPI_SUM, MPI_COMM_WORLD);
      if (myid == 0)
      {
