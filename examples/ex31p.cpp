...
int main(int argc, char *argv[])
{
   ...
   {
      fptype error = sol.ComputeHCurlError(&E, &CurlE);
      if (Mpi::Root())
      {
...
void CurlE_exact(const Vector &x, Vector &dE)
{
   if (dim == 1)
   {
      fptype c4 = cos(kappa * x(0) + 0.4 * M_PI);
      fptype c9 = cos(kappa * x(0) + 0.9 * M_PI);
   ...
   {
      fptype c0 = cos(kappa * M_SQRT1_2 * (x(0) + x(1)) + 0.0 * M_PI);
      fptype c4 = cos(kappa * M_SQRT1_2 * (x(0) + x(1)) + 0.4 * M_PI);
      fptype c9 = cos(kappa * M_SQRT1_2 * (x(0) + x(1)) + 0.9 * M_PI);
   ...
   {
      fptype s0 = sin(kappa * M_SQRT1_2 * (x(0) + x(1)) + 0.0 * M_PI);
      fptype c0 = cos(kappa * M_SQRT1_2 * (x(0) + x(1)) + 0.0 * M_PI);
      fptype s4 = sin(kappa * M_SQRT1_2 * (x(0) + x(1)) + 0.4 * M_PI);
      fptype c4 = cos(kappa * M_SQRT1_2 * (x(0) + x(1)) + 0.4 * M_PI);
      fptype c9 = cos(kappa * M_SQRT1_2 * (x(0) + x(1)) + 0.9 * M_PI);
      fptype sk = sin(kappa * x(2));
      fptype ck = cos(kappa * x(2));
...
void f_exact(const Vector &x, Vector &f)
{
   if (dim == 1)
   {
      fptype s0 = sin(kappa * x(0) + 0.0 * M_PI);
      fptype s4 = sin(kappa * x(0) + 0.4 * M_PI);
      fptype s9 = sin(kappa * x(0) + 0.9 * M_PI);
   ...
   else if (dim == 2)
   {
      fptype s0 = sin(kappa * M_SQRT1_2 * (x(0) + x(1)) + 0.0 * M_PI);
      fptype s4 = sin(kappa * M_SQRT1_2 * (x(0) + x(1)) + 0.4 * M_PI);
      fptype s9 = sin(kappa * M_SQRT1_2 * (x(0) + x(1)) + 0.9 * M_PI);
