...
int main(int argc, char *argv[])
{
   ...
   // 5.5 Set device config parameters from the command line options and switch
   ...
   m.Assemble();
   m.Finalize(); //Assemble mass integrator...
   ...
   //Project onto bernstein basis functions...
   L2_FECollection l2_fec(order, dim);
   FiniteElementSpace l2_fes(&mesh, &l2_fec);
 
   GridFunction u_l2(&l2_fes);
   u_l2.ProjectCoefficient(u0);
 
   GridFunction u(&fes);
   u.ProjectGridFunction(u_l2);
   {
