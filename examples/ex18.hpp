...
extern fptype max_char_speed;

extern const int num_equation;
extern const fptype specific_heat_ratio;
extern const fptype gas_constant;
...
public:
   RiemannSolver();
   fptype Eval(const Vector &state1, const Vector &state2,
...
inline fptype ComputePressure(const Vector &state, int dim)
{
   const fptype den = state(0);
   const Vector den_vel(state.GetData() + 1, dim);
   const fptype den_energy = state(1 + dim);
...
void ComputeFlux(const Vector &state, int dim, DenseMatrix &flux)
{
   ...
   const fptype pres = ComputePressure(state, dim);
   ...
   const fptype H = (den_energy + pres) / den;
   for (int d = 0; d < dim; d++)
   {
...
inline fptype ComputeMaxCharSpeed(const Vector &state, const int dim)
{
   const fptype den = state(0);
   const Vector den_vel(state.GetData() + 1, dim);
   ...
   const fptype pres = ComputePressure(state, dim);
   const fptype sound = sqrt(specific_heat_ratio * pres / den);
   const fptype vel = sqrt(den_vel2 / den);
...
void FE_Evolution::GetFlux(const DenseMatrix &x_, DenseTensor &flux_) const
{
   ...
   for (int i = 0; i < flux_dof; i++)
   {
      ...
      const fptype mcs = ComputeMaxCharSpeed(state, flux_dim);
...
fptype RiemannSolver::Eval(const Vector &state1, const Vector &state2,
                           const Vector &nor, Vector &flux)
{
   // NOTE: nor in general is not a unit normal
   const int dim = nor.Size();
   ...
   const fptype maxE1 = ComputeMaxCharSpeed(state1, dim);
   const fptype maxE2 = ComputeMaxCharSpeed(state2, dim);

   const fptype maxE = max(maxE1, maxE2);
                                        ...
                                        const Vector &elfun, Vector &elvect)
{
   ...
   for (int i = 0; i < ir->GetNPoints(); i++)
   {
      ...
      const fptype mcs = rsolver.Eval(funval1, funval2, nor, fluxN);
...
bool StateIsPhysical(const Vector &state, const int dim)
{
   ...
   const fptype pres = (specific_heat_ratio - 1.0) * (den_energy - 0.5 * den_vel2);
...
void InitialCondition(const Vector &x, Vector &y)
{
   ...
   const fptype xc = 0.0, yc = 0.0;

   // Nice units
   const fptype vel_inf = 1.;
   const fptype den_inf = 1.;

   // Derive remainder of background state from this and Minf
   const fptype pres_inf = (den_inf / specific_heat_ratio) * (vel_inf / Minf) *
                           (vel_inf / Minf);
   const fptype temp_inf = pres_inf / (den_inf * gas_constant);
   ...
   const fptype shrinv1 = 1.0 / (specific_heat_ratio - 1.);

   const fptype velX = vel_inf * (1 - beta * (x(1) - yc) / radius * exp(
                                     -0.5 * r2rad));
   const fptype velY = vel_inf * beta * (x(0) - xc) / radius * exp(-0.5 * r2rad);
   const fptype vel2 = velX * velX + velY * velY;

   const fptype specific_heat = gas_constant * specific_heat_ratio * shrinv1;
   const fptype temp = temp_inf - 0.5 * (vel_inf * beta) *
                       (vel_inf * beta) / specific_heat * exp(-r2rad);

   const fptype den = den_inf * pow(temp/temp_inf, shrinv1);
   const fptype pres = den * gas_constant * temp;
   const fptype energy = shrinv1 * pres / den + 0.5 * vel2;
