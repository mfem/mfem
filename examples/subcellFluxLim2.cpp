...
public:
  ...
  double xFlux(const double& u_vel, const double& soln, const int &PROBLEM_TYPE);
  double yFlux(const double& v_vel, const double& soln, const int &PROBLEM_TYPE);
  double xEntFlux(const double& u_vel, const double& soln, const int &PROBLEM_TYPE);
  double yEntFlux(const double& v_vel, const double& soln, const int &PROBLEM_TYPE);
...
double FE_Evolution::xFlux(const double& u_vel, const double& soln, const int& PROBLEM_TYPE)
{
  return u_vel*soln;
}

double FE_Evolution::yFlux(const double& v_vel, const double& soln, const int& PROBLEM_TYPE)
{
  return v_vel*soln;
}

double FE_Evolution::xEntFlux(const double& u_vel, const double& soln, const int& PROBLEM_TYPE)
{
  return u_vel*ENTROPY(soln);
}

double FE_Evolution::yEntFlux(const double& v_vel, const double& soln, const int& PROBLEM_TYPE)
{
  ...
  for (int i=0; i<dofs_per_cell; i++)
    {
      ...
      double dist_to_B_left  = fabs(DOFs_coord(0,i)-0.0);
      double dist_to_B_right = fabs(DOFs_coord(0,i)-1.0);
	  ...
	  if (i==j)
	    {
	      ...
	      if (dist_to_B_left <= tol || dist_to_B_right <= tol)
		element_MC_Q1(i,j) = 1.0; //0.75
	      else
...
void FE_Evolution::compute_uDot(GridFunction &un)
{
  ...
  for (int eN=0; eN<nElements; ++eN)
      ...
      for (int q=0; q<ir->GetNPoints(); ++q)
	{
	  ...
	  double u_vel = 1.0;
	  double v_vel = 0.0;
	  double vel_times_grad_un = u_vel * grad_un[0] + v_vel * grad_un[1]; //PROBLEM_TYPE=0
	  
	  for (unsigned int i=0; i<dofs_per_cell; ++i)
	    {
	      cell_rhs_uDot[i] += vel_times_grad_un * shape[i] * detJxW;
	    }
	}
      // distribute
...
void FE_Evolution::compute_solution(GridFunction &unp1, GridFunction &un)
{
  ...
  Vector elementMass(dofs_per_cell), elementFluxCorrection(dofs_per_cell);
  for (int eN=0; eN<nElements; ++eN)
    {
      // get local to global dofs, fe space, etc
      fespace->GetElementDofs(eN,local_dofs_indices);
      ...
      for (int q=0; q < ir->GetNPoints(); ++q)
	{
	  ...
	  double y_vel = 0.0;
	  Vector flux(dim);
	  flux[0] = xFlux(x_vel,uhn,PROBLEM_TYPE);
	  flux[1] = xFlux(y_vel,uhn,PROBLEM_TYPE);

	  for (unsigned int i=0; i<dofs_per_cell; ++i)
	    {
	      grad_shape.GetRow(i,grad_shape_i);
	      elementMass[i] += shape[i]*detJxW;
	      elementFluxCorrection[i] += -uhDot*shape[i]*detJxW + (flux*grad_shape_i)*detJxW;
	    }	  
	}
      ...
      for (unsigned int i=0; i<dofs_per_cell; ++i)
	{
	  ...
	  double qi = elementMass[i]*uDot[gi] + elementFluxCorrection[i];
	  ...
	  for (unsigned int j=0; j < dofs_per_cell; ++j)
	      ...
	      else
		{
	      ...
	      qi -= ((CxElem[eN_i_j]-PrCxElem[eN_i_j])*fxj + (CyElem[eN_i_j]-PrCyElem[eN_i_j])*fyj);
	  ...
	  // min and max 
	  umax[gi] = fmax(umax[i],umaxi);
	  umin[gi] = fmin(umin[i],umini);
