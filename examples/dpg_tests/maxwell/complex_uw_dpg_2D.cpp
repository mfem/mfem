...
double hatH_exact_r(const Vector & X);
double hatH_exact_i(const Vector & X);
...
int main(int argc, char *argv[])
{
   ...
   dim = mesh.Dimension();
   dimc = (dim == 3) ? 3 : 1;
   MFEM_VERIFY(dim == 2, "Only 2D maxwell implemented for now");
   ...
   FiniteElementSpace *H_fes = new FiniteElementSpace(&mesh,H_fec); 
...
// test integrators 
   ...
   //space-induced norm for H1 × H(curl)
   // (∇F,∇δF)
   ...
   FunctionCoefficient hatHex_r(hatH_exact_r);
   FunctionCoefficient hatHex_i(hatH_exact_i);
