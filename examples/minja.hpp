...
private:
   using ObjectType =
      nlohmann::ordered_map<json, Value>;  // Only contains primitive keys
   ...
   static void dump_string(const json & primitive, std::ostringstream & out,
                           char string_quote = '\'')
   {
...
public:
   ...
   bool operator<(const Value & other) const
   {
      ...
      throw std::runtime_error("Cannot compare values: " + dump() + " < " +
                               other.dump());
   }
      ...
      if (array_)
      {
         ...
         for (size_t i = 0; i < array_->size(); ++i)
         {
            if (!(*array_)[i].to_bool() || !(*other.array_)[i].to_bool() ||
                (*array_)[i] != (*other.array_)[i]) { return false; }
         }
         return true;
      }
      else if (object_)
      {
         ...
         for (const auto& item : *object_)
         {
            if (!item.second.to_bool() || !other.object_->count(item.first) ||
                item.second != other.object_->at(item.first)) { return false; }
         }
         return true;
      }
      else
   ...
   bool contains(const std::string & key) const
   {
      ...
      {
         throw std::runtime_error("contains can only be called on arrays and objects: " +
                                  dump());
      }
   }
...
inline json Value::get<json>() const
{
   ...
   if (object_)
   {
      ...
      for (const auto& [key, value] : *object_)
      {
         ...
         {
            throw std::runtime_error("Invalid key type for conversion to JSON: " +
                                     key.dump());
         }
      }
      if (is_callable())
   ...
   throw std::runtime_error("get<json> not defined for this value type: " +
                            dump());
}
...
public:
   Context(Value && values,
           const std::shared_ptr<Context> & parent = nullptr) : values_(std::move(values)),
      parent_(parent)
   {
...
public:
   ...
   TemplateToken(Type type, const Location & location, SpaceHandling pre,
                 SpaceHandling post) : type(type), location(location), pre_space(pre),
      post_space(post) {}
...
struct TextTemplateToken : public TemplateToken
{
   ...
   TextTemplateToken(const Location & loc, SpaceHandling pre, SpaceHandling post,
                     const std::string& t) : TemplateToken(Type::Text, loc, pre, post), text(t) {}
};
...
struct ExpressionTemplateToken : public TemplateToken
{
   ...
   ExpressionTemplateToken(const Location & loc, SpaceHandling pre,
                           SpaceHandling post, std::shared_ptr<Expression> && e) : TemplateToken(
                                 Type::Expression, loc, pre, post), expr(std::move(e)) {}
};
...
struct ElifTemplateToken : public TemplateToken
{
   ...
   ElifTemplateToken(const Location & loc, SpaceHandling pre, SpaceHandling post,
                     std::shared_ptr<Expression> && c) : TemplateToken(Type::Elif, loc, pre, post),
      condition(std::move(c)) {}
};

struct ElseTemplateToken : public TemplateToken
{
   ElseTemplateToken(const Location & loc, SpaceHandling pre,
                     SpaceHandling post) : TemplateToken(Type::Else, loc, pre, post) {}
};

struct EndIfTemplateToken : public TemplateToken
{
   EndIfTemplateToken(const Location & loc, SpaceHandling pre,
                      SpaceHandling post) : TemplateToken(Type::EndIf, loc, pre, post) {}
};
...
struct EndMacroTemplateToken : public TemplateToken
{
   EndMacroTemplateToken(const Location & loc, SpaceHandling pre,
                         SpaceHandling post) : TemplateToken(Type::EndMacro, loc, pre, post) {}
};
...
struct EndFilterTemplateToken : public TemplateToken
{
   EndFilterTemplateToken(const Location & loc, SpaceHandling pre,
                          SpaceHandling post) : TemplateToken(Type::EndFilter, loc, pre, post) {}
};
...
struct EndSetTemplateToken : public TemplateToken
{
   EndSetTemplateToken(const Location & loc, SpaceHandling pre,
                       SpaceHandling post) : TemplateToken(Type::EndSet, loc, pre, post) {}
};
...
struct CommentTemplateToken : public TemplateToken
{
   ...
   CommentTemplateToken(const Location & loc, SpaceHandling pre,
                        SpaceHandling post, const std::string& t) : TemplateToken(Type::Comment, loc,
                                                                                     pre, post), text(t) {}
};
...
public:
   ...
   LoopControlException(const std::string & message,
                        LoopControlType control_type) : std::runtime_error(message),
      control_type(control_type) {}
   LoopControlException(LoopControlType control_type)
      : std::runtime_error((control_type == LoopControlType::Continue ? "continue" :
                            "break") + std::string(" outside of a loop")),
        control_type(control_type) {}
};
...
struct LoopControlTemplateToken : public TemplateToken
{
   ...
   LoopControlTemplateToken(const Location & loc, SpaceHandling pre,
                            SpaceHandling post, LoopControlType control_type) : TemplateToken(Type::Break,
                                     loc, pre, post), control_type(control_type) {}
};

struct CallTemplateToken : public TemplateToken
{
   std::shared_ptr<Expression> expr;
   CallTemplateToken(const Location & loc, SpaceHandling pre, SpaceHandling post,
...
{
   EndCallTemplateToken(const Location & loc, SpaceHandling pre,
                        SpaceHandling post)
...
class TemplateNode
{
   Location location_;
protected:
   virtual void do_render(std::ostringstream & out,
...
public:
   ...
   void do_render(std::ostringstream & out,
...
class TextNode : public TemplateNode
{
   std::string text;
public:
   TextNode(const Location & loc, const std::string& t) : TemplateNode(loc),
      text(t) {}
   void do_render(std::ostringstream & out,
...
class ExpressionNode : public TemplateNode
{
   std::shared_ptr<Expression> expr;
public:
   ExpressionNode(const Location & loc,
                  std::shared_ptr<Expression> && e) : TemplateNode(loc), expr(std::move(e)) {}
   void do_render(std::ostringstream & out,
                  const std::shared_ptr<Context> & context) const override
   {
...
public:
   LoopControlNode(const Location & loc,
                   LoopControlType control_type) : TemplateNode(loc),
...
public:
   MacroNode(const Location & loc, std::shared_ptr<VariableExpr> && n,
      ...
      : TemplateNode(loc), name(std::move(n)), params(std::move(p)),
        body(std::move(b))
   {
                  ...
                  const std::shared_ptr<Context> & macro_context) const override
   {
      ...
      auto callable = Value::callable([this,
                                       macro_context](const std::shared_ptr<Context> & call_context,
                                                      ArgumentsValue & args)
      {
...
public:
   ...
   Value do_evaluate(const std::shared_ptr<Context> & context) const override
   {
      ...
      if (auto slice = dynamic_cast<SliceExpr*>(index.get()))
      {
         ...
         int64_t start = slice->start ? wrap(slice->start->evaluate(
                                                context).get<int64_t>()) : (step < 0 ? len - 1 : 0);
      ...
      else
      {
         ...
         if (target_value.is_null())
         {
            ...
            {
               throw std::runtime_error("'" + t->get_name() + "' is " + (context->contains(
                                                                            t->get_name()) ? "null" : "not defined"));
            }
            throw std::runtime_error("Trying to access property '" +  index_value.dump() +
                                     "' on null!");
         }
...
{
   return (((container.is_array() || container.is_object()) &&
...
public:
   ...
   Value do_evaluate(const std::shared_ptr<Context> & context) const override
   {
      ...
      if (obj.is_null())
      {
         throw std::runtime_error("Trying to call method '" + method->get_name() +
                                  "' on null");
      }
      if (obj.is_array())
      {
      ...
      else if (obj.is_object())
      {
         ...
         else if (obj.contains(method->get_name()))
         {
            ...
            {
               throw std::runtime_error("Property '" + method->get_name() +
                                        "' is not callable");
            }
...
public:
   CallNode(const Location & loc, std::shared_ptr<Expression> && e,
            std::shared_ptr<TemplateNode> && b)
                  ...
                  const std::shared_ptr<Context> & context) const override
   {
      ...
      auto caller = Value::callable([this, context](const std::shared_ptr<Context> &,
...
private:
   ...
   std::string consumeToken(const std::regex & regex,
                            SpaceHandling space_handling = SpaceHandling::Strip)
   {
      auto start = it;
      consumeSpaces(space_handling);
   ...
   std::shared_ptr<Expression> parseLogicalOr()
   {
      ...
      while (!consumeToken(or_tok).empty())
      {
         ...
         left = std::make_shared<BinaryOpExpr>(location, std::move(left),
                                               std::move(right), BinaryOpExpr::Op::Or);
      }
      return left;
   }
   ...
   std::shared_ptr<Expression> parseLogicalAnd()
   {
      ...
      while (!consumeToken(and_tok).empty())
      {
         ...
         left = std::make_shared<BinaryOpExpr>(location, std::move(left),
                                               std::move(right), BinaryOpExpr::Op::And);
      }
      return left;
   }
   ...
   std::shared_ptr<Expression> parseStringConcat()
   {
      ...
      if (!consumeToken(concat_tok).empty())
      {
         ...
         left = std::make_shared<BinaryOpExpr>(get_location(), std::move(left),
                                               std::move(right), BinaryOpExpr::Op::StrConcat);
      }
      return left;
   }
                                         ...
                                         ArgumentsExpression && args) const
   {
      return std::make_shared<CallExpr>(get_location(),
                                        std::make_shared<VariableExpr>(get_location(), name), std::move(args));
   }
   ...
   std::shared_ptr<Expression> parseExpansion()
   {
      ...
      return std::make_shared<UnaryOpExpr>(get_location(), std::move(expr),
                                           op_str == "*" ? UnaryOpExpr::Op::Expansion : UnaryOpExpr::Op::ExpansionDict);
   }
   ...
   std::shared_ptr<Expression> parseValueExpression()
   {
      ...
      while (it != end && consumeSpaces() && peekSymbols({ "[", "." }))
      {
         if (!consumeToken("[").empty())
         {
            ...
            {
               index = std::make_shared<SliceExpr>(slice_loc, std::move(start), std::move(end),
                                                   std::move(step));
            }
            else
            {
         ...
         else if (!consumeToken(".").empty())
         {
            ...
            if (peekSymbols({ "(" }))
            {
               ...
               value = std::make_shared<MethodCallExpr>(identifier->location, std::move(value),
                                                        std::move(identifier), std::move(callParams));
            }
            else
            {
               auto key = std::make_shared<LiteralExpr>(identifier->location,
                                                        Value(identifier->get_name()));
               value = std::make_shared<SubscriptExpr>(identifier->location, std::move(value),
                                                       std::move(key));
            }
         }
         consumeSpaces();
   ...
   {
      return std::runtime_error("Unexpected " + TemplateToken::typeToString(
                                   token.type)
                                + error_location_suffix(*template_str, token.location.pos));
   }
   std::runtime_error unterminated(const TemplateToken & token) const
   {
      return std::runtime_error("Unterminated " + TemplateToken::typeToString(
                                   token.type)
                                + error_location_suffix(*template_str, token.location.pos));
   }
   ...
   TemplateTokenVector tokenize()
   {
      ...
      static std::regex block_keyword_tok(
         R"((if|else|elif|endif|for|endfor|generation|endgeneration|set|endset|block|endblock|macro|endmacro|filter|endfilter|break|continue|call|endcall)\b)");
      ...
      try
      {
         while (it != end)
         {
                                                  ...
                                                  SpaceHandling::Keep)).empty())
            {
               ...
               tokens.push_back(std::make_unique<ExpressionTemplateToken>(location, pre_space,
                                                                          post_space, std::move(expr)));
            }
                                                  ...
                                                  SpaceHandling::Keep)).empty())
            {
               ...
               else if (keyword == "elif")
               {
                  ...
                  auto post_space = parseBlockClose();
                  tokens.push_back(std::make_unique<ElifTemplateToken>(location, pre_space,
                                                                       post_space, std::move(condition)));
               }
               else if (keyword == "else")
               {
                  auto post_space = parseBlockClose();
                  tokens.push_back(std::make_unique<ElseTemplateToken>(location, pre_space,
                                                                       post_space));
               }
               else if (keyword == "endif")
               {
                  auto post_space = parseBlockClose();
                  tokens.push_back(std::make_unique<EndIfTemplateToken>(location, pre_space,
                                                                        post_space));
               }
               else if (keyword == "for")
               {
                  ...
                  tokens.push_back(std::make_unique<ForTemplateToken>(location, pre_space,
                                                                      post_space, std::move(varnames), std::move(iterable), std::move(condition),
                                                                      recursive));
               }
               else if (keyword == "endfor")
               {
                  auto post_space = parseBlockClose();
                  tokens.push_back(std::make_unique<EndForTemplateToken>(location, pre_space,
                                                                         post_space));
               }
               else if (keyword == "generation")
               {
                  auto post_space = parseBlockClose();
                  tokens.push_back(std::make_unique<GenerationTemplateToken>(location, pre_space,
                                                                             post_space));
               }
               ...
               else if (keyword == "set")
               {
                  ...
                  tokens.push_back(std::make_unique<SetTemplateToken>(location, pre_space,
                                                                      post_space, ns, var_names, std::move(value)));
               }
               else if (keyword == "endset")
               {
                  auto post_space = parseBlockClose();
                  tokens.push_back(std::make_unique<EndSetTemplateToken>(location, pre_space,
                                                                         post_space));
               }
               else if (keyword == "macro")
               {
                  ...
                  auto post_space = parseBlockClose();
                  tokens.push_back(std::make_unique<MacroTemplateToken>(location, pre_space,
                                                                        post_space, std::move(macroname), std::move(params)));
               }
               else if (keyword == "endmacro")
               {
                  auto post_space = parseBlockClose();
                  tokens.push_back(std::make_unique<EndMacroTemplateToken>(location, pre_space,
                                                                           post_space));
               }
               else if (keyword == "call")
               {
                  ...
                  auto post_space = parseBlockClose();
                  tokens.push_back(std::make_unique<CallTemplateToken>(location, pre_space,
                                                                       post_space, std::move(expr)));
               }
               else if (keyword == "endcall")
               {
                  auto post_space = parseBlockClose();
                  tokens.push_back(std::make_unique<EndCallTemplateToken>(location, pre_space,
                                                                          post_space));
               }
               else if (keyword == "filter")
               {
                  ...
                  auto post_space = parseBlockClose();
                  tokens.push_back(std::make_unique<FilterTemplateToken>(location, pre_space,
                                                                         post_space, std::move(filter)));
               }
               else if (keyword == "endfilter")
               {
                  auto post_space = parseBlockClose();
                  tokens.push_back(std::make_unique<EndFilterTemplateToken>(location, pre_space,
                                                                            post_space));
               }
               else if (keyword == "break" || keyword == "continue")
               {
                  auto post_space = parseBlockClose();
                  tokens.push_back(std::make_unique<LoopControlTemplateToken>(location, pre_space,
                                                                              post_space, keyword == "break" ? LoopControlType::Break :
            ...
            else if (std::regex_search(it, end, match, non_text_open_regex))
            {
               ...
               it = text_end;
               tokens.push_back(std::make_unique<TextTemplateToken>(location,
                                                                    SpaceHandling::Keep, SpaceHandling::Keep, text));
            }
            else
            {
      ...
      {
         throw std::runtime_error(e.what() + error_location_suffix(*template_str,
      ...
      bool fully = false) const
   {
      ...
      while (it != end)
      {
         ...
         if (auto if_token = dynamic_cast<IfTemplateToken*>(token.get()))
         {
            std::vector<std::pair<std::shared_ptr<Expression>, std::shared_ptr<TemplateNode>>>
            cascade;
            cascade.emplace_back(std::move(if_token->condition), parseTemplate(begin, it,
                                                                               end));

            while (it != end && (*it)->type == TemplateToken::Type::Elif)
            {
               auto elif_token = dynamic_cast<ElifTemplateToken*>((*(it++)).get());
               cascade.emplace_back(std::move(elif_token->condition), parseTemplate(begin, it,
                                                                                    end));
            }
            ...
            children.emplace_back(std::make_shared<IfNode>(token->location,
                                                           std::move(cascade)));
         }
         else if (auto for_token = dynamic_cast<ForTemplateToken*>(token.get()))
         {
            ...
            children.emplace_back(std::make_shared<ForNode>(token->location,
                                                            std::move(for_token->var_names), std::move(for_token->iterable),
                                                            std::move(for_token->condition), std::move(body), for_token->recursive,
                                                            std::move(else_body)));
         }
         ...
         {
            SpaceHandling pre_space = (it - 1) != begin ? (*(it - 2))->post_space :
                                      SpaceHandling::Keep;
         ...
         else if (auto expr_token = dynamic_cast<ExpressionTemplateToken*>(token.get()))
         {
            children.emplace_back(std::make_shared<ExpressionNode>(token->location,
                                                                   std::move(expr_token->expr)));
         }
         else if (auto set_token = dynamic_cast<SetTemplateToken*>(token.get()))
         {
            if (set_token->value)
            {
               children.emplace_back(std::make_shared<SetNode>(token->location, set_token->ns,
                                                               set_token->var_names, std::move(set_token->value)));
            }
            else
            {
               ...
               children.emplace_back(std::make_shared<SetTemplateNode>(token->location, name,
         ...
         else if (auto macro_token = dynamic_cast<MacroTemplateToken*>(token.get()))
         {
            ...
            children.emplace_back(std::make_shared<MacroNode>(token->location,
                                                              std::move(macro_token->name), std::move(macro_token->params), std::move(body)));
         }
         else if (auto call_token = dynamic_cast<CallTemplateToken*>(token.get()))
         {
            ...
            children.emplace_back(std::make_shared<CallNode>(token->location,
                                                             std::move(call_token->expr), std::move(body)));
         }
         else if (auto filter_token = dynamic_cast<FilterTemplateToken*>(token.get()))
         {
            ...
            children.emplace_back(std::make_shared<FilterNode>(token->location,
                                                               std::move(filter_token->filter), std::move(body)));
         }
         else if (dynamic_cast<CommentTemplateToken*>(token.get()))
         {
            // Ignore comments
         }
         else if (auto ctrl_token = dynamic_cast<LoopControlTemplateToken*>
                                    (token.get()))
         {
...
public:
   ...
   {
      Parser parser(std::make_shared<std::string>(normalize_newlines(template_str)),
                    options);
...
static Value simple_function(const std::string & fn_name,
                             ...
                             fn)
{
                              ...
                              ArgumentsValue & args) -> Value
   {
      ...
      for (auto & [name, value] : args.kwargs)
      {
         ...
         {
            throw std::runtime_error("Unknown argument " + name + " for function " +
                                     fn_name);
         }
...
inline std::shared_ptr<Context> Context::builtins()
{
   ...
   {
      return Value(args.at("value").dump(args.get<int64_t>("indent",
                                                           -1), /* to_json= */ true));
   }));
   ...
   {
      return simple_function(name, { "text" }, [=](const std::shared_ptr<Context> &,
                                                   Value & args)
      {
         auto text = args.at("text");
   ...
   auto escape = simple_function("escape", { "text" }, [](const
                                                          std::shared_ptr<Context> &, Value & args)
   {
                                            ...
                                            const std::shared_ptr<Context> &, Value & args)
   {
      ...
      return simple_function("", {}, [sep, first](const std::shared_ptr<Context> &,
                                                  const Value &) -> Value
      {
         if (*first)
         {
                                          ...
                                          const std::shared_ptr<Context> &, Value & args)
   {
      ...
      {
         return simple_function("", {"items"}, [sep,
                                                do_join](const std::shared_ptr<Context> &, Value & args)
         {
            auto & items = args.at("items");
   ...
   globals.set("map", Value::callable([=](const std::shared_ptr<Context> & context,
                                          ArgumentsValue & args)
   {
      auto res = Value::array();
      if (args.args.size() == 1 &&
...
inline std::shared_ptr<Context> Context::make(Value && values,
                                              const std::shared_ptr<Context> & parent)
{
   return std::make_shared<Context>(values.is_null() ? Value::object() : std::move(
                                       values), parent);
}
