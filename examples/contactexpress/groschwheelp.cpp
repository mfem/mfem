...
int main(int argc, char *argv[])
{
   ...
   // 5. Mark the bottom boundary of the solid as attribute 5
   ...
   fespace->GetEssentialTrueDofs(ess_bdr_y, tmp, 1); ess_tdof_list.Append(tmp);

   Array<int> weak_bdr(pmesh.bdr_attributes.Max());
   weak_bdr = 0; weak_bdr[4] = 1;
   ...
   for (int k = 1; k <= max_iterations; k++)
   {
      ...
      // Step 3: Define and apply a parallel PCG solver for A X = B with the BoomerAMG
      ...
      // Step 6: Send the above data by socket to a GLVis server. Use the "n"
      ...
      // Step 8: Update previous solution for next iteration.
