...
int main(int argc, char *argv[])
{
   ...
   auto mass_timer0 = Clock::now();
   m.Assemble();
   m.Finalize();
   auto mass_timer1 = Clock::now();

   std::cout << "Mass assembly duration t2-t1: "
   ...
   auto adv_timer1 = Clock::now();

   std::cout << "Adv assembly duration t2-t1: "
   << std::chrono::duration_cast<std::chrono::nanoseconds>
   ...
   ofstream mass_file;
   mass_file.open("gpu_mass.txt");
   m.SpMat().PrintMatlab(mass_file);
   mass_file.close();

   printf(" \n \n");
   ofstream adv_file;
   adv_file.open("gpu_adv.txt");
