...
fptype bdr_func(const Vector &pt, fptype t);
fptype rhs_func(const Vector &pt, fptype t);
...
int main(int argc, char *argv[])
{
   ...
   int order = 2;
   fptype t_final = 1.0;
   fptype max_elem_error = 1.0e-4;
   fptype hysteresis = 0.25; // derefinement safety coefficient
   int ref_levels = 0;
   ...
   for (fptype time = 0.0; time < t_final + 1e-10; time += 0.01)
   {
      if (myid == 0)
      {
         cout << "\nTime " << time << "\n\nRefinement:" << endl;
...
const fptype alpha = 0.02;

// Spherical front with a Gaussian cross section and radius t
fptype front(fptype x, fptype y, fptype z, fptype t, int)
{
   fptype r = sqrt(x*x + y*y + z*z);
   return exp(-0.5*pow((r - t)/alpha, 2));
}

fptype front_laplace(fptype x, fptype y, fptype z, fptype t, int dim)
{
   fptype x2 = x*x, y2 = y*y, z2 = z*z, t2 = t*t;
   fptype r = sqrt(x2 + y2 + z2);
   fptype a2 = alpha*alpha, a4 = a2*a2;
...
fptype ball(fptype x, fptype y, fptype z, fptype t, int)
{
   fptype r = sqrt(x*x + y*y + z*z);
   return -atan(2*(r - t)/alpha);
}

fptype ball_laplace(fptype x, fptype y, fptype z, fptype t, int dim)
{
   fptype x2 = x*x, y2 = y*y, z2 = z*z, t2 = 4*t*t;
   fptype r = sqrt(x2 + y2 + z2);
   fptype a2 = alpha*alpha;
   fptype den = pow(-a2 - 4*(x2 + y2 + z2 - 2*r*t) - t2, 2.0);
...
fptype composite_func(const Vector &pt, fptype t, F0 f0, F1 f1)
{
   int dim = pt.Size();
   fptype x = pt(0), y = pt(1), z = 0.0;
   if (dim == 3) { z = pt(2); }
   ...
   if (problem == 0)
   {
      ...
      else
      {
         ...
         {
            fptype x0 = 0.5*cos(2*M_PI * i / nfeatures);
            fptype y0 = 0.5*sin(2*M_PI * i / nfeatures);
   ...
   else
   {
      ...
      for (int i = 0; i < nfeatures; i++)
      {
         fptype x0 = 0.5*cos(2*M_PI * i / nfeatures + M_PI*t);
         fptype y0 = 0.5*sin(2*M_PI * i / nfeatures + M_PI*t);
