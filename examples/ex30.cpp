...
// (2) The exact solution of Maxwells is known.
// (3) please check care fully electric field visulation and Implicitsolve 
// (4) The numericl resut is very week? 
//   The time-behaviour in the E-table is o.k. but the
//   asymptotics in h is wrong: For one and the same time step,
//    the error increases with increasing number of unknowns.
// 
//         
...
protected: 
	...
	/// precondition for the mass Matrix Nmat and Rat
...
void Maxwell::ImplicitSolve(const double dt, const Vector &EH,  Vector &dEH_t ) 
{
		...
		//curr->Print();
		GridFunction *curr1= dynamic_cast<GridFunction*>(Current_density);
		
		if(curr1!=NULL)
		{
			cout<<"curr1 point="<<endl;
		curr1->Print();
		}**/
		*Current_density*=-1;
		
		VectorFunctionCoefficient electric_exact(3, ElectricField);
        E.ProjectCoefficient(electric_exact);
		

		
		Conduct->AddMult(E, *Current_density, -1.0);
		
	   // Current_density= MB *H;
		MB->AddMultTranspose(H, *Current_density, 1);
       ...
       Array<int>ess_tdof_list;
		 ...
		 N_Solver->SetOperator(N->SpMat());
		
		}
		N_Solver->Mult(*Current_density, dE);
		//dE=0.0;
...
int main(int argc, char *argv[])
{
 ...
 // double dt= 0.03125; // t=32
   ...
   double Permittivity=0.1;
   double Conductivity=0.0;
   double Permeability=3.75;
...
/**     7. Since E and H are integrate in time so we group them together in block vector EH,  
...
/**   8. Define a solutions Vectors E (electric field )and H (magnetic field) as a Finite element Grid functions
...
/** 9.   Set the initial conditions for E (electric field) and  H (magnetics field).
...
///	 	Initialize the maxwell constructor
		...
		ofstream mesh_ofs("ex30.mesh");
		mesh_ofs.precision(8);
		mesh->Print(mesh_ofs);
		
		ofstream E_ofs("sol_E30.gf");
		...
		VisItDataCollection visit_dc("Example30, mesh");
   ...
   /// 12.GridFunction E and H. Compute the L2 error norms.
   ...
   double norm_h = ComputeLpNorm(2., magnetic_coeff, *mesh, irs);
	double err_e1= E.ComputeL2Error(electric_coeff);
	double err_h1  = H.ComputeL2Error(magnetic_coeff);
   //std::cout << "|| E_h - E_ex ||  = " << err_e  << "\n";
    //std::cout << "|| H_h - H_ex ||  = " << err_h  << "\n";
   std::cout << "|| E_h - E_ex ||  = " << err_e1  << "\n";
   std::cout << "|| H_h - H_ex ||  = " << err_h1  << "\n";
