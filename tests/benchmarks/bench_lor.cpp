...
static int config_inner_cg_max_iter = 0;

static int config_outer_cg_max_iter = 500;
...
public:
                                         ...
                                         Array<int>& ess_bdr)
   {
      ...
      if (config.inner_cg)
      {
         ...
         cg->SetMaxIter(config_inner_cg_max_iter);
         cg->SetRelTol(1e-8);
         cg->SetAbsTol(1e-8);
...
struct BakeOff
{
   ...
   const int smoothness, preconditioner;
...
struct SolverProblem: public BakeOff
{
   ...
   const int max_it = config_outer_cg_max_iter;
   ...
   SolverProblem(int preconditioner,
                 ...
                 bool rhs_1, int rhs_n,
                 int order):
      BakeOff(preconditioner, side, smoothness,
              epsy, epsz,
              rhs_1, rhs_n,
              order,
              VDIM, GLL),
      ...
      cg(MPI_COMM_WORLD)
   {
      ...
      {
         case FinePrecondType::None: { break; }
         case FinePrecondType::Jacobi:
         {
            M.reset(new OperatorJacobiSmoother(a,ess_tdof_list));
            break;
         }
         case FinePrecondType::BoomerAMG:
         {
#ifdef MFEM_USE_MPI
         ...
         case FinePrecondType::MGFAHypre:
         {
   ...
   void benchmark() override
   {
      ...
      dbg("converged:%s num_iterations:%d", converged?"yes":"no", num_iterations);
...
/// rhs_1 = true, means we are solving rhs=1, not with the rhs::RhsCoefficient
...
static void BPS##i##_##Precond(bm::State &state){\
   ...
   ceed::SolverProblem<Kernel##Integrator> bps\
      (Precond, side, smoothness, eps, eps, rhs_1, rhs_n, order);\
...
int main(int argc, char *argv[])
{
   ...
   if (bmi::global_context != nullptr)
   {
      ...
      bmi::FindInContext("inner_cg_max_iter", config_inner_cg_max_iter);

      bmi::FindInContext("max_iter", config_outer_cg_max_iter);

      bmi::FindInContext("mg_spec", config_mg_spec); // mg_spec="1 2"
      bmi::FindInContext("mg_select", config_mg_spec_select); // mg_select=2

      bmi::FindInContext("ndev", config_dev_size);
   }
