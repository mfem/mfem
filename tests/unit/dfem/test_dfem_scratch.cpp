...
struct ScratchBank
{
    ...
    // Raw pointer used by qfunctions to build tensor_array scratch views.
    real_t *GetScratchPointer(const int i) const { return ptrs[i]; }
    real_t *operator[](const int i) const { return ptrs[i]; }

    // Owned storage backing the quadrature-point scratch buffer.
    Vector &GetScratchVector(const int i) const { return *owned[i]; }

    // Get one global scratch element by tuple index.
    template <int I>
                         ...
                         const Vector &scratch, const Vector &scratch_d)
{
    ...
    if (Mpi::Root())
    {
        cout << "Scratch max error: " << global_scratch_err << endl;
        cout << "Scratch derivative max error: "
