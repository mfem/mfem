...
//    for (const auto &f : mesh.ncmesh->faces)
//    {
...
TEST_CASE("NCSubMesh", "[SubMesh], [NCMesh]")
{
   auto NCRefineInternalBoundary = [](Mesh &mesh, int target_attr)
   {
      // Refine one element on the internal boundary
      Array<Refinement> refinements(1);
      // refinements[0].index = 0;
      refinements[0].ref_type = Refinement::XYZ;
      int ninternal_faces = 0;
      for (int f = 0; f < mesh.GetNumFaces(); f++)
      {
         int e0, e1;
         mesh.GetFaceElements(f, &e0, &e1);
         if (e0 >= 0 && e1 >= 0 && mesh.GetAttribute(e0) != mesh.GetAttribute(e1))
         {
            // Interior boundary face
            refinements[0].index = mesh.GetAttribute(e0) == target_attr ? e0 : e1;
            ninternal_faces++;
         }
      }
      mesh.GeneralRefinement(refinements);
      return ninternal_faces;
   };

   bool is_hex = GENERATE(false, true);
   CAPTURE(is_hex);

   auto mesh = DividingPlaneMesh(is_hex, true, true);
   mesh.EnsureNCMesh(true);
   mesh.UniformRefinement();
   int ne = mesh.GetNE();
   int ne_boundary = NCRefineInternalBoundary(mesh, 1);
   ...
   SECTION("Volume")
   {
      ...
      CHECK(submesh.GetNE() == ((ne / 2) - 1 + 8));
      CHECK(submesh.ncmesh != nullptr);
   }
   SECTION("InteriorBoundarySurface")
   {
      subdomain_attributes[0] = mesh.bdr_attributes.Max();
      CAPTURE(ne_boundary);
      auto submesh = SubMesh::CreateFromBoundary(mesh, subdomain_attributes);
      CHECK(submesh.GetNE() == ne_boundary - 1 + 4);
      CHECK(submesh.ncmesh != nullptr);
   }
}
} // namespace mfem
