...
int tmop(int id, Req &res, int argc, char *argv[]) {
  ...
  switch (metric_id) {
    case 1:
      metric.reset(new TMOP_Metric_001);
      break;
    case 2:
      metric.reset(new TMOP_Metric_002);
      break;
    case 7:
      metric.reset(new TMOP_Metric_007);
      break;
    case 77:
      metric.reset(new TMOP_Metric_077);
      break;
    case 80:
      metric.reset(new TMOP_Metric_080(0.5));
      break;
    case 94:
      metric.reset(new TMOP_Metric_094);
      break;
    case 302:
      metric.reset(new TMOP_Metric_302);
      break;
    case 303:
      metric.reset(new TMOP_Metric_303);
      break;
    case 315:
      metric.reset(new TMOP_Metric_315);
      break;
    case 318:
      metric.reset(new TMOP_Metric_318);
      break;
    case 321:
      metric.reset(new TMOP_Metric_321);
      break;
    case 332:
      metric.reset(new TMOP_Metric_332(0.5));
      break;
    case 338:
      metric.reset(new TMOP_Metric_338);
      break;
    default: {
...
#else
  if (target_c == nullptr) {
    target_c.reset(new TargetConstructor(target_t));
  }
#endif
  ...
  switch (quad_type) {
    case 1:
      ...
      break;
    case 2:
      ir = &IntRules.Get(geom_type, quad_order);
      break;
    case 3:
      ir = &IntRulesCU.Get(geom_type, quad_order);
      break;
    default: {
  ...
  TMOP_Integrator *he_nlf_integ =
      new TMOP_Integrator(metric.get(), target_c.get());
  ...
  std::unique_ptr<ConstantCoefficient> coeff1 = nullptr;
  ...
  if (combo > 0) {
    ...
    coeff1.reset(new FunctionCoefficient(coeff_1_func));
    he_nlf_integ->SetCoefficient(*coeff1);
    // Second metric.
    if (dim == 2) {
      metric2.reset(new TMOP_Metric_077);
    } else {
      metric2.reset(new TMOP_Metric_315);
    }
    ...
    if (combo == 1) {
      ...
      he_nlf_integ2 = new TMOP_Integrator(metric2.get(), target_c2.get());
      he_nlf_integ2->SetCoefficient(coeff2);
    } else {
      he_nlf_integ2 = new TMOP_Integrator(metric2.get(), target_c.get());
    }
    he_nlf_integ2->SetIntegrationRule(*ir);
    if (fdscheme) {
      he_nlf_integ2->EnableFiniteDifferences(x);
    }
    ...
    if (normalization) {
      combo_integ->ParEnableNormalization(x0);
    }
    if (lim_const != 0.0) {
  ...
  if (diag && combo == 0) {
    if (pa) {
      ...
      TMOP_Integrator *nlfi_fa =
          new TMOP_Integrator(metric.get(), target_c.get());
      nlfi_fa->SetIntegrationRule(*ir);
      if (normalization == 1) {
        nlfi_fa->ParEnableNormalization(x0);
      }
      if (lim_const != 0.0) {
        if (lim_type == 0) {
  ...
  if (lin_solver == 0) {
    S.reset(new DSmoother(1, 1.0, max_lin_iter));
    ...
    if (lin_solver == 3 || lin_solver == 4) {
      if (pa) {
        ...
        hs->SetType(
            (lin_solver == 3) ? HypreSmoother::Jacobi : HypreSmoother::l1Jacobi,
            1);
        S_prec.reset(hs);
      }
#else
      else {
        S_prec.reset(new DSmoother((lin_solver == 3) ? 0 : 1, 1.0, 1));
      }
#endif
  ...
  for (int i = 0; i < newton_loop; i++) {
    ...
    if (datc && target_id == 5) {
      datc->SetDiscreteTargetSize(size);
    }
    if (datc && target_id == 7) {
    ...
    if (normalization == 1) {
      he_nlf_integ->ParEnableNormalization(x);
    }
...
#define DEFAULT_ARGS                                                      \
  const char *args[] = {                                                  \
      "tmop_pa_tests", "-pa", "-m",    "mesh", "-o",   "0", "-rs", "0",   \
...
static void dump_args(int id, const char *args[]) {
  ...
  const char *format =
      ...
      "-ni %s -rtol %s"       // newton iteration and tolerance
      "%s%s%s%s"              // Optional args: RS, QTY
      "%s%s%s%s%s%s%s%s%s%s"  // Optional args: LC, NOR, JI, NL, CMB, LT
      " %s\n";                // Assembly level
  printf(
      format, args[MSH], args[POR], args[QOR], args[MID], args[TID], args[LS],
      args[NI], args[RTOL],
      // Optional args: RS, QTY
      args[RS][0] == '0' ? "" : " -rs ", args[RS][0] == '0' ? "" : args[RS],
      args[QTY][0] == '1' ? "" : " -qt ", args[QTY][0] == '1' ? "" : args[QTY],
      // Optional args: LC, NOR, JI, NL & CMB
      args[LC][0] == '0' ? "" : " -lc ", args[LC][0] == '0' ? "" : args[LC],
      args[NOR][0] == '0' ? "" : " -nor",
      atof(args[JI]) == 0.0 ? "" : " -jitter",
      args[NL][0] == '1' ? "" : " -nl ", args[NL][0] == '1' ? "" : args[NL],
      args[CMB][0] == '0' ? "" : " -cmb ", args[CMB][0] == '0' ? "" : args[CMB],
      args[LT][0] == '0' ? "" : " -lt ", args[LT][0] == '0' ? "" : args[LT],
      // Assembly level
      args[ALV]);
  fflush(0);
}
...
static inline const char *dtoa(const real_t d, char *buf) {
  if (std::snprintf(buf, sz, "%g", d) < 0) {
    MFEM_ABORT("snprintf error!");
  }
  return buf;
}
...
class Launch {
 ...
 public:
  class Args {
   ...
   public:
    ...
    Args &NEWTON_RTOLERANCE(const double arg) {
      newton_rtol = arg;
      return *this;
    }
    ...
    Args &NORMALIZATION(const bool arg) {
      normalization = arg;
      return *this;
    }
 ...
 public:
  ...
  void Run(const int id = 0, bool all = false) const {
    if ((id == 0) && name) {
      mfem::out << "[" << name << "]" << std::endl;
    }
    DEFAULT_ARGS;
    char ni[sz]{}, nt[sz]{}, rs[sz]{}, li[sz]{}, lc[sz]{}, ji[sz]{}, cmb[sz]{},
        lt[sz]{};
    args[MSH] = mesh;
