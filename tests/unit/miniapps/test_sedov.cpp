                                   ...
                                   const Tensors1D *tensors1D)
{
   ...
   const double h1order = (double) H1.GetElementPolyDegree(0);
...
public:
   LagrangianHydroOperator(Coefficient &rho_coeff,
      ...
      integ_rule(IntRules.Get(h1_fes.GetMesh()->GetElementBaseGeometry(0),
                              (order_q > 0) ? order_q :
                              3*h1_fes.GetElementPolyDegree(0)
                              + l2_fes.GetElementPolyDegree(0) - 1)),
      ...
      dvc_gf(&H1compFESpace)
   {
      ...
      quad_data.h0 /= (double) H1FESpace.GetElementPolyDegree(0);
      {
         Vector d;
