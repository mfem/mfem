...
TEST_CASE("ParBilinearForm Full Ops",
          "[ParBilinearForm]"
          "[Parallel]")
{
   int num_procs;
        ...
        mt <= (int)MeshType::MIXED3D8; mt++)
   {
      ...
      for (int ft = (int)FEType::H1_FEC; ft <= (int)FEType::RT_FEC; ft++)
      {
                 ...
                 " on mesh type " + std::to_string(mt) )
         {
            ...
            ParLinearForm Au(&fespace);
            ParLinearForm ATu(&fespace);
            ParLinearForm aAu(&fespace);
            ParLinearForm aATu(&fespace);
...
TEST_CASE("ParMixedBilinearform Full Ops",
        ...
        mt <= (int)MeshType::MIXED3D8; mt++)
   {
      ...
      for (int ft = (int)FEType::H1_FEC; ft <= (int)FEType::RT_FEC; ft++)
      {
                 ...
                 " on mesh type " + std::to_string(mt) )
         {
            ...
            ParFiniteElementSpace fespace_dom(&pmesh, fec_dom);
            ParFiniteElementSpace fespace_ran(&pmesh, fec_ran);
            ...
            ParGridFunction u_dom(&fespace_dom);
            ParGridFunction u_ran(&fespace_ran);
            if (!vec)
            {
               u_dom.ProjectCoefficient(oneCoef);
               u_ran.ProjectCoefficient(oneCoef);
            ...
            ParMixedBilinearForm a(&fespace_dom, &fespace_ran);
            if (!vec)
            {
               a.AddDomainIntegrator(new MassIntegrator(oneCoef));
            }
            ...
            ParLinearForm Au(&fespace_ran);
            ParLinearForm ATu(&fespace_dom);
            ParLinearForm aAu(&fespace_ran);
            ParLinearForm aATu(&fespace_dom);
            ParLinearForm b_ran(&fespace_ran);
            ParLinearForm b_dom(&fespace_dom);
