...
void test_pa_simplex_diffusion(Mesh mesh, int p, int btype)
{
   CAPTURE(p, btype);
   if (mesh.GetTypicalElementGeometry() == Geometry::SQUARE ||
       mesh.GetTypicalElementGeometry() == Geometry::CUBE)
...
TEST_CASE("PA Simplex MMA Diffusion", "[PartialAssembly][SimplexMMA][GPU]")
{
   const auto all_tests = launch_all_non_regression_tests;
   const auto p = !all_tests ? GENERATE(1, 2, 5, 6) : GENERATE(1, 2, 3, 4, 5, 6);
   ...
   SECTION("single triangle")
   {
      test_pa_simplex_diffusion(Mesh::MakeCartesian2D(1, 1, Element::TRIANGLE),
                                p, BasisType::GaussLobatto);
   }

   SECTION("beam-tri")
   {
      test_pa_simplex_diffusion(Mesh("../../data/beam-tri.mesh"), p,
                                BasisType::GaussLobatto);
   }

   SECTION("single tet")
   {
      test_pa_simplex_diffusion(
         Mesh::MakeCartesian3D(1, 1, 1, Element::TETRAHEDRON), p,
         BasisType::GaussLobatto);
   }
}

TEST_CASE("PA Simplex MMA Diffusion HO mesh",
          "[PartialAssembly][SimplexMMA][GPU]")
{
   ...
   SECTION("quadratic triangle mesh")
   {
      Mesh mesh = Mesh::MakeCartesian2D(2, 2, Element::TRIANGLE);
      mesh.SetCurvature(2);
      test_pa_simplex_diffusion(std::move(mesh), p, BasisType::GaussLobatto);
   }
...
TEST_CASE("PA Simplex Positive Diffusion", "[PartialAssembly][SimplexPA][GPU]")
{
   const auto all_tests = launch_all_non_regression_tests;
   const auto p = !all_tests ? GENERATE(1, 2) : GENERATE(1, 2, 3, 4);
