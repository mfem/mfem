...
void Operator::AddMult(const Vector &x, Vector &y, const fptype a) const
{
...
void Operator::ArrayAddMultTranspose(const Array<const Vector *> &X,
                                     Array<Vector *> &Y, const fptype a) const
{
   MFEM_ASSERT(X.Size() == Y.Size(),
...
void TimeDependentOperator::ImplicitSolve(const fptype, const Vector &,
                                          Vector &)
{
...
Operator &TimeDependentOperator::GetImplicitGradient(
   const Vector &, const Vector &, fptype) const
{
...
int TimeDependentOperator::SUNImplicitSolve(const Vector &, Vector &, fptype)
{
...
int TimeDependentOperator::SUNMassSolve(const Vector &, Vector &, fptype)
{
...
void SecondOrderTimeDependentOperator::ImplicitSolve(const fptype dt0,
                                                     const fptype dt1,
                                                     const Vector &x,
                                                     const Vector &dxdt,
                                                     Vector &k)
...
fptype PowerMethod::EstimateLargestEigenvalue(Operator& opr, Vector& v0,
                                              int numSteps, fptype tolerance, int seed)
{
   v1.SetSize(v0.Size());
   if (seed != 0)
   {
   ...
   for (int iter = 0; iter < numSteps; ++iter)
   {
      ...
      eigenvalueNew = InnerProduct(v0, v1);
#endif
      fptype diff = std::abs((eigenvalueNew - eigenvalue) / eigenvalue);
