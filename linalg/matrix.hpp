...
public:
   ...
   virtual fptype &Elem(int i, int j) = 0;

   /// Returns constant reference to a_{ij}.
   virtual const fptype &Elem(int i, int j) const = 0;
...
public:
   ...
   /** @brief If the matrix is square, this method will place 1 on the diagonal
       ...
       error will occur. */
   virtual void EliminateZeroRows(const fptype threshold = 1e-12) = 0;

   /// Matrix-Vector Multiplication y = A*x
   virtual void Mult(const Vector &x, Vector &y) const = 0;
   /// Matrix-Vector Multiplication y = y + val*A*x
   virtual void AddMult(const Vector &x, Vector &y,
                        const fptype val = 1.) const = 0;
