...
public:
   ...
   HypreParVector(MPI_Comm comm, HYPRE_BigInt glob_size, fptype *data_,
   ...
   /// Set constant values
   HypreParVector& operator= (fptype d);
   ...
   void WrapMemoryRead(const Memory<fptype> &mem);
   ...
   /** This method must be used with HypreParVector%s that do not own the data,
       ...
       The Memory @a mem must be accessible with the hypre MemoryClass defined
       by GetHypreMemoryClass(). */
   void WrapMemoryReadWrite(Memory<fptype> &mem);
   ...
   /** This method must be used with HypreParVector%s that do not own the data,
       ...
       The Memory @a mem must be accessible with the hypre MemoryClass defined
       by GetHypreMemoryClass(). */
   void WrapMemoryWrite(Memory<fptype> &mem);
...
private:
   ...
   mutable Memory<fptype> auxX, auxY;
...
public:
   ...
   HYPRE_Int Mult(HypreParVector &x, HypreParVector &y,
                  fptype alpha = 1.0, fptype beta = 0.0) const;
   /// Computes y = alpha * A * x + beta * y
   ...
   void Mult(fptype a, const Vector &x, fptype b, Vector &y) const;

   /// Computes y = alpha * A^t * x + beta * y
   /** If the matrix is modified, call ResetTranspose() and optionally
       EnsureMultTranspose() to make sure this method uses the correct updated
       transpose. */
   void MultTranspose(fptype a, const Vector &x, fptype b, Vector &y) const;
   ...
   void AddMult(const Vector &x, Vector &y, const fptype a = 1.0) const override
   { Mult(a, x, 1.0, y); }
   ...
   /** @brief Computes y = a * |A| * x + b * y, using entry-wise absolute values
       of the matrix A. */
   void AbsMult(fptype a, const Vector &x, fptype b, Vector &y) const;

   /** @brief Computes y = a * |At| * x + b * y, using entry-wise absolute
       values of the transpose of the matrix A. */
   void AbsMultTranspose(fptype a, const Vector &x, fptype b, Vector &y) const;
   ...
   HypreParMatrix &operator=(fptype value)
   {
   ...
   HypreParMatrix &Add(const fptype beta, const HypreParMatrix &B)
   {
   ...
   void operator*=(fptype s);

   /// Remove values smaller in absolute value than some threshold
   void Threshold(fptype threshold = 0.0);
   ...
   void DropSmallEntries(fptype tol);
   ...
   Memory<fptype> &GetDiagMemoryData() { return mem_diag.data; }
   ...
   const Memory<fptype> &GetDiagMemoryData() const { return mem_diag.data; }
...
HypreParMatrix * HypreParMatrixFromBlocks(Array2D<HypreParMatrix*> &blocks,
                                          Array2D<fptype> *blockCoeff=NULL);
...
protected:
   ...
   mutable Memory<fptype> auxB, auxX;
   /// Temporary vectors
...
public:
   ...
   HypreSmoother(const HypreParMatrix &A_, int type = default_type,
                 int relax_times = 1, fptype relax_weight = 1.0,
                 fptype omega = 1.0, int poly_order = 2,
                 fptype poly_fraction = .3, int eig_est_cg_iter = 10);

   /// Set the relaxation type and number of sweeps
   void SetType(HypreSmoother::Type type, int relax_times = 1);
   /// Set SOR-related parameters
   void SetSOROptions(fptype relax_weight, fptype omega);
   ...
   void SetPolyOptions(int poly_order, fptype poly_fraction,
                       int eig_est_cg_iter = 10);
   /// Set parameters for Taubin's lambda-mu method
   void SetTaubinOptions(fptype lambda, fptype mu, int iter);

   /// Convenience function for setting canonical windowing parameters
   void SetWindowByName(const char* window_name);
   /// Set parameters for windowing function for FIR smoother.
   void SetWindowParameters(fptype a, fptype b, fptype c);
   /// Compute window and Chebyshev coefficients for given polynomial order.
   void SetFIRCoefficients(fptype max_eig);
...
public:
   ...
   void SetResidualConvergenceOptions(int res_frequency=-1, fptype rtol=0.0);
   ...
   void GetFinalResidualNorm(fptype &final_res_norm) const
   {
...
public:
   ...
   void SetParams(fptype threshold, int max_levels);
   void SetFilter(fptype filter);
   void SetLoadBal(fptype loadbal);
...
public:
   ...
   void SetStrongThresholdR(fptype strengthR)
   { HYPRE_BoomerAMGSetStrongThresholdR(amg_precond, strengthR); }

   /// Expert option - consult hypre documentation/team
   void SetFilterThresholdR(fptype filterR)
   ...
   /// Expert option - consult hypre documentation/team
   void SetStrengthThresh(fptype strength)
...
public:
   ...
   void GetEigenvalues(Array<fptype> & eigenvalues) const;
