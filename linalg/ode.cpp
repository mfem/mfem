...
void ForwardEulerSolver::Step(Vector &x, fptype &t, fptype &dt)
{
   f->SetTime(t);
   f->Mult(x, dxdt);
   x.Add(dt, dxdt);
...
void RK2Solver::Step(Vector &x, fptype &t, fptype &dt)
{
   //  0 |
   //  a |  a
   // ---+--------
...
void RK3SSPSolver::Step(Vector &x, fptype &t, fptype &dt)
{
...
void RK4Solver::Step(Vector &x, fptype &t, fptype &dt)
{
   //   0  |
   //  1/2 | 1/2
   //  1/2 |  0   1/2
...
ExplicitRKSolver::ExplicitRKSolver(int s_, const fptype *a_, const fptype *b_,
                                   const fptype *c_)
{
   s = s_;
   a = a_;
...
void ExplicitRKSolver::Step(Vector &x, fptype &t, fptype &dt)
{
   //   0     |
   //  c[0]   | a[0]
   //  c[1]   | a[1] a[2]
...
AdamsBashforthSolver::AdamsBashforthSolver(int s_, const fptype *a_)
{
   smax = std::min(s_,5);
   a = a_;
   k = new Vector[5];
...
void AdamsBashforthSolver::Step(Vector &x, fptype &t, fptype &dt)
{
   if ( (dt_ > 0.0) && (fabs(dt-dt_) >10*std::numeric_limits<fptype>::epsilon()))
   {
      s = 0;
      dt_ = dt;
...
const fptype AB1Solver::a[] =
{1.0};
const fptype AB2Solver::a[] =
{1.5,-0.5};
const fptype AB3Solver::a[] =
{23.0/12.0,-4.0/3.0, 5.0/12.0};
const fptype AB4Solver::a[] =
{55.0/24.0,-59.0/24.0, 37.0/24.0,-9.0/24.0};
const fptype AB5Solver::a[] =
{1901.0/720.0,-2774.0/720.0, 2616.0/720.0,-1274.0/720.0, 251.0/720.0};

AdamsMoultonSolver::AdamsMoultonSolver(int s_, const fptype *a_)
{
   s = 0;
   smax = std::min(s_+1,5);
   a = a_;
...
void AdamsMoultonSolver::Step(Vector &x, fptype &t, fptype &dt)
{
   if ( (dt_ > 0.0) && (fabs(dt-dt_) >10*std::numeric_limits<fptype>::epsilon()))
   {
      s = 0;
...
const fptype AM0Solver::a[] =
{1.0};
const fptype AM1Solver::a[] =
{0.5, 0.5};
const fptype AM2Solver::a[] =
{5.0/12.0, 2.0/3.0, -1.0/12.0};
const fptype AM3Solver::a[] =
{3.0/8.0, 19.0/24.0,-5.0/24.0, 1.0/24.0};
const fptype AM4Solver::a[] =
...
void BackwardEulerSolver::Step(Vector &x, fptype &t, fptype &dt)
{
   f->SetTime(t + dt);
...
void ImplicitMidpointSolver::Step(Vector &x, fptype &t, fptype &dt)
{
   f->SetTime(t + dt/2);
...
void SDIRK23Solver::Step(Vector &x, fptype &t, fptype &dt)
{
   // with a = gamma:
   //   a   |   a
   //  1-a  |  1-2a  a
...
void SDIRK34Solver::Step(Vector &x, fptype &t, fptype &dt)
{
   //   a   |    a
   //  1/2  |  1/2-a    a
   //  1-a  |   2a    1-4a   a
   ...
   const fptype a = 1./sqrt(3.)*cos(M_PI/18.) + 0.5;
   const fptype b = 1./(6.*(2.*a-1.)*(2.*a-1.));
...
void SDIRK33Solver::Step(Vector &x, fptype &t, fptype &dt)
{
   //   a  |   a
   //   c  |  c-a    a
   //   1  |   b   1-a-b  a
   // -----+----------------
   //      |   b   1-a-b  a
   const fptype a = 0.435866521508458999416019;
   const fptype b = 1.20849664917601007033648;
   const fptype c = 0.717933260754229499708010;
...
void TrapezoidalRuleSolver::Step(Vector &x, fptype &t, fptype &dt)
{
   //   0   |   0    0
   //   1   |  1/2  1/2
   // ------+-----------
...
void ESDIRK32Solver::Step(Vector &x, fptype &t, fptype &dt)
{
   //   0   |    0      0    0
   //   2a  |    a      a    0
   //   1   |  1-b-a    b    a
   // ------+--------------------
   //       |  1-b-a    b    a
   const fptype a = (2.0 - sqrt(2.0)) / 2.0;
   const fptype b = (1.0 - 2.0*a) / (4.0*a);
...
void ESDIRK33Solver::Step(Vector &x, fptype &t, fptype &dt)
{
   //   0   |      0          0        0
   //   2a  |      a          a        0
   //   1   |    1-b-a        b        a
   // ------+----------------------------
   //       |  1-b_2-b_3     b_2      b_3
   const fptype a   = (3.0 + sqrt(3.0)) / 6.0;
   const fptype b   = (1.0 - 2.0*a) / (4.0*a);
   const fptype b_2 = 1.0 / ( 12.0*a*(1.0 - 2.0*a) );
   const fptype b_3 = (1.0 - 3.0*a) / ( 3.0*(1.0 - 2.0*a) );
...
void GeneralizedAlphaSolver::SetRhoInf(fptype rho_inf)
{
...
void GeneralizedAlphaSolver::Step(Vector &x, fptype &t, fptype &dt)
{
   if (nstate == 0)
   {
      f->Mult(x,xdot);
   ...
   fptype dt_eff = (gamma*alpha_f/alpha_m)*dt;
...
void
SIA1Solver::Step(Vector &q, Vector &p, fptype &t, fptype &dt)
{
   F_->SetTime(t);
   F_->Mult(q,dp_);
   p.Add(dt,dp_);
...
void
SIA2Solver::Step(Vector &q, Vector &p, fptype &t, fptype &dt)
{
   P_->Mult(p,dq_);
   q.Add(0.5*dt,dq_);
...
void
SIAVSolver::Step(Vector &q, Vector &p, fptype &t, fptype &dt)
{
...
void NewmarkSolver::Step(Vector &x, Vector &dxdt, fptype &t, fptype &dt)
{
   fptype fac0 = 0.5 - beta;
   fptype fac2 = 1.0 - gamma;
   fptype fac3 = beta;
   fptype fac4 = gamma;
...
void GeneralizedAlpha2Solver::Step(Vector &x, Vector &dxdt,
                                   fptype &t, fptype &dt)
{
   fptype fac0 = (0.5 - (beta/alpha_m));
   fptype fac1 = alpha_f;
   fptype fac2 = alpha_f*(1.0 - (gamma/alpha_m));
   fptype fac3 = beta*alpha_f/alpha_m;
   fptype fac4 = gamma*alpha_f/alpha_m;
   fptype fac5 = alpha_m;
