...
PenaltyConstrainedSolver::PenaltyConstrainedSolver(
   HypreParMatrix& A, SparseMatrix& B, fptype penalty_)
   :
   ConstrainedSolver(A.GetComm(), A, B),
   penalty(B.Height()),
   constraintB(B),
...
PenaltyConstrainedSolver::PenaltyConstrainedSolver(
   HypreParMatrix& A, HypreParMatrix& B, fptype penalty_)
   :
   ConstrainedSolver(A.GetComm(), A, B),
   penalty(B.Height()),
   constraintB(B),
         ...
         for (int j = 0; j < dofs.Size(); ++j)
         {
            ...
            if (truek >= 0)
            {
               ...
               for (int d = 0; d < dim; ++d)
               {
                  ...
                  {
                     mout->SetColPtr(row);
                     const fptype pv = mout->SearchRow(inner_truek);
                     const fptype scaling = ((fptype) (visits - 1)) /
                                            ((fptype) visits);
