                           ...
                           HYPRE_BigInt &firstLocal, HYPRE_BigInt &globalNum)
{
   ...
   MPI_Allgather(const_cast<int*>(&num_loc), 1, MPI_INT, all_num_loc.data(), 1, MPI_INT, comm);
   ...
   for (int j = 0; j < numBlocks; ++j)
   {
      ...
      MPI_Allgather(const_cast<int*>(&blockNumRows), 1, MPI_INT, all_block_num_loc[j].data(), 1,
                    MPI_INT, comm);
