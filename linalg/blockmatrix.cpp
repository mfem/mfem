...
fptype& BlockMatrix::Elem (int i, int j)
{
   int iloc, iblock;
   int jloc, jblock;
...
const fptype& BlockMatrix::Elem (int i, int j) const
{
...
void BlockMatrix::EliminateZeroRows(const fptype threshold)
{
...
void BlockMatrix::AddMult(const Vector & x, Vector & y, const fptype val) const
{
   if (x.GetData() == y.GetData())
   {
      mfem_error("Error: x and y can't point to the same data \n");
   ...
   for (int iblock = 0; iblock != nRowBlocks; ++iblock)
   {
      for (int jblock = 0; jblock != nColBlocks; ++jblock)
      {
         if (Aij(iblock,jblock) != NULL)
         {
            ...
            fptype * data_aij = Aij(iblock, jblock)->GetData();
