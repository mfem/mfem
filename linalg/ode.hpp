...
public:
   ...
   virtual void Step(Vector &x, fptype &t, fptype &dt) = 0;
   ...
   virtual void Run(Vector &x, fptype &t, fptype &dt, fptype tf)
   {
...
public:
   ...
   void Step(Vector &x, fptype &t, fptype &dt) override;
};
...
public:
   RK2Solver(const fptype a_ = 2./3.) : a(a_) { }
...
public:
   ExplicitRKSolver(int s_, const fptype *a_, const fptype *b_,
                    const fptype *c_);
...
{
private:
   static MFEM_EXPORT const fptype a[28], b[8], c[7];
...
{
private:
   static const fptype a[66], b[12], c[11];
...
public:
   AdamsBashforthSolver(int s_, const fptype *a_);
...
{
private:
   static MFEM_EXPORT const fptype a[1];
...
{
private:
   static MFEM_EXPORT const fptype a[2];
...
{
private:
   static MFEM_EXPORT const fptype a[3];
...
{
private:
   static MFEM_EXPORT const fptype a[4];
...
{
private:
   static MFEM_EXPORT const fptype a[5];
...
public:
   AdamsMoultonSolver(int s_, const fptype *a_);
...
public:
   ...
   virtual void Step(Vector &x, fptype &t, fptype &dt);
};
...
protected:
   ...
   fptype alpha_f, alpha_m, gamma;
   int  nstate;

   void SetRhoInf(fptype rho_inf);
   void PrintProperties(std::ostream &out = mfem::out);
public:

   GeneralizedAlphaSolver(fptype rho = 1.0) { SetRhoInf(rho); };
...
public:
   ...
   virtual void Step(Vector &q, Vector &p, fptype &t, fptype &dt) = 0;

   virtual void Run(Vector &q, Vector &p, fptype &t, fptype &dt, fptype tf)
   {
...
public:
   SIA1Solver() {}
   void Step(Vector &q, Vector &p, fptype &t, fptype &dt) override;
};
...
public:
   ...
   /** @param[in,out] x    Approximate solution.
       ...
       - If the previous rule has to be broken, e.g. to restart a time stepping
         sequence, then the ODE solver must be re-initialized by calling Init()
         between the two Step() calls. */
   virtual void Step(Vector &x, Vector &dxdt, fptype &t, fptype &dt) = 0;
   ...
   /** @param[in,out] x    Approximate solution.
       ...
       - The output @a dt [out] is the last time step taken by the method which
         ...
         of time step control.
       - The output value of @a t [out] is not smaller than @a tf [in]. */
   virtual void Run(Vector &x, Vector &dxdt, fptype &t, fptype &dt, fptype tf)
   {
...
public:
   NewmarkSolver(fptype beta_ = 0.25, fptype gamma_ = 0.5) { beta = beta_; gamma = gamma_; };

   void PrintProperties(std::ostream &out = mfem::out);

   void Init(SecondOrderTimeDependentOperator &f_) override;

   void Step(Vector &x, Vector &dxdt, fptype &t, fptype &dt) override;
};
...
protected:
   ...
   fptype alpha_f, alpha_m, beta, gamma;
   int nstate;

public:
   GeneralizedAlpha2Solver(fptype rho_inf = 1.0)
   {
...
{
public:
   HHTAlphaSolver(fptype alpha = 1.0)
   {
...
{
public:
   WBZAlphaSolver(fptype rho_inf = 1.0)
   {
      rho_inf = (rho_inf > 1.0) ? 1.0 : rho_inf;
      rho_inf = (rho_inf < 0.0) ? 0.0 : rho_inf;
