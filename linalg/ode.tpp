...
void TForwardEulerSolver<TVector>::Init(TimeDependentOperator &_f)
{
   ODESolver::Init(_f);
...
template <class TVector>
void TRK2Solver<TVector>::Init(TimeDependentOperator &_f)
{
   ODESolver::Init(_f);
   int n = f->Width();
   dxdt.SetSize(n);
...
template <class TVector>
void TRK3SSPSolver<TVector>::Init(TimeDependentOperator &_f)
{
   ODESolver::Init(_f);
   int n = f->Width();
   y.SetSize(n);
...
template <class TVector>
void TRK4Solver<TVector>::Init(TimeDependentOperator &_f)
{
   ODESolver::Init(_f);
   int n = f->Width();
   y.SetSize(n);
...
template <class TVector>
void TExplicitRKSolver<TVector>::Init(TimeDependentOperator &_f)
{
   ODESolver::Init(_f);
   int n = f->Width();
   y.SetSize(n);
...
template <class TVector>
void TBackwardEulerSolver<TVector>::Init(TimeDependentOperator &_f)
{
   ODESolver::Init(_f);
   k.SetSize(f->Width());
}
...
template <class TVector>
void TImplicitMidpointSolver<TVector>::Init(TimeDependentOperator &_f)
{
   ODESolver::Init(_f);
   k.SetSize(f->Width());
}
...
template <class TVector>
void TSDIRK23Solver<TVector>::Init(TimeDependentOperator &_f)
{
   ODESolver::Init(_f);
   k.SetSize(f->Width());
   y.SetSize(f->Width());
...
template <class TVector>
void TSDIRK34Solver<TVector>::Init(TimeDependentOperator &_f)
{
   ODESolver::Init(_f);
   k.SetSize(f->Width());
   y.SetSize(f->Width());
...
template <class TVector>
void TSDIRK33Solver<TVector>::Init(TimeDependentOperator &_f)
{
   ODESolver::Init(_f);
   k.SetSize(f->Width());
   y.SetSize(f->Width());
...
template <class TVector>
void TGeneralizedAlphaSolver<TVector>::Init(TimeDependentOperator &_f)
{
   ODESolver::Init(_f);
   k.SetSize(f->Width());
   y.SetSize(f->Width());
...
template <class TVector>
void TGeneralizedAlphaSolver<TVector>::SetRhoInf(double rho_inf)
{
...
template <class TVector>
void TGeneralizedAlphaSolver<TVector>::PrintProperties(std::ostream &out)
{
...
template <class TVector>
void TGeneralizedAlphaSolver<TVector>::Step(Vector &x, double &t, double &dt)
{
...
template <class TVector>
void
TSIASolver<TVector>::Init(Operator &P, TimeDependentOperator & F)
{
   this->P_ = &P; F_ = &F;

   dp_.SetSize(F_->Height());
   this->dq_.SetSize(this->P_->Height());
}
...
{
   this->F_->SetTime(t);
   this->F_->Mult(q,this->dp_);
   p.Add(dt,this->dp_);

   this->P_->Mult(p,this->dq_);
   q.Add(dt,this->dq_);
...
{
   this->P_->Mult(p,this->dq_);
   q.Add(0.5*dt,this->dq_);

   this->F_->SetTime(t+0.5*dt);
   this->F_->Mult(q,this->dp_);
   p.Add(dt,this->dp_);
...
TSIAVSolver<TVector>::Step(TVector &q, TVector &p, double &t, double &dt)
{
   for (int i=0; i<order_; i++)
   {
      ...
      {
         this->F_->SetTime(t);
         if ( this->F_->isExplicit() )
         {
            this->F_->Mult(q, this->dp_);
         }
         else
         {
            this->F_->ImplicitSolve(b_[i] * dt, q, this->dp_);
         }
         p.Add(b_[i] * dt, this->dp_);
      }

      this->P_->Mult(p, this->dq_);
      q.Add(a_[i] * dt, this->dq_);
