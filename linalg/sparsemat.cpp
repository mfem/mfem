...
int SparseMatrix::MaxRowSize() const
{
   ...
   if (I)
   {
      ...
      {
         rowSize = I[i+1]-I[i];
         rout = (rout > rowSize) ? rout : rowSize;
      }
   }
   else
   {
...
void SparseMatrix::Print(std::ostream & pout, int width_) const
{
   int i, j;
...
void SparseMatrix::PrintMatlab(std::ostream & pout) const
{
   pout << "% size " << height << " " << width << "\n";
   pout << "% Non Zeros " << NumNonZeroElems() << "\n";
   int i, j;
   ios::fmtflags old_fmt = pout.flags();
   ...
   pout << height << " " << width << " 0.0\n";
   pout.precision(old_prec);
   pout.flags(old_fmt);
}

void SparseMatrix::PrintMM(std::ostream & pout) const
{
   int i, j;
   ios::fmtflags old_fmt = pout.flags();
   pout.setf(ios::scientific);
   ...
   pout << "%%MatrixMarket matrix coordinate real general" << '\n'
        << "% Generated by MFEM" << '\n';

   pout << height << " " << width << " " << NumNonZeroElems() << '\n';
   for (i = 0; i < height; i++)
   {
      for (j = I[i]; j < I[i+1]; j++)
      {
...
void SparseMatrix::PrintCSR(std::ostream & pout) const
{
   MFEM_VERIFY(Finalized(), "Matrix must be finalized.");
...
void SparseMatrix::PrintCSR2(std::ostream & pout) const
{
   MFEM_VERIFY(Finalized(), "Matrix must be finalized.");
...
void SparseMatrix::PrintInfo(std::ostream &pout) const
{
...
#endif
      pout << "  Memory used by LIL          : " << used_mem/MiB << " MiB\n";
   }
}
