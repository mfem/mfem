   ...
   comm_(hypParMat.GetComm())
{
   ...
   if (symm)
   {
      csr = hypre_MergeDiagAndOffd(const_cast<hypre_ParCSRMatrix*>(par_csr));
   }
   else
   {
      // PaStiX cannot convert a distributed CSR to a distributed CSC, so we need to gather the global matrix
      csr = hypre_ParCSRMatrixToCSRMatrixAll(const_cast<hypre_ParCSRMatrix*>
                                             (par_csr));
   }
   ...
   matrix_.mtxtype = symm ? SpmSymmetric : SpmGeneral;
   ...
   {
      // Convert the global CSR into global CSC
      if (spmConvert(SpmCSC, &matrix_) != 0)
      {
         MFEM_WARNING("Failed to convert global CSR to global CSC");
      }

      // Scatter the global CSC into local CSCs
      spmatrix_t* scattered_matrix;
      spmatrix_t* base_matrix = (rank == 0) ? &matrix_ : nullptr;
      scattered_matrix = spmScatter(base_matrix, 0, nullptr, true, 0, comm_);
      if (rank == 0)
      {
         spmExit(&matrix_);
      }
      matrix_ = *scattered_matrix;
   }

   // Fix the matrix if necessary
...
{
   spmMatVec(SpmNoTrans, 1.0, &matrix_, static_cast<const double*>(x), 0.0,
