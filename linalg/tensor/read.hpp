...
dTensor<D> Read(const DeviceTensor<2,T> &e_vec, const int e)
{
   dTensor<D> u;
...
dTensor<D1d,D1d,D1d> Read(const DeviceTensor<4,T> &e_vec, const int e)
{
...
// 2D tensor read
template <int D1d, typename T> MFEM_HOST_DEVICE inline
dTensor<D1d,D1d> Read(const DeviceTensor<3,T> &e_vec, const int e)
{
   dTensor<D1d,D1d> u;
   MFEM_FOREACH_THREAD(dy,y,D1d)
   {
