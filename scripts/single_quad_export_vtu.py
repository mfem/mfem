...
ref_levels = 5 # Shows up to this refinement level, 1 for base-level, 2 for base + next refinement level, etc.
M = 4
boundtxtpath = f'../examples/2DcustomboundinfoM{M}.txt'
soltxtpath = '2Dsolutioninfo_coarse.txt'
solvtupath = 'sol.vtu'

def filter(data, tlevel=None):
	level = data[:,5]
	...
	if tlevel is None:
		...
		for i in range(tlevel):
			if i == tlevel - 1:
				...
				keep_levels = np.logical_or(keep_levels, level == i+1)
		# keep_levels = np.logical_or(keep_levels, np.abs(level) == tlevel)
...
for ii in range(ref_levels):
	bounddata = np.genfromtxt(boundtxtpath)
	if (ii < ref_levels-1):
		bounddata = filter(bounddata,ii+1)
	else:
	...
	minvtupath = f'bounds_min_M{M}_r{ii+1}.vtu'
	maxvtupath = f'bounds_max_M{M}_r{ii+1}.vtu'
