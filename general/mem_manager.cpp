                          ...
                          unsigned &flags)
{
   ...
   flags = Mem::REGISTERED_ | Mem::OWNS_INTERNAL | Mem::OWNS_HOST |
                               ...
                               bool own, bool alias, unsigned &flags)
{
   ...
   flags |= Mem::REGISTERED_ | Mem::OWNS_INTERNAL;
   void *h_ptr;
...
void MemoryManager::Delete_(void *h_ptr, MemoryType h_mt, unsigned flags)
{
   ...
   const bool registered = flags & Mem::REGISTERED_;
                                ...
                                size_t bytes, unsigned &flags)
{
   ...
   if (bytes > 0) { MFEM_VERIFY(flags & Mem::REGISTERED_,""); }
...
{
   // This is called only when (base_flags & Mem::REGISTERED_) is true.
   // Note that (alias_flags & REGISTERED_) may not be true.
   ...
   {
      if (!(alias_flags & Mem::REGISTERED_))
      {
         mm.InsertAlias(base_h_ptr, alias_h_ptr, alias_bytes, base_flags & Mem::ALIAS);
         alias_flags = (alias_flags | Mem::REGISTERED_ | Mem::OWNS_INTERNAL) &
                       ~(Mem::OWNS_HOST | Mem::OWNS_DEVICE);
      }
...
void MemoryPrintFlags(unsigned flags)
{
   ...
   mfem::out
         << "\n   registered    = " << bool(flags & Mem::REGISTERED_)
