...
int socketbuf::sync()
{
   ...
   // mout << "[socketbuf::sync n=" << n << ']' << std::endl;
   while (n > 0)
   {
#ifdef MSG_NOSIGNAL
      ...
      {
#ifdef MFEM_DEBUG
         mout << "Error in send(): " << strerror(errno) << std::endl;
#endif
...
socketbuf::int_type socketbuf::underflow()
{
   ...
   // mout << "[socketbuf::underflow br=" << br << ']'
   //           << std::endl;
   if (br <= 0)
   {
#ifdef MFEM_DEBUG
      if (br < 0)
      {
         mout << "Error in recv(): " << strerror(errno) << std::endl;
      }
#endif
      setg(NULL, NULL, NULL);
...
{
   mout << "GnuTLS <" << level << "> " << str << std::flush;
}
...
void GnuTLS_global_state::generate_dh_params()
{
   if (status.good())
   {
      ...
      else
      {
         ...
         unsigned bits = 1024;
#endif
         mout << "Generating DH params (" << bits << " bits) ..."
              << std::flush;
...
static int mfem_gnutls_verify_callback(gnutls_session_t session)
{
   ...
   if (ret < 0)
   {
      mout << "Error in gnutls_certificate_verify_peers3:"
   ...
   if (ret < 0)
   {
      mout << "Error in gnutls_certificate_verification_status_print:"
           << gnutls_strerror(ret) << std::endl;
      return GNUTLS_E_CERTIFICATE_ERROR;
   }
   mout << out.data << std::endl;
   ...
   if (ret < 0)
   {
      mout << "Error in gnutls_certificate_verify_peers2:"
           << gnutls_strerror(ret) << std::endl;
      return GNUTLS_E_CERTIFICATE_ERROR;
   }
#ifdef MFEM_DEBUG
...
void GnuTLS_socketbuf::handshake()
{
   ...
   mout << "[GnuTLS_socketbuf::handshake]" << std::endl;
#endif
   ...
   do
   {
      ...
      {
#if 0
         mout << "handshake successful, TLS version is "
...
{
#ifdef MFEM_USE_GNUTLS_DEBUG
   mout << "[GnuTLS_socketbuf::start_session]" << std::endl;
#endif
   ...
   if (status.good())
   {
      ...
      if (!status.good())
      {
         mout << "Error ptr = \"" << err_ptr << '"' << std::endl;
      }
   }
...
{
#ifdef MFEM_USE_GNUTLS_DEBUG
   mout << "[GnuTLS_socketbuf::end_session]" << std::endl;
#endif
   ...
   if (is_open() && status.good())
   {
      pubsync();
#ifdef MFEM_USE_GNUTLS_DEBUG
      mout << "[GnuTLS_socketbuf::end_session: gnutls_bye]" << std::endl;
#endif
      int err;
      do
      {
...
{
#ifdef MFEM_USE_GNUTLS_DEBUG
   mout << "[GnuTLS_socketbuf::attach]" << std::endl;
#endif
...
{
#ifdef MFEM_USE_GNUTLS_DEBUG
   mout << "[GnuTLS_socketbuf::open]" << std::endl;
#endif
...
{
#ifdef MFEM_USE_GNUTLS_DEBUG
   mout << "[GnuTLS_socketbuf::close]" << std::endl;
#endif
...
int GnuTLS_socketbuf::sync()
{
   ssize_t bw, n = pptr() - pbase();
#ifdef MFEM_USE_GNUTLS_DEBUG
   mout << "[GnuTLS_socketbuf::sync n=" << n << ']' << std::endl;
#endif
...
{
#ifdef MFEM_USE_GNUTLS_DEBUG
   mout << "[GnuTLS_socketbuf::underflow ...]" << std::endl;
#endif
...
#ifdef MFEM_USE_GNUTLS_DEBUG
   mout << "[GnuTLS_socketbuf::underflow br=" << br << ']' << std::endl;
#endif
...
{
#ifdef MFEM_USE_GNUTLS_DEBUG
   mout << "[GnuTLS_socketbuf::xsgetn __n=" << __n << ']' << std::endl;
#endif
...
{
#ifdef MFEM_USE_GNUTLS_DEBUG
   mout << "[GnuTLS_socketbuf::xsputn __n=" << __n << ']' << std::endl;
#endif
   if (!session_started || !status.good()) { return 0; }
   ...
   while (remain > buflen)
   {
      ...
      if (bw == GNUTLS_E_INTERRUPTED || bw == GNUTLS_E_AGAIN) { continue; }
#ifdef MFEM_USE_GNUTLS_DEBUG
      mout << "[GnuTLS_socketbuf::xsputn bw=" << bw << ']' << std::endl;
#endif
      if (bw < 0)
      {
         status.set_result((int)bw);
...
GnuTLS_session_params &socketstream::add_socket()
{
   if (num_glvis_sockets == 0)
   {
      ...
      {
         mout << "  public key   = " << pubkey << '\n'
              << "  private key  = " << privkey << '\n'
              << "  trusted keys = " << trustedkeys << std::endl;
         mout << "Error setting GLVis client parameters.\n"
