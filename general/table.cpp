...
Table::Table(const Table &table1, const Table &table2, int offset, bool merge)
{
   ...
   {
      I[i+1] = I[i];
      if (merge || (table1.RowSize(i) < table2.RowSize(i)) )
      {
         table1.GetRow(i, row);
      ...
      if (merge || (table2.RowSize(i) < table1.RowSize(i)) )
      {
         table2.GetRow(i, row);
         for (int r = 0; r < row.Size(); r++,  I[i+1] ++)
         {
