...
template <typename T, std::size_t UID> __device__ static T mfem_shared()
{
...
template <typename Tsmem = double, typename BODY>
__global__ static void CuKernel2DSmem(const int N, BODY body)
{
   const int k = blockIdx.x * blockDim.z + threadIdx.z;
   if (k >= N)
   {
...
template <typename Tsmem = double, typename BODY>
__global__ static void CuKernel2DGmem(const int N, BODY body, Tsmem *smem,
                                      const int smem_size)
{
   const int k = blockIdx.x * blockDim.z + threadIdx.z;
   if (k >= N)
...
template <typename Tsmem = double, typename BODY>
__global__ static void CuKernel3DSmem(const int N, BODY body)
{
...
template <typename Tsmem = double, typename BODY>
__global__ static void CuKernel3DGmem(const int N, BODY body, Tsmem *smem,
                                      const int smem_size)
{
   for (int k = blockIdx.x; k < N; k += gridDim.x)
   {
...
void CuWrapSmem2D(const int N, DBODY &&d_body, const int smem_size, const int X,
                  const int Y, const int BZ, const int G)
{
   if (N == 0)
   {
      return;
   ...
   else
   {
      ...
      CuKernel2DGmem<Tsmem><<<GRID, BLCK>>>(
         N, d_body, smem.Write(MemoryClass::DEVICE, smem_size), smem_size);
   }
   MFEM_GPU_CHECK(cudaGetLastError());
}
...
template <typename Tsmem = double, typename DBODY>
void CuWrapSmem3D(const int N, DBODY &&d_body, const int smem_size, const int X,
...
template <typename Tsmem> struct CuWrapSmem<2, Tsmem>
{
   template <const int BLCK = MFEM_CUDA_BLOCKS, typename DBODY>
   static void run(const int N, DBODY &&d_body, const int smem_size, const int X,
                   const int Y, const int Z, const int G)
   {
...
template <typename Tsmem> struct CuWrapSmem<3, Tsmem>
{
   template <const int BLCK = MFEM_CUDA_BLOCKS, typename DBODY>
   static void run(const int N, DBODY &&d_body, const int smem_size, const int X,
                   const int Y, const int Z, const int G)
