...
template<> void CPU::token<TOK::NL>(string) const { /* empty */ }

template<> void CPU::token<TOK::QUOTE>(string text) const
{
   str::replace(text.c_str(), '\'','\"');
   out << text;
}

template<> void CPU::token<TOK::DOM_DX>(string) const { out << " xfl::Form()"; }

template<> void CPU::token<TOK::POW>(string) const { out << ","; }

template<> void CPU::token<TOK::INNER_OP>(string) const { out << "dot"; }
