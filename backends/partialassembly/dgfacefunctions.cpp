...
void GetFaceInfo(const Mesh* mesh, const int face, int& ind_elt1, int& ind_elt2,
                 int& face_id1, int& face_id2, int& nb_rot1, int& nb_rot2)
{
...
*/
void Permutation3D(const int face_id1, const int face_id2,
                   const int orientation, int& perm1, int& perm2)
{
   IntMatrix K1(3,3);
   K1.zero();
   ...
   perm1 += 100*(0*(P(0,0)==-1) + 1*(P(0,0)==1) + 2*(P(1,0)==-1) + 3*(P(1,
                                                                        0)==1) + 4*(P(2,0)==-1) + 5*(P(2,0)==1));
   // Encrypts second column
   perm1 += 10 *(0*(P(0,1)==-1) + 1*(P(0,1)==1) + 2*(P(1,1)==-1) + 3*(P(1,
                                                                        1)==1) + 4*(P(2,1)==-1) + 5*(P(2,1)==1));
   // Encrypts third column
   perm1 +=     (0*(P(0,2)==-1) + 1*(P(0,2)==1) + 2*(P(1,2)==-1) + 3*(P(1,
                                                                        2)==1) + 4*(P(2,2)==-1) + 5*(P(2,2)==1));
   // Encrypts the transposed permutation matrix in a second integer.
   perm2  = 0;
   perm2 += 100*(0*(P(0,0)==-1) + 1*(P(0,0)==1) + 2*(P(0,1)==-1) + 3*(P(0,
                                                                        1)==1) + 4*(P(0,2)==-1) + 5*(P(0,2)==1));
   perm2 += 10 *(0*(P(1,0)==-1) + 1*(P(1,0)==1) + 2*(P(1,1)==-1) + 3*(P(1,
                                                                        1)==1) + 4*(P(1,2)==-1) + 5*(P(1,2)==1));
   perm2 +=     (0*(P(2,0)==-1) + 1*(P(2,0)==1) + 2*(P(2,1)==-1) + 3*(P(2,
                                                                        1)==1) + 4*(P(2,2)==-1) + 5*(P(2,2)==1));
}

void GetPermutation(const int dim, const int face_id1, const int face_id2,
                    const int orientation, int& perm1, int& perm2)
{
   switch (dim)
   {
...
*/
int GetFaceQuadIndex3D(const int face_id, const int orientation, const int qind,
...
int GetFaceQuadIndex(const int dim, const int face_id, const int orientation,
                     const int qind, const int quads, Tensor<1,int>& ind_f)
{
   int res = 0;
   switch (dim)
   {
...
const int GetGlobalQuadIndex(const int dim, const int face_id, const int quads,
                             Tensor<1,int>& ind_f)
{
   switch (dim)
   {
