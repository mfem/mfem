...
void AcroDiffusionIntegrator::PAMult(const Vector &x, Vector &y)
{
    ...
    if (!U.IsInitialized())
    {
        vectorX.SetSize(ofes->GetLocalVSize());
        vectorY.SetSize(ofes->GetLocalVSize());
...
void AcroDiffusionIntegrator::Mult(const Vector &x, Vector &y) const
{
   const_cast<AcroDiffusionIntegrator*>(this)->PAMult(x, y);
}

void AcroDiffusionIntegrator::MultTranspose(const Vector &x, Vector &y) const
{
