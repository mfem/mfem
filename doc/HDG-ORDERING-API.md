...
* ~~The trace right-hand side has no slot~~ — **built, and this entry told a
  caller to work around something that exists.** `DarcyForm::GetTraceRHS()` is
  the third load beside `GetFluxRHS()` and `GetPotentialRHS()`, and **both
  routes carry it with no caller wiring**: `ReduceRHS()` adds `P^T b_λ` to the
  reduced right-hand side and `NPCResidual()` subtracts it from the trace
  block, which is the same convention read off `r = A x - b`. The sign, the
  API and why the registration happens at construction rather than at
  `Assemble()` are on the accessor; the pin is "A load on the skeleton reaches
  both routes" in `tests/unit/fem/test_darcy_npc.cpp`. The entry's own
  reasoning — that a real slot "would move the reduced route too, and nobody
  has asked" — was right about the first half and is why the slot serves both;
