...
fptype AvgElementSize(ParMesh &pmesh)
{
   // Compute average mesh size (assumes similar cells).
   fptype dx, loc_area = 0.0;
   for (int i = 0; i < pmesh.GetNE(); i++)
   {
      loc_area += pmesh.GetElementVolume(i);
   }
   fptype glob_area;
   MPI_Allreduce(&loc_area, &glob_area, 1, MPITypeMap<fptype>::mpi_type,
                                        ...
                                        ParGridFunction &dist_v)
{
   ...
   for (int i = 0; i < size; i++)
   {
      const fptype vec_magn = std::sqrt(magn(i) + 1e-12);
      for (int d = 0; d < dim; d++)
      {
                                               ...
                                               ParGridFunction &distance)
{
   ...
   fptype d_min_loc = distance.Min();
   fptype d_min_glob;
   MPI_Allreduce(&d_min_loc, &d_min_glob, 1, MPITypeMap<fptype>::mpi_type,
...
fptype NormalizationDistanceSolver::NormalizationCoeff::
Eval(ElementTransformation &T,const IntegrationPoint &ip)
{
   T.SetIntPoint(&ip);
   Vector u_grad;
   u.GetGradient(T, u_grad);
   const fptype u_value  = u.GetValue(T, ip);
...
fptype ScreenedPoisson::GetElementEnergy(const FiniteElement &el,
...
fptype PUMPLaplacian::GetElementEnergy(const FiniteElement &el,
                                       ElementTransformation &trans,
                                       const Vector &elfun)
{
   fptype energy = 0.0;
