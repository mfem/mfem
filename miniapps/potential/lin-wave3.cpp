...
protected:
   ...
   SparseMatrix Mm, Km, Fm;
   BlockMatrix *Kb, *Tb;

   SparseMatrix Kmc, Kmr;
   ...
   Array<int> fs_bdr;
   Array<int> tdof_list_fs,tdof_list_fs_inv;
   double g,current_dt;
   mutable Vector z; // auxiliary vector

public:
   WaveOperator(FiniteElementSpace &f, Array<int> &fs_bdr, Coefficient *motion, double dt_);
   void Assemble();
...
WaveOperator::WaveOperator(FiniteElementSpace &f, Array<int> &fs_bdr_, Coefficient *motion, double dt_)
...
{  
   ...
   {
      marker[tdof_list_fs[i]] = 0;
   }
   int j = 0;

   tdof_list_fs_inv.SetSize(fespace.GetVSize()-tdof_list_fs.Size());

   for (int i = 0; i<marker.Size(); i++)
   {
      if (marker[i]==1) 
      {
          tdof_list_fs_inv[j] = i;
          j++;
      }
   }
...
void WaveOperator::Assemble()
{
   ...
   for (int i = 0; i<tdof_list_fs.Size(); i++)
   {
      Km.EliminateRow(tdof_list_fs[i]);
      Kmc.EliminateRow(tdof_list_fs[i]);
      Kmr.EliminateRowCol(tdof_list_fs[i]);
   }
   // Eliminate interior cols
   for (int i = 0; i<tdof_list_fs_inv.Size(); i++)
   {
      Kmc.EliminateCol(tdof_list_fs_inv[i]);
   }

   // Flux --> at free-surface  
   ConstantCoefficient one(1.0);
   BilinearForm F(&fespace);
   F.AddBdrFaceIntegrator(new DGDiffusionIntegrator(one,0.0,0.0), fs_bdr);
   F.Assemble();
   F.Finalize();
   Fm = F.SpMat();
   ...
   T_solver.SetRelTol(1e-8);
   T_solver.SetAbsTol(0.0);
   T_solver.SetMaxIter(500);
   T_solver.SetKDim(500);
...
*/
 ...
 //  {
      ...
      current_dt = dt;
      double alpha = 1.0/dt;
      double g2a2 = g*g/(alpha*alpha);
      Kb = new BlockMatrix(block_offsets);
     // Kb->SetBlock(0, 0, &Km);
      Kb->SetBlock(0, 1, Scale(g, Mm));
      Kb->SetBlock(1, 0, Scale(-g2a2, Fm));

      Tb = new BlockMatrix(block_offsets);
      Tb->SetBlock(0, 0, &Mm);
  //    Tb->SetBlock(0, 1, Scale(dt*g, Mm));

    //  Kb->SetBlock(1, 0, Scale(-dt*g2a2, Fm));
      Tb->SetBlock(1, 1, Scale(g2a2, Mm));
...
Kb->PrintMatlab();
   ...
   // Compute interior as elliptic extension
  // if (true)
  // {
 ...
 //  }
   ...
   // Combine free-surface and interior
   for (int i = 0; i<tdof_list_fs.Size(); i++)
   {
      int j = tdof_list_fs[i];
      dudt[j] = dui[j];
      dudt[size+j] = dui[size+j];
   }
...
int main(int argc, char *argv[])
{
   ...
   // 8d. Visualisation
   VisItDataCollection visit_dc("solution/lin-wave3", mesh);
   ...
   //    time-step dt). 
   WaveOperator oper(*fespace, fs_bdr, motion, dt);
