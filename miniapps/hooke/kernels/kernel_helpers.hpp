...
CalcGrad(const tensor<fptype, q1d, d1d> &B,
         const tensor<fptype, q1d, d1d> &G,
         tensor<fptype,2,3,q1d,q1d,q1d> &smem,
         const DeviceTensor<4, const fptype> &U,
         tensor<fptype, q1d, q1d, q1d, dim, dim> &dUdxi)
{
   for (int c = 0; c < dim; ++c)
   {
      ...
      MFEM_FOREACH_THREAD(dz,z,d1d)
      {
         MFEM_FOREACH_THREAD(dy,y,d1d)
         {
            MFEM_FOREACH_THREAD(qx,x,q1d)
            {
               ...
               {
                  const fptype input = smem(0,0,dx,dy,dz);
                  u += input * B(qx,dx);
                  v += input * G(qx,dx);
               }
               smem(0,1,dz,dy,qx) = u;
...
template <int dim, int d1d, int q1d>
static inline MFEM_HOST_DEVICE void
CalcGradTSum(const tensor<fptype, q1d, d1d> &B,
             const tensor<fptype, q1d, d1d> &G,
             tensor<fptype, 2, 3, q1d, q1d, q1d> &smem,
             const tensor<fptype, q1d, q1d, q1d, dim, dim> &U,
             DeviceTensor<4, fptype> &F)
{
   for (int c = 0; c < dim; ++c)
   {
      MFEM_FOREACH_THREAD(qz, z, q1d)
 ...
 */
template <int dim, int d1d, int q1d>
static inline MFEM_HOST_DEVICE tensor<fptype, d1d, d1d, d1d, dim>
GradAllShapeFunctions(int qx, int qy, int qz,
                      const tensor<fptype, q1d, d1d> &B,
                      const tensor<fptype, q1d, d1d> &G,
                      const tensor<fptype, dim, dim> &invJ)
{
   tensor<fptype, d1d, d1d, d1d, dim> dphi_dx;
   // G (x) B (x) B
   // B (x) G (x) B
   // B (x) B (x) G
   for (int dx = 0; dx < d1d; dx++)
   {
      for (int dy = 0; dy < d1d; dy++)
      {
         for (int dz = 0; dz < d1d; dz++)
         {
            dphi_dx(dx,dy,dz) =
               transpose(invJ) *
               tensor<fptype, dim> {G(qx, dx) * B(qy, dy) * B(qz, dz),
