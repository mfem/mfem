...
//                                                     Dirichlet temperature BCs
//
//               THE TRANSIENT PROBLEMS ARE 4, 5, 7 AND 9, and until this
//               change not one of them had ever printed a meaningful error.
//               Two defects, both fixed here and both invisible for as long
//               as nothing ran a transient case.
//
...
//               source -- 7 and 9 through ft = exp(t)*ux*uy in GetFFun -- but
//               7 and 9 have u = (exp(t) - 1) ux uy, which is IDENTICALLY
//               ZERO at t = 0, so their initial condition is exact in every
//               space and no projection error is folded into the temporal
//               one. Their data is homogeneous and time INDEPENDENT,
//               ux = x*tanh((1-x)/k) vanishing at both ends, so they exercise
//               the integrator but not the time-varying-datum path; only
//               problem 4 does both, and it is the one to reach for when that
//               path is what is in question.
//
...
int main(int argc, char *argv[])
{
   ...
   args.AddOption(&free_outflow, "-ofl", "--free-outflow", "-no-ofl",
                  ...
                  " when it does. A second-order operator has no free"
   ...
   // The flux components, parsed once. An EMPTY string is "all of them",
   // which is every existing invocation and takes the stock integrators
   // unchanged; anything else selects the restricted family.
   Array<int> fcomps;
   ...
   // Heat flux FE space
   auto V_space = make_unique<ParFiniteElementSpace>(
                     &pmesh, V_coll.get(),
   ...
   for (int ti = 0; ti < nt; ti++)
   {
      ...
      real_t norm_q = (restricted && fcomps.Size() == 0)?(1.)
                      :(ComputeGlobalLpNorm(2., qecoeff, pmesh, irs));
      ...
      // it has no home in a restricted one -- same as qt_a above.
      if (bconv && !restricted)
      {
   ...
   // are the run's result, and every reference in regress_test_par/ --
   // transient or not -- is compared on exactly these two lines.
   if (btime && root)
   {
...
TFunc GetTFun(const ProblemParams &params)
{
   ...
   switch (prob)
   {
      ...
      case Problem::DegenerateAdvectionDiffusion:
         // p = x y + y^2: degree 2 and so EXACT in the discrete spaces at
         // order >= 2 on any mesh; d_xx p = 0, so a flux that also carries x
         // solves the same problem; and it is not zero on the boundary, nor
         // is its flux. See convdiff.cpp for the whole argument.
         return [=](const Vector &x, real_t) -> real_t
         {
...
TFunc GetFFun(const ProblemParams &params)
{
   ...
   switch (prob)
   {
      ...
      case Problem::DegenerateAdvectionDiffusion:
         ...
         // convention every case here follows.
         return [=](const Vector &x, real_t) -> real_t
         {
            return 2. * k - c * x(1);
         };
