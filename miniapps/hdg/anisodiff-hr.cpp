...
// Visualize the mesh in GLVis
bool VisualizeMesh(socketstream &sout, const Mesh &m,
...
int main(int argc, char *argv[])
{
   ...
   args.AddOption(&hdg_tmop_scaling, "-ts", "--tmop-scaling",
                  "HDG TMOP scaling (1=default)");
   ...
   auto X_coll = make_unique<H1_FECollection>(max(order,1), dim);
   auto X_fes = make_unique<FiniteElementSpace>(&mesh, X_coll.get(), dim);
   mesh.SetNodalFESpace(X_fes.get());

   GridFunction nodes(X_fes.get());
   mesh.SetNodalGridFunction(&nodes);

   unique_ptr<TMOP_QualityMetric> metric;

   metric = make_unique<TMOP_Metric_001>();

   TargetConstructor target_c(TargetConstructor::IDEAL_SHAPE_EQUAL_SIZE);
   target_c.SetNodes(nodes);

   auto *tmop_integ = new TMOP_Integrator(metric.get(), &target_c, metric.get());

   IntegrationRules IntRulesLo(0, Quadrature1D::GaussLobatto);
   const int tmop_quad_order = order+1;
   tmop_integ->SetIntegrationRules(IntRulesLo, tmop_quad_order);

   NonlinearForm tmop_form(X_fes.get());
   tmop_form.AddDomainIntegrator(tmop_integ);
   ...
   tmop_integ->SetInitialMeshPos(&nodes);
   Vector nodes_dx(X_fes->GetTrueVSize()); nodes_dx = 0.;
   const real_t en_pre = tmop_form.GetEnergy(nodes_dx);

   constexpr real_t hdg_tmop_scaling0 = 1e5;
   auto *hdg_integ = new HDG_TMOP_Integrator(kcoeff, t_h, tr_h, hdg_tmop_scaling0);
   tmop_form.AddInteriorFaceIntegrator(hdg_integ);

   {
      Array<int> ess_bdr(mesh.bdr_attributes.Max());
      ess_bdr = 1;
      tmop_form.SetEssentialBC(ess_bdr);
   }

   // Setup TMOP solver
   GSSmoother tmop_prec;
   FGMRESSolver tmop_lsolver;
   tmop_lsolver.SetMaxIter(200);
   tmop_lsolver.SetRelTol(1e-6);
   tmop_lsolver.SetAbsTol(0.);
   tmop_lsolver.SetPrintLevel(IterativeSolver::PrintLevel().FirstAndLast());
   tmop_lsolver.SetPreconditioner(tmop_prec);
   TMOPNewtonSolver tmop_solver(IntRulesLo.Get(mesh.GetTypicalElementGeometry(),
                                               tmop_quad_order));
   tmop_solver.SetSolver(tmop_lsolver);
   tmop_solver.SetMaxIter(10);
   tmop_solver.SetRelTol(1e-4);
   tmop_solver.SetAbsTol(0.);
   tmop_solver.SetPrintLevel(1);
   ...
   for (int amr_it = 0; amr_it <= amr_nrefs; amr_it++)
   {
      ...
      if (amr_it < amr_nrefs)
      {
         ...
         hdg_integ->SetInitialMeshPos(&nodes);
         if (nodes_dx.Size() != X_fes->GetTrueVSize())
         {
            nodes_dx.SetSize(X_fes->GetTrueVSize());
            nodes_dx = 0.;
         }
         const real_t en_post = tmop_form.GetEnergy(nodes_dx);

         const real_t tmop_td = (hdg_tmop_scaling * hdg_tmop_scaling0 * en_pre) / fabs(
                                   en_post - en_pre);
         cout << "Setting HDG scaling: " << tmop_td << endl;
         hdg_integ->SetTd(tmop_td);

         tmop_solver.SetOperator(tmop_form);
         Vector b;
         tmop_solver.Mult(b, nodes.GetTrueVector());
         nodes.SetFromTrueVector();

         if (visualization)
         {
            static socketstream m_sock;
            VisualizeMesh(m_sock, mesh, "Optimized mesh", amr_it);
         }
      }
...
bool VisualizeMesh(socketstream &sout, const Mesh &m, const char *name,
                   int iter)
{
