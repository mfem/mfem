                                   ...
                                   const ParFiniteElementSpace &fes)
{
...

   {
      ...
      fptype *A = diag_spmat.WriteData();
      const fptype *d_diag = diag.Read();
...
void HdivSaddlePointSolver::Setup()
{
   ...
   if (convert_map_type)
   {
      ...
      const fptype *d_detJ = geom->detJ.Read();
      fptype *d_w_mix = W_mix_coeff_qf.ReadWrite();
      fptype *d_w = W_coeff_qf.ReadWrite();
      const bool zero_l2 = zero_l2_block;
      MFEM_FORALL(i, n,
      {
         const fptype detJ = d_detJ[i];
   ...
   else
   {
      ...
      const fptype *d_L_diag_unweighted = L_diag_unweighted.Read();
      fptype *d_L_diag = L_diag.ReadWrite();
      MFEM_FORALL(i, L_diag.Size(),
      {
         const fptype d = d_L_diag_unweighted[i];
         d_L_diag[i] /= d*d;
      });
   }
   ...
   // Update the mass RT diagonal for essential DOFs
   {
      ...
      fptype *d_R_diag = R_diag.ReadWrite();
...
void HdivSaddlePointSolver::EliminateBC(Vector &b) const
{
   ...
   z = 0.0;
   const int *d_I = ess_rt_dofs.Read();
   const fptype *d_x_bc = x_bc.Read();
   fptype *d_z = z.ReadWrite();
   ...
   const fptype *d_w = w.Read();
   fptype *d_bF = bF.ReadWrite(); // Need read-write access to set subvector
   MFEM_FORALL(i, n_ess_dofs,
   {
      const int j = d_I[i];
      d_bF[j] = -d_w[j];
