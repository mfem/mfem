...
public:
    ...
    HeatSink(mfem::ParMesh* pmesh_, int vorder=1, int seed=std::numeric_limits<int>::max())
    {
        ...
        dsola=new mfem::DiffusionSolver(pmesh_,vorder);
        dsola->AddMaterial(mat);
        dsola->SetVolInput(1.0);
    ...
    double MeanComplSymm(mfem::Vector& grad)
    {
        ...
        dsola->DelDirichletBC();
        dsola->AddDirichletBC(2,0.0);
        dsola->AddDirichletBC(3,0.0);
        dsola->AssembleTangent();
        ...
        for(int i=0;i<n;i++){
            ...
            sols+=dsola->GetFSolution(); sols*=0.5;
            sola-=dsolv->GetFSolution(); sola*=0.5;
        ...
        grad/=double(2*n);
        rvar=rvar/double(2*n);
        robj=robj/double(2*n);
...
private:
    ...
    mfem::ParGridFunction sols;
    mfem::ParGridFunction sola;
