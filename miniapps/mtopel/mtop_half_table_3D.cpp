...
public:
    ...
    //return 0.3+0.3*sin(2.0*M_PI*x[0]/dx+M_PI/2.0)*sin(2.0*M_PI*x[2]/dx+M_PI/2.0);
    return 0.3+0.3*sin(2.0*M_PI*x[2]/dx+M_PI/2.0);
...
public:
    ...
    double  Compliance(mfem::Vector& grad)
    {
        ...
        esola->AddSurfLoad(4,1.00,0.00,1.0);
        esola->FSolve();
        //esola->GetSol(sol);
...
int main(int argc, char *argv[])
{
...

   {
      ...
      for(int i=1;i<max_it;i++){
          ...
          if(i>100){max_vol=max_vol*2.0/3.0;}
