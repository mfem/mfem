...
void LinIsoElasticityCoefficient::EvalRes(double EE,double nnu,  double* gradu,
                                          double* res)
{
...
void LinIsoElasticityCoefficient::EvalStress(DenseMatrix &ss,
...
void LinIsoElasticityCoefficient::EvalStrain(DenseMatrix &ee,
                                             ElementTransformation &T, const IntegrationPoint &ip)
{
...
void LinIsoElasticityCoefficient::EvalResidual(Vector &rr, Vector &gradu,
                                               ElementTransformation &T, const IntegrationPoint &ip)
{
...
void LinIsoElasticityCoefficient::EvalTangent(DenseMatrix &mm, Vector &gradu,
                                              ElementTransformation &T, const IntegrationPoint &ip)
{
   MFEM_ASSERT(mm.Size()==9,
               "The size of the stiffness tensor should be set to 9.");
   double EE=E->Eval(T,ip);
   double nnu=nu->Eval(T,ip);
   Eval(EE,nnu,mm.GetData());
}

double LinIsoElasticityCoefficient::EvalEnergy(Vector &gradu,
                                               ElementTransformation &T, const IntegrationPoint &ip)
{
   double EE=E->Eval(T,ip);
   double nnu=nu->Eval(T,ip);
...
void NLSurfLoadIntegrator::AssembleFaceVector(const FiniteElement &el1,
...
void NLSurfLoadIntegrator::AssembleFaceGrad(const FiniteElement &el1,
                                            const FiniteElement &el2,
...
double NLVolForceIntegrator::GetElementEnergy(const FiniteElement &el,
                                              ElementTransformation &Tr, const Vector &elfun)
{
   return 0.0;
}

void NLVolForceIntegrator::AssembleElementVector(const FiniteElement &el,
...
void NLVolForceIntegrator::AssembleElementGrad(const FiniteElement &el,
                                               ElementTransformation &Tr,
...
double NLElasticityIntegrator::GetElementEnergy(const FiniteElement &el,
                                                ElementTransformation &Tr, const Vector &elfun)
{
   const int dof=el.GetDof();
   const int dim=el.GetDim();
...
void NLElasticityIntegrator::AssembleElementVector(const FiniteElement &el,
...
void NLElasticityIntegrator::AssembleElementGrad(const FiniteElement &el,
...
void ElasticitySolver::SetNewtonSolver(double rtol, double atol,int miter,
                                       int prt_level)
{
   rel_tol=rtol;
   abs_tol=atol;
...
double ComplianceNLIntegrator::GetElementEnergy(const FiniteElement &el,
                                                ElementTransformation &Tr, const Vector &elfun)
{
   ...
   const IntegrationRule *ir = nullptr;
   int order= 2 * el.GetOrder() + Tr.OrderGrad(&el)+disp->FESpace()->GetOrder(
                 Tr.ElementNo);
   ir=&IntRules.Get(Tr.GetGeometryType(),order);
...
void ComplianceNLIntegrator::AssembleElementVector(const FiniteElement &el,
                                                   ElementTransformation &Tr,
                                                   const Vector &elfun, Vector &elvect)
{
   ...
   const IntegrationRule *ir = nullptr;
   int order= 2 * el.GetOrder() + Tr.OrderGrad(&el)+2*(disp->FESpace()->GetOrder(
                                                          Tr.ElementNo));
   ir=&IntRules.Get(Tr.GetGeometryType(),order);
...
void ComplianceNLIntegrator::AssembleElementGrad(const FiniteElement &el,
                                                 ElementTransformation &Tr,
                                                 const Vector &elfun, DenseMatrix &elmat)
{
...
double InvMechObjIntegrator::GetElementEnergy(const FiniteElement &el,
                                              ElementTransformation &Tr,
                                              const Vector &elfun)
{
   if (disp==nullptr) {return 0.0;}
...
void  InvMechObjIntegrator::AssembleElementVector(const FiniteElement &el,
                                                  ElementTransformation &Tr,
                                                  const Vector &elfun, Vector &elvect)
{
...
void InvMechObjIntegrator::AssembleElementGrad(const FiniteElement &el,
                                               ElementTransformation &Tr,
                                               const Vector &elfun, DenseMatrix &elmat)
{
   {
...
double InvMechObjective::Eval(mfem::ParGridFunction& sol,
...
void InvMechObjective::Grad(mfem::ParGridFunction& sol,
                            mfem::ParGridFunction& adj, Vector& grad)
{
   if (Ecoef==nullptr)
   {
      MFEM_ABORT("Ecoef in ComplianceObjective should be set before calling the Eval method!");
...
void DiffusionSolver::SetNewtonSolver(double rtol, double atol,int miter,
                                      int prt_level)
{
   rel_tol=rtol;
   abs_tol=atol;
...
double ComplianceDiffIntegrator::GetElementEnergy(const FiniteElement &el,
                                                  ElementTransformation &Tr, const Vector &elfun)
{
   ...
   const IntegrationRule *ir = nullptr;
   int order= 4 * el.GetOrder() + Tr.OrderGrad(&el)+2* temp->FESpace()->GetOrder(
                 Tr.ElementNo);
   ir=&IntRules.Get(Tr.GetGeometryType(),order);
...
void ComplianceDiffIntegrator::AssembleElementVector(const FiniteElement &el,
                                                     ElementTransformation &Tr,
                                                     const Vector &elfun, Vector &elvect)
{
   const int dof=el.GetDof();
...
void ComplianceDiffIntegrator::AssembleElementGrad(const FiniteElement &el,
                                                   ElementTransformation &Tr,
                                                   const Vector &elfun, DenseMatrix &elmat)
{
   {
