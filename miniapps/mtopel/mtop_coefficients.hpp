...
public:
   ...
   double EVaR_Find_t(double beta, double alpha=1.0, double eps=1e-4,
                      int max_it=1000)
   {
   ...
   double EVaR_Find_t(double beta, double t, double alpha=1.0, double eps=1e-4,
                      int max_it=1000)
   {
      //find t
      double inc=0.0;
...
{
public:
   LognormalDistributionCoefficient(Coefficient* gf_,double mu_=0.0,
                                    double ss_=1.0):mu(mu_),ss(ss_)
   {
      gf=gf_;
      scale=1.0;
   }
...
{
public:
   UniformDistributionCoefficient(Coefficient* gf_, double a_=0.0,
                                  double b_=1.0):a(a_),b(b_)
   {
      gf=gf_;
   }
...
{
public:
   ThresholdCoefficient(Coefficient* gf_, double t_=1.5, double a_=0.0,
                        double b_=1.0):t(t_),a(a_),b(b_)
   {
      gf=gf_;
   }
...
public:
   ...
   void Sample(int seed=std::numeric_limits<int>::max())
   {
      if (solver==nullptr)
      {
         ...
         solver=new spde::SPDESolver(nu,bc,fes,pmesh->GetComm(),lx,ly,lz,angle_x,
                                     angle_y, angle_z);
      }
