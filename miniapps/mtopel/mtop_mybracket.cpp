...
public:
   ...
   void Solve()
   {
      ...
      for (int i=0; i<7; i++)
      {
         ...
         for (int j=0; j<8; j++)
         {
            if (j!=i)     { esolv->AddDispBC(3+j,4,0.0);}
            if (j!=(i+1)) { esolv->AddDispBC(3+j,4,0.0);}
         }
         esolv->AddSurfLoad(1,1.0,0.0,0.0);
         esolv->FSolve();
         esolv->GetSol(bsolx[i]);
         esolv->AddSurfLoad(1,0.0,1.0,0.0);
         esolv->FSolve();
         esolv->GetSol(bsoly[i]);
         esolv->AddSurfLoad(1,0.0,0.0,1.0);
         esolv->FSolve();
      ...
      /*
      esolv->DelDispBC();
      for(int j=0;j<8;j++){esolv->AddDispBC(3+j,4,0.0);}
      esolv->AddSurfLoad(2,.001,0.0,0.0);
      esolv->FSolve();
      esolv->GetSol(bsolx[4]);
      esolv->AddSurfLoad(2,0.0,.001,0.0);
      esolv->FSolve();
      esolv->GetSol(bsoly[4]);
      esolv->AddSurfLoad(2,0.0,0.0,.001);
      esolv->FSolve();
   ...
   double AvgCompliance()
   {
      ...
      for (int i=0; i<7; i++)
      {
         ...
         //eval the objective
         cobj=Compliance(i,fx,fy,fz); rez=rez+cobj;
   ...
   void AvgCompliance(mfem::Vector& grad)
   {
      grad=0.0;
      ...
      for (int i=0; i<7; i++)
      {
         fx=1.0; fy=0.0; fz=0.0;
         //eval the objective
         GetComplianceGrad(i,fx,fy,fz,cgrad);
         grad.Add(1.0,cgrad);
         fx=0.0; fy=1.0; fz=0.0;
         //eval the objective
         GetComplianceGrad(i,fx,fy,fz,cgrad);
