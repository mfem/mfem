...
public:
   ...
   VolForce(double r_,double x, double y, double z, double fx, double fy,
            double fz):VectorCoefficient(3)
   {
      r=r_;
      cntr.SetSize(3);
      frce.SetSize(3);
   ...
   void Set(double r_,double x, double y, double z, double fx, double fy,
            double fz)
   {
      r=r_;
      cntr[0]=x;
...
int main(int argc, char *argv[])
{
   ...
   mfem::Vector vdens; vdens.SetSize(fsolv->GetDesignFES()->GetTrueVSize());
   vdens=0.0;
   oddens=0.5;
   ...
   cobj->SetDesignFES(fsolv->GetDesignFES());
   cobj->SetE(E);
   cobj->SetPoissonRatio(0.2);
   cobj->SetDens(vdens);

   //mfem::PVolumeQoI* vobj=new mfem::PVolumeQoI(fsolv->GetFilterFES());
   mfem::VolumeQoI* vobj=new mfem::VolumeQoI(fsolv->GetDesignFES());
   ...
   mfem::VolumeQoI* ivobj=new mfem::VolumeQoI(fsolv->GetDesignFES());
   ivobj->SetProjection(0.5,32);

   //gradients with respect to the filtered field
   mfem::Vector ograd(fsolv->GetDesignFES()->GetTrueVSize());
   ograd=0.0; //of the objective
   mfem::Vector vgrad(fsolv->GetDesignFES()->GetTrueVSize());
...

   {
      ...
      mfem::ParGridFunction pdesign(fsolv->GetDesignFES()); //true E-modulus
      ...
      for (int i=1; i<max_it; i++)
      {
         ...
         {
            //set xxmin and xxmax
            xxmin=vdens; xxmin-=max_ch;
            xxmax=vdens; xxmax+=max_ch;
         ...
         mma->Update(vdens,ograd,&con,&vgrad,xxmin,xxmax);
         /*
         {
             //project vtmpv
