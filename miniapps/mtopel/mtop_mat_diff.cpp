...
public:
    ...
    {
        mat=new mfem::RndDiffusionMaterial(pmesh_->SpaceDimension());
        ...
        gf->SetCorrelationLen(0.025, 1.0, 0.2);
        gf->SetRotationAngles(0.0,0.0,0.0);
        gf->SetMaternParameter(1.0);
        gf->SetScale(1.0);
        gf->Sample(seed);

        mat->AddRandField(gf,2.0,2.5);

        af=new mfem::RandFieldCoefficient(pmesh_,vorder);
        af->SetCorrelationLen(0.025, 1.0, 0.2);
        gf->SetRotationAngles(0.3,0.0,0.0);
    ...
    void GetRandProj(mfem::ParGridFunction& pgf)
    {
       mfem::Coefficient* rp=mat->GetRndProj();
       pgf.ProjectCoefficient(*rp);

       //mfem::MatrixElementCoefficient rp(mat,1,1);
       //pgf.ProjectCoefficient(rp);
    }
    ...
    double MeanComplMat(mfem::Vector& grad)
    {
        mfem::Vector lgr(grad.Size());
        grad=0.0;
...
int main(int argc, char *argv[])
{
   ...
   mfem::ParGridFunction mtdens(fsolv->GetFilterFES());
...

   {
      ...
      mfem::ParaViewDataCollection paraview_dc(paraview_file_name.str(), &pmesh);
      {
         ...
         paraview_dc.RegisterField("proj",&mtdens);
         paraview_dc.Save();
      }
