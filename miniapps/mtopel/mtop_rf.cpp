...
class TrigRandField:public mfem::Coefficient
{
public:
    TrigRandField(double fr1=1.0, double fr2=2.0, MPI_Comm comm=MPI_COMM_WORLD)
    {
        freq1=fr1;
        freq2=fr2;

        std::default_random_engine generator;
        std::normal_distribution<double> cdist(0.0,1.0);
        ...
        MPI_Bcast(A,4,MPI_DOUBLE,0,comm);
    ...
    {
        double xx[3];
        mfem::Vector transip(xx, 3);
        T.Transform(ip, transip);

        double d=sqrt(transip*transip);

        return A[0]*cos(d*freq1)+A[1]*sin(d*freq1)+
                A[2]*cos(d*freq2)+A[3]*sin(d*freq2);
...
{
public:
    RfBracket(mfem::ParMesh* pmesh, int vorder=1):E(),nu(0.2)
    {
    ...
    void Solve()
    {
        ...
        esolv->AddSurfLoad(7,0.0,0.001,0.0);
        esolv->AddDispBC(1,4,0.0);
        esolv->FSolve();
        esolv->GetSol(bsoly[5]);

        esolv->AddSurfLoad(7,0.001,0.000,0.0);
        esolv->FSolve();
        esolv->GetSol(bsolx[5]);
    ...
    void SetDensity(mfem::Vector& vdens_, mfem::Coefficient& eta,
                                    double beta=8.0,double pen=1.0){
        vdens=vdens_;
        pdens.SetFromTrueDofs(vdens);
        E.SetDens(&pdens);
        E.SetProjParam(eta,beta);
    ...
    double MeanCompliance()
    {
        ...
        for(int i=0;i<5;i++){
            cobj=Compliance(i,1.0,0.0,0.0); mean=mean+cobj;
            cobj=Compliance(i,0.0,1.0,0.0); mean=mean+cobj;
        }
...
int main(int argc, char *argv[])
{
   ...
   mfem::ParGridFunction gf(&fes); gf=0.0;
   gf.ProjectCoefficient(spderf);
   ...
   /*
   {
       ...
       paraview_dc.RegisterField("rf",&gf);
       paraview_dc.Save();

       for(int p=0;p<10;p++){
       spderf.Generate();
       gf.ProjectCoefficient(spderf);
       paraview_dc.SetCycle(1+p);
       paraview_dc.SetTime(1.0+p);
       paraview_dc.Save();
       }
   ...
   RfBracket* alco=new RfBracket(&pmesh,1);
