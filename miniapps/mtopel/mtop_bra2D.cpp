...
public:
   ...
   double CVar()
   {
      ...
      for (int i=0; i<16; i++)
      {
         vv=Compliance(i,1.0,0.0,0.0);
         vals.push_back(std::tuple<double,double,int,int>(vv,prob[i],i,0));
         vv=Compliance(i,0.0,1.0,0.0);
         vals.push_back(std::tuple<double,double,int,int>(vv,prob[i],i,1));
         vv=Compliance(i,0.0,0.0,1.0);
         vals.push_back(std::tuple<double,double,int,int>(vv,prob[i],i,2));
      }
   ...
   double CVar(mfem::Vector& grad, double beta=0.98)
   {
      ...
      std::sort(vals.begin(),vals.end(),
                std::greater<std::tuple<double,double,int,int>>());
      double var;
      double cvar;
      int vi=47;
      double cp;
