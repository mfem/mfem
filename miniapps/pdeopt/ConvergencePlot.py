...
plt.savefig('CompliancePlotOrder%i.pdf' % order)

fig2,(ax2) = plt.subplots(1, 1)
ax2.plot(steps_GD,MassFraction_GD,'-k',lw=1.5,markersize = 9.0, alpha=.7, label=r'GD: Fixed Stepsize')
ax2.plot(steps_Subgrad,MassFraction_Subgrad,'-r',lw=1.5,markersize = 9.0, alpha=.7, label=r'GD: Decaying Stepsize')
ax2.plot(steps_KL,MassFraction_KL,'-g',lw=1.5,markersize = 9.0, alpha=.7, label=r'Mirror Descent (KL-Divergence)')
ax2.plot(steps_Sigmoid,MassFraction_Sigmoid,'-b',lw=1.5,markersize = 9.0, alpha=.7, label=r'Mirror Descent (Binary Entropy)')
ax2.set_ylabel(r"Mass Fraction")
ax2.set_xlabel(r"Design Update")
ax2.legend(fontsize=15)
ax2.grid(True)

ax2.set_title(r'Convergence: Order %i FEM Discretization' % order  )
plt.savefig('MassFractionPlotOrder%i.pdf' % order)

fig3,(ax3) = plt.subplots(1, 1)
ax3.plot(steps_GD,BinaryEntropy_GD,'-k',lw=1.5,markersize = 9.0, alpha=.7, label=r'GD: Fixed Stepsize')
ax3.plot(steps_Subgrad,BinaryEntropy_Subgrad,'-r',lw=1.5,markersize = 9.0, alpha=.7, label=r'GD: Decaying Stepsize')
ax3.plot(steps_KL,BinaryEntropy_KL,'-g',lw=1.5,markersize = 9.0, alpha=.7, label=r'Mirror Descent (KL-Divergence)')
ax3.plot(steps_Sigmoid,BinaryEntropy_Sigmoid,'-b',lw=1.5,markersize = 9.0, alpha=.7, label=r'Mirror Descent (Binary Entropy)')
ax3.set_ylabel(r"Binary entropy")
ax3.set_xlabel(r"Design Update")
ax3.legend(fontsize=15)
ax3.grid(True)

ax3.set_title(r'Entropy: Order %i FEM Discretization' % order  )
