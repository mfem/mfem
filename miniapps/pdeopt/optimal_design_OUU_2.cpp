...
private:
   ...
   double compliance = 0.;
public:
   PoissonSolver(Mesh * mesh_, int order_, RandomFunctionCoefficient * damage_coeff_) :
   mesh(mesh_), order(order_), damage_coeff(damage_coeff_) { }
...
int main(int argc, char *argv[])
{
   ...
   PoissonSolver * psolver = new PoissonSolver(&mesh, order, &damage_coeff);
   psolver->Setup();
