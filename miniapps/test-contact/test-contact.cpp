...
int main(int argc, char *argv[])
{
   ...
   args.AddOption(&use_schwarz, "-schwarz", "--schwarz", "-no-schwarz",
                  "--no-schwarz", "Enable Schwarz-based subspace solver (requires -amgf).");
   ...
   // Validate Schwarz requires AMGF
   if (use_schwarz && !amgf)
   {
      if (myid == 0)
      {
   ...
   for (int i = 0; i<total_steps; i++)
   {
      ...
      if (amgf)
      {
         ...
         if (use_schwarz)
         {
            ...
            {
               // CG solver with Schwarz preconditioner
            ...
            {
               // Direct Schwarz (current behavior)
               subspacesolver = schwarz_ptr;
            }
         }
         else
         {
