/**
 * Serial examples for SQEqualityOptimizer.
 *
 * Mirrors test_mma_equality_serial.cpp exactly, swapping the
 * history-adaptive MMA asymptotes for SQEqualityOptimizer's stateless,
 * symmetric SQ trust region (SetSigmaScale() instead of SetAsymptotes()).
 *
 * Covers:
...
void TestAffineRestoration()
{
   ...
   Check(opt.NumIterations()==0,"restoration does not advance SQ history");
}
...
void TestNonlinearRestoration()
{
   ...
   Check(opt.NumIterations()==0,"nonlinear restoration leaves SQ history untouched");
}
...
void TestGlobalizationRetries()
{
   ...
   opt.UpdateGCMMA(x,df0,0.0,h,&dh,xmin,xmax,
      [&](const Vector &candidate,Vector &true_h,real_t &true_f0)
      {
         ...
         // than the initial SQ objective model.
...
void TestGlobalizationRollback()
{
   ...
   Check(opt.NumIterations()==0,"rejected step does not advance SQ history");
}
...
int main()
{
   std::printf("=== Serial equality-only SQ examples ===\n");
   ...
   std::printf("\n%s\n",failures==0 ? "All serial examples PASSED."
                                      : "Serial equality-only SQ failures detected.");
