/**
 * test_mma_device.cpp  —  Device-aware MMA test suite
 *
                                     ...
                                     bool gcmma = false)
{
    ...
    return {kkt, GlobalSum(xloc)/(double)n, opt.GetIteration(), it>0?ms/it:0};
}
    ...
    auto Bench = [&](bool on_dev, const char* label) {
        ...
        if(g_rank==0)
            printf("  %s: kkt=%.2e  xmean=%.4f  iters=%d  total=%.0fms  %.2fms/iter\n",
                   label, kkt, xmean, opt.GetIteration(), ms, it>0?ms/it:0);
