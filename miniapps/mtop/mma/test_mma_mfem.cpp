...
static void Test_ComplianceProxy(int n, double Vfrac)
{
    ...
    MMAOptimizerParallel opt(comm, n_local, 1);
    double kkt=1.0;
    for (int it=0; it<300&&kkt>1e-5; ++it) {
        double gl=0;
...
static void Test_TwoConstraints(int n, double V1, double V2)
{
    ...
    MMAOptimizerParallel opt(comm,n_local,2,a2,c2,d2);
    double kkt=1.0;
...
static void Test_ParallelScale(int n)
{
    ...
    MMAOptimizerParallel opt(comm,n_local,1,a1,c1,d1);
    double kkt=1.0;
...
static void Test_HundredConstraints()
{
    ...
    MMAOptimizerParallel opt(comm,n_local,m,av.data(),cv_v.data(),dv.data());
    double kkt=1.0;
