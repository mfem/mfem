/**
 * test_ccsa_serial.cpp  —  Serial CCSAOptimizer test suite
 *
 * Tests mfem_mma::CCSAOptimizer (single process, mfem::Vector).
 * Covers: plain Update, UpdateGCMMA, all 8 problem types mirrored from
 * test_mma_serial.cpp, so numbers are directly comparable between MMA and
 * CCSA. SetAsymptotes has no CCSA analogue (no asymptotes — see
 * CCSA_Bregman_MFEM.hpp); that slot is replaced with Test_DualSolverKinds,
 * which exercises SetDualSolver(ProjectedAscent / BarrierNewton) instead.
 *
 * CCSAOptimizer works directly on the LATENT variable eta: Update()/
 * UpdateGCMMA()/KKTresidual() take/return eta, not the physical design, and
 * take no xmin/xmax. Every test builds a BoundsGeometry up front,
 * constructs the optimiser with it, seeds eta = opt.ToLatent(x0) from an
 * initial physical guess, and converts back via opt.ToPhysical(eta)
 * whenever it needs the physical point to evaluate the objective/
 * constraints or to report results.
 *
 * The MinMax (z-variable) test carries over unchanged in spirit:
 * CCSAOptimizer uses the identical a/c/d elastic-variable penalty
 * mechanism as MMAOptimizer (same z >= 0, a^T*lambda <= a0 dual feasible
 * region), so min-max problems work exactly the same way.
 *
 * Build:  cmake --build build && ./build/test_ccsa_serial
 *         (links MMA_MFEM.cpp + CCSA_Bregman_MFEM.cpp)
 */
...
// ── helper: compliance objective and gradient (evaluated at physical x) ───
...
static void Test_ComplianceProxy(int n, double Vfrac, bool gcmma)
{
    ...
    for (int it = 0; it < 300 && kkt > 1e-5; ++it) {
        ...
        if (gcmma) {
            int inner;
            opt.UpdateGCMMA(eta, df0, f0, fival, &dg, &inner);
        } else {
...
static void Test_MinMax()
{
    ...
    for (int it = 0; it < 200 && kkt > 1e-5; ++it) {
        ...
        kkt = opt.KKTresidual(eta, df0, 0.0, fival, dg, lam.data());
        if (it % 10 == 0)
...
static void Test_TwoConstraints(int n, double V1, double V2)
{
    ...
    Vector xmin(n), xmax(n), df0(n), dg0(n), dg1(n);
    ...
    BoundsGeometry bounds = BoundsGeometry::TwoSided(xmin, xmax);
    CCSAOptimizer opt(n, 2, bounds, a2, c2, d2);
    Vector x0(n); x0=0.5;
    Vector eta = opt.ToLatent(x0);
    Vector x(n);
    Vector dg[2] = {dg0, dg1};
...
static void Test_ThreeConstraints(int n, double V1, double V2, double V3)
{
    ...
    BoundsGeometry bounds = BoundsGeometry::TwoSided(xmin, xmax);
    CCSAOptimizer opt(n,3,bounds,a3,c3,d3);
    Vector x0(n); x0=0.5;
    Vector eta = opt.ToLatent(x0);
    Vector x(n);
    double kkt=1.0;
...
static void Test_HundredConstraints()
{
    ...
    BoundsGeometry bounds = BoundsGeometry::TwoSided(xmin, xmax);
    CCSAOptimizer opt(n,m,bounds,av.data(),cv_v.data(),dv.data());
    Vector x0(n); x0=0.5;
    Vector eta = opt.ToLatent(x0);
    Vector x(n);
    double kkt=1.0;
...
// ============================================================
// Test 9 — DualSolverKinds  (CCSA-specific; replaces MMA's SetAsymptotes)
// ============================================================
static void Test_DualSolverKinds()
{
    printf("\n--- DualSolverKinds (n=100, m=1) ---\n");
    const int n=100;
    ...
    auto run = [&](DualSolverKind kind, const char* label) -> std::pair<double,double> {
        ...
        for(int it=0;it<300&&kkt>1e-5;++it){
            ...
            double g=0;for(int j=0;j<n;++j)g+=double(x(j));g=g/n-Vfrac;
            mfem::Vector fival(1); fival(0)=g;
            opt.Update(eta,df0,f0,fival,&dg);
            x = opt.ToPhysical(eta);
            ComplianceGrad(x,f0,df0);g=0;for(int j=0;j<n;++j)g+=double(x(j));g=g/n-Vfrac;fival(0)=g;
            kkt=opt.KKTresidual(eta,df0,f0,fival,&dg);
        }
        x = opt.ToPhysical(eta);
        double xmean=0;for(int j=0;j<n;++j)xmean+=double(x(j));xmean/=n;
        printf("  [%-13s] Final: xmean=%.6f  kkt=%.2e\n",label,xmean,kkt);
        return {xmean,kkt};
    };
    ...
    Check(kkt_pa<1e-4,                 "[ProjectedAscent] KKT < 1e-4 with default dual solver");
    Check(std::abs(xmean_pa-0.4)<0.01, "[ProjectedAscent] Volume fraction satisfied");
    Check(kkt_bn<1e-4,                 "[BarrierNewton] KKT < 1e-4 with custom dual solver");
...
//   (shared detail::SolveDense(), not duplicated in CCSA_Bregman_MFEM.cpp)
...
int main()
{
    printf("=== MFEM CCSA (Fermi-Dirac Bregman) Serial test suite ===\n");
