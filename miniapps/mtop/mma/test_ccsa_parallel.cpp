/**
 * test_ccsa_parallel.cpp  —  Parallel CCSAOptimizerParallel test suite
 *
 * Tests mfem_mma::CCSAOptimizerParallel (MPI, distributed mfem::Vector).
 * Covers: plain Update (ProjectedAscent dual solver, the default),
 * UpdateGCMMA, all problem types mirrored from test_mma_parallel.cpp, plus
 * two tests specific to CCSA that have no MMA analogue:
 *   - DualSolverKinds: same problem solved with both ProjectedAscent and
 *     BarrierNewton, checking they agree.
 *   - OneSidedBounds: BoundsGeometry::LowerOnly (no upper bound at all),
 *     a geometry MMA cannot express since it always has an upper asymptote.
 ...
 * whenever it needs the physical point to evaluate the objective/
 * constraints or to report results.
 *
 * Build:  cmake --build build   (links MMA_MFEM.cpp + CCSA_Bregman_MFEM.cpp)
 * Run:    mpirun -np 4 ./build/test_ccsa_parallel
 *         ./build/test_ccsa_parallel        (1 rank — verifies serial-parallel match)
 */
...
// ============================================================
// Test 1/2 — Compliance proxy  (Update and UpdateGCMMA)
...
static void Test_ComplianceProxy(int n, double Vfrac, bool gcmma)
{
    ...
    for (int it = 0; it < 300 && kkt > 1e-5; ++it) {
        ...
        if (gcmma) {
            int inner;
            opt.UpdateGCMMA(eta, df0, 0.0, fival, &dg, &inner);
        } else {
    ...
    if (g_rank==0)
        printf("  Final: xmean=%.6f (%.2f)  kkt=%.2e  iters=%d\n",
               xmean, Vfrac, kkt, opt.GetIteration());
...
static void Test_ThreeConstraints(int n, double V1, double V2, double V3)
{
    ...
    BoundsGeometry bounds = BoundsGeometry::TwoSided(xmin, xmax);
    CCSAOptimizerParallel opt(comm,nl,3,bounds,a3,c3,d3);
    Vector x0(nl); x0=0.5;
    Vector eta = opt.ToLatent(x0);
    Vector x(nl);
    double kkt=1.0;
...
// ============================================================
// Test 8 — DualSolverKinds  (CCSA-specific; replaces MMA's SetAsymptotes)
// ============================================================
static void Test_DualSolverKinds()
{
    const int n=200;
    if(g_rank==0) printf("\n--- DualSolverKinds (n=%d, m=1) ---\n",n);
    auto [nl,off] = Distribute(n);
    MPI_Comm comm = MPI_COMM_WORLD;
    ...
    auto run = [&](DualSolverKind kind, const char* label) -> std::pair<double,double> {
        ...
        double xl=0;for(int j=0;j<nl;++j)xl+=double(x(j));
        double xmean=GlobalSum(xl)/n;
        if(g_rank==0) printf("  [%-13s] Final: xmean=%.6f  kkt=%.2e\n",label,xmean,kkt);
        return {xmean,kkt};
    };
    ...
    Check(std::abs(xmean_pa-0.4)<0.01,   "[ProjectedAscent] Volume fraction satisfied");
    Check(kkt_bn<1e-4,                   "[BarrierNewton] KKT < 1e-4");
    Check(std::abs(xmean_bn-0.4)<0.01,   "[BarrierNewton] Volume fraction satisfied");
    Check(std::abs(xmean_pa-xmean_bn)<1e-3,
          "[both] ProjectedAscent and BarrierNewton agree on the optimum");
}

// ============================================================
// Test 9 — OneSidedBounds  (CCSA-specific; no MMA analogue)
//   Same objective, but x has ONLY a lower bound (no upper bound at all —
//   BoundsGeometry::LowerOnly), which MMA cannot express since MMA's
//   asymptotes always produce a finite upper move limit. The volume
//   constraint mean(x) ≤ Vfrac keeps the problem well-posed even though x
//   is formally unbounded above.
// ============================================================
static void Test_OneSidedBounds()
{
    const int n=200;
    const double Vfrac=0.4, lo=0.01, scale=1.0;
    if(g_rank==0){
        printf("\n--- OneSidedBounds (n=%d, m=1, LowerOnly bounds) ---\n",n);
        printf("  x_j > %.2f (no upper bound); mean(x) <= %.2f\n",lo,Vfrac);
    }
    auto [nl,off] = Distribute(n);
    MPI_Comm comm = MPI_COMM_WORLD;

    Vector dg(nl); dg=real_t(1.0/n);

    BoundsGeometry bounds = BoundsGeometry::LowerOnly(nl, real_t(lo), real_t(scale));
    CCSAOptimizerParallel opt(comm, nl, 1, bounds);
    ...
    for(int it=0;it<300&&kkt>1e-5;++it){
        ...
        double gl=0;
        for(int j=0;j<nl;++j){df0(j)=real_t(-1.0/(double(x(j))*double(x(j))));gl+=double(x(j));}
        double g=GlobalSum(gl)/n-Vfrac; mfem::Vector fival(1); fival(0)=g;
        opt.Update(eta,df0,0.0,fival,&dg);
        x = opt.ToPhysical(eta);
        gl=0;for(int j=0;j<nl;++j){df0(j)=real_t(-1.0/(double(x(j))*double(x(j))));gl+=double(x(j));}
        g=GlobalSum(gl)/n-Vfrac;fival(0)=g;
        kkt=opt.KKTresidual(eta,df0,0.0,fival,&dg);
        if(g_rank==0&&it%20==0) printf("  iter %3d: g=%.4e kkt=%.4e\n",it,g,kkt);
    }
    x = opt.ToPhysical(eta);
    double xl=0,xminv=1e300;
    for(int j=0;j<nl;++j){xl+=double(x(j)); xminv=std::min(xminv,double(x(j)));}
    double xmean=GlobalSum(xl)/n;
    double xmin_global; MPI_Allreduce(&xminv,&xmin_global,1,MPI_DOUBLE,MPI_MIN,comm);
    if(g_rank==0){
        printf("  Final: xmean=%.6f (%.2f)  xmin_actual=%.4f (>%.2f)  kkt=%.2e  iters=%d\n",
               xmean,Vfrac,xmin_global,lo,kkt,opt.GetIteration());
        Check(kkt<1e-4,                  "KKT < 1e-4 with LowerOnly bounds");
        Check(std::abs(xmean-Vfrac)<0.01,"Volume fraction satisfied");
        Check(xmin_global>lo,            "All x_j strictly above the lower bound");
    }
}

// ============================================================
// Test 10 — RedundantConstraints (parallel)
//   g2 = g0 + g1 exactly — rank-deficient dual Hessian. Exercises the same
//   detail::SolveDense() SVD fallback that MMA relies on (shared code,
//   not duplicated in CCSA_Bregman_MFEM.cpp).
// ============================================================
static void Test_RedundantConstraints()
{
    ...
    for(int it=0;it<300&&kkt>1e-5;++it){
        ...
        double s0=GlobalSum(s0l),s1=GlobalSum(s1l);
        mfem::Vector fival(3); fival(0)=s0/n1-0.4; fival(1)=s1/n2-0.4; fival(2)=fival(0)+fival(1);
        opt.Update(eta,df0,0.0,fival,dg);
        x = opt.ToPhysical(eta);
        s0l=s1l=0;
        for(int j=0;j<nl;++j){int g=off+j;df0(j)=real_t(-1.0/(double(x(j))*double(x(j))));if(g<n1)s0l+=double(x(j));else s1l+=double(x(j));}
        s0=GlobalSum(s0l);s1=GlobalSum(s1l);
        fival(0)=s0/n1-0.4;fival(1)=s1/n2-0.4;fival(2)=fival(0)+fival(1);
        kkt=opt.KKTresidual(eta,df0,0.0,fival,dg);
        if(g_rank==0&&it%20==0)
...
int main(int argc, char** argv)
{
    ...
    if(g_rank==0)
        printf("=== MFEM CCSA (Fermi-Dirac Bregman) Parallel test suite (%d rank(s)) ===\n", nranks);

    // ── CCSA Update (ProjectedAscent, the default dual solver) ───────────
    if(g_rank==0)
        printf("\n── CCSA Update ──────────────────────────────────────────────\n");
    ...
    // ── CCSA UpdateGCMMA (globally-convergent, conservatism-checked) ─────
    if(g_rank==0)
        printf("\n── CCSA UpdateGCMMA ─────────────────────────────────────────\n");
