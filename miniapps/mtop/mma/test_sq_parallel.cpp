 ...
 *
 * Same parallel problems as test_mma_parallel.cpp using SQOptimizerParallel.
 *
 * Tests mfem_mma::SQOptimizerParallel (MPI, distributed mfem::Vector).
 * Covers: SQ Update, SQ-GCMMA UpdateGCMMA, and parallel edge cases.
 * DOFs are distributed uniformly across all MPI ranks.
 *
 * Build:  cmake --build build
 * Run:    mpirun -np 4 ./build/test_sq_parallel
 *         ./build/test_sq_parallel        (1 rank)
 */
...
static double Compliance(const Vector& x, Vector& df0)
{
    double f0=0.0;
...
// ============================================================
// Test 1/2 — Compliance proxy  (SQ and SQ-GCMMA)
...
static void Test_ComplianceProxy(int n, double Vfrac, bool gcmma)
{
    ...
    for (int it = 0; it < 300 && kkt > 1e-5; ++it) {
        double gl = 0.0, f0=GlobalSum(Compliance(x,df0));
        for (int j=0;j<nl;++j) gl+=double(x(j));
        ...
        if (gcmma) {
            int inner;
            opt.UpdateGCMMA(x,df0,real_t(f0),fival,&dg,xmin,xmax,nullptr,&inner);
        } else {
            opt.Update(x,df0,real_t(f0),fival,&dg,xmin,xmax);
        }

        f0=GlobalSum(Compliance(x,df0));
        gl=0; for(int j=0;j<nl;++j)gl+=double(x(j));
...
// ============================================================
static void Test_TwoConstraints(int n, double V1, double V2, bool gcmma=false)
{
    if(g_rank==0) printf("\n--- TwoConstraints (n=%d, V1=%.2f, V2=%.2f, %s) ---\n",
                         n,V1,V2,gcmma?"SQ-GCMMA":"SQ");
    auto [nl,off] = Distribute(n);
    ...
    for(int it=0;it<200&&kkt>1e-5;++it){
        double s0l=0,s1l=0,f0=GlobalSum(Compliance(x,df0));
        for(int j=0;j<nl;++j){
            int g=off+j;
        ...
        if(gcmma) opt.UpdateGCMMA(x,df0,real_t(f0),fival,dg,xmin,xmax);
        else       opt.Update(x,df0,real_t(f0),fival,dg,xmin,xmax);
        f0=GlobalSum(Compliance(x,df0));
        s0l=s1l=0;
    ...
    double s0=GlobalSum(s0l),s1=GlobalSum(s1l);
    if(g_rank==0)
        printf("  Final: mean1=%.6f(%.2f) mean2=%.6f(%.2f) kkt=%.2e iters=%d\n",
               s0/n1,V1,s1/n2,V2,kkt,opt.NumIterations());
    Check(kkt<1e-4,                "KKT < 1e-4");
...
// ============================================================
static void Test_ThreeConstraints(int n,double V1,double V2,double V3,bool gcmma=false)
{
    const int b1=n/3,b2=2*n/3;
    const int sz[3]={b1,b2-b1,n-b2};
    const double Vt[3]={V1,V2,V3};
    if(g_rank==0) printf("\n--- ThreeConstraints (n=%d, V=%.2f/%.2f/%.2f, %s) ---\n",
                         n,V1,V2,V3,gcmma?"SQ-GCMMA":"SQ");
    ...
    SQOptimizerParallel opt(comm,nl,3,x,a3,c3,d3);
    double kkt=1.0;

    for(int it=0;it<200&&kkt>1e-5;++it){
        double sll[3]={0,0,0}; double f0=GlobalSum(Compliance(x,df0));
    ...
    for(int i=0;i<3;++i) MPI_Allreduce(&sll[i],&sl[i],1,MPI_DOUBLE,MPI_SUM,comm);
    if(g_rank==0)
        printf("  Final: means=[%.4f,%.4f,%.4f] kkt=%.2e iters=%d\n",
               sl[0]/sz[0],sl[1]/sz[1],sl[2]/sz[2],kkt,opt.NumIterations());
    Check(kkt<1e-4,"KKT < 1e-4");
    for(int i=0;i<3;++i)
...
static void Test_ParallelScale(int n)
{
    ...
    for(int it=0;it<100&&kkt>1e-5;++it){
        double gl=0,f0=GlobalSum(Compliance(x,df0));
        for(int j=0;j<nl;++j)gl+=double(x(j));
...
static void Test_ConstraintSwitching()
{
    ...
    for(int it=0;it<300&&kkt>1e-5;++it){
        ...
        for(int j=0;j<nl;++j){
            int g=off+j;
            if(g<n1){double xj=double(x(j));df0(j)=real_t(-1.0/(xj*xj));f0l+=1.0/xj;s0l+=xj;}
            else    {df0(j)=1.0;f0l+=double(x(j));s1l+=double(x(j));}
        }
        double s0=GlobalSum(s0l),s1=GlobalSum(s1l);
        ...
        opt.Update(x,df0,real_t(f0),fival,dg,xmin,xmax);
        s0l=s1l=f0l=0;
        for(int j=0;j<nl;++j){int g=off+j;if(g<n1){double xj=double(x(j));df0(j)=real_t(-1.0/(xj*xj));f0l+=1.0/xj;s0l+=xj;}else{df0(j)=1.0;f0l+=double(x(j));s1l+=double(x(j));}}
        s0=GlobalSum(s0l);s1=GlobalSum(s1l);
        f0=GlobalSum(f0l);
        fival(0)=s0/n1-0.5;fival(1)=s1/n2-0.5;
        kkt=opt.KKTresidual(x,df0,real_t(f0),fival,dg,xmin,xmax);
        if(g_rank==0&&it%20==0){
            printf("  iter %3d: g=[%.4f,%.4f] kkt=%.4e\n",
                   it,fival(0),fival(1),kkt);
        }
    }
    double s0l=0,s1l=0;
    ...
    double s0=GlobalSum(s0l),s1=GlobalSum(s1l);
    if(g_rank==0)
        printf("  Final: mean_left=%.5f(0.50) mean_right=%.5f(<0.1) kkt=%.2e iters=%d\n",
               s0/n1,s1/n2,kkt,opt.NumIterations());
    Check(kkt<1e-4,                   "KKT < 1e-4");
...
static void Test_HundredConstraints()
{
    ...
    for(int it=0;it<200&&kkt>1e-5;++it){
        std::vector<double> sll(m,0); double f0=GlobalSum(Compliance(x,df0));
        ...
        opt.Update(x,df0,real_t(f0),fival,dg.data(),xmin,xmax);
        f0=GlobalSum(Compliance(x,df0));
        std::fill(sll.begin(),sll.end(),0);
        for(int j=0;j<nl;++j){int g=off+j;int k=g/region;if(k<m)sll[k]+=double(x(j));}
        MPI_Allreduce(sll.data(),sl.data(),m,MPI_DOUBLE,MPI_SUM,comm);
        for(int k=0;k<m;++k) fival(k)=sl[k]/region-Vtgt[k];
        kkt=opt.KKTresidual(x,df0,real_t(f0),fival,dg.data(),xmin,xmax);
        if(g_rank==0&&it%20==0){
...
// ============================================================
// Test 8 — SetSigmaScale API (parallel)
// ============================================================
static void Test_SetSigmaScale()
{
    const int n=200;
    if(g_rank==0) printf("\n--- SetSigmaScale (n=%d, m=1, sigma=0.3) ---\n",n);
    auto [nl,off] = Distribute(n);
    (void)off;
    MPI_Comm comm = MPI_COMM_WORLD;
    ...
    if(g_rank==0)
        printf("  Final: xmean=%.6f  kkt=%.2e  iters=%d\n",
               xmean,kkt,opt.NumIterations());
    Check(kkt<1e-4,                 "KKT < 1e-4 with custom sigma");
    Check(std::abs(xmean-0.4)<0.01, "Volume fraction satisfied");
}
...
int main(int argc, char** argv)
{
    ...
    if(g_rank==0)
        printf("=== MFEM SQ Parallel test suite (%d rank(s)) ===\n", nranks);
    ...
    Test_TwoConstraints(500,0.30,0.50,true);
    Test_ThreeConstraints(3000,0.25,0.45,0.60,true);
