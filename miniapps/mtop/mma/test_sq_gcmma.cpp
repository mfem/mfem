/**
 * test_sq_gcmma.cpp  —  SQOptimizer UpdateGCMMA API test suite
 *
 * Same problems as test_mma_gcmma.cpp using SQOptimizer::UpdateGCMMA().
 * These tests exercise the no-callback UpdateGCMMA path. For SQ this uses
 * the SQ subproblem without constraint-conservatism retries; callback-loop
 * coverage is provided by test_sq_gcmma_callback.cpp.
 *
 * Tests mfem_mma::SQOptimizer::UpdateGCMMA() (serial) and
 * mfem_mma::SQOptimizerParallel::UpdateGCMMA() (parallel MPI).
 *
 * Problem catalogue
 * ─────────────────
 ...
 *
 * Most cases run both serial (rank 0) and parallel variants. The mixed
 * separable unconstrained case uses SQOptimizerParallel, including one rank.
 *
 * Build:  cmake --build build
 * Run:    ./build/test_sq_gcmma              (1 rank, serial + parallel)
 *         mpirun -np 4 ./build/test_sq_gcmma (4 ranks, parallel path exercised)
 */
...
// ============================================================
// Shared compliance objective evaluation.
// ============================================================
...
static void Test_MinMax()
{
    ...
    // ── parallel, including zero-local-DOF ranks ──────────────────────────
    MPI_Barrier(MPI_COMM_WORLD);
    if(g_rank==0) printf("  [parallel]\n");
    {
        auto [nl,off] = Distribute(1);
        (void)off;
        ...
        for(int it=0;it<200&&kkt>1e-5;++it){
            ...
            double h1=(xv-2)*(xv-2),h2=(xv+2)*(xv+2);
            if(nl){dh1(0)=real_t(2*(xv-2));dh2(0)=real_t(2*(xv+2));}
            Vector fival(2);fival(0)=real_t(h1);fival(1)=real_t(h2);
            Vector dg[2]={dh1,dh2};
            opt.UpdateGCMMA(x,df0,0.0f,fival,dg,xmin,xmax);
            xv=GSum(nl?double(x(0)):0.0);
            h1=(xv-2)*(xv-2);h2=(xv+2)*(xv+2);
            double zh=std::max(h1,h2);
            fival(0)=real_t(h1-zh);fival(1)=real_t(h2-zh);
            if(nl){dh1(0)=real_t(2*(xv-2));dh2(0)=real_t(2*(xv+2));}
            kkt=opt.KKTresidual(x,df0,0.0f,fival,dg,xmin,xmax);
        }
        double xf=GSum(nl?double(x(0)):0.0);
        if(g_rank==0)
            printf("  Final: x=%.6f  h1=%.4f  h2=%.4f  kkt=%.2e  iters=%d\n",
                   xf,(xf-2)*(xf-2),(xf+2)*(xf+2),double(kkt),opt.NumIterations());
        Check(kkt<1e-4,          "[par] KKT < 1e-4");
        Check(std::abs(xf)<0.01, "[par] x near 0");
    }
}
...
static void Test_BlockVolumes(int n, const std::vector<double>& Vtgt)
{
    ...
    if(g_rank==0) {
        ...
        for(int it=0;it<300&&kkt>1e-5;++it){
            ...
            opt.UpdateGCMMA(xs,df0s,real_t(f0),fi,dgs.data(),xmins,xmaxs);
            compliance_grad(xs,f0,df0s);
            for(int k=0;k<m;++k){
                double s=0;
                for(int j=bstart[k];j<bstart[k+1];++j) s+=double(xs(j));
                fi(k)=real_t(s/bsz[k]-Vtgt[k]);
            }
            kkt=opt.KKTresidual(xs,df0s,real_t(f0),fi,dgs.data(),xmins,xmaxs);
            if(it%30==0){
                printf("    iter %3d:", it);
    ...
    if(g_rank==0) printf("  [parallel]\n");
    {
        ...
        for(int it=0;it<300&&kkt>1e-5;++it){
            ...
            opt.UpdateGCMMA(x,df0,real_t(f0),fi,dg.data(),xmin,xmax);
            compliance_grad(x,f0_loc,df0); f0=GSum(f0_loc);
            ...
            MPI_Allreduce(sl.data(),sg.data(),m,MPI_DOUBLE,MPI_SUM,comm);
            for(int k=0;k<m;++k) fi(k)=real_t(sg[k]/bsz[k]-Vtgt[k]);
            kkt=opt.KKTresidual(x,df0,real_t(f0),fi,dg.data(),xmin,xmax);
            if(g_rank==0&&it%30==0){
                printf("    iter %3d:", it);
                for(int k=0;k<m;++k) printf(" g%d=%.3e",k,double(fi(k)));
                printf("  kkt=%.3e\n", double(kkt));
...
static void Test_HundredConstraints()
{
    ...
    if(g_rank==0){
        ...
        printf("  Final: kkt=%.2e  max_err=%.2e  wrong=%d/%d  iters=%d  time=%.0fms\n",
               double(kkt),me,nw,m,opt.NumIterations(),ms);
        Check(kkt<1e-4,   "[serial] KKT < 1e-4");
...
static void Test_ConstraintSwitching()
{
    ...
    if(g_rank==0){
        ...
        for(int it=0;it<400&&kkt>1e-5;++it){
            ...
            for(int j=0;j<n;++j){
                if(j<n1){double xj=double(xs(j));df0s(j)=real_t(-1.0/(xj*xj));f0+=1.0/xj;s0+=xj;}
                else{df0s(j)=1.0;f0+=double(xs(j));s1+=double(xs(j));}
            }
            mfem::Vector fival(2); fival(0)=real_t(s0/n1-0.5); fival(1)=real_t(s1/n2-0.5);
            opt.UpdateGCMMA(xs,df0s,real_t(f0),fival,dgs,xmins,xmaxs);
            s0=s1=f0=0;
            for(int j=0;j<n;++j){
                if(j<n1){double xj=double(xs(j));df0s(j)=real_t(-1.0/(xj*xj));f0+=1.0/xj;s0+=xj;}
                else{df0s(j)=1.0;f0+=double(xs(j));s1+=double(xs(j));}
            ...
            kkt=opt.KKTresidual(xs,df0s,real_t(f0),fival,dgs,xmins,xmaxs);
            if(it%40==0)
                printf("    iter %3d: g=[%.4f,%.4f]  kkt=%.3e\n",
                       it,double(fival(0)),double(fival(1)),double(kkt));
        }
        double s0=0,s1=0;
        ...
        printf("  Final: mean_left=%.5f(0.50)  mean_right=%.5f(<0.1)  kkt=%.2e  iters=%d\n",
               s0/n1,s1/n2,double(kkt),opt.NumIterations());
        Check(kkt<1e-4,                   "[serial] KKT < 1e-4");
    ...
    if(g_rank==0) printf("  [parallel]\n");
    {
        ...
        for(int it=0;it<400&&kkt>1e-5;++it){
            ...
            mfem::Vector fival(2); fival(0)=real_t(s0/n1-0.5); fival(1)=real_t(s1/n2-0.5);
            double f0=GSum(f0l);
            opt.UpdateGCMMA(x,df0,real_t(f0),fival,dg,xmin,xmax);
            s0l=s1l=f0l=0;
...
static void Test_LargeN(int n)
{
    ...
    if(g_rank==0){
        ...
        auto t0=Clock::now();
        for(int it=0;it<200&&kkt>1e-5;++it){
            double g=0,f0; compliance_grad(xs,f0,df0s);
            for(int j=0;j<n;++j) g+=double(xs(j));
            mfem::Vector fival(1); fival(0)=real_t(g/n-0.4);
            opt.UpdateGCMMA(xs,df0s,real_t(f0),fival,&dgs,xmins,xmaxs);
            compliance_grad(xs,f0,df0s);
            g=0;for(int j=0;j<n;++j)g+=double(xs(j));
        ...
        printf("  Final: xmean=%.6f(0.40)  kkt=%.2e  iters=%d  time=%.0fms\n",
               xmean,double(kkt),opt.NumIterations(),ms);
        Check(kkt<1e-4,                   "[serial] KKT < 1e-4");
    ...
    if(g_rank==0) printf("  [parallel]\n");
    {
        ...
        auto t0=Clock::now();
        for(int it=0;it<200&&kkt>1e-5;++it){
            double gl=0,f0_loc,f0; compliance_grad(x,f0_loc,df0); f0=GSum(f0_loc);
...
// 6. Unconstrained (m=0)
//    Two objective types:
//    (a) Quadratic bowl: min mean((x-t)^2),  x* = t  (interior)
//    (b) Mixed separable: min mean(a/x + b*x),  x* = sqrt(a/b)
// ============================================================
static void Test_Unconstrained(int n)
{
    ...
    uint64_t s = 12345ULL;
    for(int g=0;g<off;++g){ lcg(s); lcg(s); lcg(s); }
    ...
    if(g_rank==0){
        ...
        for(int it=0;it<200&&maxerr>0.005;++it){
            ...
            for(int j=0;j<n;++j){
                ...
                df0s(j)=real_t(2.0*r/n);
                f0+=r*r/n;
            }
            opt.UpdateGCMMA(xs,df0s,real_t(f0),xmins,xmaxs);
            f0=0;
            maxerr=0;
            for(int j=0;j<n;++j){
                double r=double(xs(j))-double(tgts(j));
            ...
            kkt=opt.KKTresidual(xs,df0s,real_t(f0),xmins,xmaxs);
            if(it%20==0) printf("    iter %3d: kkt=%.3e\n",it,double(kkt));
        }
        printf("  Final: maxerr=%.2e  kkt=%.2e  iters=%d\n",maxerr,double(kkt),opt.NumIterations());
    ...
    if(g_rank==0) printf("  [parallel, quadratic bowl]\n");
    {
        ...
        for(int it=0;it<200&&maxerr>0.005;++it){
            ...
            double errl=0;
            for(int j=0;j<nl;++j) errl=std::max(errl,std::abs(double(x(j))-double(target(j))));
            maxerr=GMax(errl);
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    if(g_rank==0) printf("  [parallel, mixed separable f=mean(a/x+b*x), sigma=0.1]\n");
    {
        Vector x(nl),xmin(nl),xmax(nl),df0(nl);
        x=0.5;xmin=0.001;xmax=1.0;
        SQOptimizerParallel opt(comm,nl,0);
        ...
        opt.SetSigmaScale(0.1);
        real_t kkt=1.0; double maxerr=1.0;
        // Bound-active components converge asymptotically with the tighter
        // move width, so allow enough iterations to meet the pointwise test.
        for(int it=0;it<400&&maxerr>0.01;++it){
            double f0_loc=0;
            for(int j=0;j<nl;++j){
                double xj=double(x(j)), a=double(alpha_v(j)), b=double(beta_v(j));
                double g=(-a/(xj*xj)+b)/n; df0(j)=real_t(g);
                f0_loc+=(a/xj+b*xj)/n;
            }
            double f0=GSum(f0_loc);
            opt.UpdateGCMMA(x,df0,real_t(f0),xmin,xmax);
...
int main(int argc, char** argv)
{
    ...
    if(g_rank==0)
        printf("╔══════════════════════════════════════════════╗\n"
               "║   SQ UpdateGCMMA API suite  (%2d rank(s))      ║\n"
               "╚══════════════════════════════════════════════╝\n\n",
               nranks);
    ...
    if(g_rank==0){
        printf("\n╔══════════════════════════════════════════════╗\n");
        if(g_nfail==0)
            printf("║   All UpdateGCMMA API tests PASSED.          ║\n");
        else
            printf("║   %d UpdateGCMMA test(s) FAILED.%-15s║\n",g_nfail,"");
        printf("╚══════════════════════════════════════════════╝\n");
    }
    MPI_Finalize();
