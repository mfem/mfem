/**
 * MPI examples for SQEqualityOptimizerParallel.
 *
 * Mirrors test_mma_equality_parallel.cpp exactly, swapping the
 * history-adaptive MMA asymptotes for SQEqualityOptimizerParallel's
 * stateless, symmetric SQ trust region (SetSigmaScale() instead of
 * SetAsymptotes()).
 */
...
void TestAffineRestoration()
{
   ...
   SQEqualityOptimizerParallel opt(MPI_COMM_WORLD,nl,1);
   ...
   Check(opt.NumIterations()==0,"parallel restoration does not advance SQ history");
}
...
void TestRedundantEqualities()
{
   ...
   SQEqualityOptimizerParallel opt(MPI_COMM_WORLD,nl,2);
   opt.SetSigmaScale(0.3);
   real_t kkt=1.0;
...
void TestManyEqualities()
{
   ...
   SQEqualityOptimizerParallel opt(MPI_COMM_WORLD,nl,m);
   opt.SetSigmaScale(0.3);
   real_t kkt=1.0;
...
int main(int argc,char **argv)
{
   ...
   if(rank_id==0)
      std::printf("=== Parallel equality-only SQ examples (%d ranks) ===\n",rank_count);
   ...
   if(rank_id==0)
      std::printf("\n%s\n",failures==0 ? "All parallel examples PASSED."
                                         : "Parallel equality-only SQ failures detected.");
