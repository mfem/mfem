/**
 * Nonconvex density-filtered SIMP tests for equality-only SQ.
 *
 * Mirrors test_mma_equality_nonconvex.cpp exactly (which itself mirrors
 * test_mma_nonconvex.cpp), but drives SQEqualityOptimizerParallel instead
 * of MMAEqualityOptimizerParallel -- a stateless, symmetric SQ trust
 * region (SetSigmaScale()) in place of history-adaptive MMA asymptotes.
 * Fixed volume is represented as the single true equality
 *
 *                 h(x) = mean(x) - Vfrac = 0,
 *
 * rather than as two inequalities in the generalized MMA formulation.
 * Both the ordinary equality-only SQ update and its callback-based,
 * objective-globalized GCMMA update are exercised in MPI runs.
 */
               ...
               int radius,bool gcmma,int max_iterations)
{
   ...
   dh=real_t(1.0/n);
   SQEqualityOptimizerParallel optimizer(MPI_COMM_WORLD,problem.n_local,1);
   optimizer.SetSigmaScale(0.3);
              ...
              bool gcmma,int max_iterations,const char *label)
{
   if(rank_id==0)
      std::printf("\n--- %s n=%d Vfrac=%.2f r=%d p=%s [%s] ---\n",
                  ...
                  gcmma ? "equality GCMMA" : "equality SQ");
   ...
   Check(result.iterations>0,tag+"performs accepted SQ iterations");
}
