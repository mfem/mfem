 ...
 * Equality-constraint tests for SQOptimizer and SQOptimizerParallel.
 *
 ...
 *
 *   Callers supply logical layouts [fi_ineq | h_eq] and [dfi_ineq | dh_eq];
 *   PackFival and PackedDfidx expand the internal +/- equality representation.
 *
 ...
 *     min  (1/n) Σ 1/xⱼ
 *     s.t. mean(x[n/2:]) ≤ Vfrac+0.05  (inequality)
 *          mean(x) = Vfrac  (equality)
 *     Tests that multiplier signs are correct simultaneously.
 *
 *  4. UpdateGCMMA equality API  (same as Test 1 using UpdateGCMMA)
 *
 ...
 * Run:    ./build/test_sq_equalities
 *         mpirun -np 4 ./build/test_sq_equalities
 */
...
static void Test_PureEquality()
{
    ...
    for (int it=0;it<1000&&kkt>1e-9 && !std::isnan(double(kkt));++it){
        ...
        // re-evaluate for KKT
        f0=0;
        for(int j=0;j<n;++j){double r=double(x(j))-double(a(j));df0(j)=real_t(2.0*r/n);f0+=r*r/n;}
    ...
    if(g_rank==0)printf("  kkt=%.2e  xmean=%.6f(%.2f)  max_err_vs_analytic=%.2e  iters=%d\n",
                        double(kkt),xmean,Vfrac,err,opt.NumIterations());
...
static void Test_MultipleEqualities()
{
    ...
    double a1mean=0,a2mean=0;
    for(int j=0;j<half;++j)a1mean+=double(a(j))/half;
    for(int j=half;j<n;++j)a2mean+=double(a(j))/half;
    double err=0;
    for(int j=0;j<n;++j){
        double expected=double(a(j))-(j<half?a1mean:a2mean)+(j<half?V1:V2);
        err=std::max(err,std::abs(double(x(j))-expected));
    }

    if(g_rank==0)printf("  kkt=%.2e  mean_lo=%.6f(%.2f)  mean_hi=%.6f(%.2f)  err=%.2e  iters=%d\n",
                        double(kkt),s1,V1,s2,V2,err,opt.NumIterations());
    Check(kkt<1e-5,                "converges: KKT < 1e-5");
    ...
    Check(err<1e-2,                 "multiple equalities match analytic optimum");
}
...
static void Test_MixedConstraints()
{
    ...
    for (int it=0;it<1000&&kkt>1e-9&&!std::isnan(double(kkt));++it){
        ...
        f0=0;
        for(int j=0;j<n;++j){double xj=double(x(j));df0(j)=real_t(-1.0/(n*xj*xj));f0+=1.0/(n*xj);}
        xmean=0; for (int j=0;j<n;++j) xmean+=double(x(j)); xmean/=n;
    ...
    double xmean=0; for (int j=0;j<n;++j) xmean+=double(x(j)); xmean/=n;
    double xright=0; for (int j=nhalf;j<n;++j) xright+=double(x(j)); xright/=nright;
    if(g_rank==0)printf("  kkt=%.2e  xmean=%.6f(%.2f)  xright=%.6f(<%.2f)  iters=%d\n",
                        double(kkt),xmean,Vfrac,xright,Vfrac+0.05,opt.NumIterations());
    Check(kkt<1e-5,                   "converges: KKT < 1e-5");
    ...
    Check(xright<Vfrac+0.05-1e-3,     "mixed inequality remains slack");
}

// ============================================================
// Test 4: UpdateGCMMA equality API
...
static void Test_GCMMAEquality()
{
    ...
    xmean/=n;
    if(g_rank==0)printf("  kkt=%.2e  xmean=%.6f(%.2f)  err_vs_analytic=%.2e  iters=%d\n",
                        double(kkt),xmean,Vfrac,err,opt.NumIterations());
    Check(kkt<1e-5,                   "UpdateGCMMA converges: KKT < 1e-5");
    Check(std::abs(xmean-Vfrac)<1e-6, "UpdateGCMMA: equality satisfied");
    Check(err<1e-2,                   "UpdateGCMMA: matches analytic optimum");
}
...
static void Test_ParallelEquality()
{
    ...
    // target: a_j = sin(2*pi*j/n)*0.3 + 0.5
    constexpr double pi=3.141592653589793238462643383279502884;
    Vector a(nl),x(nl),xmin(nl),xmax(nl),df0(nl),dh(nl);
    for(int j=0;j<nl;++j)a(j)=real_t(0.3*std::sin(2*pi*(off+j)/n)+0.5);
    x=real_t(Vfrac); xmin=real_t(0.01); xmax=real_t(1.0);
    ...
    Check(err<1e-2,                    "parallel: matches analytic optimum");
}

// ============================================================
// Test 6: equality recovers feasibility from infeasible start
// Start at x₀ = 0.8 (mean = 0.8 ≠ Vfrac = 0.4).
// The first SQ step must move toward feasibility.
// ============================================================
static void Test_FeasibilityRecovery()
{
    ...
    if(g_rank==0)printf("  kkt=%.2e  xmean=%.6f(%.2f)  iters=%d\n",
                        double(kkt),xmean,Vfrac,opt.NumIterations());
...
int main(int argc, char** argv)
{
    ...
    if (g_rank==0)
        printf("╔══════════════════════════════════════════════════════════╗\n"
               ...
               "║  SQ equality constraints through SolveDualSQ             ║\n"
               "╚══════════════════════════════════════════════════════════╝\n",
               nranks);
