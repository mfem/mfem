/**
 * test_sq_serial.cpp  —  Serial SQOptimizer test suite
 *
 * Same problems as test_mma_serial.cpp but using SQOptimizer.
 * Verifies that the SQ approximation (Svanberg 2007 §5.1) converges
 * to the same optima as MMA on all standard test problems.
 *
 * Problem catalogue (identical to test_mma_serial):
 *  1/2. ComplianceProxy   — min Σ1/xj  s.t. mean(x)≤Vfrac
 *  3.   MinMax            — min max{(x-2)², (x+2)²}
 *  4/5. TwoConstraints    — two block volumes
 *  6.   ThreeConstraints  — three block volumes
 *  7.   ConstraintSwitching
 *  8.   HundredConstraints
 *  9.   SetSigmaScale     — SQ-specific: vary sigma scale parameter
 * 10.   RedundantConstraints
 */
...
static void Test_ComplianceProxy(int n, double Vfrac, bool gcmma)
{
    ...
    for (int it=0; it<300 && kkt>1e-5; ++it) {
        ...
        g=0; for(int j=0;j<n;++j) g+=double(x(j)); g=g/n-Vfrac;
        mfem::Vector fival(1); fival(0)=g;
        if (gcmma) { int inner; opt.UpdateGCMMA(x,df0,f0,fival,&dg,xmin,xmax,nullptr,&inner); }
        else         opt.Update(x,df0,f0,fival,&dg,xmin,xmax);
        ComplianceGrad(x,f0,df0); g=0; for(int j=0;j<n;++j) g+=double(x(j)); g=g/n-Vfrac;
        fival(0)=g; kkt=opt.KKTresidual(x,df0,f0,fival,&dg,xmin,xmax);
        if (it%20==0) printf("  iter %3d: g=%.4e  kkt=%.4e\n", it, g, kkt);
    }
    double xmean=0, maxerr=0;
    for(int j=0;j<n;++j){ xmean+=double(x(j)); maxerr=std::max(maxerr,std::abs(double(x(j))-Vfrac)); }
    xmean/=n;
    printf("  Final: xmean=%.6f(%.2f) max_err=%.2e kkt=%.2e iters=%d\n",
           xmean, Vfrac, maxerr, kkt, opt.NumIterations());
...
static void Test_TwoConstraints(int n, double V1, double V2)
{
    ...
    for(int it=0;it<200&&kkt>1e-5;++it){
        double s1=0,s2=0;
        for(int j=0;j<n;++j){df0(j)=real_t(-1.0/(double(x(j))*double(x(j))));if(j<n1)s1+=double(x(j));else s2+=double(x(j));}
        mfem::Vector fival(2); fival(0)=s1/n1-V1; fival(1)=s2/n2-V2;
        opt.Update(x,df0,0.0,fival,dg,xmin,xmax);
        s1=s2=0; for(int j=0;j<n;++j){df0(j)=real_t(-1.0/(double(x(j))*double(x(j))));if(j<n1)s1+=double(x(j));else s2+=double(x(j));}
        fival(0)=s1/n1-V1; fival(1)=s2/n2-V2;
        kkt=opt.KKTresidual(x,df0,0.0,fival,dg,xmin,xmax);
        if(it%20==0) printf("  iter %3d: g=[%.4e,%.4e] kkt=%.4e\n",it,fival(0),fival(1),kkt);
    }
    double s1=0,s2=0; for(int j=0;j<n;++j){if(j<n1)s1+=double(x(j));else s2+=double(x(j));}
...
static void Test_ThreeConstraints(int n, double V1, double V2, double V3)
{
    ...
    for(int it=0;it<200&&kkt>1e-5;++it){
        ...
        opt.Update(x,df0,0.0,fival,dg,xmin,xmax);
        sl[0]=sl[1]=sl[2]=0; for(int j=0;j<n;++j){int blk=(j<b1)?0:(j<b2)?1:2;df0(j)=real_t(-1.0/(double(x(j))*double(x(j))));sl[blk]+=double(x(j));}
        for(int i=0;i<3;++i) fival(i)=sl[i]/sz[i]-Vt[i];
        kkt=opt.KKTresidual(x,df0,0.0,fival,dg,xmin,xmax);
        if(it%20==0) printf("  iter %3d: g=[%.3e,%.3e,%.3e] kkt=%.4e\n",it,fival(0),fival(1),fival(2),kkt);
    }
    double sl[3]={0,0,0}; for(int j=0;j<n;++j){int blk=(j<b1)?0:(j<b2)?1:2;sl[blk]+=double(x(j));}
    printf("  Final: means=[%.4f,%.4f,%.4f] kkt=%.2e\n",sl[0]/sz[0],sl[1]/sz[1],sl[2]/sz[2],kkt);
    Check(kkt<1e-4,"KKT < 1e-4");
    for(int i=0;i<3;++i) Check(std::abs(sl[i]/sz[i]-Vt[i])<0.01,
        (std::string("Block ")+std::to_string(i+1)+" volume").c_str());
}

// ── Test 7: ConstraintSwitching ───────────────────────────────────────────
static void Test_ConstraintSwitching()
{
    ...
    for(int it=0;it<300&&kkt>1e-5;++it){
        ...
        opt.Update(x,df0,0.0,fival,dg,xmin,xmax);
        s1=s2=0; for(int j=0;j<n;++j){if(j<n1){df0(j)=real_t(-1.0/(double(x(j))*double(x(j))));s1+=double(x(j));}else{df0(j)=1.0;s2+=double(x(j));}}
        fival(0)=s1/n1-0.5; fival(1)=s2/n2-0.5;
        kkt=opt.KKTresidual(x,df0,0.0,fival,dg,xmin,xmax);
        if(it%20==0) printf("  iter %3d: g=[%.4f,%.4f] kkt=%.4e\n",it,fival(0),fival(1),kkt);
    }
    double s1=0,s2=0; for(int j=0;j<n;++j){if(j<n1)s1+=double(x(j));else s2+=double(x(j));}
...
static void Test_HundredConstraints()
{
    ...
    Vector x(n),xmin(n),xmax(n),df0(n); std::vector<Vector> dg(m);
    ...
    for(int it=0;it<200&&kkt>1e-5;++it){
        ...
        if(it%20==0){double gmax=*std::max_element(fival.begin(),fival.end());printf("  iter %3d: g_max=%.4e kkt=%.4e\n",it,gmax,kkt);}
    }
    std::vector<double> sl(m,0); for(int j=0;j<n;++j){int k=j/region;sl[k]+=double(x(j));}
...
// ── Test 9: SetSigmaScale (SQ-specific) ───────────────────────────────────
static void Test_SetSigmaScale()
{
    printf("\n--- SetSigmaScale (n=100, m=1, sigma=0.3/0.5/0.7) ---\n");
    for (double sig : {0.3, 0.5, 0.7}) {
        const int n=100;
        Vector x(n),xmin(n),xmax(n),df0(n),dg(n);
        x=0.5; xmin=0.001; xmax=1.0; dg=real_t(1.0/n);
        SQOptimizer opt(n,1,x);
        ...
        for(int it=0;it<300&&kkt>1e-5;++it){
            ...
            ComplianceGrad(x,f0,df0); g=0; for(int j=0;j<n;++j) g+=double(x(j)); g=g/n-0.4;
            fival(0)=g; kkt=opt.KKTresidual(x,df0,f0,fival,&dg,xmin,xmax);
        }
        double xmean=0; for(int j=0;j<n;++j) xmean+=double(x(j)); xmean/=n;
        printf("  sigma=%.1f: xmean=%.6f kkt=%.2e iters=%d\n",sig,xmean,kkt,opt.NumIterations());
        Check(kkt<1e-4,                   (std::string("KKT<1e-4 sigma=")+std::to_string(sig)).c_str());
        Check(std::abs(xmean-0.4)<0.01,   (std::string("vol satisfied sigma=")+std::to_string(sig)).c_str());
    }
}

// ── Test 10: RedundantConstraints ─────────────────────────────────────────
static void Test_RedundantConstraints()
{
    ...
    for(int it=0;it<300&&kkt>1e-5;++it){
        ...
        opt.Update(x,df0,0.0,fival,dg,xmin,xmax);
        s0=s1=0; for(int j=0;j<n;++j){df0(j)=real_t(-1.0/(double(x(j))*double(x(j))));if(j<n1)s0+=double(x(j));else s1+=double(x(j));}
        fival(0)=s0/n1-0.4; fival(1)=s1/n2-0.4; fival(2)=(s0+s1)/n-0.8;
        kkt=opt.KKTresidual(x,df0,0.0,fival,dg,xmin,xmax);
        if(it%20==0) printf("  iter %3d: g=[%.3e,%.3e,%.3e] kkt=%.4e\n",it,fival(0),fival(1),fival(2),kkt);
    }
    double s0=0,s1=0; for(int j=0;j<n;++j){if(j<n1)s0+=double(x(j));else s1+=double(x(j));}
    printf("  Final: xmean=%.6f m1=%.4f m2=%.4f kkt=%.2e iters=%d\n",(s0+s1)/n,s0/n1,s1/n2,kkt,opt.NumIterations());
