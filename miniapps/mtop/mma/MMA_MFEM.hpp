...
/// @brief Dual IP solver for the Separable Quadratic (SQ) subproblem.
/// Separate from SolveDualIP: uses the correct quadratic primal/grad/hess.
/// @param xk_sq   x^k (linearisation point, device ptr)
/// @param rho_sq  rho[0..m] curvature weights (host, length m+1)
/// @param fival_sq  fi(x^k) values (host, length m)
void SolveDualSQ(
    MPI_Comm comm,
    int n_loc, int m, int n_eq,
    bool use_dev,
    const mfem::real_t* L_loc,    // L = xk - sigma
    const mfem::real_t* U_loc,    // U = xk + sigma
    const mfem::real_t* alpha_loc,
    const mfem::real_t* beta_loc,
    const double* p0_loc,         // sigma^2 * max(df0,0) + reg (host read)
    const double* q0_loc,         // sigma^2 * max(-df0,0) + reg (host read)
    const std::vector<mfem::Vector>& pij_loc,
    const std::vector<mfem::Vector>& qij_loc,
    const std::vector<double>& a_pen,
    const std::vector<double>& c_pen,
...
public:
    ...
    int NumIterations()  const { return iter_; }  ///< alias
...
/**
 * @class SQOptimizer
 * @brief Serial Separable Quadratic (SQ) approximation optimiser.
 *
 * Implements the GCMMA separable quadratic model (Svanberg 2007, §5.1):
 * @f[
 *   \tilde{g}_i^{(k,\ell)}(x) = f_i(x^k)
 *     + \nabla f_i(x^k)^T (x - x^k)
 *     + \frac{\rho_i^{(k,\ell)}}{2}
 *       \sum_j \left(\frac{x_j - x_j^k}{\sigma_j}\right)^2
 * @f]
 * where @f$\sigma_j = s \cdot (x_j^{\max} - x_j^{\min})@f$ is a fixed
 * half-width controlled by SetSigmaScale() (default @f$s = 0.5@f$).
 *
 * The subproblem is separable so the dual has the same interior-point
 * structure as MMA.  Symmetric artificial asymptotes
 * @f$L_j = x_j^k - \sigma_j@f$, @f$U_j = x_j^k + \sigma_j@f$ are
 * rebuilt fresh each outer iteration; no asymptote adaptation is needed.
 *
 * The interface mirrors @c MMAOptimizer exactly.  Use @c Update() for a
 * plain SQ step and @c UpdateGCMMA() for a globally convergent step that
 * increases @f$\rho@f$ until the approximation is conservative.
 *
 * **Equality constraints** are supported via WithEqualities() together with
 * PackFival() / PackedDfidx() at call sites.  Internally an equality
 * @f$h_i(x) = 0@f$ is split into two inequalities @f$+h_i \le 0@f$ and
 * @f$-h_i \le 0@f$; the dual solver handles this ±h pair with an exact
 * analytic update that avoids numerical null-space amplification.
 *
 ...
 *       opt.Update(x, df0, f0, fi, dfi.data(), xmin, xmax);
 *       if (opt.KKTresidual(x, df0, f0, fi, dfi.data(), xmin, xmax) < tol)
 *           break;
 *   }
 * @endcode
 *
 ...
 *       PackedDfidx  dfidx(dfi.data(), n_ineq, dh.data(), n_eq);
 *       opt.Update(x, df0, f0, fival, dfidx.data(), xmin, xmax);
 *       if (opt.KKTresidual(x, df0, f0, fival, dfidx.data(), xmin, xmax) < tol)
 *           break;
 *   }
 * @endcode
 *
 * @note The optimiser is **not** thread-safe.  Each instance maintains mutable
 *       dual state (lam, mu, y, z, rho) and an iteration counter.
 *
 * @see SQOptimizerParallel for the MPI-parallel variant.
 * @see MMAOptimizer for the MMA-based optimiser with adaptive asymptotes.
 */
// ============================================================
...
public:
    ...
    /**
     * @brief Set the σ scaling factor.
     *
     * The half-width of the move region is @f$\sigma_j = s \cdot
     * (x_j^{\max} - x_j^{\min})@f$.  A larger @f$s@f$ allows wider steps;
     * a smaller @f$s@f$ tightens the quadratic approximation.
     *
     * Default: 0.5 (σ = half the variable range).
     *
     * @param s  Scaling factor in (0, 1].
     */
    void SetSigmaScale(mfem::real_t s) { sigma_scale_ = double(s); }

    // ── Equality-constraint factory ───────────────────────────────────────

    /**
     * @brief Create an SQOptimizer with mixed inequality + equality constraints.
     *
     * Internally allocates @c m = n_ineq + 2*n_eq constraint slots and sets
     * @f$c_i = 10^{30}@f$ for the equality slots so that the dual variable
     * for each ±h pair is unconstrained.
     *
     * Use PackFival() and PackedDfidx() to prepare the @c fival and @c dfidx
     * arguments at call sites, and call NumEqualities() / NumConstraints()
     * to query the resulting layout.
     *
     * @param n       Number of design variables.
    ...
    /** @brief Total number of outer iterations performed so far. */
    int NumIterations() const { return iter_; }

    /** @brief Number of equality constraints (as passed to WithEqualities). */
    int NumEqualities()   const { return n_eq_; }
    ...
    /**
     * @brief User-visible constraint count: n_ineq + n_eq.
     *
     * This is the number of constraints the caller supplied (not the
     * internal doubled count for equality encoding).
     */
    ...
    /**
     * @brief Perform one SQ outer iteration (plain, no conservatism check).
     *
     * Builds symmetric asymptotes @f$L = x^k - \sigma@f$,
     * @f$U = x^k + \sigma@f$, assembles the separable quadratic
     * subproblem, solves the dual interior-point system, and updates @p x
     * in place.  The curvature parameter @f$\rho@f$ is set from the
     * gradient ratio formula and kept constant (no inner conservatism loop).
     *
     * @param[in,out] x       Design variables (local on entry, updated on exit).
     * @param[in]     df0dx   Gradient of the objective, @f$\nabla f_0(x^k)@f$.
     * @param[in]     f0val   Objective value @f$f_0(x^k)@f$ (unused internally,
     *                        kept for API symmetry with MMAOptimizer).
     * @param[in]     fival   Constraint values.  Length @c m for pure
     *                        inequalities, or assembled via PackFival() when
     *                        equalities are present.
     * @param[in]     dfidx   Array of @c m constraint gradients (may be
     *                        @c nullptr when @c m == 0).  For equalities use
     *                        PackedDfidx::data().
     * @param[in]     xmin    Lower bounds on @p x.
     * @param[in]     xmax    Upper bounds on @p x.
     */
    void Update(mfem::Vector& x,
                const mfem::Vector& df0dx,
                mfem::real_t f0val,
    ...
    /**
     * @brief Perform one GCMMA outer iteration with conservatism check.
     *
     * Identical to Update() except that after each candidate step the
     * approximation quality is verified.  If @f$f_i(\tilde x) >
     * \tilde{g}_i^{(k,\ell)}(\tilde x)@f$ for any @f$i@f$, @f$\rho@f$ is
     * doubled and the subproblem is resolved (inner iteration).  This
     * guarantees that the accepted step satisfies the approximation.
     *
     * @param[in,out] x       Design variables.
     * @param[in]     df0dx   Objective gradient at @f$x^k@f$.
     * @param[in]     f0val   Objective value at @f$x^k@f$.
     * @param[in]     fival   Constraint values at @f$x^k@f$.
     * @param[in]     dfidx   Constraint gradients at @f$x^k@f$.
     * @param[in]     xmin    Lower bounds.
     * @param[in]     xmax    Upper bounds.
     * @param[in]     eval_fi Callable @c void(x_trial, fival_out, dfidx_out)
     *                        that evaluates constraints at a trial point.
     *                        If @c nullptr, conservatism checking is skipped
     *                        (falls back to plain SQ).
     * @param[out]    innerIter  If non-null, receives the number of inner
     *                           (ρ-doubling) iterations performed.
     */
    void UpdateGCMMA(mfem::Vector& x,
                     const mfem::Vector& df0dx,
                     mfem::real_t f0val,
                     ...
                     std::function<void(const mfem::Vector&,
                                        mfem::Vector&,
                                        mfem::Vector*)> eval_fi = nullptr,
                     int* innerIter = nullptr);

    /**
     * @brief Compute the KKT residual at the current iterate.
     *
     * Uses the same formula as MMAOptimizer::KKTresidual(): the maximum
     * of the projected-gradient norm and complementary-slackness terms,
     * normalised by @f$n + m@f$.
     *
     * @param x       Current design variables.
     * @param df0dx   Objective gradient at @p x.
     * @param f0val   Objective value at @p x (unused, for API symmetry).
     * @param fival   Constraint values at @p x.
     * @param dfidx   Constraint gradients at @p x.
     * @param xmin    Lower bounds.
     * @param xmax    Upper bounds.
     * @return        KKT residual (non-negative; zero at an exact KKT point).
     */
    mfem::real_t KKTresidual(const mfem::Vector& x,
                              const mfem::Vector& df0dx,
                              mfem::real_t f0val,
    ...
    /// @brief Update for unconstrained problems (no fival / dfidx needed).
    ...
    /// @brief GCMMA update for unconstrained problems.
    void UpdateGCMMA(mfem::Vector& x, const mfem::Vector& df0dx, mfem::real_t f0val,
                     const mfem::Vector& xmin, const mfem::Vector& xmax)
    { static const mfem::Vector e; UpdateGCMMA(x,df0dx,f0val,e,nullptr,xmin,xmax); }

    /// @brief KKT residual for unconstrained problems.
...
private:
    ...
    mfem::Vector L_, U_;     ///< Symmetric asymptotes (rebuilt each iter)
    mfem::Vector p0_, q0_;
    mfem::Vector alpha_, beta_;  ///< move limits (= xmin, xmax clipped)
    std::vector<mfem::Vector> pij_, qij_;
    std::vector<double> b_, a_, c_, d_, lam_, mu_, y_;
    double z_ = 0.0;
    std::vector<double> rho_;  ///< GCMMA curvature parameters (length m+1)
 ...
 * @brief MPI-parallel Separable Quadratic (SQ) approximation optimiser.
 *
 * Distributes the @f$n@f$ design variables across MPI ranks; each rank
 * holds a contiguous slice of length @c n_local.  The objective gradient
 * @c df0dx_local and constraint gradients @c dfidx_local are likewise
 * local.  Constraint values @c fival and the objective value @c f0val are
 * **global** (identical on all ranks).
 *
 * The algorithmic behaviour is identical to SQOptimizer: symmetric
 * asymptotes @f$\sigma_j = s \cdot (x_j^{\max} - x_j^{\min})@f$, a
 * separable quadratic subproblem dual, and an optional GCMMA conservatism
 * loop.  All MPI reductions (inner products, norms) are handled internally.
 *
 * Equality constraints are supported through the same WithEqualities() /
 * PackFival() / PackedDfidx() pattern as SQOptimizer.
 *
 * ### Typical usage
 * @code
 ...
 *       real_t kkt = opt.KKTresidual(
 *           x_local, df0_local, f0, fi, dfi_local, xmin, xmax);
 *       if (kkt < tol) break;
 *   }
 * @endcode
 *
 * @note
 * - @c f0val must be the **global** sum (already reduced by the caller).
 * - @c fival must be the **global** constraint vector (same on all ranks).
 * - @c df0dx_local and @c dfidx_local hold only the local slice.
 * - The optimiser is **not** thread-safe within a rank.
 *
 * @see SQOptimizer for the serial variant.
...
public:
     ...
     * @brief Construct with default penalty parameters.
     *
     * @param comm     MPI communicator spanning all ranks that share the
    ...
    /**
     * @brief Set the σ scaling factor (see SQOptimizer::SetSigmaScale).
     * Default: 0.5.
     */
    void SetSigmaScale(mfem::real_t s) { sigma_scale_ = double(s); }
    ...
    /**
     * @brief Create an SQOptimizerParallel with mixed inequality + equality
     *        constraints.
     *
     * Same semantics as SQOptimizer::WithEqualities().
     *
     * @param comm     MPI communicator.
     * @param n_local  Local design-variable count.
    ...
    /**
     * @brief User-visible constraint count: n_ineq + n_eq.
     * Does not count the internal doubled encoding for equalities.
     */
    int NumConstraints()  const { return m_ - n_eq_; }
     ...
     * @brief Perform one SQ outer iteration (plain, no conservatism check).
     *
     * Each rank passes only its local slice of @p x and @p df0dx_local.
     * The solve is collective: all ranks must call this together.
     *
     * @param[in,out] x_local       Local design variables (updated on exit).
     * @param[in]     df0dx_local   Local objective gradient @f$\nabla f_0@f$.
     * @param[in]     f0val         Global objective value (same on all ranks).
     * @param[in]     fival         Global constraint values (length m, same on
     *                              all ranks).  Use PackFival() for equalities.
     * @param[in]     dfidx_local   Local constraint gradients (m vectors, each
     *                              of length n_local; nullptr if m==0).
     *                              Use PackedDfidx::data() for equalities.
     * @param[in]     xmin_local    Local lower bounds.
     * @param[in]     xmax_local    Local upper bounds.
     */
    void Update(mfem::Vector& x_local,
     ...
     * @brief Perform one GCMMA outer iteration with conservatism check.
     *
     * Collective; all ranks must call this together.  The @p eval_fi
     * callable receives the full local trial vector and must write global
     * fival and local dfidx_local (same convention as Update).
     *
     * @param[in,out] x_local       Local design variables.
     * @param[in]     df0dx_local   Local objective gradient.
     * @param[in]     f0val         Global objective value.
     * @param[in]     fival         Global constraint values.
     * @param[in]     dfidx_local   Local constraint gradients.
     * @param[in]     xmin_local    Local lower bounds.
     * @param[in]     xmax_local    Local upper bounds.
     * @param[in]     eval_fi       @c void(x_trial_local, fival_out,
     *                              dfidx_local_out) evaluator.  Pass @c nullptr
     *                              to skip the conservatism check.
     * @param[out]    innerIter     If non-null, receives the inner iteration
     *                              count (number of ρ doublings).
     */
    void UpdateGCMMA(mfem::Vector& x_local,
                     const mfem::Vector& df0dx_local,
                     mfem::real_t f0val,
    ...
    /**
     * @brief Compute the global KKT residual.
     *
     * Each rank contributes its local slice; the result is reduced across
     * all ranks and returned identically on all of them.
     *
     * @param x_local       Local design variables.
     * @param df0dx_local   Local objective gradient.
     * @param f0val         Global objective value.
     * @param fival         Global constraint values.
     * @param dfidx_local   Local constraint gradients.
     * @param xmin_local    Local lower bounds.
     * @param xmax_local    Local upper bounds.
     * @return              Global KKT residual (same value on all ranks).
     */
    mfem::real_t KKTresidual(const mfem::Vector& x_local,
                              const mfem::Vector& df0dx_local,
                              mfem::real_t f0val,
                              ...
                              const mfem::Vector* dfidx_local,
                              const mfem::Vector& xmin_local,
                              const mfem::Vector& xmax_local) const;

    // ── Unconstrained convenience overloads (m=0) ─────────────────────────

    /// @brief Update for unconstrained problems (no fival / dfidx needed).
    void Update(mfem::Vector& x, const mfem::Vector& df0, mfem::real_t f0,
                const mfem::Vector& xmin, const mfem::Vector& xmax)
    { static const mfem::Vector e; Update(x,df0,f0,e,nullptr,xmin,xmax); }

    /// @brief GCMMA update for unconstrained problems.
    void UpdateGCMMA(mfem::Vector& x, const mfem::Vector& df0, mfem::real_t f0,
                     const mfem::Vector& xmin, const mfem::Vector& xmax)
    { static const mfem::Vector e; UpdateGCMMA(x,df0,f0,e,nullptr,xmin,xmax); }

    /// @brief KKT residual for unconstrained problems.
    mfem::real_t KKTresidual(const mfem::Vector& x, const mfem::Vector& df0,
                              mfem::real_t f0, const mfem::Vector& xmin,
                              const mfem::Vector& xmax) const
