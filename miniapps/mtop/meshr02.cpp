...
// Mesh refinement based only in density
//define threshold coefficient
   ...
   for(int par_ref_levels=1; par_ref_levels<max_par;par_ref_levels++)
   {
       ...
       mfem::ParMesh pmesh(MPI_COMM_WORLD, mesh);
       {
          ...
          for (int l = 0; l < par_ref_levels; l++)
          {
             ...
             mfem::L2_FECollection pfec(filter_o, dim);
             ...
             filter.Filter(*densco,gf);
             ThresholdCoefficient tcoeff(gf,eta,512);

             //Define error estimator
             mfem::LpErrorEstimator lpest(2,tcoeff,gf);
             mfem::ThresholdRefiner refiner(lpest);
