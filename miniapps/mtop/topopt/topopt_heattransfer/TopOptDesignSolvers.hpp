...
//    ParFiniteElementSpace filter_fes;
//    ParFiniteElementSpace control_fes;
//    TopOptSteadyStateStokesSolver *stokes_oper;
...
//    ParFiniteElementSpace P_fes, V_fes;
//    Array<int> inlet_bdr;
//    Array<int> noslip_bdr;
//    Array<int> all_ess_bdr;
//    BrinkmanCoefficient &brink_cf;
//    ConstantCoefficient &viscosity_cf;
//    VectorFunctionCoefficient &inlet_func;
//    ParGridFunction u_gf;



//    public:
//    DesignSolverWithStokes(ParFiniteElementSpace &state_fes_,
//                          ParFiniteElementSpace &filter_fes_,
//                          ParFiniteElementSpace &control_fes_,
...
//                          BrinkmanCoefficient &brink_cf_,
//                          ConstantCoefficient &viscosity_cf_,
//                          VectorFunctionCoefficient &inlet_func_,
//                          int imex_integrator_, int vis_steps_, int problem_type_, MPI_Comm comm_, bool pa_)
//       : state_fes(state_fes_), filter_fes(filter_fes_), control_fes(control_fes_),
//         V_fes(V_fes_),
//         P_fes(P_fes_),
...
//         inlet_func(inlet_func_),
//         filter(filter_),
//         objective(objective_), 
//         dt_diff_term(dt_diff_term_), raw_inflow(raw_inflow_), q0(q0_), raw_diff_term(raw_diff_term_),
//         nsteps(nsteps_), dt(dt_), t_final(t_final_),
//         rho(rho_), rho_tilde(rho_tilde_), q_gf(&state_fes_), imex_integrator(imex_integrator_), SIMP_cf(SIMP_cf_), u_gf(&V_fes_),
//         q_vec(nullptr), oper(nullptr), stokes_oper(nullptr), vis_steps(vis_steps_), problem_type(problem_type_), comm(comm_), pa(pa_)
//    { 
//       outer_it = 0;
//       dJ_drho_tilde.SetSize(filter_fes.GetTrueVSize());
//       dJ_drho_tilde = 0.0;
...
//    ~DesignSolverWithStokes() 
//    { 
//       if (oper) delete oper; 
//       if (q_vec) delete q_vec;
//       if (stokes_oper) delete stokes_oper;
//    }
...
//       if (oper) { delete oper; oper = nullptr; }
//       if (q_vec) { delete q_vec; q_vec = nullptr; }
//       if (stokes_oper) { delete stokes_oper; stokes_oper = nullptr; }

//       stokes_oper = new TopOptSteadyStateStokesSolver(
//          V_fes, P_fes,
//          filter_fes,
//          inlet_bdr,
//          noslip_bdr,
...
//       stokes_oper -> SolveStokes();

//       u_gf = stokes_oper->GetU();
...
//          //if(Mpi::Root()){std::cout << "time step: " << ti << ", time: " << t << ", ||q|| = " << q_vec->Norml2() << std::endl;}
//          ti++;
//          oper->SetStep(ti);
//          oper->StoreTraj(ti, *q_vec);
//          oper->SetTime(t);
...
//       // 3. Set the primal GridFunction from the True-Dofs
//       HypreParVector stokes_adjoint_forcing(&V_fes); 
//       stokes_adjoint_forcing = 0.0;
//       HypreParVector lam_vec = *grad_vec;
//       lam_vec *= -1.0;
//       lam_gf.SetFromTrueDofs(lam_vec);
//       oper->SetStep(nsteps);
...
//       // } 
//       real_t t = t_final;
//       //if(Mpi::Root()){std::cout<<"stokes forcing adjoint norm = " << stokes_adjoint_forcing.Norml2() << std::endl;}
//       bool done = false;
//       for (int ti = 0; !done;)
//       {
//          real_t dti = times[nsteps-ti-1]; 
...
//          oper->GetTraj(oper->GetStep() - 1, *q_vec);
//          q_gf.SetFromTrueDofs(*q_vec);
//          Vector lam_vec2 = lam_vec; // need to keep previous lambda
//          //ode_solver->StateGradStep(lam_vec2,*q_vec, stokes_adjoint_forcing, t_dummy, dti, V_fes);
//          //oper->GetTraj(oper->GetStep() - 1, *q_vec);
//          ode_solver->AdjointStep(lam_vec,*q_vec, dJ_drho_tilde, t_dummy, dti);
//          //if(Mpi::Root()){std::cout<<"stokes forcing adjoint norm = " << stokes_adjoint_forcing.Norml2() << std::endl;}
//          ParLinearForm grad_form2(&state_fes);
...
//          lam_vec.Add(-1.0, *grad_vec);
//          ti++;
//          if(Mpi::Root()){std::cout << "time step: " << dti << ", time: " << t << ", ||l|| = " << lam_vec.Norml2() << std::endl;}
...
//       //dJ_drho_tilde = oper->GetDesignGrad();
//       stokes_oper -> AdjointSolve(stokes_adjoint_forcing);
//       // stokes_oper -> AddDesignGradient(dJ_drho_tilde, dt);
//       delete grad_vec;
//    } 
