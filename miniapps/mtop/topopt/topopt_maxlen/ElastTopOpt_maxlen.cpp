...
int main(int argc, char *argv[])
{
    ...
    ParGridFunction alpha(&filter_fes);
    ParGridFunction gamma(&filter_fes);
    alpha = gamma_s * vol_fraction;  // initialize alpha differently to create initial residual
    gamma = vol_fraction;
    ...
    for (; (it <= init_it) || (it <= max_it && iterationError > tol); it++)
    {
        ...
        // (10) tighten the max-length tolerance and update beta
