...
// Transfer operator S^T: submesh -> parent.
// original transfermap would overwrite overlapping dofs
class SubMeshDualTransfer
{
private:
   Array<int> sub_to_par;

public:
   SubMeshDualTransfer(ParFiniteElementSpace &sub_fes,
                        ParFiniteElementSpace &par_fes)
   {
      MFEM_VERIFY(par_fes.IsDGSpace() && sub_fes.IsDGSpace(),
                  "SubMeshDualTransfer: both spaces must be DG");

      ParGridFunction probe_par(&par_fes);
      for (int j = 0; j < probe_par.Size(); j++) { probe_par(j) = j + 1; }

      ParGridFunction probe_sub(&sub_fes);
      probe_sub = 0.0;
      ParSubMesh::Transfer(probe_par, probe_sub);

      sub_to_par.SetSize(sub_fes.GetVSize());
      for (int i = 0; i < sub_to_par.Size(); i++)
      {
         const int j = static_cast<int>(std::lround(probe_sub(i))) - 1;
         MFEM_VERIFY(j >= 0 && j < probe_par.Size(),
                     "SubMeshDualTransfer: trace is not a dof selection");
         sub_to_par[i] = j;
      }
   }

   // y_par = S^T x_sub
   void MultTranspose(const ParGridFunction &x_sub, ParGridFunction &y_par) const
   {
      y_par = 0.0;
      for (int i = 0; i < sub_to_par.Size(); i++)
      {
         y_par(sub_to_par[i]) += x_sub(i);
      }
   }

   // sub dof -> parent dof
   const Array<int> &GetMap() const { return sub_to_par; }
};