...
int main(int argc, char *argv[])
{
    ...
    // 7. Setup Helmholtz filter:  (r² ∇·∇ + I) ρ̃ = ρ
    ConstantCoefficient filter_r2_cf(filter_r * filter_r);

    FilterSolver filter;
    filter.SetMesh(&pmesh);
    filter.SetOrder(order);
    filter.SetDiffusionCoefficient(&filter_r2_cf);
    ...
    for (; k < max_it && iterationError > tol; k++)
    {
        ...
        // (3) Adjoint filter:  (r² K + M) w̃ = -r'(ρ̃) ψ₀(u)
        filter.ASolve();
        ParGridFunction &w_filter = *filter.GetFEMSolution();

        // (4) Objective gradient:  dc/dρ = (w̃, ·)_L2
