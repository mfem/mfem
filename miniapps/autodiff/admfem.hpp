...
private:
   ...
   TFunctor<fptype,const mfem::Vector, mfem::Vector,
...
public:
   ...
   fptype Eval(const mfem::Vector &vparam, mfem::Vector &uu)
   {
      return rf(vparam,uu);
   }
   ...
   typedef codi::RealForwardGen<fptype>    ADFType;
...
private:
   TFunctor<fptype, const mfem::Vector,
...
typedef internal::dual<fptype, fptype> ADFloatType;
...
private:
   /// MFEM native forward AD-type
   typedef internal::dual<fptype, fptype> ADFType;
   /// Vector type for AD-type numbers
   typedef TAutoDiffVector<ADFType> ADFVector;
   /// Matrix type for AD-type numbers
   typedef TAutoDiffDenseMatrix<ADFType> ADFDenseMatrix;
...
private:
   ...
   TFunctor<fptype, const Vector, Vector,
...
public:
   /// Evaluates a function for arguments vparam and uu. The evaluation is
   /// based on the operator() in the user provided functor TFunctor.
   fptype Eval(const Vector &vparam, Vector &uu)
   {
      return tf(vparam,uu);
   }
...
private:
   TFunctor<fptype, const Vector, Vector, state_size, param_size> tf;
