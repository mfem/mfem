   ...
   for (it_oc = 0; it_oc<max_it; it_oc++)
   {
      ...
      if (it_oc % vis_steps == 0)
      {
         ...
         if (use_paraview && !(paraview_dc->Error()))
         {
            ...
            {
               paraview_dc->SetCycle(it_oc);
               paraview_dc->SetTime(it_oc);
            }
            paraview_dc->Save();
         }
      ...
      {
         if (it_oc > min_it) { break; }
      }
   }
   if (Mpi::Root())
   {
      out << filename.str() << " terminated after " << it_oc
