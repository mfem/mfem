...
int main(int argc, char *argv[])
{
   ...
   args.AddOption(&E, "-E", "--youngs-modulus", "Young's modulus E");
   args.AddOption(&nu, "-nu", "--poisson-ratio", "Poinsson ration nu");
   ...
   args.AddOption(&step_size, "-a0", "--init-step", "Initial step size");
   args.AddOption(&use_bregman_backtrack, "-bb", "--bregman-backtrack", "-ab",
                  "--armijo-backtrack",
                  "Option to choose Bregman backtracking algorithm or Armijo "
                  "backtracking algorithm");
   args.AddOption(&use_bregman_stationary, "-bs", "--bregman-stationarity",
                  "-L2", "--L2-stationarity",
                  "Option to choose Bregman stationarity or L2 stationarity for "
                  "stopping criteria");
   ...
   std::unique_ptr<ParMesh> mesh((ParMesh *)GetTopoptMesh(
                                    prob, filename, r_min, tot_vol, min_vol, max_vol, E, nu, ess_bdr_state,
                                    ess_bdr_filter, solid_attr, void_attr, ser_ref_levels, par_ref_levels));
   ...
   if (Mpi::Root())
   {
      std::cout << "\tThe number of elements: " << num_elem << std::endl
                << "\tThe number of control unknowns: " << control_ndof
                << std::endl
                << "\tThe number of filter  unknowns: " << filter_ndof
                << std::endl
                << "\tThe number of state   unknowns: " << state_ndof
                << std::endl;
   }
   ...
   for (auto &isSolid : solid_bdr_filter)
   {
      isSolid = isSolid == 1;
   };
   ...
   MappedGFCoefficient der_simp_cf(filter_gf, [exponent, rho0](const real_t x)
   {
   ...
   filter.GetLinearForm()->AddDomainIntegrator(
      new DomainLFIntegrator(density_cf));
   ...
   DensityBasedTopOpt optproblem(density, control_gf, grad_gf, filter, filter_gf,
                                 grad_filter_gf, elasticity, state_gf);
   if (elasticity.HasAdjoint())
   {
      energy.SetAdjState(optproblem.GetAdjState());
   }
   if (Mpi::Root())
   {
      out << "done" << std::endl;
   ...
   MappedGFCoefficient density_eps_dual_cf =
      entropy.GetBackwardCoeff(control_eps_gf);
   ...
   diff_density_form.AddDomainIntegrator(
      new DomainLFIntegrator(diff_density_cf));
   ...
   if (elasticity.HasAdjoint())
   {
      glvis.Append(optproblem.GetAdjState(), "adjoint displacement", keys);
   }

   real_t stationarity0, obj0, stationarity_error_L2, stationarity_error_bregman,
          stationarity_error, curr_vol, objval(infinity()), old_objval(infinity()),
          succ_obj_diff(infinity());
   ...
   for (it_md = 0; it_md < max_it; it_md++)
   {
      if (Mpi::Root())
      {
         out << "Mirror Descent Step " << it_md << std::endl;
      }
      ...
      for (num_reeval = 0; num_reeval < 1; num_reeval++)
      {
         ...
         real_t grad_diffrho =
            InnerProduct(MPI_COMM_WORLD, diff_density_form, grad_gf);
         real_t bregman_diff = zero_gf.ComputeL1Error(bregman_diff_old);
         real_t target_objval =
            use_bregman_backtrack
         ...
         if (objval < target_objval)
         {
            if (Mpi::Root())
            {
               out << "   Backtracking terminated after " << num_reeval << " re-eval"
                   << std::endl;
            }
      ...
      stationarity_error_bregman =
         std::sqrt(zero_gf.ComputeL1Error(bregman_diff_eps)) / eps_stationarity;

      add(density_gf, -eps_stationarity, grad_gf, control_eps_gf);
      density.ApplyVolumeProjection(control_eps_gf, false);
      stationarity_error_L2 =
         density_gf.ComputeL2Error(density_eps_primal_cf) / eps_stationarity;

      stationarity_error = use_bregman_stationary ? stationarity_error_bregman
                           : stationarity_error_L2;
      logger.Print(true);
      if (Mpi::Root())
      {
   ...
   if (Mpi::Root())
   {
      out << filename.str() << " terminated after " << it_md << " with "
          << tot_reeval << " re-eval" << std::endl;
   }
   logger.CloseFile();
   return 0;
}
