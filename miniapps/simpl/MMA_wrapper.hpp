...
MMA::MMA(PetscInt nn, PetscInt mm, PetscInt kk, Vec xo1t, Vec xo2t, Vec Ut,
         Vec Lt, PetscScalar* at, PetscScalar* ct,
         PetscScalar* dt)
{
...
MMA::MMA(MPI_Comm comm_, PetscInt nn, PetscInt mm, Vec x, PetscScalar* at,
...
// Set the aggresivity of the moving asymptotes
PetscErrorCode MMA::SetAsymptotes(PetscScalar init, PetscScalar decrease,
                                  PetscScalar increase)
{
   PetscErrorCode ierr = 0;
...
PetscErrorCode MMA::KKTresidual(Vec x, Vec dfdx, PetscScalar* fx, Vec* dgdx,
                                Vec xmin, Vec xmax, PetscScalar* norm2,
                                PetscScalar* normInf)
{
   PetscErrorCode ierr = 0;
...
// Set and solve a subproblem: return new xval
PetscErrorCode MMA::Update(Vec xval, Vec dfdx, PetscScalar* gx, Vec* dgdx,
                           Vec xmin, Vec xmax)
{
   PetscErrorCode ierr = 0;
...
PetscErrorCode MMA::GenSub(Vec xval, Vec dfdx, PetscScalar* gx, Vec* dgdx,
                           Vec xmin, Vec xmax)
{
   PetscErrorCode ierr = 0;
   ...
   for (PetscInt i = 0; i < nloc; i++)
   {
      ...
      p0v[i] = pow(Uv[i] - xv[i],
                   2.0) * (dfdxp + 0.001 * Abs(dfdxv[i]) + 0.5 * feps / (Uv[i] - Lv[i]));
      q0v[i] = pow(xv[i] - Lv[i],
                   2.0) * (dfdxm + 0.001 * Abs(dfdxv[i]) + 0.5 * feps / (Uv[i] - Lv[i]));
      for (PetscInt j = 0; j < m; j++)
      {
         ...
         {
            pijv[j][i] =
               pow(Uv[i] - xv[i], 2.0) * (dfdxp + 0.001 * Abs(dgdxv[j][i]) + 0.5 * feps /
                                          (Uv[i] - Lv[i]));
            qijv[j][i] =
               pow(xv[i] - Lv[i], 2.0) * (dfdxm + 0.001 * Abs(dgdxv[j][i]) + 0.5 * feps /
                                          (Uv[i] - Lv[i]));
         }
         else
         {
...
PetscErrorCode MMA::XYZofLAMBDA(Vec x)
{
   ...
   for (PetscInt i = 0; i < nloc; i++)
   {
      ...
      xv[i] = (sqrt(pjlam) * Lv[i] + sqrt(qjlam) * Uv[i]) / (sqrt(pjlam) + sqrt(
                                                                qjlam));
      if (xv[i] < alf[i])
      {
         xv[i] = alf[i];
...
PetscErrorCode MMA::DualHess(Vec x)
{
   ...
   for (PetscInt i = 0; i < nloc; i++)
   {
      ...
      for (PetscInt j = 0; j < m; j++)
      {
         pjlam += pijv[j][i] * lam[j];
         qjlam += qijv[j][i] * lam[j];
         PQ[i * m + j] = pijv[j][i] / pow(Uv[i] - xv[i],
                                          2.0) - qijv[j][i] / pow(xv[i] - Lv[i], 2.0);
      }
      df2[i]         = -1.0 / (2.0 * pjlam / pow(Uv[i] - xv[i],
                                                 3.0) + 2.0 * qjlam / pow(xv[i] - Lv[i], 3.0));
      PetscScalar xp = (sqrt(pjlam) * Lv[i] + sqrt(qjlam) * Uv[i]) / (sqrt(
                                                                         pjlam) + sqrt(qjlam));
      if (xp < alf[i])
      {
         df2[i] = 0.0;
...
inline PetscScalar MMA::Min(PetscScalar d1, PetscScalar d2) { return d1 < d2 ? d1 : d2; }

inline PetscScalar MMA::Max(PetscScalar d1, PetscScalar d2) { return d1 > d2 ? d1 : d2; }

inline PetscInt MMA::Min(PetscInt d1, PetscInt d2) { return d1 < d2 ? d1 : d2; }

inline PetscInt MMA::Max(PetscInt d1, PetscInt d2) { return d1 > d2 ? d1 : d2; }

inline PetscScalar MMA::Abs(PetscScalar d1) { return d1 > 0 ? d1 : -1.0 * d1; }
