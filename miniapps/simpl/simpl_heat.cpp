...
int main(int argc, char *argv[])
{
   ...
   args.AddOption(&tol_kkt_rel, "-rtol", "--rel-tol",
                  "Tolerance for relative KKT residual");
   args.AddOption(&tol_kkt_abs, "-atol", "--abs-tol",
   ...
   {
      Mesh ser_mesh = Mesh::MakeCartesian2D(10, 10, Element::Type::QUADRILATERAL,
                                            false,
                                            1, 1);
      ...
      mesh.reset(new ParMesh(MPI_COMM_WORLD, ser_mesh));
      for (int i=0; i<par_ref_levels; i++)
      {
         mesh->UniformRefinement();
      }
      MarkBoundaries(*mesh, 5, [](const Vector &x) {return (x[0]-0.5)<0.1 && x[1] > 1-1e-09;});
      ess_bdr_filter.SetSize(5);
      ess_bdr_state.SetSize(5);
      ess_bdr_filter = 0;
      ess_bdr_state = 0;
      ess_bdr_state[4] = 1;
      r_min = 0.02;
   ...
   L2_FECollection fec_filter(order_filter, dim);
   H1_FECollection fec_state(order_state);
   ...
   ParGridFunction state_gf(&fes_state); state_gf = 1.0;
   ...
   ProductCoefficient K_simp_cf(K_cf, simp_cf);
   ...
   DiffusionEnergyDensityCoefficient energy(K_cf, der_simp_cf, state_gf, nullptr);
   ...
   // elasticity
   DiffusionProblem diffusion(fes_state, ess_bdr_state, K_simp_cf, prob < 0);
   ...
   if (use_glvis)
   {
      ...
      if (diffusion.HasAdjoint()) {glvis->Append(optproblem.GetAdjState(), "adjoint displacement", keys);}
   }
