...
void vel_tgv(const Vector &x, fptype t, Vector &u)
{
   fptype xi = x(0);
   fptype yi = x(1);
   fptype zi = x(2);
...
public:
   ...
   fptype ComputeKineticEnergy(ParGridFunction &v)
   {
      ...
      for (int i = 0; i < fes->GetNE(); i++)
      {
         ...
         for (int j = 0; j < ir->GetNPoints(); j++)
         {
            ...
            fptype vel2 = velx(j) * velx(j) + vely(j) * vely(j)
                          + velz(j) * velz(j);
...
void ComputeQCriterion(ParGridFunction &u, ParGridFunction &q)
{
   ...
   for (int e = 0; e < fes->GetNE(); ++e)
   {
      ...
      for (int dof = 0; dof < elndofs; ++dof)
      {
         ...
         fptype q_val = 0.5 * (sq(grad(0, 0)) + sq(grad(1, 1)) + sq(grad(2, 2)))
   ...
   gcomm.Reduce<fptype>(q.GetData(), GroupCommunicator::Sum);
   gcomm.Bcast<fptype>(q.GetData());
...
int main(int argc, char *argv[])
{
   ...
   *nodes *= M_PI;
   ...
   fptype u_inf_loc = u_gf->Normlinf();
   fptype p_inf_loc = p_gf->Normlinf();
   fptype u_inf = GlobalLpNorm(infinity(), u_inf_loc, MPI_COMM_WORLD);
   fptype p_inf = GlobalLpNorm(infinity(), p_inf_loc, MPI_COMM_WORLD);
   fptype ke = kin_energy.ComputeKineticEnergy(*u_gf);
