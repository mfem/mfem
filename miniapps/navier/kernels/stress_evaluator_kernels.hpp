...
)
{
   ...
   auto fq = Reshape(Y_.ReadWrite(), q1d, q1d, q1d, dim, ne);
   auto dkv = Reshape(dkv_.Read(), q1d, q1d, q1d, dim, ne);

   MFEM_FORALL_3D(e, ne, q1d, q1d, q1d,
                  // for (int e = 0; e < ne; e++)
   {
      MFEM_SHARED tensor<double, 2, 3, q1d, q1d, q1d> smem;
      MFEM_SHARED tensor<double, q1d, q1d, q1d, dim> gradkvS;
      ...
      MFEM_FOREACH_THREAD(qx, x, q1d)
      {
         MFEM_FOREACH_THREAD(qy, y, q1d)
         {
            MFEM_FOREACH_THREAD(qz, z, q1d)
            {
               ...
               auto gradkv = make_tensor<dim>(
               [&](int i) { return dkv(qx, qy, qz, i, e); });

               const auto dudx = dudxi(qz, qy, qx) * invJqp;
               gradkvS(qx, qy, qz) = dot(gradkv, dudx + transpose(dudx));

               for (int d = 0; d < dim; d++)
               {
                  fq(qx, qy, qz, d, e) = gradkvS(qx, qy, qz, d);
               }
            }
         }
      }
