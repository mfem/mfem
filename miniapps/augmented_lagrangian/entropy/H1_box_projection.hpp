...
double lnit(double x, double xmin = 0.0, double xmax = 1.0);

double expit(double x, double ymin = 0.0, double ymax = 1.0);

double dexpitdx(double x, double ymin = 0.0, double ymax = 1.0);
...
public:
   ...
   void SetBounds(double a_, double b_)
   {
      MFEM_VERIFY(a_ < b_, "Inconsistent domain bounds given");
      a = a_;
      b = b_;
   }
...
public:
   ExpitGridFunctionCoefficient(GridFunction &u_)
      : u(&u_)  { }
