...
fptype dielectric_sphere(const Vector &);
fptype epsilon(const Vector &x) { return dielectric_sphere(x); }
...
fptype muInv(const Vector & x) { return 1.0/magnetic_shell(x); }

// Conductivity Function
static Vector cs_params_(0);  // Center, Radius, and Conductivity
//                               of conductive sphere
fptype conductive_sphere(const Vector &);
fptype sigma(const Vector &x) { return conductive_sphere(x); }

// Current Density Function
static Vector dp_params_(0);  // Axis Start, Axis End, Rod Radius,
//                               Total Current of Rod, and Frequency
void dipole_pulse(const Vector &x, fptype t, Vector &j);
void j_src(const Vector &x, fptype t, Vector &j) { dipole_pulse(x, t, j); }

// dE/dt Boundary Condition: The following function returns zero but any time
// dependent function could be used.
void dEdtBCFunc(const Vector &x, fptype t, Vector &E);
...
static fptype tScale_ = 1e-9;  // Input time in nanosecond

int SnapTimeStep(fptype tmax, fptype dtmax, fptype & dt);
...
int main(int argc, char *argv[])
{
   ...
   fptype energy = Maxwell.GetEnergy();
   if ( Mpi::Root() )
   {
   ...
   fptype dtmax = Maxwell.GetMaximumTimeStep();
...
fptype dielectric_sphere(const Vector &x)
{
   fptype r2 = 0.0;
...
fptype conductive_sphere(const Vector &x)
{
   fptype r2 = 0.0;
...
void dipole_pulse(const Vector &x, fptype t, Vector &j)
{
   ...
   fptype r = dp_params_[2*x.Size()+0];
   fptype a = dp_params_[2*x.Size()+1] * tScale_;
   fptype b = dp_params_[2*x.Size()+2] * tScale_;
   fptype c = dp_params_[2*x.Size()+3] * tScale_;
...
void
dEdtBCFunc(const Vector &x, fptype t, Vector &dE)
{
   dE.SetSize(3);
   dE = 0.0;
}
...
int
SnapTimeStep(fptype tmax, fptype dtmax, fptype & dt)
{
   fptype dsteps = tmax/dtmax;
