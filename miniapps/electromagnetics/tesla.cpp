...
fptype magnetic_shell(const Vector &);
fptype magnetic_shell_inv(const Vector & x) { return 1.0/magnetic_shell(x); }
...
fptype phi_m_bc_uniform(const Vector &x);
...
int main(int argc, char *argv[])
{
   ...
   for (int it = 1; it <= maxit; it++)
   {
      ...
      fptype local_max_err = errors.Max();
      fptype global_max_err;
      ...
      const fptype frac = 0.5;
      fptype threshold = frac * global_max_err;
...
fptype magnetic_shell(const Vector &x)
{
   fptype r2 = 0.0;
...
void current_ring(const Vector &x, Vector &j)
{
   ...
   fptype ra = cr_params_[2*x.Size()+0];
   fptype rb = cr_params_[2*x.Size()+1];
   if ( ra > rb )
   {
      fptype rc = ra;
      ra = rb;
