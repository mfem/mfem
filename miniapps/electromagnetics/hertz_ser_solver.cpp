...
HertzSolver::HertzSolver(Mesh & mesh, int order, double freq,
   ...
   : order_(order),
     ...
     erCoef_(NULL),
     eiCoef_(NULL),
     err_integ_(new CurlCurlIntegrator(*muInvCoef_)),
     err_flux_fec_(new RT_FECollection(order_-1, mesh_->SpaceDimension())),
     // mCoef_(NULL),
     // a_bc_(a_bc),
     j_r_src_(j_r_src),
     j_i_src_(j_i_src),
     ...
     visit_dc_(NULL)
{
   ...
   blockTrueOffsets_[1] = HCurlFESpace_->GetTrueVSize();
   blockTrueOffsets_[2] = HCurlFESpace_->GetTrueVSize();
   ...
   */
   ...
   if ( abcs.Size() > 0 )
   {
      ...
      abc_marker_.SetSize(mesh.bdr_attributes.Max());
   ...
   if ( j_r_src_ != NULL )
   {
      jrCoef_ = new VectorFunctionCoefficient(mesh_->SpaceDimension(),
                                              j_r_src_);
   }
   else
   {
   ...
   if ( j_i_src_ != NULL )
   {
      jiCoef_ = new VectorFunctionCoefficient(mesh_->SpaceDimension(),
                                              j_i_src_);
   }
   else
   {
   ...
   */
   // Bilinear Forms
   a1_ = new SesquilinearForm(HCurlFESpace_, conv_);
   ...
   b1_ = new BilinearForm(HCurlFESpace_);
   ...
   */
   ...
   e_  = new ComplexGridFunction(HCurlFESpace_);
   *e_ = 0.0;
   ...
   j_ = new ComplexGridFunction(HCurlFESpace_);
   ...
   */
   jd_ = new ComplexLinearForm(HCurlFESpace_, conv_);
...
{
   return 2 * HCurlFESpace_->GetTrueVSize();
}
...
{
   if ( logging_ > 0 ) { cout << "Assembling ..." << flush; }
   ...
   */
   if ( logging_ > 0 ) { cout << " done." << endl; }
}

void
HertzSolver::Update()
{
   if ( logging_ > 0 ) { cout << "Updating ..." << endl; }
...
HertzSolver::Solve()
{
   if ( logging_ > 0 ) { cout << "Running solver ... " << endl; }
   ...
   */
   ...
   // e_->Distribute(E);
   a1_->RecoverFEMSolution(E, *jd_, *e_);
   ...
   */
   if ( logging_ > 0 ) { cout << "done." << flush; }

   if ( logging_ > 0 ) { cout << " Solver done. " << endl; }
}
/*
void
HertzSolver::GetErrorEstimates(Vector & errors)
{
 ...
 RT_FECollection flux_fec(order_-1, mesh_->SpaceDimension());
 FiniteElementSpace flux_fes(mesh_, &flux_fec);

 // Space for the smoothed (conforming) flux
 double norm_p = 1;
 ND_FECollection smooth_flux_fec(order_, mesh_->Dimension());
 FiniteElementSpace smooth_flux_fes(mesh_, &smooth_flux_fec);
...
HertzSolver::WriteVisItFields(int it)
{
   if ( visit_dc_ )
   {
      ...
      int prob_size = this->GetProblemSize();
