...
fptype prodFunc(fptype a, fptype b) { return a * b; }
...
void
MaxwellSolver::ImplicitSolve(fptype dt, const Vector &B, Vector &dEdt)
{
   implicitSolve(dt, B, dEdt);
}

void
MaxwellSolver::setupSolver(const int idt, const fptype dt) const
{
...
void
MaxwellSolver::implicitSolve(fptype dt, const Vector &B, Vector &dEdt) const
{
...
MaxwellSolver::GetMaximumTimeStep() const
{
   ...
   int iter = 0, nstep = 20;
   fptype dt0 = 1.0, dt1 = 1.0, change = 1.0, ptol = 0.001;
   ...
   {
      fptype normV0 = InnerProduct(*v0,*v0);
      *v0 /= sqrt(normV0);
      ...
      fptype lambda = InnerProduct(*v0,*v1);
      dt1 = 2.0/sqrt(lambda);
