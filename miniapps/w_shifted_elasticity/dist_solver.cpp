...
void DistanceSolver::ScalarDistToNormalVector(ParGridFunction &dist_s, ParGridFunction &dist_v,
                                        ParGridFunction &normal_v)
{
   ParFiniteElementSpace &pfes = *dist_s.ParFESpace();
   MFEM_VERIFY(pfes.GetOrdering()==Ordering::byNODES,
   ...
   for (int i = 0; i < size; i++)
     {
       for (int d = 0; d < dim; d++)
	 {
	   magn(i) += dist_v(i+d*size) * dist_v(i+d*size);
	 }
       magn(i) = sqrt(magn(i));
     }
   ...
   for (int i = 0; i < size; i++)
   {
     ...
     if (dist_s(i) <= 0){
      ...
      for (int d = 0; d < dim; d++)
      {
        normal_v(i + d*size) = sign * dist_v(i+d*size) / magn(i);
      }
   }
}
...
void DistanceSolver::ComputeVectorNormal(Coefficient &zero_level_set,
					 ParGridFunction &distance,
					 ParGridFunction &normal)
{
   ParFiniteElementSpace &pfes = *normal.ParFESpace();
   MFEM_VERIFY(pfes.GetVDim() == pfes.GetMesh()->Dimension(),
               "This function expects a vector ParGridFunction!");
   ...
   ParGridFunction dist_s(&pfes_s);
   ComputeScalarDistance(zero_level_set, dist_s);
   ScalarDistToNormalVector(dist_s, distance, normal);
}
