...
class AlfvenOperator : public TimeDependentOperator {
 public:
  ...
  void UpdateMatrices(double dt, Array<int>& ess_tdof_list, const Vector& u);
  void UpdateLinearSolver(double dt, Array<int>& ess_tdof_list, const Vector& u);
  void FormBlockSystem(double dt, int mode);
 ...
 private:
  ...
  std::shared_ptr<BlockOperator> system_;
  Array<int> block_offsets_, block_true_offsets_; // number of variables + 1

  double eta_;

  std::shared_ptr<HypreParMatrix> M00_, M11_, S11_, B01_, P11add_;
  std::shared_ptr<HypreParMatrix> A11_;
  TransposeOperator *B10_;
...
****************************************************************** */
void AlfvenOperator::UpdateMatrices(double dt, Array<int>& ess_tdof_list, const Vector& u)
{
  // block 00 is mass matrix for velocity uses the unit coefficient
  ConstantCoefficient one(1.0);
  auto form00 = std::make_shared<ParBilinearForm>(vspace_.get());
  form00->AddDomainIntegrator(new VectorFEMassIntegrator(one));
  form00->Assemble();
  form00->Finalize();

  form00->EliminateEssentialBC(ess_tdof_list);
  ...
  form01->EliminateTestDofs(ess_tdof_list);
...
****************************************************************** */
void AlfvenOperator::UpdateLinearSolver(double dt, Array<int>& ess_tdof_list, const Vector& u)
{
  // update matrices
  ParGridFunction bfun(bspace_.get());
...
****************************************************************** */
void AlfvenOperator::FormBlockSystem(double dt, int mode)
{
...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  AlfvenOperator op(vspace, bspace, gmres_tol, myid);
  op.Init(eta);
  op.SetTime(0.0);
  ode_solver->Init(op);
  ...
  while (t < tfin) {
    ...
    op.UpdateMatrices(dt_limited, ess_tdof_list, trueSol);
    op.FormBlockSystem(dt_limited, (implicit ? 2 : 1));
    if (implicit) op.UpdateLinearSolver(dt, ess_tdof_list, trueSol);
