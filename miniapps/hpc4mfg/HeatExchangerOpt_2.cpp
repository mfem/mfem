...
int main(int argc, char *argv[])
{
   ...
   //    with the same code.
   mfem::Mesh *mesh = new mfem::Mesh("mesh33.g");
   ...
   mfem::Vector constgrad(desFESpace_scalar_H1.GetTrueVSize()); constgrad=0.0; //of the volume contr.
   ...
   mfem::ParGridFunction desingVarVec(&desFESpace_scalar_H1); desingVarVec=0.35;
   mfem::Vector vdens; vdens.SetSize(desFESpace_scalar_H1.GetTrueVSize()); vdens=0.3;

   mfem::ParGridFunction indicatofunc(&desFESpace_scalar_H1); indicatofunc=0.0;
   ...
   for(int Ij = 0; Ij< desFESpace_scalar_H1.GetTrueVSize(); Ij++)
   {
      ...
      if( locationVector[0] < -0.0)
      {
         indicatofunc[Ij] = 1.0;
         desingVarVec[Ij] = 0.15;
   ...
   std::string tName = "AdvDiff";

   mfem::SurrogateAdvDiffCoefficientnCoefficient * SurrogateDiffCoeff = new mfem::SurrogateAdvDiffCoefficientnCoefficient( tName );
   ...
   //double max_vol = 1.08;
   double maxEnergydissipationAllowed = 1e-0;
   ...
   std::cout<<"Load: "<<tLoad<<std::endl;
   solver->AddNeumannLoadVal( tLoad, Attribute );

   solver->AddDirichletBC(2,0.0);

   solverAdvDiff->AddDirichletBC(1 , 0.0);

   solverAdvDiff->SetLinearSolver(1e-8, 1e-12, 2000);
   ...
   if(true)
   {
      ...
      {
         //----------------------------------------------------------------
         // solve nonlinear Diffusion problem
         solver->FSolve();
         ...
         mfem::VectorGridFunctionCoefficient VelCoeff(&velGF);
         solverAdvDiff->SetVelocity(&VelCoeff);
         solverAdvDiff->SetPreassureAndDesignGF(     
                        &tPreassureGF,
                        &desingVarVec,
                        SurrogateDiffCoeff );
         ...
         mfem::ParGridFunction tTempGF;
         solverAdvDiff->GetSol(tTempGF);
         ...
         //mfem::MeanTempQoI tObj;
         mfem::ThermalComplianceQoI tObj;
         tObj.SetFESpaceFieldsAndMicromodel( 
                  &desFESpace_scalar_H1,
                  &tPreassureGF,
                  &desingVarVec,
                  &tTempGF,
                  tMatCoeff,
         ...
         mfem::EnergyDissipationObjective tConstr;
         tConstr.SetNLDiffusionSolver( solver );
         tConstr.SetDesignFES( &desFESpace_scalar_H1 );
         tConstr.SetDesField( desingVarVec );
         tConstr.SetNLDiffusionCoeff( tMatCoeff );
         energyDissip =tConstr.Eval();
         ...
         if(myrank==0)
         {
             std::cout<<"it: "<<i<<" | obj= "<<thermalCompliance<<" | energyDissip= "
             <<energyDissip<<" | Constraint: "<< energyDissip/maxEnergydissipationAllowed -1.0<<std::endl;
         }

         tObj   .Grad(objgrad);
         tConstr.Grad(constgrad);

        // objgrad.Print();

         if(myrank==0)
         {
             std::cout<<"... Eval obj/const done"<<std::endl;
         }

         mfem::ThermalComplianceIntegrator_1 * ImpThermalCompliance = new mfem::ThermalComplianceIntegrator_1;
         //mfem::MeanTempIntegrator_1 * ImpThermalCompliance = new mfem::MeanTempIntegrator_1;
         ImpThermalCompliance->SetFieldsAndMicrostructure(
                SurrogateDiffCoeff,
                &tTempGF,
                &tPreassureGF,
                &desingVarVec);

         mfem::ParLinearForm ParLinerFormDQObjDT(fes);
         ParLinerFormDQObjDT.AddDomainIntegrator(ImpThermalCompliance);
         ParLinerFormDQObjDT.Assemble();        

         //Adjoint solve
         solverAdvDiff->ASolve(ParLinerFormDQObjDT);

         mfem::ParGridFunction tAdjointGF_Obj;
         solverAdvDiff->GetAdj(tAdjointGF_Obj);
         ...
         //------------------------------------------------------------
         mfem::AdjointDResidualDGradPPostIntegrator * ImplDRTempDpAdjointLaodIntegrator = new mfem::AdjointDResidualDGradPPostIntegrator;
         ImplDRTempDpAdjointLaodIntegrator->SetAdjoint(&tAdjointGF_Obj);
         ImplDRTempDpAdjointLaodIntegrator->SetCoeffsAndGF(
                  tMatCoeff,
                  SurrogateDiffCoeff,
                  &desingVarVec ,
                  &tPreassureGF,
                  &tTempGF );
         mfem::ParLinearForm ImplDRTempDpAdjointLF(fes);
         ImplDRTempDpAdjointLF.AddDomainIntegrator(ImplDRTempDpAdjointLaodIntegrator);
         ImplDRTempDpAdjointLF.Assemble();
              
         solver->ASolve(ImplDRTempDpAdjointLF);

         mfem::ParGridFunction tAdjointGF_1;
         solver->GetAdj(tAdjointGF_1);
         ...
         mfem::AdvDiffAdjointPostIntegrator * AdvDiffAdjointPostIntegrator = new mfem::AdvDiffAdjointPostIntegrator;
         AdvDiffAdjointPostIntegrator->SetAdjoint(&tAdjointGF_Obj);
         AdvDiffAdjointPostIntegrator->SetCoeffsAndGF(
                  tMatCoeff,
                  SurrogateDiffCoeff,
                  &desingVarVec ,
                  &tPreassureGF,
                  &tTempGF );
         mfem::ParLinearForm AdvDiffParLinerFormPostAdjoint(&desFESpace_scalar_H1);
         AdvDiffParLinerFormPostAdjoint.AddDomainIntegrator(AdvDiffAdjointPostIntegrator);
         AdvDiffParLinerFormPostAdjoint.Assemble();
         ...
         //------------------------------------------------------------
         mfem::AdjointPostIntegrator * AdjointPostIntegrator_TempSys = new mfem::AdjointPostIntegrator;
         AdjointPostIntegrator_TempSys->SetAdjoint(&tAdjointGF_1);
         AdjointPostIntegrator_TempSys->SetNLDiffusionCoeff(tMatCoeff);
         AdjointPostIntegrator_TempSys->SetDesingField(&desingVarVec);
         AdjointPostIntegrator_TempSys->SetPreassure(&tPreassureGF);
         mfem::ParLinearForm ParLinerFormPostAdjoint_TempSys(&desFESpace_scalar_H1);
         ParLinerFormPostAdjoint_TempSys.AddDomainIntegrator(AdjointPostIntegrator_TempSys);
         ParLinerFormPostAdjoint_TempSys.Assemble();



         if(myrank==0)
         {
             std::cout<<"... PostMultiply Solve done"<<std::endl;
         }
         ...
         //ParLinerFormPostAdjoint.Print();
                ...
                // AdvDiffParLinerFormPostAdjoint.Print();

         objgrad -= AdvDiffParLinerFormPostAdjoint;
         objgrad -= ParLinerFormPostAdjoint_TempSys;
         constgrad -= ParLinerFormPostAdjoint;
        ...
        //objgrad *= -1.0;
         ...
         paraview_dc.RegisterField("temp",&tTempGF);
         ...
         double con=energyDissip/maxEnergydissipationAllowed -1.0;
         constgrad /=maxEnergydissipationAllowed;

         mma->Update(desingVarVec,objgrad,&con,&constgrad,xxmin,xxmax);
