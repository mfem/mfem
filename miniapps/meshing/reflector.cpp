...
public:
   ...
   int AddVertex(const fptype *coords) const
   {
...
fptype GetElementEdgeMin(Mesh const& mesh, const int elem)
{
   Array<int> edges, cor;
   ...
   for (auto e : edges)
   {
      ...
      const fptype *v0 = mesh.GetVertex(vert[0]);
      const fptype *v1 = mesh.GetVertex(vert[1]);
...
{
   const fptype relTol = 1.0e-6;
   Vector diff(3);

   for (int e=0; e<mesh.GetNE(); ++e)
   {
      const fptype diam = GetElementEdgeMin(mesh, e);
      Array<int> vert;
      ...
      for (auto v : vert)
      {
         const fptype *vcrd = mesh.GetVertex(v);
         for (int i=0; i<3; ++i)
         {
...
Mesh* ReflectHighOrderMesh(Mesh & mesh, Vector origin, Vector normal)
{
   ...
   for (int i=0; i<mesh.GetNE(); i++)
   {
      ...
      Vector diff(3);
      subtract(v0, v1, diff);
      const fptype length = diff.Norml2();
