...
int main (int argc, char *argv[])
{
   ...
   for (int i = 0; i < pmesh->GetNE(); i++)
   {
      ...
      const fptype hi = pmesh->GetElementSize(i);
   ...
   fptype vol_glb;
   MPI_Allreduce(&vol_loc, &vol_glb, 1, MPITypeMap<fptype>::mpi_type,
                 MPI_SUM, MPI_COMM_WORLD);
   const fptype small_phys_size = pow(vol_glb, 1.0 / dim) / 100.0;
   ...
   switch (target_id)
   {
      ...
      case 6: // material indicator 2D
      {
         ...
         fptype max_all;
         MPI_Allreduce(&max, &max_all, 1, MPITypeMap<fptype>::mpi_type,
         ...
         fptype volume_all, volume_ind_all;
         MPI_Allreduce(&volume, &volume_all, 1, MPITypeMap<fptype>::mpi_type,
                       MPI_SUM, MPI_COMM_WORLD);
         ...
         const fptype avg_zone_size = volume_all / NE_ALL;

         const fptype small_avg_ratio =
   ...
   fptype minJ0;
   MPI_Allreduce(&min_detJ, &minJ0, 1, MPITypeMap<fptype>::mpi_type,
   ...
   if (min_detJ < 0.0)
   {
      ...
      fptype h0min = h0.Min(), h0min_all;
      MPI_Allreduce(&h0min, &h0min_all, 1, MPITypeMap<fptype>::mpi_type,
                    MPI_MIN, MPI_COMM_WORLD);
