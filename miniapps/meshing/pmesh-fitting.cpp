...
int main (int argc, char *argv[])
{
   ...
   int metric_id         = 2;
   int target_id         = 1;
   fptype surface_fit_const = 100.0;
   int quad_type         = 1;
   int quad_order        = 8;
   int solver_type       = 0;
   int solver_iter       = 20;
   ...
   fptype surface_fit_adapt = 0.0;
   fptype surface_fit_threshold = -10;
   ...
   if (surf_bg_mesh)
   {
      ...
      for (int i = 0; i < num_nodes; i++)
      {
         for (int d = 0; d < dim; d++)
         {
            fptype length_d = p_max(d) - p_min(d),
   ...
   MPI_Allreduce(MPI_IN_PLACE, &min_detJ, 1,
                 MPITypeMap<fptype>::mpi_type, MPI_MIN, MPI_COMM_WORLD);
   if (myid == 0)
   ...
   const fptype init_energy = a.GetParGridFunctionEnergy(x);
   ...
   const fptype fin_energy = a.GetParGridFunctionEnergy(x);
