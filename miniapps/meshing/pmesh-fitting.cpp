...
// Sample runs:
//  Surface fitting:
//    mpirun -np 4 pmesh-fitting -o 3 -mid 58 -tid 1 -vl 1 -sfc 5e4 -rtol 1e-5 -no-vis
//    mpirun -np 4 pmesh-fitting -m square01-tri.mesh -o 3 -rs 0 -mid 58 -tid 1 -vl 1 -sfc 1e4 -rtol 1e-5 -no-vis
//  Surface fitting with weight adaptation and termination based on fitting error:
//    mpirun -np 4 pmesh-fitting -o 2 -mid 2 -tid 1 -vl 2 -sfc 10 -rtol 1e-20 -sfa 10.0 -sft 1e-5 -no-resid -ni 40 -no-vis
//  Surface fitting with weight adaptation, limit on max weight, and convergence based on residual.
//  * mpirun -np 4 pmesh-fitting -m ../../data/inline-tri.mesh -o 2 -mid 2 -tid 4 -vl 2 -sfc 10 -rtol 1e-10 -sfa 10.0 -sft 1e-5 -bgamriter 3 -sbgmesh -ae 1 -marking -slstype 3 -resid -sfcmax 10000 -mod-bndr-attr -no-vis
//  Surface fitting to Fischer-Tropsch reactor like domain (requires GSLIB):
//  * mpirun -np 6 pmesh-fitting -m ../../data/inline-tri.mesh -o 2 -rs 4 -mid 2 -tid 1 -vl 2 -sfc 100 -rtol 1e-12 -li 20 -ae 1 -bnd -sbgmesh -slstype 2 -smtype 0 -sfa 10.0 -sft 1e-4 -no-resid -bgamriter 5 -dist -mod-bndr-attr -ni 50 -no-vis
...
int main (int argc, char *argv[])
{
   ...
   else if (surf_ls_type == 4) // sphere
   {
      MFEM_VERIFY(dim == 3, "The sphere level set requires a 3D mesh.");
      ls_coeff = new FunctionCoefficient(sphere_level_set);
   }
