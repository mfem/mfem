...
int main (int argc, char *argv[])
{
   ...
   if (surf_amr_iters)
   {
      ...
      // We can re-mark after AMR, requiring that the new interface does not
      // have any hanging nodes.
      if (amr_remarking)
      {
         SetMaterialGridFunction(pmesh, mat, surf_fit_gf0, material, adapt_marking);
         {
         ...
         {
            VisItDataCollection visit_dc("debug", pmesh);
            visit_dc.RegisterField("level_set_fun", &mat);
            visit_dc.SetFormat(DataCollection::SERIAL_FORMAT);
            visit_dc.Save();
         }
         //breaking run
         //make pmesh-fitting -j && mpirun -np 4 pmesh-fitting -m ../../data/inline-tet.mesh -o 1 -mid 303 -tid 4 -ni 100 -vl 2 -sfc 10 -rtol 1e-10 -st 0 -sfa 10.0 -sft 1e-5 -bgamriter 1 -sbgmesh -ae 1 -marking -slstype 3 -resid -sfcmax 1000 -mod-bndr-attr -samriter 2 -refint 1 -vis -rs 0 -remark

         if (marking_type == 0)
         {
            //need to check material consistency for AMR meshes
            int matcheck = CheckMaterialConsistency(pmesh, mat);
            MFEM_VERIFY(matcheck, "Remarking after AMR resulting in hanging "
   ...
   // Setup gradient and Hessian on the current mesh.
   if (surf_bg_mesh)
   {
      surf_fit_grad_fes =
