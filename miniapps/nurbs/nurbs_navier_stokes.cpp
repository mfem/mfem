...
int main(int argc, char *argv[])
{
   ...
   for (int iter = 0; iter < 2; iter++)
   {
    ...
    //  GridFunctionVectorCoefficient u_gf_cf(&u_gf);
      
      VectorGridFunctionCoefficient u_gf_cf(&u_gf);
      
      kVarf.AddDomainIntegrator(new VectorFEConvectionIntegrator(u_gf_cf));
      ...
      chrono.Stop();
      std::cout<<" Assembly of convection - diffusion matrix took "<<chrono.RealTime()
               <<"s\n";
      ...
      solver.SetKDim(maxIter+2); // restart!!!
      ...
      u_gf0 -= u_gf;
      p_gf0 -= p_gf;
      double u_norm = u_gf0.Norml2();
      double p_norm = p_gf0.Norml2();
      std::cout << " |du| = " << u_norm<< "\n";
      std::cout << " |dp| = " << p_norm << "\n";

      if (max(u_norm, p_norm) < 1e-6) { break; }
