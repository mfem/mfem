//                       MFEM Example 24 - Parallel NURBS Version
//
...
//               mpirun -np 4 ex24p -m ../../ata/star.mesh -pa -d raja-omp
...
int main(int argc, char *argv[])
{
   ...
   NURBSExtension *trial_ext = nullptr;
   NURBSExtension *test_ext = nullptr;

   if (pmesh->NURBSext)
   {
      trial_ext = new NURBSExtension(pmesh->NURBSext, order);
      test_ext = new NURBSExtension(pmesh->NURBSext, order);
      if (prob == 0)
      {
   ...
   ParFiniteElementSpace trial_fes(pmesh, trial_ext, trial_fec);
   ParFiniteElementSpace test_fes(pmesh, test_ext, test_fec);
   ...
   if (myid == 0)
   {
      ...
      else if (prob == 1)
      {
         ...
         cout << "Number of HDiv finite element unknowns: " << test_size << endl;
      }
      else
      {
         cout << "Number of HDiv finite element unknowns: " << trial_size << endl;
