...
void ParInteriorPointSolver::SaveLambda(int i)
{
   std::ofstream lStream;
   char lString[100];
   snprintf(lString, 100, "logBarrierHessiandata/lambda%d_rank%d.dat", i, MyRank);
   lStream.open(lString, ios::out | ios::trunc);
   for(int ii = 0; ii < lk.Size(); ii++)
   {
      lStream << setprecision(30) << lk(ii) << endl;
   }
   lStream.close();
}

void ParInteriorPointSolver::SaveZl(int i)
{
   std::ofstream zlStream;
   char zlString[100];
   snprintf(zlString, 100, "logBarrierHessiandata/zl%d_rank%d.dat", i, MyRank);
   zlStream.open(zlString, ios::out | ios::trunc);
   for(int ii = 0; ii < zlk.Size(); ii++)
   {
      zlStream << setprecision(30) << zlk(ii) << endl;
   }
   zlStream.close();
}
                                             ...
                                             BlockOperator &Ak)
{
   ...
   if(saveLogBarrierIterates)
   {
      ...
      std::ofstream lStream;
      char lString[100];
      snprintf(lString, 100, "logBarrierHessiandata/lambda%d_rank%d.dat", jOpt, MyRank);
      lStream.open(lString, ios::out | ios::trunc);
      ...
      std::ofstream zlStream;
      char zlString[100];
      snprintf(zlString, 100, "logBarrierHessiandata/zl%d_rank%d.dat", jOpt, MyRank);
      zlStream.open(zlString, ios::out | ios::trunc);
