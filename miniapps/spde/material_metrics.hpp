...
public:
   ...
   fptype GetDistanceTo(const Vector &x) const;
...
public:
   ...
   virtual fptype ComputeMetric(const Vector &x) = 0;
};
...
public:
   ...
   ParticleTopology(fptype length_x, fptype length_y, fptype length_z,
                    std::vector<fptype> &random_positions,
   ...
   /// particle shape.)
   fptype ComputeMetric(const Vector &x) final;

private:
   /// Initialize the particle topology with positions x_k and matrices A_k.
   void Initialize(std::vector<fptype> &random_positions,
                   std::vector<fptype> &random_rotations);
