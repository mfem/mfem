// MMS test for DiffusionMassGeometricMultigrid.
//
// The test solves
//     -div(a grad u) + m u = f
// on (0,1)^d, d = 2 or 3, with homogeneous Dirichlet boundary conditions.  The
// manufactured solution u = product_i sin(pi x_i) gives
//     f = (d a pi^2 + m) u
// for constant user-provided diffusion coefficient a and mass coefficient m.
// The multigrid level operators use partial assembly for both the diffusion and
// mass terms; the solver itself is implemented in diffusion_mass_gmg.hpp/cpp.
...
int main(int argc, char *argv[])
{
   ...
   args.AddOption(&nx, "-nx", "--num-elements-x",
                  "Number of coarse elements in the x direction.");
   args.AddOption(&ny, "-ny", "--num-elements-y",
                  "Number of coarse elements in the y direction.");
   args.AddOption(&nz, "-nz", "--num-elements-z",
                  "Number of coarse elements in the z direction.");
   ...
   args.AddOption(&par_ref_levels, "-prl", "--par-ref-levels",
                  "Number of parallel uniform refinement levels.");
   ...
   ParFiniteElementSpaceHierarchy hierarchy(pmesh, coarse_fespace,
                                            true, true);
   ...
   if (Mpi::Root())
   {
      cout << "Diffusion+mass GMG MMS test\n"
           << "  dim=" << dim
           << " order=" << order
   ...
   if (paraview)
   {
      ParaViewDataCollection pvdc("DiffusionMassGMGMMS", pmesh);
