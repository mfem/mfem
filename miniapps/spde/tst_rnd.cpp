...
int main(int argc, char *argv[])
{
   ...
   if (Mpi::Root()) { device.Print(); }
    ...
    real_t nu=(4.0*(1-s)-dim)/2.0;
    real_t sca=pow((2*nu), (2*nu+dim)/4);
    if (Mpi::Root())
    {
       std::cout<<"nu="<<nu<<" sca="<<sca<<" 1/sca="<<1.0/sca<<std::endl;
    }

    // Create the FracRandomFieldGenerator object
    FracRandomFieldGenerator* frac_rng = new FracRandomFieldGenerator(*pmesh, par_ref_levels, order, /*sqrt(2*nu)*/ 1.0, s);
    ...
    frac_rng->Mult(*r, rfv); // rfv/=sca;


    ParGridFunction spderf(&fes);
    spderf.UseDevice(true);
    spderf.SetTrueVector(); spderf.GetTrueVector().UseDevice(true);

    // define the SPDE solver
    {
    ...
    //diffusion + mass
    ParGridFunction diffrf(&fes); diffrf=0.0; diffrf.SetTrueVector();
    {
      ParFiniteElementSpace& fes = frac_rng->GetFinestFESpace();
      mfem::ParBilinearForm bf(&fes);
      bf.AddDomainIntegrator(new DiffusionIntegrator());
      ConstantCoefficient mc(/*2*nu*/ 1.0);
      ...
      diffrf.SetFromTrueVector(); //diffrf/=sca;
