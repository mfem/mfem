...
//     f = d a pi^2 u.
//
// The test exercises the solver-owned RHS path through Solve() by default and
// the direct true-vector RHS path through Mult() with -mult.
...
static real_t RHSValue(const Vector &x, int dim, real_t diffusion)
{
   const real_t pi = 4.0*std::atan(1.0);
   return dim*diffusion*pi*pi*ExactSolution(x);
}
...
int main(int argc, char *argv[])
{
   ...
   args.AddOption(&use_qf_coefficients, "-qf", "--quadrature-coefficients",
                  ...
                  "Use QuadratureFunction coefficients for diffusion and RHS.");
   args.AddOption(&use_pgf_coefficients, "-pgf",
                  "--pargridfunction-coefficients",
                  "-no-pgf", "--no-pargridfunction-coefficients",
                  "Use ParGridFunction coefficients for diffusion and RHS.");
   args.AddOption(&use_mult_rhs, "-mult", "--mult-rhs",
                  "-solve", "--solve-rhs",
                  "Provide the RHS as a true vector through Mult().");
   ...
   DiffusionSolver solver(fespace);
   ...
   auto rhs = std::make_shared<FunctionCoefficient>(
      [dim, diffusion](const Vector &x)
      {
         return RHSValue(x, dim, diffusion);
      });
   if (use_qf_coefficients)
   {
      auto diffusion_qf = std::make_shared<QuadratureFunction>(qspace);
      auto rhs_qf = std::make_shared<QuadratureFunction>(qspace);
      *diffusion_qf = diffusion;
      for (int e = 0; e < pmesh.GetNE(); e++)
      {
         ...
         for (int q = 0; q < ir.GetNPoints(); q++)
         {
            ...
            Vector x;
            Tr->Transform(ip, x);
            (*rhs_qf)(qspace.Offset(e) + q) = rhs->Eval(*Tr, ip);
         }
      }
      solver.SetDiffusionCoefficient(diffusion_qf);
      for (int attr = 1; attr <= max_attr; attr++)
      {
         solver.RHS().Add(attr, rhs_qf);
      }
   }
   else if (use_pgf_coefficients)
   {
   ...
   if (Mpi::Root())
   {
      cout << "DiffusionSolver MMS test\n"
           ...
           << " rhs_path=" << (use_mult_rhs ? "Mult" : "Solve")
   ...
   if (paraview)
   {
      ParaViewDataCollection pvdc("DiffusionSolverMMS", &pmesh);
