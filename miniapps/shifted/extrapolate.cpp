...
fptype domainLS(const Vector &coord)
{
   // Map from [0,1] to [-1,1].
   const int dim = coord.Size();
   const fptype x = coord(0)*2.0 - 1.0,
...
fptype solution0(const Vector &coord)
{
   // Map from [0,1] to [-1,1].
   const int dim = coord.Size();
   const fptype x = coord(0)*2.0 - 1.0 + 0.25,
                y = (dim > 1) ? coord(1)*2.0 - 1.0 : 0.0,
                z = (dim > 2) ? coord(2)*2.0 - 1.0 : 0.0;
...
{
   fptype norm = v.Norml1();
   MPI_Allreduce(MPI_IN_PLACE, &norm, 1, MPITypeMap<fptype>::mpi_type, MPI_SUM,
                 MPI_COMM_WORLD);
   if (myid == 0)
   {
...
void PrintIntegral(int myid, ParGridFunction &g, std::string text)
{
   ...
   fptype norm = g.ComputeL1Error(zero);
   if (myid == 0)
   {
      std::cout << std::setprecision(12) << std::fixed
                << text << norm << std::endl;
...
int main(int argc, char *argv[])
{
   ...
   fptype err_L1 = ux.ComputeL1Error(u_exact_coeff),
   ...
   fptype loc_error_L1, loc_error_L2, loc_error_LI;
