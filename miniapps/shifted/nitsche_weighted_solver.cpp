...
namespace mfem
{
  ...
  void WeightedStrainBoundaryForceIntegrator::AssembleFaceMatrix(const FiniteElement &fe,
  ...
  void WeightedStrainBoundaryForceTransposeIntegrator::AssembleFaceMatrix(const FiniteElement &fe,
								  const FiniteElement &fe2,
								  FaceElementTransformations &Tr,
								  DenseMatrix &elmat)
  {
  ...
  void WeightedPressureBoundaryForceIntegrator::AssembleFaceMatrix(const FiniteElement &trial_fe,
							   ...
							   DenseMatrix &elmat)
  {
    ...
    if (ir == NULL)
      {
	// a simple choice for the integration order; is this OK?
	const int order = 5 * max(test_fe1.GetOrder(), 1);
	ir = &IntRules.Get(Tr.GetGeometryType(), order);
      }
	...
	for (int i = 0; i < testdofs_cnt; i++)
	  {
		...
		for (int vd = 0; vd < dim; vd++) // Velocity components.
		  {
		    elmat(i, j + vd * trialdofs_cnt) -= tr_shape(j) * nor(vd) * te_shape(i) * ip_f.weight * volumeFraction;
		  }
	      }
	  }
      }
  ...
  void WeightedPressureBoundaryForceTransposeIntegrator::AssembleFaceMatrix(const FiniteElement &trial_fe,
								    const FiniteElement &test_fe1,
								    FaceElementTransformations &Tr,
								    DenseMatrix &elmat)
  {
	...
	for (int i = 0; i < testdofs_cnt; i++)
	  {
		...
		for (int j = 0; j < trialdofs_cnt; j++)
		  {
		    elmat(i + vd * testdofs_cnt, j) += tr_shape(j) * nor(vd) * te_shape(i) * ip_f.weight * volumeFraction;
		  }
	      }
	  }
      }
  ...
  void WeightedVelocityPenaltyIntegrator::AssembleFaceMatrix(const FiniteElement &fe,
						     const FiniteElement &fe2,
						     FaceElementTransformations &Tr,
						     DenseMatrix &elmat)
  {
	...
	for (int i = 0; i < h1dofs_cnt; i++)
	  {
		...
		for (int j = 0; j < h1dofs_cnt; j++)
		  {
		    elmat(i + vd * h1dofs_cnt, j + vd * h1dofs_cnt) += 4.0 * shape(i) * shape(j) * (nor_norm / Tr.Elem1->Weight()) * ip_f.weight * nor_norm * penaltyParameter * Mu * volumeFraction;
		  }
	      }
	  }
      }
  ...
  void WeightedStrainNitscheBCForceIntegrator::AssembleRHSElementVect(const FiniteElement &el,
							      FaceElementTransformations &Tr,
							      Vector &elvect)
  {
  ...
  void WeightedStrainNitscheBCForceIntegrator::AssembleRHSElementVect(
							      const FiniteElement &el, ElementTransformation &Tr, Vector &elvect)
  {
    mfem_error("DGDirichletLFIntegrator::AssembleRHSElementVect");
  }

  void WeightedPressureNitscheBCForceIntegrator::AssembleRHSElementVect(const FiniteElement &el,
								FaceElementTransformations &Tr,
								Vector &elvect)
  {
    const int dim = el.GetDim();
	...
	for (int i = 0; i < testdofs_cnt; i++)
	  {
	    for (int vd = 0; vd < dim; vd++) // Velocity components.
	      {
		elvect(i) -= nor(vd) * bcEval(vd) * te_shape(i) * ip_f.weight  * volumeFraction;
	      }
	  }
      }
  }
  void WeightedPressureNitscheBCForceIntegrator::AssembleRHSElementVect(
								const FiniteElement &el, ElementTransformation &Tr, Vector &elvect)
  {
    mfem_error("DGDirichletLFIntegrator::AssembleRHSElementVect");
  }

  void WeightedVelocityBCPenaltyIntegrator::AssembleRHSElementVect(const FiniteElement &el,
							   FaceElementTransformations &Tr,
							   Vector &elvect)
  {
    const int dim = el.GetDim();
	...
	for (int i = 0; i < h1dofs_cnt; i++)
	  {
	    for (int vd = 0; vd < dim; vd++) // Velocity components.
	      {
		elvect(i + vd * h1dofs_cnt) += 4.0 * shape(i) * bcEval(vd) * penaltyParameter * (nor_norm / Tr.Elem1->Weight()) * ip_f.weight  * nor_norm * Mu  * volumeFraction;
	      }
	  }
      }
  }

  void WeightedVelocityBCPenaltyIntegrator::AssembleRHSElementVect(
							   const FiniteElement &el, ElementTransformation &Tr, Vector &elvect)
  {
    mfem_error("DGDirichletLFIntegrator::AssembleRHSElementVect");
  }
