...
struct SimpHyperelasticEnergyQFunction
{
                   ...
                   dscalar_t &energy) const
  {
    ...
    const real_t rho_clamped = rho < 0.0 ? 0.0 : (rho > 1.0 ? 1.0 : rho);
    const real_t rho_simp = rho_min + (1.0 - rho_min) * SimpPower(rho_clamped, penal);
  ...
  SimpHyperelasticEnergyQFunction<HolzapfelEnergy<dscalar_t>, dscalar_t>
{
                ...
                const tensor<dscalar_t, dim, dim> & /* dudx */) const
  {
    ...
    const auto psi_aniso = (k1 / (2.0 * k2)) * (exp(k2 * fiber_strain * fiber_strain) - 1.0);
...
class HyperelasticTopOptOperator : public Operator
{
  ...
  public:
    HessianOperator(const HyperelasticTopOptOperator &oper, const Vector &state) :
      Operator(oper.Height()), ess_tdofs(oper.ess_tdofs), state(state), z(oper.Height())
    {
...
public:
  ...
  void ComplianceSensitivity(const Vector &u, const Vector &w,
  ...
  // TODO: implement mixed second derivatives for dfem
  // Mixed second derivative d^2V/(dDisplacement dRho) applied to the
  // adjoint vector w, needed for c2 (K&S eq. 34-35).
  //std::shared_ptr<DerivativeOperator> GetMixedHessian(const Vector &u) const
  //{
  //  MultiVector X{u, mesh_nodes_tdofs, rho_tdofs};
  //  return internal_energy_dop->GetMixedDerivative(Displacement, Density, X);
  //}
...
public:
  HelmholtzFilter(ParFiniteElementSpace &fes_, real_t epsilon, real_t mass = 1.0) :
...
int main(int argc, char *argv[])
{
  ...
  args.AddOption(&max_it, "-mi", "--max-it", "Maximum number of design iterations.");
  ...
  args.AddOption(&prec_type, "-pc", "--preconditioner",
                 "Preconditioner: 0 = none, 1 = diagonal/Jacobi.");
  ...
  L2_FECollection control_fec(std::max(order - 1, 0), dim, BasisType::GaussLobatto); // space for psi
  ...
  mass_control.AddDomainIntegrator(new InverseIntegrator(new MassIntegrator(one)));
  ...
  for (int k = 1; k <= max_it; k++)
  {
    ...
    else
    {
      ...
      // Temporary hardcoded mixed derivative for c2.
      // This evaluates the residual at rho +/- h in each density dof and
      // contracts the finite-difference change with the adjoint vector.
      elasticity_op.ComplianceSensitivity(U, w_adj, rho_filter_tdofs, s_phys);
    }
