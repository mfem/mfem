...
//                   -----------------------------------------
//                   Minimal dFEM Global QFunction  Demo
//                   -----------------------------------------
//
...
struct CubicQF
{
                    ...
                    tensor_array<real_t> &y) const
    {
        ...
        {
            y(q) = coef(q) * x(q) * x(q) * x(q) * det(J(q)) * w(q);
        }
           ...
           Vector &y, Vector &dy)
{
    ...
    const real_t local_dy_norm = dy.Normlinf();
    const real_t local_expected_dy_norm = expected_dy.Normlinf();
    real_t global_dy_norm = 0.0;
    real_t global_expected_dy_norm = 0.0;
    MPI_Allreduce(&local_dy_norm, &global_dy_norm, 1, MPITypeMap<real_t>::mpi_type,
                  MPI_MAX, MPI_COMM_WORLD);
    MPI_Allreduce(&local_expected_dy_norm, &global_expected_dy_norm, 1, MPITypeMap<real_t>::mpi_type,
                  MPI_MAX, MPI_COMM_WORLD);
...
int main(int argc, char *argv[])
{
    ...
    auto dop_deriv = dop.GetDerivative(U, X, false);
    MultiVector dY{dy};
