...
public:
   ...
   virtual ~DoubleIntegralBFIntegrator() = 0;
...
void test(FiniteElementSpace &fes)
{
   BilinearForm a(&fes);

   // Add standard integrators to 'a'
   // ...

   a.Assemble();

   auto M_lambda = [](const Vector &x1, const Vector &x2) -> double
   {
      return 1.0;
   };
   ...
   AssembleDoubleBoundaryIntegrator(a, i);
