...
static fptype cosa_ = cos(0.5 * M_PI / nstep_);
static fptype sina_ = sin(0.5 * M_PI / nstep_);
...
int main(int argc, char *argv[])
{
   ...
   for (int i=1; i<=7; i++) { pwCoef(i) = (fptype)(i-1)/6.0; }
...
void
rotate_step(char axis, int incr, fptype * x)
{
   if (incr == 0) { return; }
...
void determine_random_moves(Array<Move> & moves)
{
   ...
   {
      fptype ran = fptype(rand()) / RAND_MAX;
      int  ir   = (int)(26 * ran);
