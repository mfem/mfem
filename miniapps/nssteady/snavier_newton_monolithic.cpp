#include "snavier_newton_monolithic.hpp"

namespace mfem{

/// Constructor
SNavierNewtonMonolithicSolver::SNavierNewtonMonolithicSolver(ParMesh* mesh,
                                             int vorder,
                                             int porder,
                                             double kin_vis_,
                                             bool verbose):
...
// Boundary conditions
void SNavierNewtonMonolithicSolver::AddVelDirichletBC(VectorCoefficient *coeff, Array<int> &attr)
{
...
void SNavierNewtonMonolithicSolver::AddVelDirichletBC(VecFunc func, Array<int> &attr)
{
   AddVelDirichletBC(new VectorFunctionCoefficient(pmesh->Dimension(), func), attr);
}

void SNavierNewtonMonolithicSolver::AddVelDirichletBC(Coefficient *coeff, Array<int> &attr, int &dir)
{
...
void SNavierNewtonMonolithicSolver::AddVelDirichletBC(VectorCoefficient *coeff, int &attr)
{
...
void SNavierNewtonMonolithicSolver::AddVelDirichletBC(VecFunc func, int &attr)
{
   AddVelDirichletBC(new VectorFunctionCoefficient(pmesh->Dimension(), func), attr);
}

void SNavierNewtonMonolithicSolver::AddVelDirichletBC(Coefficient *coeff, int &attr, int &dir)
{
   // Create array for attributes and mark given mark given mesh boundary
   ess_attr_tmp = 0;
   ess_attr_tmp[ attr - 1] = 1;
...
void SNavierNewtonMonolithicSolver::AddTractionBC(VectorCoefficient *coeff, Array<int> &attr)
{
...
void SNavierNewtonMonolithicSolver::AddTractionBC(VecFunc func, Array<int> &attr)
{
   AddTractionBC(new VectorFunctionCoefficient(pmesh->Dimension(), func), attr);
}

void SNavierNewtonMonolithicSolver::AddTractionBC(VectorCoefficient *coeff, int &attr)
{
   // Create array for attributes and mark given mark given mesh boundary
   trac_attr_tmp = 0;
...
void SNavierNewtonMonolithicSolver::AddTractionBC(VecFunc func, int &attr)
{
   AddTractionBC(new VectorFunctionCoefficient(pmesh->Dimension(), func), attr);
}


void SNavierNewtonMonolithicSolver::AddAccelTerm(VectorCoefficient *coeff, Array<int> &attr)
{
...
void SNavierNewtonMonolithicSolver::AddAccelTerm(VecFunc func, Array<int> &attr)
{
...
// Solver setup
void SNavierNewtonMonolithicSolver::SetOuterSolver(SolverParams params)
{
   sParams = params;    
}

void SNavierNewtonMonolithicSolver::SetGamma(double &gamma_)
{
   gamma = gamma_;
}

void SNavierNewtonMonolithicSolver::SetAlpha(double &alpha_, const AlphaType &type_)
{
   alpha0    = alpha_;
   alphaType = type_;
}

void SNavierNewtonMonolithicSolver::SetLift(double &lift_)
{
   lift    = lift_;
}

void SNavierNewtonMonolithicSolver::SetInitialConditionVel(VectorCoefficient &v_in)
{
...
void SNavierNewtonMonolithicSolver::SetInitialConditionPres(Coefficient &p_in)
{
...
void SNavierNewtonMonolithicSolver::Setup()
{
...
void SNavierNewtonMonolithicSolver::FSolve()
{
#ifdef MFEM_DEBUG
...
void SNavierNewtonMonolithicSolver::SetupOutput( const char* folderPath, bool visit_, bool paraview_,
...
void SNavierNewtonMonolithicSolver::Step()
{
   // Assemble convective term 
   delete C1_form; C1_form = nullptr;
   delete C2_form; C2_form = nullptr;
   C1_form = new ParBilinearForm(vfes);
   C2_form = new ParBilinearForm(vfes);
   C1_form->AddDomainIntegrator(new VectorConvectionIntegrator(*vk_vc, 1.0));
   C2_form->AddDomainIntegrator(new VectorGradCoefficientIntegrator(*vk_vc, 1.0));
   C1_form->Assemble(); C1_form->Finalize();
   C2_form->Assemble(); C2_form->Finalize();
   C1 = C1_form->ParallelAssemble();  
   C2 = C2_form->ParallelAssemble();  

   A = Add(1.0, *K, alpha, *C2);                      // A = K + alpha C1                     
   A->Add(alpha,*C1);                                 // A += alpha C2 

   // Assemble RHS velocity block
   (rhs->GetBlock(0)).Set(1.0,*fv);                             // rhs_v = fv
   C1->AddMult(x_k->GetBlock(0),rhs->GetBlock(0), 2*alpha-1.);  // rhs_v += (2 alpha - 1) * C * vk
...
void SNavierNewtonMonolithicSolver::ComputeError()
{
...
void SNavierNewtonMonolithicSolver::UpdateSolution()
{
   *x_k = *x;
   vk_gf->SetFromTrueDofs(x_k->GetBlock(0));
...
void SNavierNewtonMonolithicSolver::UpdateAlpha()
{
   if ( alphaType == AlphaType::CONSTANT) { alpha = alpha0;}
   else {  MFEM_ABORT("Error: SNavierNewtonMonolithicSolver::UpdateAlpha does not implement"
                       "adaptive update of the segregation parameter yet!");} // NYI!
}


void SNavierNewtonMonolithicSolver::MeanZero(ParGridFunction &p)
{
...
void SNavierNewtonMonolithicSolver::Orthogonalize(Vector &v)
{
...
void SNavierNewtonMonolithicSolver::SaveResults( int iter )
{
...
void SNavierNewtonMonolithicSolver::PrintInfo()
{
...
void SNavierNewtonMonolithicSolver::PrintMatricesVectors( const char* id, int num )
{
   // Create folder
   ...
   if(C1==nullptr)
   {
      C1 = new HypreParMatrix();
      C1->PrintMatlab(C1_file);
      delete C1; C1 = nullptr;
   }
   else
   {
...
/// Destructor
SNavierNewtonMonolithicSolver::~SNavierNewtonMonolithicSolver()
{
