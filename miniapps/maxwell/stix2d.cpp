...
void visualize_exact(ParFiniteElementSpace &, ColdPlasmaPlaneWave &, ColdPlasmaPlaneWave &, ParGridFunction &);
void set_plasma_defaults(Vector & , Vector & , Vector &, double &);
...
int main(int argc, char *argv[])
{
   ...
   double freq = 1.0e6; //frequecy in hertz
   ...
   set_plasma_defaults(numbers, charges,masses, hz);
   //
   ...
   if (visualization) 
   {
      visualize_exact(*HCurlFESpace,EReCoef,EImCoef,BField);
   }
...
void set_plasma_defaults(Vector & numbers, Vector & charges, Vector &masses, double & hz)
{
   if (numbers.Size() == 0)
   {
      numbers.SetSize(2);
