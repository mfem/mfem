...
real_t exact_fun(const Vector & x)
{
   ...
   return sx*sy*sz;
   //return 4*x[0]*(1.0-x[0])*4*x[1]*(1.0-x[1]);
}
...
int main(int argc, char *argv[])
{
   ...
   {
      real_t order = log(normL2[s-1]/normL2[s])/log(2.0);
      cout<<normL2[s]<<"\t| "<<order<<endl;
   }
   cout<<"---------------------------\n";
   cout<<normH1[0]<<"\t| "<<"-"<<endl;
   for (int s = 1; s < stages; s++)
   {
      real_t order = log(normH1[s-1]/normH1[s])/log(2.0);
      cout<<normH1[s]<<"\t| "<<order<<endl;
   }
   cout<<"---------------------------\n";
   cout<<normH2[0]<<"\t| "<<"-"<<endl;
   for (int s = 1; s < stages; s++)
   {
      real_t order = log(normH2[s-1]/normH2[s])/log(2.0);
      cout<<normH2[s]<<"\t| "<<order<<endl;
   }
   return 0;
}
