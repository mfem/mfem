...
void Solve(FiniteElementSpace *fespace, GridFunction *sln, Coefficient *exsol,
           Coefficient *rhs,
...
void FindHPRef(int elem, FiniteElementSpace *fes, Array<double> elemError,
               int n,
               Array<Solution*> solution, int max_order, std::map<int, HPRefinement> *hp_refs)
{
   ...
   cout << "Element " << elem << " (order " << o << "): err = " << s_err <<
        ", dof = " << o*o << "\n  ";
   ...
   if (best_id == 0)
   {
      cout << "Best candidate: " << best_id << ", err = " << sqrt(
              candidate[best_id].err) << ", dof = "
...
int main(int argc, char *argv[])
{
   ...
   bool vis_orders = false, vis_basis = false;
   ...
   for (int it = 0; ; it++)
   {
      ...
      Array<int> ref_type;
      Array<double> elemError;
      double error = sqrt(CalculateH10Error2(&sol, &exgrad, &elemError, &ref_type,
                                             int_order));
      ...
      if (hp)
      {
         ...
         for (int k = 0; k < n_enriched; k++)
         {
            Solve(&(solution[k]->fes), &(solution[k]->sol), &exsol, &rhs, pa, int_order,
                  relaxed_hp);
            CalculateH10Error2(&(solution[k]->sol), &exgrad, &(solution[k]->elemError),
                               &ref_type, int_order);
         }
