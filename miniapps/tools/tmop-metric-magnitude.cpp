...
void Form2DJac(fptype perturb_v, fptype perturb_ar, fptype perturb_s,
               DenseMatrix &J);
void Form3DJac(fptype perturb_v, fptype perturb_ar, fptype perturb_s,
               DenseMatrix &J);

int main(int argc, char *argv[])
{
   int metric_id = 2;
   fptype perturb_v = 1.0;
   fptype perturb_ar = 1.0;
   fptype perturb_s  = 1.0;
...
{
   // Volume.
   const fptype volume = 1.0 * perturb_v;

   // Aspect Ratio.
   const fptype a_r = 1.0 * perturb_ar;
   DenseMatrix M_ar(2); M_ar = 0.0;
   M_ar(0, 0) = 1.0 / sqrt(a_r);
   M_ar(1, 1) = sqrt(a_r);

   // Skew.
   const fptype skew_angle = M_PI / 2.0 / perturb_s;
   DenseMatrix M_skew(2);
   M_skew(0, 0) = 1.0; M_skew(0, 1) = cos(skew_angle);
   M_skew(1, 0) = 0.0; M_skew(1, 1) = sin(skew_angle);

   // Rotation.
   const fptype rot_angle = 0.0; // not sure how to choose
   DenseMatrix M_rot(2);
               ...
               DenseMatrix &J)
{
   ...
   const fptype ar_1 = 1.0 * perturb_ar,
                ar_2 = 1.0,
                ar_3 = 1.0;

   // Skew - only in one direction, the others are pi/2.
   const fptype skew_angle_12 = M_PI / 2.0 / perturb_s,
   ...
   fptype sin3 = sin(skew_angle_12)*sin(skew_angle_13)*sin(skew_angle_23),
