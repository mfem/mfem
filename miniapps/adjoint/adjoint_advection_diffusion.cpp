...
int main(int argc, char *argv[])
{
   ...
   fptype reltol = 1e-8, abstol = 1e-6;
   ...
   for (int ti = 0; !done; )
   {
      fptype dt_real = max(dt, t_final - t);
...
int AdvDiffSUNDIALS::SUNImplicitSolve(const Vector &b, Vector &x, fptype tol)
{
   Vector z(b.Size());
   M->Mult(b,z);
                                                ...
                                                Vector &qBdot) const
{
   ...
   fptype dp1_result = InnerProduct(pfes->GetComm(), yB, b1);
   fptype dp2_result = InnerProduct(pfes->GetComm(), yB, b2);
