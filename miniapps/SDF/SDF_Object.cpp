...
namespace mfem
{
    namespace sdf
    {
                ...
                bool               & aError )
        {
            ...
            {
                aCoordinate = aPoint( aAxis ) + ( mHesse - mNormal * aPoint ) / mNormal( aAxis );
                aError      = false;
            }
        }
                ...
                const mfem::Vector & aPoint )
        {
            ...
            // R
            double tPredictYR = mPredictYRA( aEdge, aAxis ) * aPoint( tI ) + mPredictYRB( aEdge, aAxis );
                ...
                const mfem::Vector & aPoint )
        {
            ...
            mfem::Vector tLocalPointCoords = this->project_point_to_local_cartesian( aPoint );
            // step 2: calculate barycentric coordinates
            mfem::Vector tXi = this->get_barycentric_from_local_cartesian( tLocalPointCoords );
            ...
            else
            {
                if ( tXi( 0 ) > 0 )
                {
                    ...
                    double tDist1 = distance_point_to_edge_in_local_cartesian( tLocalPointCoords, 1 );
                    double tDist2 = distance_point_to_edge_in_local_cartesian( tLocalPointCoords, 2 );
                    return std::min( tDist1, tDist2 );
                }
                else if ( tXi( 1 ) > 0 )
                {
                    // this rules out edge 1
                    double tDist0 = distance_point_to_edge_in_local_cartesian( tLocalPointCoords, 0 );
                    double tDist2 = distance_point_to_edge_in_local_cartesian( tLocalPointCoords, 2 );
        ...
        void Triangle_Vertex::rotate_node_coords( const mfem::DenseMatrix & aRotationMatrix )
        {
