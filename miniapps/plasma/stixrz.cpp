...
//
//   -----------------------------------------------------------------------
//       StixRZ Miniapp: Cold Plasma Electromagnetic Simulation Code
//   -----------------------------------------------------------------------
//
...
void Update(ParFiniteElementSpace & H1FESpace,
            ...
            ParGridFunction & B3Field,
            ParGridFunction & BFieldRZ,
            ParGridFunction & BFieldPhi,
            VectorCoefficient & B3Coef,
            PartitionedVectorCoefficient & BCoef,
            Coefficient & rhoCoef,
            Coefficient & TCoef,
            int & size_h1,
            int & size_l2,
...
int main(int argc, char *argv[])
{
   ...
   Array<int> r0bcs; // Boundary attributes where r = 0
   ...
   args.AddOption(&r0bcs, "-r0bcs", "--r0-bc-surf",
                  "Boundary Surfaces where r = 0");
   ...
   VectorConstantCoefficient B3Coef(BVec);
   VectorConstantCoefficient kCoef(kVec);

   Array<unsigned int> part(2);
   part[0] = 2; part[1] = 1;
   PartitionedVectorCoefficient BCoef(B3Coef, part);
   ...
   ParGridFunction B3Field(&H1VFESpace);
   ParGridFunction BFieldRZ(&HDivFESpace);
   ParGridFunction BFieldPhi(&L2FESpace);
   ParGridFunction temperature_gf;
   ParGridFunction density_gf;

   B3Field.ProjectCoefficient(B3Coef);
   BFieldRZ.ProjectCoefficient(BCoef.GetVectorPartition(0));
   BFieldPhi.ProjectCoefficient(BCoef.GetScalarPartition(1));
   ...
   DielectricTensor epsilon_real(B3Field, density, temperature,
                                 L2FESpace, H1FESpace,
                                 omega, charges, masses, true);
   DielectricTensor epsilon_imag(B3Field, density, temperature,
                                 L2FESpace, H1FESpace,
                                 omega, charges, masses, false);
   SPDDielectricTensor epsilon_abs(B3Field, density, temperature,
                                   L2FESpace, H1FESpace,
   ...
   if (visualization)
   {
      ...
      socketstream  sock_Brz, sock_Bphi;
      sock_Brz.precision(8);
      sock_Bphi.precision(8);

      Wx += 2 * offx;
      VisualizeField(sock_Brz, vishost, visport,
                     BFieldRZ, "Background Magnetic Field (RZ)",
                     Wx, Wy, Ww, Wh);

      Wx += offx;
      VisualizeField(sock_Bphi, vishost, visport,
                     BFieldPhi, "Background Magnetic Field (Phi)",
                     Wx, Wy, Ww, Wh);
   ...
   */
   ...
   CPDSolver CPD(pmesh, order, omega,
                 ...
                 conv, B3Coef, epsilon_real, epsilon_imag, epsilon_abs,
   ...
   VisItDataCollection visit_dc("STIXRZ-AMR-Parallel", &pmesh);
   ...
   for (int it = 1; it <= maxit; it++)
   {
      ...
      */
      ...
      Update(H1FESpace, H1VFESpace, HCurlFESpace, HDivFESpace, L2FESpace,
             B3Field, BFieldRZ, BFieldPhi, B3Coef, BCoef,
...
void display_banner(ostream & os)
{
   os << "  _________ __   __       ____________________" << endl
      << " /   _____//  |_|__|__  __\\______   \\____    /" << endl
      << " \\_____  \\\\   __\\  \\  \\/  /|       _/ /     / " << endl
      << " /        \\|  | |  |>    < |    |   \\/     /_ " << endl
      << "/_______  /|__| |__/__/\\_ \\|____|_  /_______ \\" << endl
      << "        \\/               \\/       \\/        \\/" << endl
      << endl
