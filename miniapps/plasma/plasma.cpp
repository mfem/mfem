...
double G_EQDSK_Data::InterpVar(const Vector &pt, vector<double> &VarField)
{
   double x = pt[0];
   ...
   c.SetSize(NW_ - 1, NH_);
   d.SetSize(NW_, NH_ - 1);
   e.SetSize(NW_ - 1, NH_ - 1);

   double dr = RDIM_ / (NW_ - 1);
   double dz = ZDIM_ / (NH_ - 1);

   for (int i=0; i<NW_-1; i++)
      for (int j=0; j<NH_; j++)
      {
         c(i,j) = (VarField[NH_ * (i + 1) + j] - VarField[NH_ * i + j]) / dr;
      }

   for (int i=0; i<NW_; i++)
      for (int j=0; j<NH_ - 1; j++)
      {
         d(i,j) = (VarField[NH_ * i + j + 1] - VarField[NH_ * i + j]) / dz;
      }

   for (int i=0; i<NW_ - 1; i++)
      for (int j=0; j<NH_ - 1; j++)
      {
         e(i,j) = (c(i,j+1) - c(i,j)) / dz;
      }
   ...
   double p00 = VarField[NH_ * i + j];
   double p10 = VarField[NH_ * (i + 1) + j];
   double p01 = VarField[NH_ * i + j + 1];
   double p11 = VarField[NH_ * (i + 1) + j + 1];
...
void G_EQDSK_Data::InterpNxGradVar(const Vector &pt, Vector &b,
                                   vector<double> &VarField)
{
   b.SetSize(2);
   b = 0.0;
   ...
   // Prepare position dependent factors
   double wra = (r1 - x) / dr, dwra = -1.0 / dr;
   double wrb = (x - r0) / dr, dwrb = 1.0 / dr;
   ...
   double wza = (z1 - y) / dz, dwza = -1.0 / dz;
   double wzb = (y - z0) / dz, dwzb = 1.0 / dz;
   ...
   b[0] -= dr *
           ((px00 * wra2 * wrb - px10 * wrb2 * wra) * (dwza2 * wzd + wza2 * dwzd) +
            (px01 * wrb * wra2 - px11 * wra * wrb2) * (dwzb2 * wzc + wzb2 * dwzc));

   b[1] += dr *
           ((px00 * wza2 * wzd + px01 * wzb2 * wzc) * (dwra2 * wrb + wra2 * dwrb ) -
            (px10 * wza2 * wzd + px11 * wzb2 * wzc) * (dwra * wrb2 + wra * dwrb2));
   ...
   b[0] -= dz *
           ((py00 * wra2 * wrd + py10 * wrb2 * wrc) * (dwza2 * wzb + wza2 * dwzb) -
            (py01 * wra2 * wrd + py11 * wrb2 * wrc) * (dwza * wzb2 + wza * dwzb2));
   b[1] += dz *
           ((py00 * wza2 * wzb - py01 * wza * wzb2) * (dwra2 * wrd + wra2 * dwrd) +
            (py10 * wza2 * wzb - py11 * wza * wzb2) * (dwrb2 * wrc + wrb2 * dwrc));
   ...
   b[0] -= dr * dz * ((pxy00 * wra2 * wrb - pxy10 * wra * wrb2)
                      * (dwza2 * wzb + wza2 * dwzb) +
                      (pxy11 * wra * wrb2 - pxy01 * wra2 * wrb)
                      * (dwza * wzb2 + wza * dwzb2));
   b[1] += dr * dz * ((pxy00 * wza2 * wzb - pxy01 * wza * wzb2)
                      * (dwra2 * wrb + wra2 * dwrb) +
