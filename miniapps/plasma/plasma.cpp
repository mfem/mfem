...
vector<double> G_EQDSK_Data::FPoltoBtor()
{
    vector<double> V_;
    V_.resize(FPOL_.size());
    for (int i=0; i<NW_; i++)
    {
       V_[i] = FPOL_[i] / ( RLEFT_ + RDIM_ * i  / (NW_ - 1) );
    }
    return V_;
}
...
double G_EQDSK_Data::InterpVar(const Vector &pt, vector<double> VarField_)
{
    double x = pt[0];
    ...
    double dr_ = RDIM_ / (NW_ - 1);
    double dz_ = ZDIM_ / (NH_ - 1);

    for (int i=0; i<NW_-1; i++)
       for (int j=0; j<NH_; j++)
       {
          c_(i,j) = (VarField_[NH_ * (i + 1) + j] - VarField_[NH_ * i + j]) / dr_;
       }

    for (int i=0; i<NW_; i++)
       for (int j=0; j<NH_ - 1; j++)
       {
          d_(i,j) = (VarField_[NH_ * i + j + 1] - VarField_[NH_ * i + j]) / dz_;
       }
   ...
   double p00 = VarField_[NH_ * i + j];
   double p10 = VarField_[NH_ * (i + 1) + j];
   double p01 = VarField_[NH_ * i + j + 1];
   double p11 = VarField_[NH_ * (i + 1) + j + 1];
   ...
   double vary = py00 * wra2 * wrd * wza2 * wzb + py10 * wrb2 * wrc * wza2 * wzb
...
void G_EQDSK_Data::InterpNxGradVar(const Vector &pt, Vector &b, vector<double> VarField_)
{
    b.SetSize(2);
    b = 0.0;
