...
int main(int argc, char *argv[])
{
   ...
   */
   ...
   DGAdvectionDiffusionTDO oper(dg, fes, pgf, oneCoef, imex);
   // DGTransportTDO oper(dg, fes, ffes, pgf, mnCoef, niCoef, neCoef, imex);

   oper.SetLogging(max(0, logging - (mpi.Root()? 0 : 1)));

   oper.SetDiffusionCoefficient(DnCoef);
   oper.SetSourceCoefficient(SnCoef);
   /*
   ...
   */
   ...
   DiffusionIntegrator integ(DnCoef);
   ...
   if (visit)
   {
      if (binary)
      {
         ...
         dc = new SidreDataCollection("DG_Adv_Diff_Test-Parallel", &pmesh);
#else
      ...
      {
         dc = new VisItDataCollection("DG_Adv_Diff_Test-Parallel", &pmesh);
   ...
   while (t < t_final)
   {
      ...
      ode_controller.Run(neu_density, t, t_final);
