...
void fillRandomMatrix(SparseMatrix & M)
{
   ...
   for(int i(0); i < nrows; ++i)
   {
	 ...
	 M.Set(i,rand()%ncols, static_cast<double>( rand() )/static_cast<double>(RAND_MAX) -.5 );
   }
   M.Finalize();
}
...
TEST_CASE("BlockMatrix", "[BlockMatrix]")
{
   ...
   SECTION("Check method BlockMatrix::RowSize")
   {
	 ...
	 {
	    std::cout<< "BlockMatrix::RowSize failure: " << i <<"\t"<< A->RowSize(i) <<"\t" <<  Amono->RowSize(i) << "\n";
	    ++nfails;
	 }
      }
      REQUIRE(nfails == 0);
   ...
   SECTION("Check BlockMatrix::AddMult #2")
   {
	 ...
	 y.Randomize();
	 ymono = y;
	 double a = 10 * static_cast<double>( rand() )  / static_cast<double>( RAND_MAX );
	 a -= 5;
	 A->AddMult(x,y,a);
   ...
   SECTION("Check Mult(const BlockMatrix &, const BlockMatrix &)")
   {
      ...
      SparseMatrix B00(size0, size0), B10(size1,size0), B01(size0,size1), B11(size1,size1);
