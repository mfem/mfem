...
/// Data and methods for element-assembled nonlinear forms.
...
public:
   ...
   /// Reflect changes in the finite element space.
   void Update() override;

   ~EANonlinearFormExtension() override = default;

protected:
   mutable int ne;
   /// The number of vdofs per element
   mutable int elem_vdofs;
   /// Element gradient matrices.
   mutable Vector ea_data;
   mutable EAGradient eaGrad;
};

/// Data and methods for fully assembled nonlinear forms.
...
protected:
   /// Reusable gradient matrix on the local dofs.
