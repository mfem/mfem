...
std::pair<bool, int> GridFunction::IsFunctionPositiveInElement(
   const int elem, const PLBound &plb, const int vdim,
   const int max_depth)
{
   const int dim = this->FESpace()->GetMesh()->Dimension();
   ...
   // Lambda to select a value and flag based on lower/upper.
   static auto check_positive_fun = [](real_t lower,
                                       real_t upper) -> std::pair<bool, int>
   {
      if (upper <= 0)
      {
         return {false, 1};
   ...
   if (max_depth == 0)
   {
      return check_positive_fun(val_min, val_max);
   }
   ...
   while (!pq.empty())
   {
      ...
      // if lower bound of interval is positive or max depth reached, don't
      // need to explore further
      if (current->node->val_min > 0 || curr_depth >= max_depth)
      {
         delete current;
         continue;
      }
      ...
      GetElementBoundsAtControlPoints(elem, plb, current->ref_range,
                                      vdim, lower, upper, cp_ref_loc);
      if (upper.Min() <= 0) // found guarantee negative function
      {
         IntervalNode *child_node = new IntervalNode(lower.Min(), upper.Min());
         current->node->AddChild(child_node);
         min_upper_bound = std::min(min_upper_bound, upper.Min());
         delete current;
         break;
      }
      else if (lower.Min() > 0) // this interval is positive.
      {
         delete current;
         continue;
      }
   ...
   return check_positive_fun(min_lower_bound, min_upper_bound);
}
