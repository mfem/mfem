                                                   ...
                                                   const int vdim)
{
   ...
   for (int d = 0; d < fes_dim; d++)
   {
      ...
      if (check_gslib)
      {
         int nbg = plb.nb;
         int ncpg = plb.ncp;
         Vector data_r(lob_bnd_size(nbg, ncpg));
         lob_bnd_setup(data_r.GetData(), nbg,ncpg);

         Vector lowerG, upperG;
         GetBounds(nbg, ncpg, data_r, nodal_data, lowerG, upperG, rdim);
         Vector diffl;
         diffl = lowerG;
         diffl -= lowerT;
         Vector diffu;
         ...
         diffu -= upperT;
         auto diff_tol = 1e-8;
         if (diffl.Norml2() > diff_tol || diffu.Norml2() > diff_tol)
         {
            mfem::out << diffl.Norml2() << " " << diffu.Norml2() <<
                      " k10-diffl-diffu-norm" << std::endl;
         }
      }
#endif
   }
   ...
   // fes->GetElementVDofs(elem, dof_idx);
   // GetSubVector(dof_idx, loc_data);
   // const FiniteElement *fe = fes->GetFE(elem);
   // int vdim  = fes->GetVDim();
   // int ndofs = dof_idx.Size()/vdim;
   // int rdim  = fe->GetDim();
   ...
   // int n_c_pts = std::pow(plb.ncp, rdim);
   // lower.SetSize(n_c_pts*vdim);
   // upper.SetSize(n_c_pts*vdim);
   // bool check_gslib = plb.b_type == 1 && plb.cp_type == 1;

   // for (int d = 0; d < vdim; d++)
   // {
   //    Vector lowerT(lower.GetData() + d*n_c_pts, n_c_pts);
   //    Vector upperT(upper.GetData() + d*n_c_pts, n_c_pts);
   //    Vector nodal_data;
   //    if (dof_map.Size() == 0)
   //    {
   //       nodal_data.SetDataAndSize(loc_data.GetData() + d*ndofs, ndofs);
   //    }
   //    else
   //    {
   //       nodal_data.SetSize(ndofs);
   //       for (int j = 0; j < ndofs; j++)
   //       {
   //          nodal_data(j) = loc_data(dof_map[j] + d*ndofs);
   //       }
   //    }
   //    GetnDBounds(rdim, plb, nodal_data, lowerT, upperT);
   //    // check only if b_type = 1, cp_type = 1
   //    if (check_gslib)
   //    {
   //       int nbg = plb.nb;
   //       int ncpg = plb.ncp;
...
// void GridFunction::GetElementControlPointsPhysicalLocation(const int elem,
//                                                            PLBound &plb,
//                                                            Vector &xyz)
// {
//    MFEM_VERIFY(!fes->IsVariableOrder(),
...
//    Array<int> dof_idx;
//    const FiniteElement *fe = fes->GetFE(elem);
//    ElementTransformation *Transf = fes->GetElementTransformation(elem);

//    int rdim  = fe->GetDim();
//    int n_c_pts = std::pow(plb.ncp, rdim);
//    int sdim  = fes->GetMesh()->SpaceDimension();
//    xyz.SetSize(n_c_pts*sdim);
//    DenseMatrix xyzM(xyz.GetData(), n_c_pts, sdim);
//    Vector loc(sdim);
//    IntegrationPoint ip;
//    int count = 0;
//    for (int k = 0; k < (rdim == 3 ? plb.ncp : 1); k++)
//    {
//       ip.z = plb.control_points(k);
//       for (int j = 0; j < (rdim == 2 ? plb.ncp : 1); j++)
//       {
//          ip.y = plb.control_points(j);
//          for (int i = 0; i < plb.ncp; i++)
//          {
//             ip.x = plb.control_points(i);
//             Transf->SetIntPoint(&ip);
...
#ifdef MFEM_USE_GSLIB
void GridFunction::GetBounds(int nb, int ncp, Vector &data_r,
                             Vector &vect, Vector &qpmin, Vector &qpmax, int dim)
{
   int nqpts = std::pow(ncp, dim);

   qpmin.SetSize(nqpts);
   qpmax.SetSize(nqpts);
   qpmin = 0.0;
   qpmax = 0.0;
   MFEM_VERIFY(std::pow(nb,dim) == vect.Size(), "Incompatible size.");

   int wrksize = 2*ncp;
   if (dim == 2)
   {
      wrksize = 2*ncp*(nb+ncp+1);
   }
   else if (dim == 3)
   {
      wrksize = 2*ncp*ncp*(nb+ncp+1);
   }
   ...
   if (dim == 1)
   {
      bound = lob_bnd_1(data_r.GetData(),nb,ncp, vect.GetData(),
                        work.GetData());
   }
   else if (dim == 2)
   {
      bound = lob_bnd_2(data_r.GetData(),nb,ncp,
                        data_r.GetData(),nb,ncp,
                        vect.GetData(),
                        work.GetData()); // compute bounds on u2
   }
   else
   {
      bound = lob_bnd_3(data_r.GetData(),nb,ncp,
                        data_r.GetData(),nb,ncp,
                        data_r.GetData(),nb,ncp,
                        vect.GetData(),
                        work.GetData()); // compute bounds on u2
   ...
   else if (dim == 2)
   {
      ...
      for (int j = 0; j < ncp; j++)
      {
         for (int i = 0; i < ncp; i++)
         {
            qpmin(j*ncp+i) = work[2*(i*ncp+j)+0];
            qpmax(j*ncp+i) = work[2*(i*ncp+j)+1];
         }
      }
   }
   else if (dim == 3)
   {
      DenseTensor Jtrl(ncp,ncp,ncp), Jtru(ncp,ncp,ncp);
      DenseTensor Jtrls(Jtrl.Data(), ncp*ncp*ncp,1,1),
                  Jtrus(Jtru.Data(), ncp*ncp*ncp,1,1);
      int count = 0;
      for (int k = 0; k < ncp; k++)
      {
         for (int j = 0; j < ncp; j++)
