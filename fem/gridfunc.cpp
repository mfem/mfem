...
void GridFunction::GetDerivative(int comp, int der_comp, GridFunction &der)
{
   ...
   for (i = 0; i < der_fes->GetNE(); i++)
   {
      ...
      for (j = 0; j < dof; j++)
      {
         loc_func(j) = ((ind = vdofs[comp * dof + j]) >= 0) ? (data[ind]) :
                       (-data[-1 - ind]);
      }
      for (k = 0; k < der_dof; k++)
      {
...
{
   cout << "gridfunc.cpp: Doing accum + count zeros (includes Project call)" <<
        endl;
   ...
   Array<int> &values_counter)
{
   ...
   {
      if (attr[fes->GetBdrAttribute(i) - 1] == 0)
      {
         continue;
      }
...
{
   cout << "* gridfunc.cpp: Calling ProjectCoeff" << endl
        << endl;
