...
static void InterpolateSurfLocal3DKernel(const double *const gf_in,
                                         int *const el,
                                         double *const r,
...
void FindPointsGSLIB::InterpolateSurfLocal3(const Vector &field_in,
                                            ...
                                            int dof1Dsol)
{
   MFEM_VERIFY(dim == 2 && spacedim == 3, "Kernel for 3D surface only.");
   if (npt == 0) { return; }
   ...
   switch (dof1Dsol)
   {
      case 2: return InterpolateSurfLocal3DKernel<2>(pfin, pgsl, pgslr, pfout,
                                                     npt, ncomp, nel, gf_offset,
                                                     pgll, plcf);
      case 3: return InterpolateSurfLocal3DKernel<3>(pfin, pgsl, pgslr, pfout,
                                                     npt, ncomp, nel, gf_offset,
                                                     pgll, plcf);
      case 4: return InterpolateSurfLocal3DKernel<4>(pfin, pgsl, pgslr, pfout,
                                                     npt, ncomp, nel, gf_offset,
                                                     pgll, plcf);
      case 5: return InterpolateSurfLocal3DKernel<5>(pfin, pgsl, pgslr, pfout,
                                                     npt, ncomp, nel, gf_offset,
                                                     pgll, plcf);
      default: return InterpolateSurfLocal3DKernel(pfin, pgsl, pgslr, pfout,
                                                   npt, ncomp, nel, gf_offset,
                                                   pgll, plcf, dof1Dsol);
   }
}
