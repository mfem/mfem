...
static MFEM_HOST_DEVICE inline dfloat obbox_axis_test(const obbox_t *const b, const dfloat x[3])
{
  dfloat test = 1;
  for (dlong d = 0; d < 3; ++d) {
...
/* positive when possibly inside */
static MFEM_HOST_DEVICE inline dfloat obbox_test(const obbox_t *const b, const dfloat x[3])
{
...
static MFEM_HOST_DEVICE inline dlong hash_index(const findptsLocalHashData_t *p, const dfloat x[3])
{
...
/* A is row-major */
static MFEM_HOST_DEVICE inline void lin_solve_3(dfloat x[3], const dfloat A[9], const dfloat y[3])
{
...
static MFEM_HOST_DEVICE inline void lin_solve_sym_2(dfloat x[2], const dfloat A[3], const dfloat y[2])
{
...
/* assumes x = 0, 1, or 2 */
static MFEM_HOST_DEVICE inline dlong plus_1_mod_3(const dlong x) { return ((x | x >> 1) + 1) & 3u; }
static MFEM_HOST_DEVICE inline dlong plus_2_mod_3(const dlong x)
{
  const dlong y = (x - 1) & 3u;
  return y ^ (y >> 1);
}
static MFEM_HOST_DEVICE inline dlong plus_1_mod_2(const dlong x) { return x ^ 1u; }
...
static MFEM_HOST_DEVICE inline dlong face_index(const dlong x) { return which_bit(x) - 1; }
...
static MFEM_HOST_DEVICE inline dlong point_index(const dlong x) { return ((x >> 1) & 1u) | ((x >> 2) & 2u) | ((x >> 3) & 4u); }
...
#define CONSTRAINT_SIZE size2
        infok[0] = size1;
        infok[1] = CONSTRAINT_SIZE;
        dfloat constraint_workspace[CONSTRAINT_SIZE];
...
//                    mfem::forall(p_innerSize, [=] MFEM_HOST_DEVICE (dlong j)
                    {
                      if (j == 0) {
                        fpt->dist2 = DBL_MAX;
                        fpt->dist2p = 0;
...
{
    std::cout << " k10do-findptslocalkernel\n";
    if (dim == 3)
    {
        FindPointsLocal3D_Kernel(points_cnt, DEV.tol,
                                 point_pos.Read(),
                                 point_pos.Read()+points_cnt,
                                 point_pos.Read()+2*points_cnt,
                                 DEV.o_x.ReadWrite(),
                                 DEV.o_y.ReadWrite(),
                                 DEV.o_z.ReadWrite(),
                                 ...
                                 gsl_code_dev.ReadWrite(),
                                 gsl_elem_dev.ReadWrite(),
                                 gsl_ref.ReadWrite(),
                                 gsl_dist.ReadWrite(),
                                 DEV.gll1d.ReadWrite(),
