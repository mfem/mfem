...
// Compute kernels for PABilinearFormExtension::AddMultWithMarkers and
// MFBilinearFormExtension::AddMultWithMarkers. Cannot be in member function
...
void MFBilinearFormExtension::Mult(const Vector &x, Vector &y) const
{
   ...
   if (DeviceCanUseCeed() || !elem_restrict || allPatchwise)
   {
      ...
      for (int i = 0; i < iSz; ++i)
      {
         if (integrators[i]->Patchwise())
         {
            integrators[i]->AddMultNURBSMF(x, y);
         }
         else
         {
   ...
   if (int_face_restrict_lex && iFISz>0)
   {
      ...
      if (int_face_X.Size() > 0)
      {
         ...
         for (int i = 0; i < iFISz; ++i)
         {
            ...
            {
               intFaceIntegrators[i]->AddMultMFFaceNormalDerivatives(
                  int_face_X, int_face_dXdn, int_face_Y, int_face_dYdn);
            }
            else
            {
   ...
   if (bdr_face_restrict_lex && has_bdr_integs)
   {
      ...
      if (bdr_face_X.Size() > 0)
      {
         ...
         if (bdr_face_dYdn.Size() > 0)
         {
            bdr_face_restrict_lex->NormalDerivativeAddMultTranspose(
               bdr_face_dYdn, y);
         }
      }
   }
...
void MFBilinearFormExtension::AddMultNormalDerivativesWithMarkers(
   const BilinearFormIntegrator &integ, const Vector &x, const Vector &dxdn,
   const Array<int> *markers, const Array<int> &attributes, Vector &y,
   Vector &dydn) const
{
   if (markers)
   {
      ...
      integ.AddMultMFFaceNormalDerivatives(x, dxdn, tmp_y, tmp_dydn);
   ...
   {
      integ.AddMultMFFaceNormalDerivatives(x, dxdn, y, dydn);
   }
}

void MFBilinearFormExtension::AddMultWithMarkers(
   const BilinearFormIntegrator &integ, const Vector &x,
   const Array<int> *markers, const Array<int> &attributes,
   const bool transpose, Vector &y) const
{
   if (markers)
   {
      tmp_evec.SetSize(y.Size());
      tmp_evec = 0.0;
      if (transpose) { integ.AddMultTransposeMF(x, tmp_evec); }
      else { integ.AddMultMF(x, tmp_evec); }
      const int ne = attributes.Size();
   ...
   {
      if (transpose) { integ.AddMultTransposeMF(x, y); }
      else { integ.AddMultMF(x, y); }
   }
}
