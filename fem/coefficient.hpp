...
public:
   ...
   virtual void SetTime(fptype t) { time = t; }

   /// Get the time for time dependent coefficients
   fptype GetTime() { return time; }
   ...
   fptype Eval(ElementTransformation &T,
               const IntegrationPoint &ip, fptype t)
   {
      SetTime(t);
      return Eval(T, ip);
   }
...
public:
   ...
   explicit ConstantCoefficient(fptype c = 1.0) { constant=c; }
...
public:
   ...
   fptype &operator()(int i) { return constants(i-1); }

   /// Set the constants for all attributes to constant @a c.
   void operator=(fptype c) { constants = c; }
...
protected:
   ...
   std::function<fptype(const Vector &, fptype)> TDFunction;

public:
   /// Define a time-independent coefficient from a std function
   /** \param F time-independent std::function */
   FunctionCoefficient(std::function<fptype(const Vector &)> F)
      : Function(std::move(F))
   { }

   /// Define a time-dependent coefficient from a std function
   /** \param TDF time-dependent function */
   FunctionCoefficient(std::function<fptype(const Vector &, fptype)> TDF)
   ...
   MFEM_DEPRECATED FunctionCoefficient(fptype (*f)(Vector &))
   {
      Function = reinterpret_cast<fptype(*)(const Vector&)>(f);
      TDFunction = NULL;
   }
   ...
   /** @deprecated Use the method where the C-function, @a tdf, uses a const
       Vector argument instead of Vector. */
   MFEM_DEPRECATED FunctionCoefficient(fptype (*tdf)(Vector &, fptype))
   {
      Function = NULL;
      TDFunction = reinterpret_cast<fptype(*)(const Vector&,fptype)>(tdf);
   }
...
public:
   ...
   DeltaCoefficient(fptype x, fptype s)
   {
   ...
   DeltaCoefficient(fptype x, fptype y, fptype s)
   {
   ...
   DeltaCoefficient(fptype x, fptype y, fptype z, fptype s)
   {
   ...
   void SetScale(fptype s_) { scale = s_; }

   /// Set a time-dependent function that multiplies the Scale().
   void SetFunction(fptype (*f)(fptype)) { tdf = f; }
   ...
   /** @brief Set the tolerance used during projection onto GridFunction to
       ...
       lies. (default 1e-12)*/
   void SetTol(fptype tol_) { tol = tol_; }
   ...
   virtual fptype EvalDelta(ElementTransformation &T, const IntegrationPoint &ip);
...
private:
   ...
   std::function<void(const Vector &, fptype, Vector &)> TDFunction;
   Coefficient *Q;
...
public:
   ...
   /** \param dim - the size of the vector
       ...
       \param q - optional scalar Coefficient to scale the vector coefficient */
   VectorFunctionCoefficient(int dim,
                             std::function<void(const Vector &, fptype, Vector &)> TDF,
                             Coefficient *q = nullptr)
...
public:
   ...
   /// value.
   fptype Eval(int i, ElementTransformation &T, const IntegrationPoint &ip)
...
public:
   ...
   VectorDeltaCoefficient(const Vector& dir_, fptype x, fptype s)
      : VectorCoefficient(dir_.Size()), dir(dir_), d(x,s) { }

   /** @brief Construct with a Vector object representing the direction and a
       delta function scaled by @a s and centered at (x,y,0.0) */
   VectorDeltaCoefficient(const Vector& dir_, fptype x, fptype y, fptype s)
      : VectorCoefficient(dir_.Size()), dir(dir_), d(x,y,s) { }

   /** @brief Construct with a Vector object representing the direction and a
       delta function scaled by @a s and centered at (x,y,z) */
   VectorDeltaCoefficient(const Vector& dir_, fptype x, fptype y, fptype z,
                          fptype s)
   ...
   void SetScale(fptype s) { d.SetScale(s); }
...
private:
   ...
   std::function<void(const Vector &, fptype, DenseMatrix &)> TDFunction;
...
public:
   ...
   /** \param dim - the size of the matrix
       ...
       \param q - optional scalar Coefficient to scale the matrix coefficient */
   MatrixFunctionCoefficient(int dim,
                             std::function<void(const Vector &, fptype, DenseMatrix &)> TDF,
                             Coefficient *q = nullptr)
...
public:
   ...
   /// point @a ip.
   fptype Eval(int i, int j, ElementTransformation &T, const IntegrationPoint &ip)
...
public:
   ...
   SumCoefficient(fptype A, Coefficient &B,
                  fptype alpha_ = 1.0, fptype beta_ = 1.0)
   ...
   void SetAConst(fptype A) { a = NULL; aConst = A; }
   /// Return the first term in the linear combination
   fptype GetAConst() const { return aConst; }
...
private:
   ...
   std::function<void(const Vector &, fptype, DenseSymmetricMatrix &)> TDFunction;
...
public:
   ...
   /** \param dim - the size of the matrix
       ...
       \param q - optional scalar Coefficient to scale the matrix coefficient */
   SymmetricMatrixFunctionCoefficient(int dim,
                                      std::function<void(const Vector &, fptype, DenseSymmetricMatrix &)> TDF,
                                      Coefficient *q = nullptr)
...
public:
   ...
   ProductCoefficient(fptype A, Coefficient &B)
      : aConst(A), a(NULL), b(&B) { }
...
public:
   ...
   RatioCoefficient(fptype A, Coefficient &B)
   ...
   RatioCoefficient(Coefficient &A, fptype B)
   ...
   void SetBConst(fptype B) { b = NULL; bConst = B; }
   /// Return the denominator of the ratio
   fptype GetBConst() const { return bConst; }
   ...
   virtual fptype Eval(ElementTransformation &T,
                       const IntegrationPoint &ip)
   {
      fptype den = (b == NULL ) ? bConst : b->Eval(T, ip);
...
public:
   ...
   PowerCoefficient(Coefficient &A, fptype p_)
      : a(&A), p(p_) { }
   ...
   /// Reset the exponent
   void SetExponent(fptype p_) { p = p_; }
   /// Return the exponent
   fptype GetExponent() const { return p; }
...
public:
   ...
   void SetAlpha(fptype alpha_) { alpha = alpha_; alphaCoef = NULL; }
   /// Return the factor in front of the first vector coefficient
   fptype GetAlpha() const { return alpha; }

   /// Reset the factor in front of the second vector coefficient as a constant
   void SetBeta(fptype beta_) { beta = beta_; betaCoef = NULL; }
   /// Return the factor in front of the second vector coefficient
   fptype GetBeta() const { return beta; }
...
public:
   ...
   ScalarVectorProductCoefficient(fptype A, VectorCoefficient &B);
...
public:
   /** @brief Return a vector normalized to a length of one
       ...
       vector is returned.
   */
   NormalizedVectorCoefficient(VectorCoefficient &A, fptype tol = 1e-6);
...
public:
   /// Constructor with one coefficient.  Result is A*B.
   ScalarMatrixProductCoefficient(fptype A, MatrixCoefficient &B);
...
public:
   CrossCrossCoefficient(fptype A, VectorCoefficient &K);
...
public:
   ...
   void SetConstant(fptype constant);
...
fptype ComputeLpNorm(fptype p, Coefficient &coeff, Mesh &mesh,
                     const IntegrationRule *irs[]);

/** @brief Compute the Lp norm of a vector function f = {f_i}_i=1...N.
    \f$ \| f \|_{Lp} = ( \sum_i \| f_i \|_{Lp}^p )^{1/p} \f$ */
fptype ComputeLpNorm(fptype p, VectorCoefficient &coeff, Mesh &mesh,
                     const IntegrationRule *irs[]);
