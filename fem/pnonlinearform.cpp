...
ParTimeDepNonlinearForm::ParTimeDepNonlinearForm(ParFiniteElementSpace *pf)
   : TimeDepNonlinearForm(pf), pGrad(Operator::Hypre_ParCSR)
{
   X.MakeRef(pf, NULL);
   Y.MakeRef(pf, NULL);
   MFEM_VERIFY(!Serial(), "internal MFEM error");
   ...
   const Vector &dx) const
{
   ...
   TimeDepNonlinearForm::GetGradient(x); // (re)assemble Grad, no b.c.
...
Operator &ParTimeDepNonlinearForm::GetGradient(const Vector &dx) const
{
   if (TimeDepNonlinearForm::ext) { return TimeDepNonlinearForm::GetGradient(dx); }
   ...
   if (fnfi.Size())
   {
      ...
      TimeDepNonlinearForm::GetGradient(dx, false); // (re)assemble Grad, no b.c.

      GradientSharedFaces(dx, skip_zeros);
   ...
   {
      TimeDepNonlinearForm::GetGradient(dx); // (re)assemble Grad, no b.c.
...
void ParTimeDepNonlinearForm::Update()
{
   Y.MakeRef(ParFESpace(), NULL);
   X.MakeRef(ParFESpace(), NULL);
   pGrad.Clear();
   TimeDepNonlinearForm::Update();
}
...
// Here, rhs is a true dof vector
void ParBlockTimeDepNonlinearForm::SetEssentialBC(
   const Array<Array<int>*> &bdr_attr_is_ess, Array<Vector*> &rhs)
{
   Array<Vector *> nullarray(fes.Size());
   nullarray = NULL;

   BlockTimeDepNonlinearForm::SetEssentialBC(bdr_attr_is_ess, nullarray);
...
void ParBlockTimeDepNonlinearForm::SetEssentialTrueDofs(
   const Array<Array<int>*> &ess_tdof_list, Array<Vector*> &rhs)
{
   Array<Vector *> nullarray(fes.Size());
   nullarray = nullptr;

   BlockTimeDepNonlinearForm::SetEssentialTrueDofs(ess_tdof_list, nullarray);
...
real_t ParBlockTimeDepNonlinearForm::GetEnergy(const Vector &dx) const
{
   ...
   for (int s = 0; s < fes.Size(); ++s)
   {
      fes[s]->GetProlongationMatrix()->Mult(dxs_true.GetBlock(s), dxs.GetBlock(s));
   }

   real_t enloc = BlockTimeDepNonlinearForm::GetEnergyBlocked(xs, dxs);
   real_t englo = 0.0;
...
/// Return the local gradient matrix for the given true-dof vector x
const BlockOperator & ParBlockTimeDepNonlinearForm::GetLocalGradient(
   const Vector &dx) const
{
   add(x0,dt,dx,x);   // x = x0 + dt*dx
   ...
   // (re)assemble Grad without b.c. into 'Grads'
   BlockTimeDepNonlinearForm::ComputeGradientBlocked(xs, dxs);
...
// Set the operator type id for the parallel gradient matrix/operator.
void ParBlockTimeDepNonlinearForm::SetGradientType(Operator::Type tid)
{
   for (int s1=0; s1<fes.Size(); ++s1)
   {
      for (int s2=0; s2<fes.Size(); ++s2)
   ...
   const Vector &dx) const
{
   ...
   if (fnfi.Size() > 0)
   {
      ...
      // (re)assemble Grad without b.c. into 'Grads'
      BlockTimeDepNonlinearForm::ComputeGradientBlocked(xs, dxs, false);
