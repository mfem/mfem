...
public:
   ...
   virtual
   void Project(const GridFunction& gf, fptype lerr) = 0;
};
...
private:
   ...
   std::unique_ptr<KDTree<int,fptype,kdim>> kdt;
...
public:
   ...
   KDTreeNodalProjection(GridFunction& dest_)
   {
      ...
      kdt=std::unique_ptr<KDTree<int,fptype,kdim>>(
             new KDTree<int,fptype,kdim>());
   ...
   virtual
   void Project(const Vector& coords,const Vector& src,
                int ordering=Ordering::byNODES, fptype lerr=1e-8);
   ...
   /// between them is smaller than lerr.
   virtual
   void Project(const GridFunction& gf, fptype lerr=1e-8);
};
