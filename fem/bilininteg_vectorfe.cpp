                            ...
                            Vector &_y)
{
   ...
   auto op = Reshape(_op, scalarCoeff ? 1 : 9, Q1D, Q1D, Q1D, NE);
   auto x = Reshape(_x, 3*(D1D-1)*D1D*(trialHcurl ? D1D : D1D-1), NE);
   auto y = Reshape(_y, 3*(D1Dtest-1)*D1Dtest*
                            ...
                            Vector &_y)
{
   ...
   auto Bot = Reshape(_Bot, D1Dtest-1, Q1D);
   auto Bct = Reshape(_Bct, D1Dtest, Q1D);
   auto op = Reshape(_op, scalarCoeff ? 1 : 4, Q1D, Q1D, NE);
   auto x = Reshape(_x, 2*(D1D-1)*D1D, NE);
   auto y = Reshape(_y, 2*(D1Dtest-1)*D1Dtest, NE);
