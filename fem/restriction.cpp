...
void L2ElementRestriction::FillI(SparseMatrix &mat) const
{
   ...
   {
      I[dof] = dof<interior_dofs ? elem_dofs*vd : 0;
   });
}
                                        ...
                                        SparseMatrix &mat) const
{
   const int elem_dofs = ndof;
   const int vd = vdim;
   const int scalar_dofs = ndofs;
   const bool t = byvdim;
   auto I = mat.ReadWriteI();
   auto J = mat.WriteJ();
   auto Data = mat.WriteData();
   auto mat_ea = Reshape(ea_data.Read(), elem_dofs, vd, elem_dofs, vd, ne);
   mfem::forall(ne*elem_dofs, [=] MFEM_HOST_DEVICE (int iE)
   {
      const int e = iE/elem_dofs;
      const int i = iE%elem_dofs;
      for (int ci = 0; ci < vd; ++ci)
      {
         const int row = t ? iE*vd + ci : ci*scalar_dofs + iE;
         const int offset = AddNnz(row, I, elem_dofs*vd);
         for (int cj = 0; cj < vd; ++cj)
         {
            for (int j = 0; j < elem_dofs; ++j)
            {
               const int jE = e*elem_dofs + j;
               const int col = t ? jE*vd + cj : cj*scalar_dofs + jE;
               const int k = j + elem_dofs*cj;
               J[offset+k] = col;
               Data[offset+k] = mat_ea(j, cj, i, ci, e);
            }
         }
      }
   });
