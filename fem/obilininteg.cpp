...
template <int Dim>
PAOperator<Dim>::PAOperator(FiniteElementSpace *_fes, const int ir_order)
{
...
void PAOperator<1>::MultBtDB(const DenseMatrix Dmat, const Vector &V, Vector &U)
{
...
void PAOperator<2>::MultBtDB(const DenseMatrix Dmat, const Vector &V, Vector &U)
{
...
void PAOperator<3>::MultBtDB(const DenseMatrix Dmat, const Vector &V, Vector &U)
{
   const int dofs   = fe->GetDof();
   const int quads  = IntRule->GetNPoints();
...
void PAOperator<1>::MultGtDG(const DenseTensor Dtensor, const Vector &V, Vector &U)
{
   const int dofs1d = shape1d.Height();
   const int quads1d = shape1d.Width();
   const int quads = quads1d;
...
void PAOperator<2>::MultGtDG(const DenseTensor Dtensor, const Vector &V, Vector &U)
{
   const int dim = 2;
...
void PAOperator<3>::MultGtDG(const DenseTensor Dtensor, const Vector &V, Vector &U)
{
   const int dim = 3;
   const int terms = dim*(dim+1)/2;
