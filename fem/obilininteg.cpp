...
void PAOperator::MultBtDB1(DenseMatrix &Dmat, const Vector &V, Vector &U) const
{
...
void PAOperator::MultBtDB2(DenseMatrix &Dmat, const Vector &V, Vector &U) const
{
...
void PAOperator::MultBtDB3(DenseMatrix &Dmat, const Vector &V, Vector &U) const
{
   const FiniteElement *fe = fes->GetFE(0);
   const int dofs   = fe->GetDof();
   const int quads  = IntRule->GetNPoints();
...
void PAOperator::MultGtDG1(DenseTensor &Dtensor, const Vector &V, Vector &U) const
{
   const int dofs1d = shape1d.Height();
   const int quads1d = shape1d.Width();
   const int quads = quads1d;
...
void PAOperator::MultGtDG2(DenseTensor &Dtensor, const Vector &V, Vector &U) const
{
   const int dim = 2;
...
void PAOperator::MultGtDG3(DenseTensor &Dtensor, const Vector &V, Vector &U) const
{
   const int dim = 3;
   const int terms = dim*dim;
...
void PAOperator::MultBtDG1(DenseTensor &Dtensor, const Vector &V, Vector &U) const
{
   const int dofs1d = shape1d.Height();
   const int quads1d = shape1d.Width();
   const int quads = quads1d;
...
void PAOperator::MultBtDG2(DenseTensor &Dtensor, const Vector &V, Vector &U) const
{
   const int dim = 2;
   const int terms = dim*dim;
...
void PAOperator::MultBtDG3(DenseTensor &Dtensor, const Vector &V, Vector &U) const
{
   const int dim = 3;
   const int terms = dim*dim;
...
void PAOperator::MultGtDB1(DenseTensor &Dtensor, const Vector &V, Vector &U) const
{
   const int dofs1d = shape1d.Height();
   const int quads1d = shape1d.Width();
   const int quads = quads1d;
...
void PAOperator::MultGtDB2(DenseTensor &Dtensor, const Vector &V, Vector &U) const
{
   const int dim = 2;
   const int terms = dim*dim;
...
void PAOperator::MultGtDB3(DenseTensor &Dtensor, const Vector &V, Vector &U) const
{
   const int dim = 3;
   ...
   pao(_fes, ir_order)
{
   ...
   for (int e = 0; e < fes->GetNE(); e++)
   {
      ...
      for (int k = 0; k < quads; k++)
      {
         ...
         Tr->SetIntPoint(&ip);
         //const DenseMatrix &temp = Tr->AdjugateJacobian();
         DenseMatrix mat = (Tr->AdjugateJacobian());
         //MultABt(temp, temp, mat);
         mat *= ip.weight;
...
void PAConvectionIntegrator::AssembleVector(const Vector &fun, Vector &vect)
{
   ...
   {
   case 1:
      pao.MultGtDB1(Dtensor,fun,vect);
      break;
   case 2:
      pao.MultGtDB2(Dtensor,fun,vect);
      break;
   case 3:
      pao.MultGtDB3(Dtensor,fun,vect);
      break;
   }
}
