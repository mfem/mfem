                                            ...
                                            Vector &d_error_estimates)
{
   ...
   switch (type)
   {
      case Type::Residual:
      {
         auto sum_res = [](const DenseMatrix &fmat, const Vector &p,
                           const Vector &x) -> real_t
         {
            real_t res = 0.;
         ...
         bfi.AssembleHDGFaceMatrix(0, *fe_tr, fe1, FTr, fmat1);
         error_estimates(FTr.Elem1No) += fabs(sum_res(fmat1, p1, tr));

         if (FTr.Elem2No >= 0)
         {
            bfi.AssembleHDGFaceMatrix(1, *fe_tr, fe2, FTr, fmat2);
            error_estimates(FTr.Elem2No) += fabs(sum_res(fmat2, p2, tr));
         }
      }
      break;
      case Type::Energy:
