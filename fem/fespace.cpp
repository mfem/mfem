...
{
   int fine_elno = T.ElementNo;
   int coarse_element = refine_relation_[fine_elno];

   double x[3];
   Vector physical_point(x, 3);
   T.Transform(ip, physical_point);

   GridFunction * ho_gf = ho_coeff_.GetGridFunction();
   FiniteElementSpace * ho_fes = ho_gf->FESpace();

   ElementTransformation * ho_eltrans =
      ho_fes->GetElementTransformation(coarse_element);
   IntegrationPoint ho_ip;
   int result = ho_eltrans->TransformBack(physical_point, ho_ip);
   MFEM_ASSERT(result == 0, "Unable to evaluate coefficient!");
   double out = ho_coeff_.Eval(*ho_eltrans, ho_ip);
   return out;
}
...
{
   int ho_size = ho_fespace.GetVSize();
   ...
   {
      int coarsee = refine_relation[finee];
      Array<int> ho_dofs;
      ho_fespace.GetElementVDofs(coarsee, ho_dofs);
      Array<int> lor_dofs;
      lor_fespace.GetElementVDofs(finee, lor_dofs);
      ...
      for (int j=0; j<ho_dofs.Size(); ++j)
      {
         ...
         VectorRefinedCoefficient ref_coeff(refine_relation, ho_vgfc);

         Vector lor_proj(lor_dofs.Size());
         ...
         {
            localP(i,j) = lor_proj[i]; // no sign correction here...
         }
      }
   ...
   const Array<int>& refine_relation)
{
   ...
   for (int finee=0; finee<lor_mesh->GetNE(); ++finee)
   {
      int coarsee = refine_relation[finee];
      Array<int> ho_dofs;
      ho_fespace.GetElementDofs(coarsee, ho_dofs);
      Array<int> lor_dofs;
      lor_fespace.GetElementDofs(finee, lor_dofs);
      DenseMatrix localP(lor_dofs.Size(), ho_dofs.Size());
      for (int j=0; j<ho_dofs.Size(); ++j)
      {
         Vector v(ho_size);
         ...
         GridFunction ho_vgf(&ho_fespace);
         ho_vgf = v;
         GridFunctionCoefficient ho_vgfc(&ho_vgf);
         RefinedCoefficient ref_coeff(refine_relation, ho_vgfc);
...
/// @todo put this in fespace.cpp (not pfespace.cpp)
