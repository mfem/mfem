...
public:
   ...
   virtual fptype GetLocalStateEnergyPA(const Vector &x) const;
...
public:
   /// Compute the local energy
   virtual fptype GetElementEnergy(const Array<const FiniteElement *>&el,
                                   ElementTransformation &Tr,
...
public:
   virtual fptype EvalW(const DenseMatrix &J) const;
...
public:
   NeoHookeanModel(fptype mu_, fptype K_, fptype g_ = 1.0)
