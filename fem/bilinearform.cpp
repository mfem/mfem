...
void BilinearForm::AssembleDiagonal(Vector &diag) const
{
   if (ext)
   {
      ...
      // For an AMR mesh, a convergent diagonal is assembled with
      // |P^T| d_e, where |P^T| has the entry-wise absolute values
      // of the conforming prolongation transpose operator.
      if (P && !fes->Conforming())
      {
