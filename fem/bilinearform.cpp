...
#pragma omp parallel for private(tmp, eltrans)
   ...
   BilinearForm::~BilinearForm()
   {
      ...
      if (!extern_bfs)
      {
         ...
         for (k = 0; k < fbfi.Size(); k++)
         {
            delete fbfi[k];
         }
         for (k = 0; k < bfbfi.Size(); k++)
         {
            delete bfbfi[k];
         }
      }
   ...
   MixedBilinearForm::~MixedBilinearForm()
   {
      ...
      {
         int i;
         for (i = 0; i < dbfi.Size(); i++)
         {
            delete dbfi[i];
         }
         for (i = 0; i < bbfi.Size(); i++)
         {
            delete bbfi[i];
         }
         for (i = 0; i < tfbfi.Size(); i++)
         {
            delete tfbfi[i];
         }
         for (i = 0; i < btfbfi.Size(); i++)
         {
            delete btfbfi[i];
         }
      }
