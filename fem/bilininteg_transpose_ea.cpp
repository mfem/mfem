                                     ...
                                     Vector &ea_data, const bool add)
{
   ...
   else
   {
      ...
      if (ne == 0) { return; }
      const int dofs = fes.GetFE(0)->GetDof();
      auto A = Reshape(ea_data, dofs, dofs, ne);
      MFEM_FORALL(e, ne,
      {
         for (int i = 0; i < dofs; i++)
         {
                                                  ...
                                                  const bool add)
{
   ...
   else
   {
      ...
      const int faceDofs = fes.GetTraceElement(0,
                                               fes.GetMesh()->GetFaceBaseGeometry(0))->GetDof();
      auto A_int = Reshape(ea_data_int, faceDofs, faceDofs, 2, nf);
      auto A_ext = Reshape(ea_data_ext, faceDofs, faceDofs, 2, nf);
      MFEM_FORALL(f, nf,
      {
         for (int i = 0; i < faceDofs; i++)
         {
                                                  ...
                                                  const bool add)
{
   ...
   else
   {
      ...
      const int faceDofs = fes.GetTraceElement(0,
                                               fes.GetMesh()->GetFaceBaseGeometry(0))->GetDof();
      auto A_bdr = Reshape(ea_data_bdr, faceDofs, faceDofs, nf);
      MFEM_FORALL(f, nf,
      {
         for (int i = 0; i < faceDofs; i++)
         {
