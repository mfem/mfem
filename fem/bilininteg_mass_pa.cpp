...
void MassIntegrator::SetupPA(const FiniteElementSpace &fes)
{
   ...
   if (dim==2)
   {
      ...
      const auto W = Reshape(ir->GetWeights(), Q1D,Q1D);
      const auto J = Reshape(geom->J, Q1D,Q1D,2,2,NE);
      const auto C = const_c ? Reshape(coeff, 1,1,1) :
                     Reshape(coeff, Q1D,Q1D,NE);
   ...
   if (dim==3)
   {
      ...
      const int Q1D = quad1D;
      const bool const_c = coeff.Size() == 1;
      const auto W = Reshape(ir->GetWeights(), Q1D,Q1D,Q1D);
      const auto J = Reshape(geom->J, Q1D,Q1D,Q1D,3,3,NE);
      const auto C = const_c ? Reshape(coeff, 1,1,1,1) :
                     Reshape(coeff, Q1D,Q1D,Q1D,NE);
