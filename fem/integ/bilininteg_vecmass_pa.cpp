...
void VectorMassIntegrator::AssemblePA(const FiniteElementSpace &fes)
{
   ...
   if (dim == 2)
   {
      ...
      mfem::forall(NE, [=] MFEM_HOST_DEVICE (int e)
      {
         for (int q = 0; q < NQ; ++q)
         {
            ...
            const fptype J12 = J(q,1,0,e);
            const fptype J21 = J(q,0,1,e);
   ...
   if (dim == 3)
   {
      ...
      mfem::forall(NE, [=] MFEM_HOST_DEVICE (int e)
      {
         for (int q = 0; q < NQ; ++q)
         {
            const fptype J11 = J(q,0,0,e), J12 = J(q,0,1,e), J13 = J(q,0,2,e);
            const fptype J21 = J(q,1,0,e), J22 = J(q,1,1,e), J23 = J(q,1,2,e);
            const fptype J31 = J(q,2,0,e), J32 = J(q,2,1,e), J33 = J(q,2,2,e);
                                           ...
                                           const int q1d = 0)
{
   ...
   mfem::forall(NE, [=] MFEM_HOST_DEVICE (int e)
   {
      ...
      fptype temp[max_Q1D][max_Q1D][max_D1D];
      for (int qx = 0; qx < Q1D; ++qx)
      {
         for (int qy = 0; qy < Q1D; ++qy)
         {
      ...
      fptype temp2[max_Q1D][max_D1D][max_D1D];
      for (int qx = 0; qx < Q1D; ++qx)
      {
         for (int dz = 0; dz < D1D; ++dz)
         {
                                ...
                                const int q1d = 0)
{
   ...
   mfem::forall(NE, [=] MFEM_HOST_DEVICE (int e)
   {
      ...
      constexpr int max_D1D = T_D1D ? T_D1D : DofQuadLimits::MAX_D1D;
      constexpr int max_Q1D = T_Q1D ? T_Q1D : DofQuadLimits::MAX_Q1D;
      fptype sol_xyz[max_Q1D][max_Q1D][max_Q1D];
      for (int c = 0; c < VDIM; ++ c)
      {
         for (int qz = 0; qz < Q1D; ++qz)
         {
         ...
         for (int qz = 0; qz < Q1D; ++qz)
         {
            ...
            for (int qy = 0; qy < Q1D; ++qy)
            {
               ...
               for (int qx = 0; qx < Q1D; ++qx)
               {
                  const fptype s = sol_xyz[qz][qy][qx];
                  for (int dx = 0; dx < D1D; ++dx)
                  {
                     sol_x[dx] += Bt(dx,qx) * s;
                  }
