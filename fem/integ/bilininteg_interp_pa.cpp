...
static void PAHcurlApplyGradientTranspose2D(
   ...
   const Array<fptype> &B_, const Array<fptype> &G_,
                                   ...
                                   Vector &y_)
{
   ...
   {
      constexpr static int MAX_D1D = DofQuadLimits::HCURL_MAX_D1D;
      fptype w1[MAX_D1D][MAX_D1D][MAX_D1D];
      fptype w2[MAX_D1D][MAX_D1D][MAX_D1D];
...
static void PAHcurlVecH1IdentityApply2D(const int c_dofs1D,
                                        const int o_dofs1D,
                                        const int NE,
                                        const Array<fptype> &Bclosed,
                                        const Array<fptype> &Bopen,
                                                 ...
                                                 Vector &y_)
{
   ...
   mfem::forall(NE, [=] MFEM_HOST_DEVICE (int e)
   {
      ...
      for (int ey = 0; ey < c_dofs1D; ++ey)
      {
         ...
         {
            const int local_index = ey*o_dofs1D + ex;
            const fptype xd = x(local_index, e);
                                        ...
                                        Vector &y_)
{
   ...
   mfem::forall(NE, [=] MFEM_HOST_DEVICE (int e)
   {
      ...
      fptype w1[3][MAX_D1D][MAX_D1D][MAX_D1D];
      fptype w2[3][MAX_D1D][MAX_D1D][MAX_D1D];
      ...
      for (int ez = 0; ez < c_dofs1D; ++ez)
      {
         for (int ey = 0; ey < c_dofs1D; ++ey)
         {
            for (int j=0; j<3; ++j)
            {
               ...
               {
                  const int local_index = ez*c_dofs1D*o_dofs1D + ey*o_dofs1D + ex;
                  const fptype xv = x(local_index, e) * vk(j, local_index, e);
                  for (int dx = 0; dx < c_dofs1D; ++dx)
                  {
                                      ...
                                      const FiniteElementSpace &test_fes)
{
   ...
   if (dim == 3)
   {
      ...
      const fptype tk[9] = { 1.,0.,0.,  0.,1.,0.,  0.,0.,1. };
