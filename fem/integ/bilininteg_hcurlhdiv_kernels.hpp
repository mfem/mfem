         ...
         for (int qy = 0; qy < Q1D; ++qy)
         {
            for (int qx = 0; qx < Q1D; ++qx)
            {
               ...
               const fptype c1 = (O11 * curl[qz][qy][qx][0]) + (O12 * curl[qz][qy][qx][1]) +
                                 (O13 * curl[qz][qy][qx][2]);
               const fptype c2 = (O12 * curl[qz][qy][qx][0]) + (O22 * curl[qz][qy][qx][1]) +
                                 (O23 * curl[qz][qy][qx][2]);
               const fptype c3 = (O13 * curl[qz][qy][qx][0]) + (O23 * curl[qz][qy][qx][1]) +
                                 (O33 * curl[qz][qy][qx][2]);
      ...
      for (int qz = 0; qz < Q1D; ++qz)
      {
         ...
         for (int c = 0; c < VDIM; ++c)  // loop over x, y, z components
         {
            ...
            for (int qy = 0; qy < Q1D; ++qy)
            {
               ...
               for (int dy = 0; dy < D1Dy; ++dy)
               {
                  const fptype wy = (c == 1) ? Bct(dy,qy) : Bot(dy,qy);
                  for (int dx = 0; dx < D1Dx; ++dx)
                  {
            ...
            for (int dz = 0; dz < D1Dz; ++dz)
            {
               const fptype wz = (c == 2) ? Bct(dz,qz) : Bot(dz,qz);
               for (int dy = 0; dy < D1Dy; ++dy)
               {
                  for (int dx = 0; dx < D1Dx; ++dx)
                  {
      ...
      for (int c = 0; c < VDIM; ++c)  // loop over x, y, z components
      {
         ...
         for (int dz = 0; dz < D1Dz; ++dz)
         {
            ...
            for (int dy = 0; dy < D1Dy; ++dy)
            {
               ...
               for (int qy = 0; qy < Q1D; ++qy)
               {
                  const fptype wy = (c == 1) ? Bc(qy,dy) : Bo(qy,dy);
                  for (int qx = 0; qx < Q1D; ++qx)
                  {
                     const fptype wx = massX[qx];
                     massXY[qy][qx] += wx * wy;
            ...
            for (int qz = 0; qz < Q1D; ++qz)
            {
               const fptype wz = (c == 2) ? Bc(qz,dz) : Bo(qz,dz);
               for (int qy = 0; qy < Q1D; ++qy)
               {
                  for (int qx = 0; qx < Q1D; ++qx)
                  {
