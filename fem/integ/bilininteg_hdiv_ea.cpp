...
static void EAHdivMassAssemble2D(const int NE,
                                 const Array<real_t> &o_basis,
                                 const Array<real_t> &c_basis,
                                 ...
                                 const int q1d = 0)
{
   ...
   const auto Bo = Reshape(o_basis.Read(), Q1D, D1D-1);
   const auto Bc = Reshape(c_basis.Read(), Q1D, D1D);
   const int D_size = symmetric ? 3 : 4;
   const auto D = Reshape(pa_data.Read(), Q1D, Q1D, D_size, NE);
