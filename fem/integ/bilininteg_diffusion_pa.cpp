                                         ...
                                         Vector &y) const
{
   ...
   Array3D<fptype> gradXY(3, std::max(Q1D[0], D1D[0]), std::max(Q1D[1], D1D[1]));
   Array2D<fptype> gradX(3, std::max(Q1D[0], D1D[0]));
   ...
   for (int dz = 0; dz < D1D[2]; ++dz)
   {
      ...
      for (int dy = 0; dy < D1D[1]; ++dy)
      {
         ...
         for (int qy = minD[1][dy]; qy <= maxD[1][dy]; ++qy)
         {
            ...
            for (int qx = 0; qx < Q1D[0]; ++qx)
            {
               const fptype wx  = gradX(0,qx);
               const fptype wDx = gradX(1,qx);
   ...
   for (int qz = 0; qz < Q1D[2]; ++qz)
   {
      for (int qy = 0; qy < Q1D[1]; ++qy)
      {
         for (int qx = 0; qx < Q1D[0]; ++qx)
         {
            ...
            const fptype O00 = qd(q,0);
            const fptype O01 = qd(q,1);
            const fptype O02 = qd(q,2);
            const fptype O10 = symmetric ? O01 : qd(q,3);
            const fptype O11 = symmetric ? qd(q,3) : qd(q,4);
            const fptype O12 = symmetric ? qd(q,4) : qd(q,5);
            const fptype O20 = symmetric ? O02 : qd(q,6);
            const fptype O21 = symmetric ? O12 : qd(q,7);
            const fptype O22 = symmetric ? qd(q,5) : qd(q,8);

            const fptype grad0 = grad[0](qx,qy,qz);
            const fptype grad1 = grad[1](qx,qy,qz);
            const fptype grad2 = grad[2](qx,qy,qz);
