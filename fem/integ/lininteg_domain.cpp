...
static void DLFEvalAssemble2D(const int vdim, const int ne, const int d,
                              const int q,
                              const int map_type, const int *markers, const fptype *b,
                              const fptype *detj, const fptype *weights,
                              const Vector &coeff, fptype *y)
{
   ...
   mfem::forall_2D(ne, q, q, [=] MFEM_HOST_DEVICE (int e)
   {
      ...
      MFEM_SHARED fptype sBt[Q*D];
      MFEM_SHARED fptype sQQ[Q*Q];
      MFEM_SHARED fptype sQD[Q*D];
      ...
      {
         const fptype cst_val = C(c,0,0,0);
         MFEM_FOREACH_THREAD(x,x,q)
         {
            MFEM_FOREACH_THREAD(y,y,q)
            {
               const fptype detJ = (map_type == FiniteElement::VALUE) ? DETJ(x,y,e) : 1.0;
               const fptype coeff_val = cst ? cst_val : C(c,x,y,e);
                              ...
                              const Vector &coeff, fptype *y)
{
   ...
   mfem::forall_2D(ne, q, q, [=] MFEM_HOST_DEVICE (int e)
   {
      ...
      for (int c = 0; c < vdim; ++c)
      {
         const fptype cst_val = C(c,0,0,0,0);
         MFEM_FOREACH_THREAD(x,x,q)
         {
            MFEM_FOREACH_THREAD(y,y,q)
            {
               for (int z = 0; z < q; ++z)
               {
                  const fptype detJ = (map_type == FiniteElement::VALUE) ? DETJ(x,y,z,e) : 1.0;
                  const fptype coeff_val = cst_coeff ? cst_val : C(c,x,y,z,e);
                            ...
                            Vector &y)
{
   ...
   const fptype *detJ = geom->detJ.Read();
