                       ...
                       Vector &op)
{
   ...
   mfem::forall(NE, [=] MFEM_HOST_DEVICE (int e)
   {
      for (int q = 0; q < NQ; ++q)
      {
         ...
         const fptype c_detJ = W[q] / ((J11*J22)-(J21*J12));

         // (1/detJ) J^T C J
         if (coeffDim == 3 || coeffDim == 4) // Matrix coefficient
         {
            const fptype C11 = C(0,q,e);
            const fptype C12 = C(1,q,e);
            const fptype C21 = symmetric ? C12 : C(2,q,e);
            const fptype C22 = symmetric ? C(2,q,e) : C(3,q,e);
            const fptype R11 = C11*J11 + C12*J21;
            const fptype R21 = C21*J11 + C22*J21;
            const fptype R12 = C11*J12 + C12*J22;
            const fptype R22 = C21*J12 + C22*J22;
         ...
         {
            const fptype C1 = C(0,q,e);
            const fptype C2 = (coeffDim == 2 ? C(1,q,e) : C1);
      ...
      for (int c = 0; c < VDIM; ++c)  // loop over x, y components
      {
         ...
         for (int dy = 0; dy < D1Dy; ++dy)
         {
            ...
            for (int dx = 0; dx < D1Dx; ++dx)
            {
               ...
               for (int qx = 0; qx < Q1D; ++qx)
               {
                  const fptype wx = (c == 0) ? Bc(qx,dx) : Bo(qx,dx);
                  val += mass[qx] * wx * wx;
               }
                                  ...
                                  Vector &diag_)
{
   ...
   mfem::forall(NE, [=] MFEM_HOST_DEVICE (int e)
   {
      ...
      for (int c = 0; c < VDIM; ++c)  // loop over x, y, z components
      {
         ...
         fptype mass[DofQuadLimits::HDIV_MAX_Q1D];
   ...
   mfem::forall(NE, [=] MFEM_HOST_DEVICE (int e)
   {
      ...
      for (int qy = 0; qy < Q1D; ++qy)
      {
         for (int qx = 0; qx < Q1D; ++qx)
         {
            ...
            const fptype O21 = symmetric ? O12 : op(qx,qy,2,e);
                       ...
                       Vector &y_)
{
   ...
   mfem::forall(NE, [=] MFEM_HOST_DEVICE (int e)
   {
      fptype mass[DofQuadLimits::HDIV_MAX_Q1D][DofQuadLimits::HDIV_MAX_Q1D][DofQuadLimits::HDIV_MAX_Q1D][VDIM];
      ...
      for (int c = 0; c < VDIM; ++c)  // loop over x, y, z components
      {
         ...
         for (int dz = 0; dz < D1Dz; ++dz)
         {
            fptype massXY[DofQuadLimits::HDIV_MAX_Q1D][DofQuadLimits::HDIV_MAX_Q1D];
            for (int qy = 0; qy < Q1D; ++qy)
            {
               for (int qx = 0; qx < Q1D; ++qx)
               {
            ...
            for (int dy = 0; dy < D1Dy; ++dy)
            {
               fptype massX[DofQuadLimits::HDIV_MAX_Q1D];
               for (int qx = 0; qx < Q1D; ++qx)
               {
                  massX[qx] = 0.0;
               }
      ...
      for (int c = 0; c < VDIM; ++c)  // loop over x, y components
      {
         ...
         for (int dy = 0; dy < D1Dy; ++dy)
         {
            for (int qx = 0; qx < Q1D; ++qx)
            {
               ...
               for (int qy = 0; qy < Q1D; ++qy)
               {
                  const fptype wy = (c == 0) ? Bo(qy,dy) : Gc(qy,dy);
            ...
            for (int dx = 0; dx < D1Dx; ++dx)
            {
               ...
               for (int qx = 0; qx < Q1D; ++qx)
               {
                  const fptype wx = (c == 0) ? Gc(qx,dx) : Bo(qx,dx);
                  val += div[qx] * wx * wx;
               }
               diag(dx + (dy * D1Dx) + osc, e) += val;
            }
   ...
   mfem::forall(NE, [=] MFEM_HOST_DEVICE (int e)
   {
      ...
      for (int c = 0; c < VDIM; ++c)  // loop over x, y, z components
      {
         ...
         for (int dz = 0; dz < D1Dz; ++dz)
         {
            for (int dy = 0; dy < D1Dy; ++dy)
            {
               fptype a[DofQuadLimits::HDIV_MAX_Q1D];
               ...
               for (int qx = 0; qx < Q1D; ++qx)
               {
                  ...
                  for (int qy = 0; qy < Q1D; ++qy)
                  {
                     const fptype wy = (c == 1) ? Gc(qy,dy) : Bo(qy,dy);

                     for (int qz = 0; qz < Q1D; ++qz)
                     {
                        const fptype wz = (c == 2) ? Gc(qz,dz) : Bo(qz,dz);
   ...
   mfem::forall(NE, [=] MFEM_HOST_DEVICE (int e)
   {
      ...
      for (int qy = 0; qy < Q1D; ++qy)
      {
         ...
         for (int c = 0; c < VDIM; ++c)  // loop over x, y components
         {
            ...
            for (int dy = 0; dy < D1Dy; ++dy)
            {
               const fptype wy = (c == 0) ? Bot(dy,qy) : Gct(dy,qy);
               for (int dx = 0; dx < D1Dx; ++dx)
               {
                     ...
                     Vector &y_)
{
   ...
   mfem::forall(NE, [=] MFEM_HOST_DEVICE (int e)
   {
      fptype div[DofQuadLimits::HDIV_MAX_Q1D][DofQuadLimits::HDIV_MAX_Q1D][DofQuadLimits::HDIV_MAX_Q1D];
      ...
      for (int c = 0; c < VDIM; ++c)  // loop over x, y, z components
      {
         ...
         for (int dz = 0; dz < D1Dz; ++dz)
         {
            fptype aXY[DofQuadLimits::HDIV_MAX_Q1D][DofQuadLimits::HDIV_MAX_Q1D];
            for (int qy = 0; qy < Q1D; ++qy)
            {
               for (int qx = 0; qx < Q1D; ++qx)
               {
            ...
            for (int dy = 0; dy < D1Dy; ++dy)
            {
               fptype aX[DofQuadLimits::HDIV_MAX_Q1D];
               for (int qx = 0; qx < Q1D; ++qx)
               {
                  aX[qx] = 0.0;
               }
      ...
      for (int qz = 0; qz < Q1D; ++qz)
      {
         fptype aXY[DofQuadLimits::HDIV_MAX_D1D][DofQuadLimits::HDIV_MAX_D1D];
         ...
         for (int c = 0; c < VDIM; ++c)  // loop over x, y, z components
         {
            ...
            for (int qy = 0; qy < Q1D; ++qy)
            {
               fptype aX[DofQuadLimits::HDIV_MAX_D1D];
               for (int dx = 0; dx < D1Dx; ++dx)
               {
                  aX[dx] = 0;
               }
               ...
               for (int dy = 0; dy < D1Dy; ++dy)
               {
                  const fptype wy = (c == 1) ? Gct(dy,qy) : Bot(dy,qy);
                  for (int dx = 0; dx < D1Dx; ++dx)
                  {
                     aXY[dy][dx] += aX[dx] * wy;
                  }
            ...
            for (int dz = 0; dz < D1Dz; ++dz)
            {
               const fptype wz = (c == 2) ? Gct(dz,qz) : Bot(dz,qz);
               for (int dy = 0; dy < D1Dy; ++dy)
               {
                  for (int dx = 0; dx < D1Dx; ++dx)
                  {
   ...
   mfem::forall(NE, [=] MFEM_HOST_DEVICE (int e)
   {
      for (int ry = 0; ry < L2D1D; ++ry)
      {
         for (int rx = 0; rx < L2D1D; ++rx)
         {
            ...
            fptype row[2*DofQuadLimits::HDIV_MAX_D1D*(DofQuadLimits::HDIV_MAX_D1D-1)];
            fptype div[DofQuadLimits::HDIV_MAX_Q1D][DofQuadLimits::HDIV_MAX_Q1D];
      ...
      for (int rz = 0; rz < L2D1D; ++rz)
      {
         for (int ry = 0; ry < L2D1D; ++ry)
         {
            for (int rx = 0; rx < L2D1D; ++rx)
            {
               ...
               fptype row[3*DofQuadLimits::HDIV_MAX_D1D*(DofQuadLimits::HDIV_MAX_D1D-1)*
