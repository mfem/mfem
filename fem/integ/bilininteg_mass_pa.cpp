      ...
      mfem::forall_3D(NE, Q1D, Q1D, Q1D, [=] MFEM_HOST_DEVICE (int e)
      {
         MFEM_FOREACH_THREAD(qx,x,Q1D)
         {
            MFEM_FOREACH_THREAD(qy,y,Q1D)
            {
               ...
               {
                  const fptype detJ = J(qx,qy,qz,e);
                  const fptype coeff = const_c ? C(0,0,0,0) : C(qx,qy,qz,e);
...
void MassIntegrator::AssemblePABoundary(const FiniteElementSpace &fes)
{
   ...
   if (dim==1)
   {
      ...
      mfem::forall_2D(NE, Q1D, 1, [=] MFEM_HOST_DEVICE (int e)
      {
         MFEM_FOREACH_THREAD(qx,x,Q1D)
         {
            const fptype detJ = J(qx,e);
            const fptype coeff = const_c ? C(0,0) : C(qx,e);
