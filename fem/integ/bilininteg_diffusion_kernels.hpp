...
void PADiffusionSetup(const int dim, const int sdim, const int D1D,
                      const int Q1D, const int coeffDim, const int NE,
                      const Array<real_t> &W, const Vector &J, const Vector &C,
                      Vector &D);

// PA Diffusion Assemble 2D kernel
template <int T_SDIM>
void PADiffusionSetup2D(const int Q1D, const int coeffDim, const int NE,
                        const Array<real_t> &w, const Vector &j,
                        const Vector &c, Vector &d);

// PA Diffusion Assemble 3D kernel
void PADiffusionSetup3D(const int Q1D, const int coeffDim, const int NE,
                        const Array<real_t> &w, const Vector &j,
                        const Vector &c, Vector &d);

#ifdef MFEM_USE_OCCA
// OCCA 2D Assemble kernel
void OccaPADiffusionSetup2D(const int D1D, const int Q1D, const int NE,
                            const Array<real_t> &W, const Vector &J,
                            const Vector &C, Vector &op);

// OCCA 3D Assemble kernel
void OccaPADiffusionSetup3D(const int D1D, const int Q1D, const int NE,
                            const Array<real_t> &W, const Vector &J,
                            const Vector &C, Vector &op);
#endif // MFEM_USE_OCCA

void PADiffusionAssembleDiagonal(const int dim, const int D1D, const int Q1D,
                                 const int NE, const bool symm,
                                 const Array<real_t> &B, const Array<real_t> &G,
                                 const Vector &D, Vector &Y);

// PA Diffusion Diagonal 2D kernel
template <int T_D1D = 0, int T_Q1D = 0>
inline void PADiffusionDiagonal2D(const int NE, const bool symmetric,
                                  const Array<real_t> &b,
                                  const Array<real_t> &g, const Vector &d,
                                  Vector &y, const int d1d = 0,
                                  const int q1d = 0)
{
...
inline void SmemPADiffusionDiagonal2D(const int NE, const bool symmetric,
                                      const Array<real_t> &b_,
                                      const Array<real_t> &g_, const Vector &d_,
...
template <int T_D1D = 0, int T_Q1D = 0>
inline void PADiffusionDiagonal3D(const int NE, const bool symmetric,
                                  const Array<real_t> &b,
                                  const Array<real_t> &g, const Vector &d,
                                  Vector &y, const int d1d = 0,
                                  const int q1d = 0)
               ...
               for (int qy = 0; qy < Q1D; ++qy)
               {
                  for (int dz = 0; dz < D1D; ++dz)
                  {
                     ...
                     for (int qz = 0; qz < Q1D; ++qz)
                     {
                        ...
                        const int ksym = j >= i ? 3 - (3 - i) * (2 - i) / 2 + j
                                         : 3 - (3 - j) * (2 - j) / 2 + i;
...
template <int T_D1D = 0, int T_Q1D = 0>
inline void SmemPADiffusionDiagonal3D(const int NE, const bool symmetric,
                                      const Array<real_t> &b_,
                                      const Array<real_t> &g_, const Vector &d_,
                                      Vector &y_, const int d1d = 0,
                                      const int q1d = 0)
...
void PADiffusionApply(const int dim, const int D1D, const int Q1D, const int NE,
                      const bool symm, const Array<real_t> &B,
                      const Array<real_t> &G, const Array<real_t> &Bt,
                      const Array<real_t> &Gt, const Vector &D, const Vector &X,
                      Vector &Y);

#ifdef MFEM_USE_OCCA
// OCCA PA Diffusion Apply 2D kernel
void OccaPADiffusionApply2D(const int D1D, const int Q1D, const int NE,
                            const Array<real_t> &B, const Array<real_t> &G,
                            const Array<real_t> &Bt, const Array<real_t> &Gt,
                            const Vector &D, const Vector &X, Vector &Y);

// OCCA PA Diffusion Apply 3D kernel
void OccaPADiffusionApply3D(const int D1D, const int Q1D, const int NE,
                            const Array<real_t> &B, const Array<real_t> &G,
                            const Array<real_t> &Bt, const Array<real_t> &Gt,
                            const Vector &D, const Vector &X, Vector &Y);
#endif // MFEM_USE_OCCA
...
template <int T_D1D = 0, int T_Q1D = 0>
inline void
PADiffusionApply2D(const int NE, const bool symmetric, const Array<real_t> &b_,
                   const Array<real_t> &g_, const Array<real_t> &bt_,
                   const Array<real_t> &gt_, const Vector &d_, const Vector &x_,
...
template <int T_D1D = 0, int T_Q1D = 0, int T_NBZ = 0>
inline void SmemPADiffusionApply2D(const int NE, const bool symmetric,
                                   const Array<real_t> &b_,
                                   const Array<real_t> &g_, const Vector &d_,
...
template <int T_D1D = 0, int T_Q1D = 0>
inline void PADiffusionApply3D(const int NE, const bool symmetric,
                               const Array<real_t> &b, const Array<real_t> &g,
                               const Array<real_t> &bt, const Array<real_t> &gt,
...
template <int T_D1D = 0, int T_Q1D = 0>
inline void SmemPADiffusionApply3D(const int NE, const bool symmetric,
                                   const Array<real_t> &b_,
                                   const Array<real_t> &g_, const Vector &d_,
                                   const Vector &x_, Vector &y_,
                                   const int d1d = 0, const int q1d = 0)
