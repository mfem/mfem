                             ...
                             Vector &y)
{
   ...
   mfem::forall(NE, [=] MFEM_HOST_DEVICE (int e)
   {
      ...
      for (int dy = 0; dy < D1D; ++dy)
      {
         ...
         for (int qy = 0; qy < Q1D; ++qy)
         {
            const fptype wy  = Bc(qy,dy);
            const fptype wDy = Gc(qy,dy);
            for (int qx = 0; qx < Q1D; ++qx)
            {
      ...
      for (int qy = 0; qy < Q1D; ++qy)
      {
         for (int qx = 0; qx < Q1D; ++qx)
         {
            ...
            const fptype O12 = op(qx,qy,1,e);
            const fptype O22 = op(qx,qy,2,e);
                                      ...
                                      Vector &y)
{
   ...
   mfem::forall(NE, [=] MFEM_HOST_DEVICE (int e)
   {
      ...
      for (int qy = 0; qy < Q1D; ++qy)
      {
         ...
         for (int qx = 0; qx < Q1D; ++qx)
         {
            const fptype gX = mass[qy][qx][0];
            const fptype gY = mass[qy][qx][1];
            for (int dx = 0; dx < D1D; ++dx)
            {
                             ...
                             Vector &y)
{
   ...
   mfem::forall(NE, [=] MFEM_HOST_DEVICE (int e)
   {
      ...
      for (int dz = 0; dz < D1D; ++dz)
      {
         ...
         for (int qz = 0; qz < Q1D; ++qz)
         {
            const fptype wz  = Bc(qz,dz);
            const fptype wDz = Gc(qz,dz);
            for (int qy = 0; qy < Q1D; ++qy)
            {
               for (int qx = 0; qx < Q1D; ++qx)
               {
      ...
      for (int qz = 0; qz < Q1D; ++qz)
      {
         for (int qy = 0; qy < Q1D; ++qy)
         {
            for (int qx = 0; qx < Q1D; ++qx)
            {
               ...
               const fptype O22 = op(qx,qy,qz,3,e);
               const fptype O23 = op(qx,qy,qz,4,e);
               const fptype O33 = op(qx,qy,qz,5,e);
   ...
   mfem::forall(NE, [=] MFEM_HOST_DEVICE (int e)
   {
      ...
      for (int qz = 0; qz < Q1D; ++qz)
      {
         ...
         for (int qy = 0; qy < Q1D; ++qy)
         {
            ...
            for (int qx = 0; qx < Q1D; ++qx)
            {
               const fptype gX = mass[qz][qy][qx][0];
               const fptype gY = mass[qz][qy][qx][1];
               const fptype gZ = mass[qz][qy][qx][2];
               for (int dx = 0; dx < D1D; ++dx)
               {
                  const fptype wx  = Bt(dx,qx);
                  const fptype wDx = Gt(dx,qx);
