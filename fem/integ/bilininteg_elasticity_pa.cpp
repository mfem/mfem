                                          ...
                                          Vector &y) const
{
   ...
   const int dim = vdim;
   auto X = Reshape(x.Read(), vdim, D1D[0], D1D[1], D1D[2]);
   auto Y = Reshape(y.Read(), vdim, D1D[0], D1D[1], D1D[2]);
   // auto Y = Reshape(y.HostReadWrite(), dim, D1D[0], D1D[1], D1D[2]);
