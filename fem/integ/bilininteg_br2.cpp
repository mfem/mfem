...
DGDiffusionBR2Integrator::DGDiffusionBR2Integrator(
   FiniteElementSpace &fes, fptype e) : eta(e), Q(NULL)
{
   PrecomputeMassInverse(fes);
}

DGDiffusionBR2Integrator::DGDiffusionBR2Integrator(
   FiniteElementSpace &fes, Coefficient &Q_, fptype e) : eta(e), Q(&Q_)
{
   PrecomputeMassInverse(fes);
}
...
void DGDiffusionBR2Integrator::PrecomputeMassInverse(FiniteElementSpace &fes)
{
   ...
   for (int i=0; i<nel; ++i)
   {
      ...
      int dof = fe->GetDof();
      fptype *Minv_el = &Minv[Minv_offsets[i]];
   ...
   FaceElementTransformations &Trans, DenseMatrix &elmat)
{
   ...
   fptype factor = Geometries.NumBdr(Trans.Elem1->GetGeometryType());
   ...
   if (Trans.Elem2No >= 0)
   {
      ...
      factor = std::max(factor, fptype(Geometries.NumBdr(geom2)));
   }
   else
   {
      ndof2 = 0;
   ...
   for (int p = 0; p < ir->GetNPoints(); p++)
   {
      ...
      fptype q = Q ? Q->Eval(*Trans.Elem1, eip1) : 1.0;
      if (ndof2)
      {
