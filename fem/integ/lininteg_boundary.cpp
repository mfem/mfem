...
static void BLFEvalAssemble2D(const int vdim, const int nbe, const int d,
                              const int q,
                              const bool normals, const int *markers, const fptype *b,
                              const fptype *detj, const fptype *n, const fptype *weights,
                              const Vector &coeff, fptype *y)
{
   ...
   mfem::forall(nbe, [=] MFEM_HOST_DEVICE (int e)
   {
      ...
      for (int c = 0; c < vdim; ++c)
      {
         ...
         {
            fptype coeff_val = 0.0;
            if (normals)
            {
               for (int cd = 0; cd < 2; ++cd)
               {
                  const fptype cval = cst ? C(cd,0,0) : C(cd,qx,e);
         ...
         MFEM_FOREACH_THREAD(x,x,q)
         {
            MFEM_FOREACH_THREAD(y,y,q)
            {
               ...
               if (normals)
               {
                  ...
                  {
                     fptype cval = cst ? C(cd,0,0,0) : C(cd,x,y,e);
                            ...
                            Vector &y)
{
   ...
   const fptype *n = geom->normal.Read();
