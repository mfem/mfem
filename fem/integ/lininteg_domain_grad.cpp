...
void DLFGradAssemble2D(const int vdim, const int ne, const int d, const int q,
                       const int *markers, const fptype *b, const fptype *g,
                       const fptype *jacobians,
                       const fptype *weights, const Vector &coeff, fptype *y)
{
   ...
   mfem::forall_2D(ne, q, q, [=] MFEM_HOST_DEVICE (int e)
   {
      ...
      MFEM_SHARED fptype sBGt[2][Q*D];
      MFEM_SHARED fptype sQQ[2][Q*Q];
      MFEM_SHARED fptype sDQ[2][D*Q];
      ...
      {
         const fptype cst_val0 = C(0,c,0,0,0);
         const fptype cst_val1 = C(1,c,0,0,0);

         MFEM_FOREACH_THREAD(x,x,q)
         {
            MFEM_FOREACH_THREAD(y,y,q)
            {
               const fptype w = W(x,y);
               const fptype J11 = J(x,y,0,0,e);
               const fptype J21 = J(x,y,1,0,e);
               const fptype J12 = J(x,y,0,1,e);
               const fptype J22 = J(x,y,1,1,e);
               const fptype u = cst ? cst_val0 : C(0,c,x,y,e);
               const fptype v = cst ? cst_val1 : C(1,c,x,y,e);
...
void DLFGradAssemble3D(const int vdim, const int ne, const int d, const int q,
                       const int *markers, const fptype *b, const fptype *g,
                       const fptype *jacobians,
                       const fptype *weights, const Vector &coeff,
                       fptype *output)
{
   const auto F = coeff.Read();
   const auto M = Reshape(markers, ne);
   ...
   mfem::forall_2D(ne, q, q, [=] MFEM_HOST_DEVICE (int e)
   {
      ...
      MFEM_SHARED fptype sQQQ[MQD*MQD*MQD];
      ...
      for (int c = 0; c < vdim; ++c)
      {
         const fptype cst_val_0 = C(0,c,0,0,0,0);
         const fptype cst_val_1 = C(1,c,0,0,0,0);
         const fptype cst_val_2 = C(2,c,0,0,0,0);
         ...
         for (int k = 0; k < 3; ++k)
         {
            for (int z = 0; z < q; ++z)
            {
               MFEM_FOREACH_THREAD(y,y,q)
               {
                  MFEM_FOREACH_THREAD(x,x,q)
                  {
                     const fptype J11 = J(x,y,z,0,0,e);
                     const fptype J21 = J(x,y,z,1,0,e);
                     const fptype J31 = J(x,y,z,2,0,e);
                     const fptype J12 = J(x,y,z,0,1,e);
                     const fptype J22 = J(x,y,z,1,1,e);
                     const fptype J32 = J(x,y,z,2,1,e);
                     const fptype J13 = J(x,y,z,0,2,e);
                     const fptype J23 = J(x,y,z,1,2,e);
                     const fptype J33 = J(x,y,z,2,2,e);

                     const fptype u = cst ? cst_val_0 : C(0,c,x,y,z,e);
                     const fptype v = cst ? cst_val_1 : C(1,c,x,y,z,e);
                     const fptype w = cst ? cst_val_2 : C(2,c,x,y,z,e);
                            ...
                            Vector &y)
{
   ...
   const fptype *J = geom->J.Read();
   const fptype *W = ir->GetWeights().Read();
   fptype *Y = y.ReadWrite();
