...
template <>
void PADiffusionSetup2D<2>(const int Q1D, const int coeffDim, const int NE,
                           const Array<real_t> &w, const Vector &j,
                           const Vector &c, Vector &d);

template <>
void PADiffusionSetup2D<3>(const int Q1D, const int coeffDim, const int NE,
                           const Array<real_t> &w, const Vector &j,
                           const Vector &c, Vector &d);
                      ...
                      Vector &D)
{
   if (dim == 1)
   {
      MFEM_ABORT("dim==1 not supported in PADiffusionSetup");
   }
   if (dim == 2)
   {
#ifdef MFEM_USE_OCCA
      ...
      if (sdim == 2)
      {
         PADiffusionSetup2D<2>(Q1D, coeffDim, NE, W, J, C, D);
      }
      if (sdim == 3)
      {
         PADiffusionSetup2D<3>(Q1D, coeffDim, NE, W, J, C, D);
      }
   }
   if (dim == 3)
   {
                           ...
                           const Vector &c, Vector &d)
{
   ...
   MFEM_VERIFY(coeffDim < 3 || !const_c,
      ...
      MFEM_FOREACH_THREAD(qx, x, Q1D)
      {
         MFEM_FOREACH_THREAD(qy, y, Q1D)
         {
            ...
            if (coeffDim == 3 || coeffDim == 4) // Matrix coefficient
            {
               ...
               D(qx, qy, 2, e) =
                  w_detJ * (symmetric ? (-J21 * R12 + J11 * R22)
                           ...
                           const Vector &c, Vector &d)
{
   ...
   const auto C =
      const_c ? Reshape(c.Read(), 1, 1, 1) : Reshape(c.Read(), Q1D, Q1D, NE);
                        ...
                        const Vector &c, Vector &d)
{
   ...
   const bool const_c = c.Size() == 1;
   MFEM_VERIFY(coeffDim < 6 || !const_c,
               "Constant matrix coefficient not supported");
         ...
         MFEM_FOREACH_THREAD(qy, y, Q1D)
         {
            MFEM_FOREACH_THREAD(qz, z, Q1D)
            {
               ...
               if (coeffDim == 6 || coeffDim == 9) // Matrix coefficient version
               {
                  ...
                  const real_t M22 =
                     (!symmetric) ? C(4, qx, qy, qz, e) : C(3, qx, qy, qz, e);
                  const real_t M23 =
                     (!symmetric) ? C(5, qx, qy, qz, e) : C(4, qx, qy, qz, e);
                  const real_t M31 = (!symmetric) ? C(6, qx, qy, qz, e) : M13;
                  const real_t M32 = (!symmetric) ? C(7, qx, qy, qz, e) : M23;
                  const real_t M33 =
                     (!symmetric) ? C(8, qx, qy, qz, e) : C(5, qx, qy, qz, e);
                  ...
                  {
                     D(qx, qy, qz, 3, e) =
                        w_detJ * (A21 * R11 + A22 * R21 + A23 * R31); // 2,1
                     D(qx, qy, qz, 6, e) =
                        w_detJ * (A31 * R11 + A32 * R21 + A33 * R31); // 3,1
                     D(qx, qy, qz, 7, e) =
                        w_detJ * (A31 * R12 + A32 * R22 + A33 * R32); // 3,2
               ...
               else   // Vector or scalar coefficient version
               {
                  ...
                  const real_t C2 = const_c ? C(0, 0, 0, 0, 0)
                                    : (coeffDim == 3 ? C(1, qx, qy, qz, e)
                                       : C(0, qx, qy, qz, e));
                  const real_t C3 = const_c ? C(0, 0, 0, 0, 0)
                                    : (coeffDim == 3 ? C(2, qx, qy, qz, e)
                                       : C(0, qx, qy, qz, e));

                  // detJ J^{-1} J^{-T} = (1/detJ) adj(J) adj(J)^T
                  D(qx, qy, qz, 0, e) = w_detJ * (C1 * A11 * A11 + C2 * A12 * A12 +
                                                  C3 * A13 * A13); // 1,1
                  D(qx, qy, qz, 1, e) = w_detJ * (C1 * A11 * A21 + C2 * A12 * A22 +
                                                  C3 * A13 * A23); // 2,1
                  D(qx, qy, qz, 2, e) = w_detJ * (C1 * A11 * A31 + C2 * A12 * A32 +
                                                  C3 * A13 * A33); // 3,1
                  D(qx, qy, qz, 3, e) = w_detJ * (C1 * A21 * A21 + C2 * A22 * A22 +
                                                  C3 * A23 * A23); // 2,2
                  D(qx, qy, qz, 4, e) = w_detJ * (C1 * A21 * A31 + C2 * A22 * A32 +
                                                  C3 * A23 * A33); // 3,2
                  D(qx, qy, qz, 5, e) = w_detJ * (C1 * A31 * A31 + C2 * A32 * A32 +
                                                  C3 * A33 * A33); // 3,3
               }
            }
         }
                            ...
                            const Vector &C, Vector &op)
{
   ...
   {
      const occa::kernel DiffusionSetup2D = mfem::OccaDev().buildKernel(
                            ...
                            const Vector &C, Vector &op)
{
   ...
   {
      const occa::kernel DiffusionSetup3D = mfem::OccaDev().buildKernel(
                                               "occa://mfem/fem/occa.okl", "DiffusionSetup3D", props);
                      ...
                      Vector &Y)
{
   ...
   if (dim == 2)
   {
      if (MFEM_KERNEL_CHECK && id == 0x33)
      {
         ...
         StaticSmemPADiffusionApply2D(NE, symm, B, G, D, X, stcY, 3, 3);
      ...
      {
         return StaticSmemPADiffusionApply2D(NE, symm, B, G, D, X, Y, D1D, Q1D);
      }
      else
      {
         constexpr int NBZ_TABLE[8] = {16, 16, 8, 8, 4, 4, 2, 2};
         const int NBZ = D1D < 10 ? NBZ_TABLE[D1D - 2] : 1;
         return DynamicSmemPADiffusionApply2D(NE, symm, B, G, D, X, Y, D1D, Q1D,
                                              NBZ);
      }
   }
                            ...
                            const Vector &D, const Vector &X, Vector &Y)
{
   ...
   if (!Device::Allows(Backend::OCCA_CUDA))
   {
      ...
      {
         const occa::kernel DiffusionApply2D_CPU = mfem::OccaDev().buildKernel(
                                                      "occa://mfem/fem/occa.okl", "DiffusionApply2D_CPU", props);
   ...
   else
   {
      ...
      if (OccaDiffApply2D_gpu.find(id) == OccaDiffApply2D_gpu.end())
      {
         const occa::kernel DiffusionApply2D_GPU = mfem::OccaDev().buildKernel(
                                                      "occa://mfem/fem/occa.okl", "DiffusionApply2D_GPU", props);
                            ...
                            const Vector &D, const Vector &X, Vector &Y)
{
   ...
   if (!Device::Allows(Backend::OCCA_CUDA))
   {
      ...
      {
         const occa::kernel DiffusionApply3D_CPU = mfem::OccaDev().buildKernel(
                                                      "occa://mfem/fem/occa.okl", "DiffusionApply3D_CPU", props);
   ...
   else
   {
      ...
      {
         const occa::kernel DiffusionApply3D_GPU = mfem::OccaDev().buildKernel(
                                                      "occa://mfem/fem/occa.okl", "DiffusionApply3D_GPU", props);
