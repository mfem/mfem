      ...
      //std::cout << "% " << __LINE__ << " in " << __FUNCTION__ << " in " << __FILE__ << std::endl;
            for( int i = 0 ; i < dof1d ; i++ )
            {
               ...
               if(type==FaceType::Interior && e2>=0)
               {
                  ...
                  dudn_face(i,p,f_ind,1,0) = gf(i)*coeffs(0);//coeffn;
                  dudn_face(i,p,f_ind,1,1) = gf(i)*coeffs(1);//coefft1;
                  ...
                  if(dim==3)
                  {
                     dudn_face(i,p,f_ind,1,2) = -gf(i)*coeffs(2);//coefft2;
                  }
               }
...
*/
   ...
   if (m==L2FaceValues::DoubleValued)
   {
      ...
      MFEM_FORALL(i, dof1d*dof*vd*nf,
      {
         ...
         const int idxt2s = d_indicestan2self[i];
         ...
         // other side 
         const int idx2 = d_indices2[i];
         const int idxt1o = d_indicestan2other[i];
         const int idxt2o = d_indicestan2other[i];
         if( idx2==-1 )
         {
            for (int c = 0; c < vd; ++c)
            {
