...
public:
                 ...
                 CoeffType *Q)
   {
      Ceed ceed(internal::ceed);
      mfem::Mesh &mesh = *fes.GetMesh();
      InitCoefficientWithIndices(Q, mesh, irm, nelem, indices, coeff, info.ctx);
      bool const_coeff = coeff->IsConstant();
      std::string build_func = const_coeff ? info.build_func_const
                               : info.build_func_quad;
      CeedQFunctionUser build_qf = const_coeff ? info.build_qf_const
      ...
      mesh.EnsureNodes();
      InitBasisAndRestrictionWithIndices(fes, irm, nelem, indices,
                                         ceed, &basis, &restr);

      const mfem::FiniteElementSpace *mesh_fes = mesh.GetNodalFESpace();
      MFEM_VERIFY(mesh_fes, "the Mesh has no nodal FE space");
      InitBasisAndRestrictionWithIndices(*mesh_fes, irm, nelem, indices,
      ...
      if (GridCoefficient *gridCoeff = dynamic_cast<GridCoefficient*>(coeff))
      {
         InitBasisAndRestrictionWithIndices(*gridCoeff->gf.FESpace(), irm,
                                            nelem, indices, ceed,
                                            &gridCoeff->basis,
                                            &gridCoeff->restr);
         CeedOperatorSetField(build_oper, "coeff", gridCoeff->restr,
