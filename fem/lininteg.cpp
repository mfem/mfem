                                                ...
                                                Vector &elvect)
{
   ...
   for (int i = 0; i < ir->GetNPoints(); i++)
   {
      ...
      Tr.SetIntPoint (&ip);
      fptype val = Tr.Weight() * Q.Eval(Tr, ip);
   ...
   const FiniteElement &el, FaceElementTransformations &Tr, Vector &elvect)
{
   ...
   for (int i = 0; i < ir->GetNPoints(); i++)
   {
      ...
      fptype val = Tr.Face->Weight() * ip.weight * Q.Eval(*Tr.Face, ip);
...
{
   int dim, ndof, order;
   fptype un, w, vu_data[3], nor_data[3];
   ...
   const FiniteElement &el, FaceElementTransformations &Tr, Vector &elvect)
{
   ...
   for (int pi = 0; pi < ir->GetNPoints(); ++pi)
   {
      ...
      fptype wL, wM, jcoef;
      {
         const fptype w = ip.weight / Tr.Elem1->Weight();
      ...
      wM *= alpha;
      const fptype t1 = alpha * wL * (u_dir*nor);
      for (int im = 0, i = 0; im < dim; ++im)
      {
         const fptype t2 = wM * u_dir(im);
         const fptype t3 = wM * nor(im);
         const fptype tj = jcoef * u_dir(im);
   ...
   const FiniteElement &fe, ElementTransformation &Tr, Vector &elvect)
{
   ...
   for (int q = 0; q < nqp; q++)
   {
      const IntegrationPoint &ip = ir->IntPoint(q);
      Tr.SetIntPoint(&ip);
      const fptype w = Tr.Weight() * ip.weight;
