...
fptype ElementTransformation::EvalWeight()
{
   ...
   const Vector& pt, const IntegrationRule &ir)
{
   ...
   {
      fptype dist = pt.DistanceTo(physPts.GetColumn(i));
      if (dist < minDist)
      {
         minDist = dist;
         minIndex = i;
...
void InverseElementTransformation::NewtonPrint(int mode, fptype val)
{
...
fptype FaceElementTransformations::CheckConsistency(int print_level,
                                                    std::ostream &os)
{
