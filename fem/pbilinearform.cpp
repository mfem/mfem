...
ParMixedBilinearForm::Assemble(int skip_zeros)
{
   ...
   const Mesh *trial_mesh = trial_pfes->GetParMesh();
   const Mesh *test_mesh = test_pfes->GetParMesh();

   if (trial_mesh == test_mesh)
   {
      MixedBilinearForm::Assemble(skip_zeros);
   }
   else if (ParSubMesh::IsParSubMesh(trial_mesh, test_mesh))
   {
      const auto *trial_submesh = static_cast<const ParSubMesh *>(trial_mesh);
      MFEM_VERIFY(trial_submesh->GetParent() == test_mesh,
                  "ParMixedBilinearForm::Assemble currently supports a trial "
                  "space on a direct ParSubMesh of the test space");
      SubMeshTolerantAssemble(skip_zeros);
   }
   else
   {
      MFEM_ABORT("ParMixedBilinearForm::Assemble requires trial and test "
                 "spaces on the same mesh, or the trial space on a direct "
                 "ParSubMesh of the test space");
   }
...
ParMixedBilinearForm::SubMeshTolerantAssemble(int skip_zeros)
{
   ...
   // In the submesh case we iterate over trial elements, mapping each one to
   // the corresponding parent element in the test space.
   FiniteElementSpace &iterated_fes = *trial_fes;
   Mesh *mesh = iterated_fes.GetMesh();
   const auto *trial_submesh = static_cast<const ParSubMesh *>(trial_pfes->GetParMesh());
   const auto *trial_parent_mesh = trial_submesh->GetParent();
   MFEM_VERIFY(trial_parent_mesh == test_pfes->GetParMesh(),
               "ParMixedBilinearForm::SubMeshTolerantAssemble expects the "
               "trial space to be defined on a direct ParSubMesh of the test "
               "space");
