      ...
      for (int c = 0; c < vdim; c++)
      {
         for (int q = 0; q < q1d; q++)
         {
            ...
            if (GRAD_PHYS)
            {
               ...
               {
                  const fptype Jloc[2] = {j(q,0,e), j(q,1,e)};
                  fptype Jinv[3];
                  kernels::CalcLeftInverse<2,1>(Jloc, Jinv);
                  const fptype U = Jinv[0]*du[0];
                  const fptype V = Jinv[1]*du[0];
                  du[0] = U;
                  du[1] = V;
               }
               else // sdim == 3
               {
                  const fptype Jloc[3] = {j(q,0,e), j(q,1,e), j(q,2,e)};
                  fptype Jinv[3];
                  kernels::CalcLeftInverse<3,1>(Jloc, Jinv);
                  const fptype U = Jinv[0]*du[0];
                  const fptype V = Jinv[1]*du[0];
                  const fptype W = Jinv[2]*du[0];
                  du[0] = U;
                  du[1] = V;
                  du[2] = W;
               }
                          ...
                          const int q1d = 0)
{
   ...
   mfem::forall_2D_batch(NE, Q1D, Q1D, NBZ, [=] MFEM_HOST_DEVICE (int e)
   {
      ...
      DeviceTensor<2> X((fptype*)(XY+tidz), D1D, D1D);

      MFEM_SHARED fptype s_DQ[2][NBZ][MD1*MQ1];
            ...
            MFEM_FOREACH_THREAD(qx,x,Q1D)
            {
               ...
               if (GRAD_PHYS)
               {
                  if (SDIM == 2)
                  {
                     ...
                     const fptype U = Jinv[0]*du[0] + Jinv[1]*du[1];
                     const fptype V = Jinv[2]*du[0] + Jinv[3]*du[1];
                     du[0] = U;
                     du[1] = V;
                  }
                  else
                  {
                     fptype Jloc[6], Jinv[6];
                     Jloc[0] = j(qx,qy,0,0,e);
                     Jloc[1] = j(qx,qy,1,0,e);
                     ...
                     const fptype U = Jinv[0]*du[0] + Jinv[1]*du[1];
                     const fptype V = Jinv[2]*du[0] + Jinv[3]*du[1];
                     const fptype W = Jinv[4]*du[0] + Jinv[5]*du[1];
                     du[0] = U;
                     du[1] = V;
                     du[2] = W;
                  }
            ...
            MFEM_FOREACH_THREAD(qy,y,Q1D)
            {
               MFEM_FOREACH_THREAD(qx,x,Q1D)
               {
                  ...
                  if (GRAD_PHYS)
                  {
                     ...
                     const fptype U = Jinv[0]*u + Jinv[1]*v + Jinv[2]*w;
                     const fptype V = Jinv[3]*u + Jinv[4]*v + Jinv[5]*w;
                     const fptype W = Jinv[6]*u + Jinv[7]*v + Jinv[8]*w;
                     u = U; v = V; w = W;
                  }
                  if (Q_LAYOUT == QVectorLayout::byVDIM)
                  {
