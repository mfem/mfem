...
static void Det1D(const int NE, const real_t *b, const real_t *g,
                  ...
                  Vector *d_buff = nullptr)
{
                ...
                [=] MFEM_HOST_DEVICE(int e)
   {
      ...
      {
         real_t u = 0.0;
         for (int d = 0; d < d1d; d++) { u += G(q, d) * X(d, e); }
         Y(q, e) = u;
      }
   });
}
...
static void Det2D(const int NE, const real_t *b, const real_t *g,
...
template <int T_D1D = 0, int T_Q1D = 0>
static void Det2DSurface(const int NE, const real_t *b, const real_t *g,
                         const real_t *x, real_t *y, const int d1d = 0,
                         const int q1d = 0, Vector *d_buff = nullptr)
{
   MFEM_CONTRACT_VAR(d_buff);
         ...
         MFEM_FOREACH_THREAD(qx, x, Q1D)
         {
            ...
            for (int d = 0; d < SDIM; ++d)
            {
               for (int dy = 0; dy < D1D; ++dy)
               {
                  ...
                  J_[3 + d] +=
                     DQ[3 + d][tidz][dy + qx * D1D] * G_mat(dy, qy);
               }
            }
...
static void Det3D(const int NE, const real_t *b, const real_t *g,
                  const real_t *x, real_t *y, const int d1d = 0,
                  const int q1d = 0,
                  Vector *d_buff = nullptr) // used only with SMEM = false
{
   ...
   mfem::forall_3D_grid(NE, Q1D, Q1D, Q1D, GRID, [=] MFEM_HOST_DEVICE(int e)
   {
      ...
      auto DDD = (real_t(*)[MDQ * MDQ * MDQ])(lm0);
      auto DDQ = (real_t(*)[MDQ * MDQ * MDQ])(lm1);
      auto DQQ = (real_t(*)[MDQ * MDQ * MDQ])(lm0);
      auto QQQ = (real_t(*)[MDQ * MDQ * MDQ])(lm1);

      kernels::internal::LoadX_v<MDQ>(e, D1D, X, DDD);
      kernels::internal::LoadBG<MD1, MQ1>(D1D, Q1D, B, G, BG);
      ...
      MFEM_FOREACH_THREAD(qz, z, Q1D)
      {
         MFEM_FOREACH_THREAD(qy, y, Q1D)
         {
            ...
            {
               real_t J[9];
               kernels::internal::PullGrad<MDQ>(Q1D, qx, qy, qz, QQQ, J);
...
DetKernel QuadratureInterpolator::DetKernels::Fallback(int DIM, int SDIM,
                                                       int D1D, int Q1D)
{
   if (DIM == 1) { return internal::quadrature_interpolator::Det1D; }
   else if (DIM == 2 && SDIM == 2)
