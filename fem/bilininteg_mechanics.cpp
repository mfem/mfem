...
void MechanicsIntegrator::AssemblePA(const FiniteElementSpace &fes)
{
   ...
   if (Device::Allows(Backend::CEED_MASK))
   {
      ...
      ceedDataPtr = ptr;
      CeedPAMechanicsAssemble(fes, *ir, ktan_ptr, *ptr);
   }
   else
#endif
   {
...
// PA Mechanics Apply kernel
void MechanicsIntegrator::AddMultPA(const Vector &x, Vector &y) const
{
#ifdef MFEM_USE_CEED
   if (Device::Allows(Backend::CEED_MASK))
   {
...
#endif
   {
      // PAMechanicsApply(dim, dofs1D, quad1D, ne,
