...
protected:
   ...
   void SaveSTLTri(std::ostream &out, fptype p1[], fptype p2[], fptype p3[]);
...
public:
    ...
    */
   GridFunction(FiniteElementSpace *f, fptype *data)
   ...
   void GetNodalValues(int i, Array<fptype> &nval, int vdim = 1) const;
   ...
   */
   ...
   fptype GetDivergence(ElementTransformation &tr) const;
   ...
   void ImposeBounds(int i, const Vector &weights,
                     fptype min_ = 0.0, fptype max_ = infinity());
...
public:
   ...
   virtual fptype ComputeElementGradError(int ielem, VectorCoefficient *exgrad,
   ...
   MFEM_DEPRECATED
   fptype ComputeDGFaceJumpError(Coefficient *exsol,
                                 Coefficient *ell_coeff,
                                 fptype Nu,
                                 const IntegrationRule *irs[] = NULL) const;
   ...
   virtual fptype ComputeW11Error(Coefficient *exsol, VectorCoefficient *exgrad,
   ...
   virtual void ComputeElementLpErrors(const fptype p, Coefficient &exsol,
                                       Vector &error,
                                       Coefficient *weight = NULL,
                                       const IntegrationRule *irs[] = NULL
                                      ) const;
   ...
   /** Compute the Lp error in each element of the mesh and store the results in
       ...
       for example an L2 GridFunction of order zero using map type VALUE. */
   virtual void ComputeElementLpErrors(const fptype p, VectorCoefficient &exsol,
                                       Vector &error,
                                       Coefficient *weight = NULL,
                                       VectorCoefficient *v_weight = NULL,
                                       const IntegrationRule *irs[] = NULL
   ...
   GridFunction &operator=(fptype value);
   ...
   void MakeTRef(FiniteElementSpace *f, fptype *tv);
...
public:
   JumpScaling(fptype nu_=1.0, JumpScalingType type_=CONSTANT)
      : nu(nu_), type(type_) { }
   fptype Eval(fptype h, int p) const
   {
      fptype val = nu;
...
fptype ZZErrorEstimator(BilinearFormIntegrator &blfi,
                        GridFunction &u,
                        GridFunction &flux,
...
void TensorProductLegendre(int dim,                      // input
                           ...
                           Vector &poly,                 // output
                           fptype angle=0.0,             // input (optional)
 ...
 */
fptype LSZZErrorEstimator(BilinearFormIntegrator &blfi,         // input
                          ...
                          fptype tichonov_coeff = 0.0);         // input (optional)

/// Compute the Lp distance between two grid functions on the given element.
fptype ComputeElementLpDistance(fptype p, int i,
