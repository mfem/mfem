...
static void NDPADiffusionApply3D(const int NE,
                                 ...
                                 double *d_y)
{
   dbg("D1D:%d, Q1D:%d B3:%d",D1D,Q1D,Q1D*Q1D*Q1D);
   ...
   SYCL_KERNEL( // Q.submit
   {
      ...
      SYCL_FORALL_3D(e, 5/*NE*/, Q1D, Q1D, Q1D,
      {
         grp.parallel_for_work_item([&](sycl::h_item<1> itm)
         {
            // global, logical_local, physical_local
            kout << e << ":" << itm.get_global_id() << sycl::endl;
         });
         /*
         ...
         /*MFEM_FOREACH_THREAD(dy,1,D1D)
         {
            MFEM_FOREACH_THREAD(dx,0,D1D)
            {
               MFEM_UNROLL(MD1)
