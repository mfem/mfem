...
MFEM_HOST_DEVICE inline void LoadX(const int e, const int D1D,
                                   const DeviceTensor<3, const fptype> &x,
                                   DeviceMatrix &DD)
{
...
MFEM_HOST_DEVICE inline void LoadX(const int e, const int D1D, const int c,
                                   const DeviceTensor<4, const fptype> &x,
                                   DeviceMatrix &DD)
{
   MFEM_FOREACH_THREAD(dy,y,D1D)
   {
...
MFEM_HOST_DEVICE inline void EvalX(const int D1D, const int Q1D,
                                   const fptype (&sB)[MQ1*MD1],
                                   fptype (&sDD)[NBZ][MD1*MD1],
                                   fptype (&sDQ)[NBZ][MD1*MQ1])
{
   const int tidz = MFEM_THREAD_ID(z);
...
MFEM_HOST_DEVICE inline void EvalX(const int D1D, const int Q1D,
                                   const fptype (&sB)[MQ1*MD1],
                                   const fptype (&sX)[2][NBZ][MD1*MD1],
                                   fptype (&sDQ)[2][NBZ][MD1*MQ1])
{
   const int tidz = MFEM_THREAD_ID(z);
   ConstDeviceMatrix B(sB, D1D, Q1D);
   ...
   MFEM_FOREACH_THREAD(dy,y,D1D)
   {
      MFEM_FOREACH_THREAD(qx,x,Q1D)
      {
         ...
         for (int dx = 0; dx < D1D; ++dx)
         {
            const fptype xx = X0(dx,dy);
            const fptype xy = X1(dx,dy);
            u[0] += B(dx,qx) * xx;
            u[1] += B(dx,qx) * xy;
         }
         DQ0(qx,dy) = u[0];
...
MFEM_HOST_DEVICE inline void EvalY(const int D1D, const int Q1D,
                                   const fptype (&sB)[MQ1*MD1],
                                   const fptype (&sDQ)[2][NBZ][MD1*MQ1],
                                   fptype (&sQQ)[2][NBZ][MQ1*MQ1])
{
   const int tidz = MFEM_THREAD_ID(z);
   ConstDeviceMatrix B(sB, D1D, Q1D);
...
MFEM_HOST_DEVICE inline void PullEval(const int Q1D,
                                      const int qx, const int qy,
                                      const fptype (&sQQ)[2][NBZ][MQ1*MQ1],
                                      fptype (&P)[2])
{
   const int tidz = MFEM_THREAD_ID(z);
...
MFEM_HOST_DEVICE inline void GradX(const int D1D, const int Q1D,
                                   const fptype (&sBG)[2][MQ1*MD1],
                                   const fptype (&sX)[2][NBZ][MD1*MD1],
                                   fptype (&sDQ)[4][NBZ][MD1*MQ1])
{
   const int tidz = MFEM_THREAD_ID(z);
   ...
   MFEM_FOREACH_THREAD(dy,y,D1D)
   {
      MFEM_FOREACH_THREAD(qx,x,Q1D)
      {
         ...
         for (int dx = 0; dx < D1D; ++dx)
         {
            const fptype Bx = B(dx,qx);
            const fptype Gx = G(dx,qx);
            const fptype x0 = X0(dx,dy);
            const fptype x1 = X1(dx,dy);
            u[0] += Bx * x0;
            v[0] += Gx * x0;
            u[1] += Bx * x1;
            v[1] += Gx * x1;
...
MFEM_HOST_DEVICE inline void GradY(const int D1D, const int Q1D,
                                   const fptype (&sBG)[2][MQ1*MD1],
                                   const fptype (&sDQ)[4][NBZ][MD1*MQ1],
                                   fptype (&sQQ)[4][NBZ][MQ1*MQ1])
{
   const int tidz = MFEM_THREAD_ID(z);
   ConstDeviceMatrix B(sBG[0], D1D, Q1D);
   ConstDeviceMatrix G(sBG[1], D1D, Q1D);
   ...
   MFEM_FOREACH_THREAD(qy,y,Q1D)
   {
      MFEM_FOREACH_THREAD(qx,x,Q1D)
      {
         ...
         for (int dy = 0; dy < D1D; ++dy)
         {
            const fptype By = B(dy,qy);
            const fptype Gy = G(dy,qy);
            u[0] += X0G(qx,dy) * By;
            v[0] += X0B(qx,dy) * Gy;
            u[1] += X1G(qx,dy) * By;
            v[1] += X1B(qx,dy) * Gy;
...
MFEM_HOST_DEVICE inline void PullGrad(const int Q1D,
                                      const int qx, const int qy,
                                      const fptype (&sQQ)[4][NBZ][MQ1*MQ1],
                                      fptype *Jpr)
{
   const int tidz = MFEM_THREAD_ID(z);
...
MFEM_HOST_DEVICE inline void GradYt(const int D1D, const int Q1D,
                                    const fptype (&sBG)[2][MQ1*MD1],
                                    const fptype (&GQ)[4][NBZ][MQ1*MQ1],
                                    fptype (&GD)[4][NBZ][MD1*MQ1])
{
   const int tidz = MFEM_THREAD_ID(z);
...
MFEM_HOST_DEVICE inline void LoadX(const int e, const int D1D, const int c,
                                   const DeviceTensor<5, const fptype> &x,
                                   DeviceTensor<3> &X)
{
   MFEM_FOREACH_THREAD(dz,z,D1D)
   {
...
MFEM_HOST_DEVICE inline void EvalX(const int D1D, const int Q1D,
                                   const fptype (&sB)[MQ1*MD1],
                                   const fptype (&sDDD)[MD1*MD1*MD1],
                                   fptype (&sDDQ)[MD1*MD1*MQ1])
{
   ConstDeviceMatrix B(sB, D1D, Q1D);
...
MFEM_HOST_DEVICE inline void EvalY(const int D1D, const int Q1D,
                                   const fptype (&sB)[MQ1*MD1],
                                   const fptype (&sDDQ)[MD1*MD1*MQ1],
                                   fptype (&sDQQ)[MD1*MQ1*MQ1])
{
   ConstDeviceMatrix B(sB, D1D, Q1D);
...
MFEM_HOST_DEVICE inline void EvalZ(const int D1D, const int Q1D,
                                   const fptype (&sB)[MQ1*MD1],
                                   const fptype (&sDQQ)[MD1*MQ1*MQ1],
                                   fptype (&sQQQ)[MQ1*MQ1*MQ1])
{
   ConstDeviceMatrix B(sB, D1D, Q1D);
...
MFEM_HOST_DEVICE inline void EvalX(const int D1D, const int Q1D,
                                   const fptype (&sB)[MQ1*MD1],
                                   const fptype (&sDDD)[3][MD1*MD1*MD1],
                                   fptype (&sDDQ)[3][MD1*MD1*MQ1])
{
   ConstDeviceMatrix B(sB, D1D, Q1D);
...
MFEM_HOST_DEVICE inline void EvalY(const int D1D, const int Q1D,
                                   const fptype (&sB)[MQ1*MD1],
                                   const fptype (&sDDQ)[3][MD1*MD1*MQ1],
                                   fptype (&sDQQ)[3][MD1*MQ1*MQ1])
{
   ConstDeviceMatrix B(sB, D1D, Q1D);
...
MFEM_HOST_DEVICE inline void EvalZ(const int D1D, const int Q1D,
                                   const fptype (&sB)[MQ1*MD1],
                                   const fptype (&sDQQ)[3][MD1*MQ1*MQ1],
                                   fptype (&sQQQ)[3][MQ1*MQ1*MQ1])
{
   ConstDeviceMatrix B(sB, D1D, Q1D);
...
MFEM_HOST_DEVICE inline void PullEval(const int Q1D,
                                      const int x, const int y, const int z,
                                      const fptype (&sQQQ)[3][MQ1*MQ1*MQ1],
                                      fptype (&X)[3])
{
                                   ...
                                   fptype (*sDDQ)[MD1*MD1*MQ1])
{
   ...
   MFEM_FOREACH_THREAD(dz,z,D1D)
   {
      MFEM_FOREACH_THREAD(dy,y,D1D)
      {
         ...
         {
            fptype u[3] = {0.0, 0.0, 0.0};
            fptype v[3] = {0.0, 0.0, 0.0};
            for (int dx = 0; dx < D1D; ++dx)
            {
               const fptype xx = Xx(dx,dy,dz);
               const fptype xy = Xy(dx,dy,dz);
               const fptype xz = Xz(dx,dy,dz);
               const fptype Bx = B(dx,qx);
               const fptype Gx = G(dx,qx);
               u[0] += Bx * xx;
               u[1] += Bx * xy;
...
MFEM_HOST_DEVICE inline void GradZt(const int D1D, const int Q1D,
                                    const fptype (&sBG)[2][MQ1*MD1],
                                    const fptype (&sQQQ)[9][MQ1*MQ1*MQ1],
                                    fptype (&sDQQ)[9][MD1*MQ1*MQ1])
{
...
MFEM_HOST_DEVICE inline void GradYt(const int D1D, const int Q1D,
                                    const fptype (&sBG)[2][MQ1*MD1],
                                    const fptype (&sDQQ)[9][MD1*MQ1*MQ1],
                                    fptype (&sDDQ)[9][MD1*MD1*MQ1])
{
   ConstDeviceMatrix Bt(sBG[0], Q1D, D1D);
   ConstDeviceMatrix Gt(sBG[1], Q1D, D1D);
   ConstDeviceCube XxBB(sDQQ[0], Q1D, Q1D, D1D);
...
MFEM_HOST_DEVICE inline void GradXt(const int D1D, const int Q1D,
                                    const fptype (&sBG)[2][MQ1*MD1],
                                    const fptype (&sDDQ)[9][MD1*MD1*MQ1],
                                    const DeviceTensor<5> &Y, // output
                                    const int e)
{
   ConstDeviceMatrix Bt(sBG[0], Q1D, D1D);
