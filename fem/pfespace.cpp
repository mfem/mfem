...
void ParFiniteElementSpace::Build_Dof_TrueDof_Matrix() const // matrix P
{
   ...
   HYPRE_Int *i_diag = mm::New<HYPRE_Int>(ldof+1);
   HYPRE_Int *j_diag = mm::New<HYPRE_Int>(ltdof);
   int diag_counter;

   HYPRE_Int *i_offd = mm::New<HYPRE_Int>(ldof+1);
   HYPRE_Int *j_offd = mm::New<HYPRE_Int>(ldof-ltdof);
   int offd_counter;

   HYPRE_Int *cmap   = mm::New<HYPRE_Int>(ldof-ltdof);
                      ...
                      Array<HYPRE_Int> &col_starts) const
{
   ...
   HYPRE_Int *cmap = mm::New<HYPRE_Int>(col_map.size());
   int offd_col = 0;
   ...
   HYPRE_Int *I_diag = mm::New<HYPRE_Int>(vdim*local_rows + 1);
   HYPRE_Int *I_offd = mm::New<HYPRE_Int>(vdim*local_rows + 1);

   HYPRE_Int *J_diag = mm::New<HYPRE_Int>(nnz_diag);
   HYPRE_Int *J_offd = mm::New<HYPRE_Int>(nnz_offd);

   double *A_diag = mm::New<double>(nnz_diag);
   double *A_offd = mm::New<double>(nnz_offd);
...
{
   HYPRE_Int *I = mm::New<HYPRE_Int>(nrows+1);
...
static HYPRE_Int* make_j_array(HYPRE_Int* I, int nrows)
{
   ...
   HYPRE_Int *J = mm::New<HYPRE_Int>(nnz);
                                       ...
                                       const Table* old_elem_dof)
{
   ...
   HYPRE_Int* cmap = mm::New<HYPRE_Int>(offd_cols);
   for (int i = 0; i < offd_cols; i++)
   {
                                                  ...
                                                  const Table* old_elem_dof)
{
   ...
   HYPRE_Int *cmap = mm::New<HYPRE_Int>(offd->Width());
   for (std::map<HYPRE_Int, int>::iterator
        it = col_map.begin(); it != col_map.end(); ++it)
   {
