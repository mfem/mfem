...
public:
   ...
   LinearForm(FiniteElementSpace *f, fptype *data) : Vector(data, f->GetVSize())
   { fes = f; }
   ...
   /** Linear forms are linear functionals which map GridFunctions to
       ...
       and GridFunction. */
   fptype operator()(const GridFunction &gf) const { return (*this)*gf; }

   /// Redefine '=' for LinearForm = constant.
   LinearForm &operator=(fptype value);
