...
{
   const fptype *op = poly1d.OpenPoints(p, btype);
...
{
   const int p = order;
   const fptype *op = poly1d.OpenPoints(p, b_type);
                                         ...
                                         DenseMatrix &div) const
{
   if (basis1d.IsIntegratedType())
   {
      ...
      const fptype *gll_pts = poly1d.GetPoints(order+1, BasisType::GaussLobatto);

      // Loop over subcells
      for (int iy = 0; iy < order+1; ++iy)
      {
         const fptype hy = gll_pts[iy+1] - gll_pts[iy];
         for (int ix = 0; ix < order+1; ++ix)
         {
            const int i = ix + iy*(order+1);
            const fptype hx = gll_pts[ix+1] - gll_pts[ix];
            ...
            for (int iq = 0; iq < ir.Size(); ++iq)
            {
               ...
               if (map_type == VALUE)
               {
                  const fptype detJ = Trans.Weight();
                  w /= detJ;
               }
               ...
               {
                  const fptype div_j = div_shape(j);
                  div(i,j) += w*div_j;
               }
            }
         }
      ...
      for (int iy = 0; iy < order+1; ++iy)
      {
         ...
         for (int ix = 0; ix < order+1; ++ix)
         {
            ...
            for (int iq = 0; iq < ir.Size(); ++iq)
            {
               ...
               ip.y = gll_pts[iy] + hy*ip.y;
               Trans.SetIntPoint(&ip);
               const fptype val = coeff.Eval(Trans, ip);
               fptype w = ip.weight;
                                      ...
                                      DenseMatrix &div) const
{
   if (basis1d.IsIntegratedType())
   {
      ...
      {
         const fptype hz = gll_pts[iz+1] - gll_pts[iz];
         for (int iy = 0; iy < order+1; ++iy)
         {
            const fptype hy = gll_pts[iy+1] - gll_pts[iy];
            for (int ix = 0; ix < order+1; ++ix)
...
L2_TriangleElement::L2_TriangleElement(const int p, const int btype)
   : NodalFiniteElement(2, Geometry::TRIANGLE, ((p + 1)*(p + 2))/2, p,
                        FunctionSpace::Pk)
{
   const fptype *op = poly1d.OpenPoints(p, VerifyOpen(btype));
                        ...
                        p, FunctionSpace::Pk)
{
   ...
   for (int o = 0, k = 0; k <= p; k++)
      for (int j = 0; j + k <= p; j++)
         ...
         {
            fptype w = op[i] + op[j] + op[k] + op[p-i-j-k];
