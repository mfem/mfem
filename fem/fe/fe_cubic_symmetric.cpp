...
void ApplyDofs(ElementTransformation &T, int cell_modes, int n, int order,
               ...
               DenseMatrix &I)
{
   ...
   MFEM_VERIFY(T.GetSpaceDim() == 2 && T.Hessian().FNorm2() < 1e-20,
               "Cubic symmetric stress elements require affine 2D transformations");
   I.SetSize(21+3*cell_modes,n);
   I = 0.0;
   ...
   const IntegrationRule &rule = IntRules.Get(Geometry::TRIANGLE,
                                              order+(cell_modes > 1));
   for (int q = 0; q < rule.GetNPoints(); q++)
   {
      ...
      for (int c = 0; c < 3; c++)
      {
         for (int j = 0; j < n; j++)
         {
            ...
            for (int mode = 0; mode < cell_modes; mode++)
            {
               I(21+3*mode+c,j) += ip.weight*det2*Monomial(mode,ip)*value;
            }
         }
      }
   }
...
CubicSymmetricTriangleFiniteElement::CubicSymmetricTriangleFiniteElement(
   bool linear_divergence)
   : FiniteElement(2, Geometry::TRIANGLE, linear_divergence ? 24 : 30,
                   3, FunctionSpace::Pk),
     cell_modes(linear_divergence ? 1 : 3), basis(dof,raw_dof)
{
   range_type = MATRIX;
   ...
   ApplyDofs(T,cell_modes,raw_dof,3,RawShape,moments);
   M = 0.0;
   for (int r = 0; r < raw_dof; r++)
   {
      for (int j = 0; j < dof; j++) { M(r,j) = moments(j,r); }
      if (!linear_divergence) { continue; }
   ...
   for (int j = 21; j < dof; j++) { Nodes.IntPoint(j).Set2(1.0/3,1.0/3); }
}

void CubicSymmetricTriangleFiniteElement::CalcMShape(
...
void CubicSymmetricTriangleFiniteElement::CalcDivShape(
...
void CubicSymmetricTriangleFiniteElement::GetBasisTransform(
   const DenseMatrix &J, DenseMatrix &A) const
{
   A.SetSize(dof);
   A = 0.0;
   for (int i = 0; i < dof; i++) { A(i,i) = 1.0; }
   ...
   ElementTransformation &Trans, DenseTensor &shape) const
{
   MFEM_VERIFY(Trans.GetSpaceDim() == 2 && Trans.Hessian().FNorm2() < 1e-20,
               "Cubic symmetric stress elements require affine 2D transformations");
   MFEM_ASSERT(Trans.GetSpaceDim() == 2, "Cubic symmetric stress is a 2D element");
   MFEM_ASSERT(shape.SizeI() == 2 && shape.SizeJ() == 2 &&
               shape.SizeK() == dof, "invalid matrix shape size");
   DenseTensor ref_shape(2, 2, dof);
   CalcMShape(Trans.GetIntPoint(), ref_shape);
   const DenseMatrix &J = Trans.Jacobian();
   const real_t det = J(0,0)*J(1,1) - J(0,1)*J(1,0);
   const real_t scale = 1.0/(det*det);
   DenseTensor mapped(2, 2, dof);
   for (int k = 0; k < dof; k++)
   {
      for (int i = 0; i < 2; i++)
      {
...
void CubicSymmetricTriangleFiniteElement::CalcPhysDivShape(
   ElementTransformation &Trans, DenseMatrix &divshape) const
{
   MFEM_VERIFY(Trans.GetSpaceDim() == 2 && Trans.Hessian().FNorm2() < 1e-20,
               "Cubic symmetric stress elements require affine 2D transformations");
   MFEM_ASSERT(divshape.Height() == dof && divshape.Width() == 2,
               "invalid matrix divergence shape size");
   DenseMatrix ref_div(dof, 2), mapped(dof, 2), A;
...
void CubicSymmetricTriangleFiniteElement::Project(
   const FiniteElement &fe, ElementTransformation &Trans, DenseMatrix &I) const
{
   MFEM_VERIFY(fe.GetDim() == 2 && fe.GetGeomType() == Geometry::TRIANGLE &&
               ...
               "Cubic symmetric stress projection requires a scalar H1 triangle "
               "with three byVDIM components");
   const int n = fe.GetDof();
   Vector values(n);
   ApplyDofs(Trans,cell_modes,3*n,fe.GetOrder(),
...
void CubicSymmetricTriangleFiniteElement::GetTransferMatrix(
   const FiniteElement &fe, ElementTransformation &Trans, DenseMatrix &I) const
{
   MFEM_VERIFY(dynamic_cast<const CubicSymmetricTriangleFiniteElement *>(&fe) &&
               fe.GetDof() == dof,
               "Cubic stress transfer requires matching source and target spaces");
   Vector point(2);
   ApplyDofs(Trans,cell_modes,fe.GetDof(),fe.GetOrder(),
             [&](const IntegrationPoint &ip, DenseTensor &shape)
   {
...
void CubicSymmetricTriangleFiniteElement::GetFaceDofs(
...
void CubicSymmetricTriangleFiniteElement::GetPhysicalTransferMatrix(
   ...
   ElementTransformation &fine, DenseMatrix &I) const
{
   ...
   DenseMatrix Ac, Af, Ar, inverse_Af(dof);
   ...
   DenseMatrix right(dof), middle(dof);
