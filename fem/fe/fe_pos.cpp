                                         ...
                                         Vector &shape) const
{
   ...
   fptype l1x, l2x, l3x, l1y, l2y, l3y;
                                          ...
                                          DenseMatrix &dshape) const
{
   fptype x = ip.x, y = ip.y;
   fptype l1x, l2x, l3x, l1y, l2y, l3y;
   fptype d1x, d2x, d3x, d1y, d2y, d3y;
   ...
   : PositiveTensorFiniteElement(1, p, H1_DOF_MAP)
{
   ...
   for (int i = 1; i < p; i++)
   {
      Nodes.IntPoint(i+1).x = fptype(i)/p;
   }
}
   ...
   : PositiveTensorFiniteElement(2, p, H1_DOF_MAP)
{
   ...
   for (int j = 0; j <= p; j++)
      for (int i = 0; i <= p; i++)
      {
         Nodes.IntPoint(dof_map[o++]).Set2(fptype(i)/p, fptype(j)/p);
      }
}
   ...
   : PositiveTensorFiniteElement(3, p, H1_DOF_MAP)
{
   ...
   for (int k = 0; k <= p; k++)
      for (int j = 0; j <= p; j++)
         for (int i = 0; i <= p; i++)
            Nodes.IntPoint(dof_map[o++]).Set3(fptype(i)/p, fptype(j)/p,
                                              fptype(k)/p);
}
                           ...
                           FunctionSpace::Pk)
{
   ...
   {
      dof_map[idx(i,0)] = o;
      Nodes.IntPoint(o++).Set2(fptype(i)/p, 0.);
   }
   for (int i = 1; i < p; i++)
   {
      dof_map[idx(p-i,i)] = o;
      Nodes.IntPoint(o++).Set2(fptype(p-i)/p, fptype(i)/p);
   }
   for (int i = 1; i < p; i++)
   {
      dof_map[idx(0,p-i)] = o;
      Nodes.IntPoint(o++).Set2(0., fptype(p-i)/p);
   }
   ...
   for (int j = 1; j < p; j++)
      ...
      {
         dof_map[idx(i,j)] = o;
         Nodes.IntPoint(o++).Set2(fptype(i)/p, fptype(j)/p);
      }
}

// static method
void H1Pos_TriangleElement::CalcShape(
   const int p, const fptype l1, const fptype l2, fptype *shape)
{
   const fptype l3 = 1. - l1 - l2;
...
void H1Pos_TriangleElement::CalcDShape(
   const int p, const fptype l1, const fptype l2,
                           ...
                           ((p + 1)*(p + 2)*(p + 3))/6, p, FunctionSpace::Pk)
{
   ...
   {
      dof_map[idx(i,0,0)] = o;
      Nodes.IntPoint(o++).Set3(fptype(i)/p, 0., 0.);
   }
   for (int i = 1; i < p; i++)  // (0,2)
   {
      dof_map[idx(0,i,0)] = o;
      Nodes.IntPoint(o++).Set3(0., fptype(i)/p, 0.);
   }
   for (int i = 1; i < p; i++)  // (0,3)
   {
      dof_map[idx(0,0,i)] = o;
      Nodes.IntPoint(o++).Set3(0., 0., fptype(i)/p);
   }
   for (int i = 1; i < p; i++)  // (1,2)
   {
      dof_map[idx(p-i,i,0)] = o;
      Nodes.IntPoint(o++).Set3(fptype(p-i)/p, fptype(i)/p, 0.);
   }
   for (int i = 1; i < p; i++)  // (1,3)
   {
      dof_map[idx(p-i,0,i)] = o;
      Nodes.IntPoint(o++).Set3(fptype(p-i)/p, 0., fptype(i)/p);
   }
   for (int i = 1; i < p; i++)  // (2,3)
   {
      dof_map[idx(0,p-i,i)] = o;
      Nodes.IntPoint(o++).Set3(0., fptype(p-i)/p, fptype(i)/p);
   }
   ...
   for (int j = 1; j < p; j++)
      for (int i = 1; i + j < p; i++)  // (1,2,3)
      {
         ...
         Nodes.IntPoint(o++).Set3(fptype(p-i-j)/p, fptype(i)/p, fptype(j)/p);
      }
   for (int j = 1; j < p; j++)
      for (int i = 1; i + j < p; i++)  // (0,3,2)
      {
         dof_map[idx(0,j,i)] = o;
         Nodes.IntPoint(o++).Set3(0., fptype(j)/p, fptype(i)/p);
      }
   for (int j = 1; j < p; j++)
      for (int i = 1; i + j < p; i++)  // (0,1,3)
      {
         dof_map[idx(i,0,j)] = o;
         Nodes.IntPoint(o++).Set3(fptype(i)/p, 0., fptype(j)/p);
      }
   for (int j = 1; j < p; j++)
      for (int i = 1; i + j < p; i++)  // (0,2,1)
      {
         dof_map[idx(j,i,0)] = o;
         Nodes.IntPoint(o++).Set3(fptype(j)/p, fptype(i)/p, 0.);
      }
   ...
   for (int k = 1; k < p; k++)
      for (int j = 1; j + k < p; j++)
         ...
         {
            dof_map[idx(i,j,k)] = o;
            Nodes.IntPoint(o++).Set3(fptype(i)/p, fptype(j)/p, fptype(k)/p);
         }
}

// static method
void H1Pos_TetrahedronElement::CalcShape(
   const int p, const fptype l1, const fptype l2, const fptype l3,
   fptype *shape)
{
   const fptype l4 = 1. - l1 - l2 - l3;
   ...
   for (int o = 0, k = 0; k <= p; k++)
   {
      ...
      const fptype ek = bp[k]*l3k;
      fptype l2j = 1.;
      for (int j = 0; j <= p - k; j++)
      {
         Poly_1D::CalcBinomTerms(p - k - j, l1, l4, &shape[o]);
         fptype ekj = ek*bpk[j]*l2j;
   ...
   fptype *dshape_1d, fptype *dshape)
{
   ...
   for (int j = 0; j <= p; j++)
   {
      ...
      const fptype ej = bp[j]*l2j;
      fptype l1i = 1.;
      for (int i = 0; i <= p - j; i++)
      {
         Poly_1D::CalcDBinomTerms(p - j - i, l3, l4, dshape_1d);
         fptype eji = ej*bpj[i]*l1i;
         int m = ((p + 2)*(p + 1))/2;
         int n = ((p - j + 2)*(p - j + 1))/2;
