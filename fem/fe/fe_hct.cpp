...
void HCTTriangleFiniteElement::GetTransferMatrix(const FiniteElement &fe,
                                                 ElementTransformation &T, DenseMatrix &I) const
{
   MFEM_VERIFY(dynamic_cast<const HCTTriangleFiniteElement *>(&fe),
               "HCT transfer requires a HCT source element");
   T.SetIntPoint(&Geometries.GetCenter(Geometry::TRIANGLE));
   MFEM_VERIFY(T.GetSpaceDim() == 2 && T.Hessian().FNorm2() < 1e-20,
               "HCT transfer requires affine two-dimensional "
               "transformations");
   IsoparametricTransformation coarse;
   coarse.SetIdentityTransformation(Geometry::TRIANGLE);
   InterpolateC1Dofs(fe, 3, true, T, coarse, I);
}

void HCTTriangleFiniteElement::GetPhysicalTransferMatrix(
   ...
   ElementTransformation &fine, DenseMatrix &I) const
{
   ...
   InterpolateC1Dofs(*this, 3, true, child, coarse, I);
}

void ReducedHCTTriangleFiniteElement::GetTransferMatrix(const FiniteElement &fe,
                                                        ElementTransformation &T, DenseMatrix &I) const
{
   MFEM_VERIFY(dynamic_cast<const ReducedHCTTriangleFiniteElement *>(&fe),
               "Reduced HCT transfer requires a Reduced HCT source element");
   T.SetIntPoint(&Geometries.GetCenter(Geometry::TRIANGLE));
   MFEM_VERIFY(T.GetSpaceDim() == 2 && T.Hessian().FNorm2() < 1e-20,
               "Reduced HCT transfer requires affine two-dimensional "
               "transformations");
   IsoparametricTransformation coarse;
   coarse.SetIdentityTransformation(Geometry::TRIANGLE);
   InterpolateC1Dofs(fe, 3, false, T, coarse, I);
}

void ReducedHCTTriangleFiniteElement::GetPhysicalTransferMatrix(
   const DenseMatrix &, ElementTransformation &child,
   ElementTransformation &fine, DenseMatrix &I) const
{
   // The reference matrix cannot carry the physical DOF bases; interpolate the
   ...
   IsoparametricTransformation coarse;
   SetRelativeCoarseTransformation(child, fine, coarse);
   InterpolateC1Dofs(*this, 3, false, child, coarse, I);
}
