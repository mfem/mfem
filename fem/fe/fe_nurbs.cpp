                                        ...
                                        DenseMatrix &hessian) const
{
   ...
   fptype sum = 0.0, dsum = 0.0, d2sum = 0.0;
   for (int i = 0; i <= order; i++)
   {
      sum   += (shape_x(i) *= weights(i));
      dsum  += (   grad(i) *= weights(i));
                                      ...
                                      DenseMatrix &dshape) const
{
   ...
   for (int o = 0, j = 0; j <= orders[1]; j++)
   {
      const fptype sy = shape_y(j), dsy = dshape_y(j);
      for (int i = 0; i <= orders[0]; i++, o++)
      {
                                        ...
                                        DenseMatrix &hessian) const
{
   ...
   for (int o = 0, j = 0; j <= orders[1]; j++)
   {
      const fptype sy = shape_y(j), dsy = dshape_y(j), d2sy = d2shape_y(j);
      for (int i = 0; i <= orders[0]; i++, o++)
      {
         const fptype sx = shape_x(i), dsx = dshape_x(i), d2sx = d2shape_x(i);
                                     ...
                                     Vector &shape) const
{
   ...
   {
      const fptype sz = shape_z(k);
      for (int j = 0; j <= orders[1]; j++)
      {
         const fptype sy_sz = shape_y(j)*sz;
         for (int i = 0; i <= orders[0]; i++, o++)
         {
                                      ...
                                      DenseMatrix &dshape) const
{
   ...
   for (int o = 0, k = 0; k <= orders[2]; k++)
   {
      const fptype sz = shape_z(k), dsz = dshape_z(k);
      for (int j = 0; j <= orders[1]; j++)
      {
         const fptype  sy_sz  =  shape_y(j)* sz;
         const fptype dsy_sz  = dshape_y(j)* sz;
         const fptype  sy_dsz =  shape_y(j)*dsz;
         for (int i = 0; i <= orders[0]; i++, o++)
         {
                                        ...
                                        DenseMatrix &hessian) const
{
   ...
   for (int o = 0, k = 0; k <= orders[2]; k++)
   {
      const fptype sz = shape_z(k), dsz = dshape_z(k), d2sz = d2shape_z(k);
      for (int j = 0; j <= orders[1]; j++)
      {
         const fptype sy = shape_y(j), dsy = dshape_y(j), d2sy = d2shape_y(j);
         for (int i = 0; i <= orders[0]; i++, o++)
