...
H1Bubble_TriangleElement::H1Bubble_TriangleElement(int p, int q, int btype)
   : NodalFiniteElement(2, Geometry::TRIANGLE, 3*p + (q-2)*(q-1)/2, p,
                        FunctionSpace::Pk),
     bubble_order(q)
{
   MFEM_VERIFY(bubble_order >= 3, "Bubble order must be at least 3");

   const real_t *cp = poly1d.ClosedPoints(p, VerifyNodal(VerifyClosed(btype)));
   const real_t *cp2 = poly1d.ClosedPoints(q, VerifyNodal(VerifyClosed(btype)));
   ...
   for (int j = 1; j < q; j++)
   {
      for (int i = 1; i + j < q; i++)
      {
         const real_t w = cp2[i] + cp2[j] + cp2[q-i-j];
   ...
   const int npq = (p + 1)*(p + 2)/2 + (q-2)*(q-1)/2;
   // DenseMatrix T(npq, dof);
   ...
   for (int k = 0; k < dof; ++k)
   {
      ...
      poly1d.CalcBasis(q - 3, ip.x, shape_x);
      poly1d.CalcBasis(q - 3, ip.y, shape_y);
      poly1d.CalcBasis(q - 3, 1. - ip.x - ip.y, shape_l);
      const real_t b_T = ip.x * ip.y * (1 - ip.x - ip.y);
      for (int j = 0; j <= q-3; j++)
      {
         for (int i = 0; i + j <= q-3; i++)
         {
            Tt(k, o++) = b_T*shape_x[i]*shape_y[j]*shape_l[q-3-i-j];
         }
      }
   }
                                         ...
                                         Vector &shape) const
{
...
#ifdef MFEM_THREAD_SAFE
   const int n1d = std::max(p + 1, q - 2);
   Vector shape_x(n1d), shape_y(n1d), shape_l(n1d), u(dof);
#endif
   ...
   for (int j = 0; j <= q-3; j++)
   {
      for (int i = 0; i + j <= q-3; i++)
      {
         u(o++) = b_T*shape_x[i]*shape_y[j]*shape_l[q-3-i-j];
      }
   }
                                          ...
                                          DenseMatrix &dshape) const
{
   ...
   poly1d.CalcBasis(q - 3, ip.x, shape_x, dshape_x);
   poly1d.CalcBasis(q - 3, ip.y, shape_y, dshape_y);
   poly1d.CalcBasis(q - 3, lambda, shape_l, dshape_l);
