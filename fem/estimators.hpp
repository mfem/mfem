...
public:
   /** @brief Construct a new ComplexLpErrorEstimator object for a scalar field.
       ...
       @param rel  Compute relative as opposed to absolute error.
       Note: the coefficient must be set before use with the SetCoef method.
   */
   ComplexLpErrorEstimator(int p, ComplexGridFunction &sol, bool rel = false)
      : current_sequence(-1), local_norm_p(p), rel_error(rel),
        error_estimates(0),
   ...
   */
   ComplexLpErrorEstimator(int p,
                           ...
                           ComplexGridFunction &sol, bool rel = false)
      : current_sequence(-1), local_norm_p(p), rel_error(rel),
        error_estimates(0),
   ...
   */
   ...
   /** @brief Construct a new ComplexLpErrorEstimator object for a scalar field.
       ...
       Note: the coefficient must be set before use with the SetCoef method.
   */
   ComplexLpErrorEstimator(int p, ParComplexGridFunction &psol, bool rel = false)
      : current_sequence(-1), local_norm_p(p), rel_error(rel),
        error_estimates(0),
        real_coef(NULL), imag_coef(NULL),
   ...
   */
   ComplexLpErrorEstimator(int p,
                           Coefficient &real_coef, Coefficient &imag_coef,
                           ParComplexGridFunction &psol, bool rel = false)
      : current_sequence(-1), local_norm_p(p), rel_error(rel),
        error_estimates(0),
        real_coef(&real_coef), imag_coef(&imag_coef),
        real_vcoef(NULL), imag_vcoef(NULL),
