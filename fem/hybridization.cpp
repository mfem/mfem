...
#undef MFEM_DEBUG_HERE
   ...
   Af_data = new fptype[Af_offsets[NE]];
   ...
   for (int tdof = 0; tdof < R->Height(); tdof++)
   {
      ...
      const fptype *vals = R->GetRowEntries(tdof);
...
void Hybridization::ComputeH()
{
   ...
   for (int el = 0; el < NE; el++)
   {
      ...
      fptype *A_ib_data = LU_ii.data + i_dofs_size*i_dofs_size;
      fptype *A_bi_data = A_ib_data + i_dofs_size*b_dofs.Size();
      ...
      for (int i = 0; i < b_dofs.Size(); i++)
      {
         ...
         const int ncols = Ct->RowSize(row);
         const int *cols = Ct->GetRowColumns(row);
         const fptype *vals = Ct->GetRowEntries(row);
         for (int j = 0; j < ncols; j++)
         {
                              ...
                              int mode) const
{
   ...
   for (int i = 0; i < NE; i++)
   {
      ...
      fptype *U_ib = LU_ii.data + i_dofs.Size()*i_dofs.Size();
      fptype *L_bi = U_ib + i_dofs.Size()*b_dofs.Size();
