...
// PA Advection Integrator

// OCCA 2D Assemble kernel
#ifdef MFEM_USE_OCCA
static void OccaPAAdvectionSetup2D(const int D1D,
                                   const int Q1D,
                                   const int NE,
                                   ...
                                   Vector &op)
{
   ...
   {
      const occa::kernel AdvectionSetup2D =
         mfem::OccaDev().buildKernel("occa://mfem/fem/occa.okl",
                                     "AdvectionSetup2D", props);
      OccaDiffSetup2D_ker.emplace(id, AdvectionSetup2D);
   }
   OccaDiffSetup2D_ker.at(id)(NE, o_W, o_J, COEFF, o_op);
}

static void OccaPAAdvectionSetup3D(const int D1D,
                                   const int Q1D,
                                   const int NE,
                                   const Array<double> &W,
                                   const Vector &J,
                                   ...
                                   Vector &op)
{
   ...
   {
      const occa::kernel AdvectionSetup3D =
         mfem::OccaDev().buildKernel("occa://mfem/fem/occa.okl",
                                     "AdvectionSetup3D", props);
      OccaDiffSetup3D_ker.emplace(id, AdvectionSetup3D);
   }
   OccaDiffSetup3D_ker.at(id)(NE, o_W, o_J, COEFF, o_op);
}
#endif // MFEM_USE_OCCA

// PA Advection Assemble 2D kernel
static void PAAdvectionSetup2D(const int Q1D,
                               const int NE,
                               const Array<double> &w,
                               const Vector &j,
                               const double COEFF,
...
// PA Advection Assemble 3D kernel
static void PAAdvectionSetup3D(const int Q1D,
                               const int NE,
                               const Array<double> &w,
                               const Vector &j,
                               const double COEFF,
...
static void PAAdvectionSetup(const int dim,
                             const int D1D,
                             const int Q1D,
                             const int NE,
                             const Array<double> &W,
...
// PA Advection Apply 3D kernel
template<int T_D1D = 0, int T_Q1D = 0> static
void PAAdvectionApply3D(const int NE,
                        const Array<double> &b,
                        const Array<double> &g,
                        const Array<double> &bt,
                        const Array<double> &gt,
                        ...
                        const int q1d = 0)
{
   ...
   MFEM_FORALL(e, NE,
   {
      ...
      double Bu[D1D][D1D][Q1D];
      double Gu[D1D][D1D][Q1D];
      for (int dz = 0; dz < D1D; ++dz)
      {
        for (int dy = 0; dy < D1D; ++dy)
        {
      ...
      double BBu[D1D][Q1D][Q1D];
      double GBu[D1D][Q1D][Q1D];
      double BGu[D1D][Q1D][Q1D];
      for (int dz = 0; dz < D1D; ++dz)
      {
        for (int qx = 0; qx < Q1D; ++qx)
        {
      ...
      double GBBu[D1D][Q1D][Q1D];
      double BGBu[D1D][Q1D][Q1D];
      double BBGu[D1D][Q1D][Q1D];
      for (int qx = 0; qx < Q1D; ++qx)
      {
        for (int qy = 0; qy < Q1D; ++qy)
        {
...
void AdvectionIntegrator::AssemblePA(const FiniteElementSpace &fes)
{
   ...
   PAAdvectionSetup(dim, dofs1D, quad1D, ne, ir->GetWeights(), geom->J,
                    coeff, pa_data);
}

#ifdef MFEM_USE_OCCA
// OCCA PA Advection Apply 2D kernel
static void OccaPAAdvectionApply2D(const int D1D,
                                   const int Q1D,
                                   const int NE,
                                   const Array<double> &B,
                                   const Array<double> &G,
                                   ...
                                   Vector &y)
{
   ...
   if (!Device::Allows(Backend::OCCA_CUDA))
   {
      ...
      {
         const occa::kernel AdvectionApply2D_CPU =
            mfem::OccaDev().buildKernel("occa://mfem/fem/occa.okl",
                                        "AdvectionApply2D_CPU", props);
         OccaDiffApply2D_cpu.emplace(id, AdvectionApply2D_CPU);
      }
   ...
   else
   {
      ...
      {
         const occa::kernel AdvectionApply2D_GPU =
            mfem::OccaDev().buildKernel("occa://mfem/fem/occa.okl",
                                        "AdvectionApply2D_GPU", props);
         OccaDiffApply2D_gpu.emplace(id, AdvectionApply2D_GPU);
      }
      OccaDiffApply2D_gpu.at(id)(NE, o_B, o_G, o_Bt, o_Gt, o_op, o_x, o_y);
   }
}

// OCCA PA Advection Apply 3D kernel
static void OccaPAAdvectionApply3D(const int D1D,
                                   const int Q1D,
                                   const int NE,
                                   const Array<double> &B,
                                   const Array<double> &G,
                                   ...
                                   Vector &y)
{
   ...
   if (!Device::Allows(Backend::OCCA_CUDA))
   {
      ...
      {
         const occa::kernel AdvectionApply3D_CPU =
            mfem::OccaDev().buildKernel("occa://mfem/fem/occa.okl",
                                        "AdvectionApply3D_CPU", props);
         OccaDiffApply3D_cpu.emplace(id, AdvectionApply3D_CPU);
      }
   ...
   else
   {
      ...
      {
         const occa::kernel AdvectionApply3D_GPU =
            mfem::OccaDev().buildKernel("occa://mfem/fem/occa.okl",
                                        "AdvectionApply3D_GPU", props);
         OccaDiffApply3D_gpu.emplace(id, AdvectionApply3D_GPU);
      }
...
// Shared memory PA Advection Apply 2D kernel
template<const int T_D1D = 0,
         const int T_Q1D = 0,
         const int T_NBZ = 0>
static void SmemPAAdvectionApply2D(const int NE,
...
// Shared memory PA Advection Apply 3D kernel
template<const int T_D1D = 0,
         const int T_Q1D = 0>
static void SmemPAAdvectionApply3D(const int NE,
                                   const Array<double> &_b,
                                   const Array<double> &_g,
                                   const Array<double> &_bt,
                                   const Array<double> &_gt,
...
static void PAAdvectionApply(const int dim,
                             const int D1D,
                             const int Q1D,
                             const int NE,
                             const Array<double> &B,
                             ...
                             Vector &y)
{
   ...
   if (DeviceCanUseOcca())
   {
      if (dim == 2)
      {
         OccaPAAdvectionApply2D(D1D, Q1D, NE, B, G, Bt, Gt, op, x, y);
         return;
      }
      if (dim == 3)
      {
         OccaPAAdvectionApply3D(D1D, Q1D, NE, B, G, Bt, Gt, op, x, y);
         return;
      }
      MFEM_ABORT("OCCA PAAdvectionApply unknown kernel!");
   }
#endif // MFEM_USE_OCCA
   if (dim == 2)
   {
      switch ((D1D << 4 ) | Q1D)
      {
         case 0x22: return SmemPAAdvectionApply2D<2,2,16>(NE,B,G,Bt,Gt,op,x,y);
         case 0x33: return SmemPAAdvectionApply2D<3,3,16>(NE,B,G,Bt,Gt,op,x,y);
         case 0x44: return SmemPAAdvectionApply2D<4,4,8>(NE,B,G,Bt,Gt,op,x,y);
         case 0x55: return SmemPAAdvectionApply2D<5,5,8>(NE,B,G,Bt,Gt,op,x,y);
         case 0x66: return SmemPAAdvectionApply2D<6,6,4>(NE,B,G,Bt,Gt,op,x,y);
         case 0x77: return SmemPAAdvectionApply2D<7,7,4>(NE,B,G,Bt,Gt,op,x,y);
         case 0x88: return SmemPAAdvectionApply2D<8,8,2>(NE,B,G,Bt,Gt,op,x,y);
         case 0x99: return SmemPAAdvectionApply2D<9,9,2>(NE,B,G,Bt,Gt,op,x,y);
         default:   return PAAdvectionApply2D(NE,B,G,Bt,Gt,op,x,y,D1D,Q1D);
      }
   }
   else if (dim == 3)
   {
      switch ((D1D << 4 ) | Q1D)
      {
         case 0x23: return SmemPAAdvectionApply3D<2,3>(NE,B,G,Bt,Gt,op,x,y);
         case 0x34: return SmemPAAdvectionApply3D<3,4>(NE,B,G,Bt,Gt,op,x,y);
         case 0x45: return SmemPAAdvectionApply3D<4,5>(NE,B,G,Bt,Gt,op,x,y);
         case 0x56: return SmemPAAdvectionApply3D<5,6>(NE,B,G,Bt,Gt,op,x,y);
         case 0x67: return SmemPAAdvectionApply3D<6,7>(NE,B,G,Bt,Gt,op,x,y);
         case 0x78: return SmemPAAdvectionApply3D<7,8>(NE,B,G,Bt,Gt,op,x,y);
         case 0x89: return SmemPAAdvectionApply3D<8,9>(NE,B,G,Bt,Gt,op,x,y);
         default:   return PAAdvectionApply3D(NE,B,G,Bt,Gt,op,x,y,D1D,Q1D);
      }
   }
   MFEM_ABORT("Unknown kernel.");
}

// PA Advection Apply kernel
void AdvectionIntegrator::AddMultPA(const Vector &x, Vector &y) const
{
   PAAdvectionApply(dim, dofs1D, quad1D, ne,
