...
private:
   ...
   // This is a pointer to a temporary quadrature rule for integrating
   // over the current element in an assembly loop. It may be reset when
   // moving to a new element, and is therefore not thread-safe.
   mutable std::unique_ptr<const IntegrationRule> temporaryElementRule;
#endif
