...
public:
   ...
   /** @brief Returns an integration rule for given GeomType and Order.

       @warning Triangle and tetrahedron rules are tabulated only to order 25
       and 20 respectively. Above those the rule is generated by
       IntegrationRule::GrundmannMollerSimplexRule(), which is exact to its
       stated degree in EXACT ARITHMETIC but whose weights are partly negative
       -- a known property of that family rather than a defect in it. In double
       precision the negative weights grow fast enough to destroy the answer,
       and by construction nothing about the returned rule says so.

       Measured on the reference triangle and tetrahedron, integrating the
       constant 1:

           triangle                        tetrahedron
           order  least weight  rel err    order  least weight  rel err
              25     0.0        6.7e-16       20     0.0        3.2e-15
              26    -3.6e+01    3.2e-12       21    -7.1e-01    2.1e-12
              34    -5.2e+02    9.5e-11       34    -4.5e+01    9.4e-10
              50    -1.2e+05    2.5e-07       50    -8.0e+03    9.5e-07
              64    -1.9e+07    2.7e-05       64    -9.6e+05    1.3e-03

       So a caller asking for order 64 on a triangle is handed a rule that
       integrates a constant to five digits. It is worse on an integrand that
       is not smooth -- with weights of that size the cancellation amplifies a
       jump rather than averaging it out -- and worse again on tetrahedra,
       which leave the tabulated range five orders earlier.

       The practical ceiling is therefore the tabulated one. Above it the rule
       is returned with a one-time warning rather than refused, because it is
       correct in exact arithmetic and a caller may know what they are doing.

       @sa "Simplex rule positivity" in tests/unit/fem/test_intrules.cpp, which
