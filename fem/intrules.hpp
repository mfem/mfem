...
public:
   ...
   void Set(const fptype *p, const int dim)
   {
   ...
   void Get(fptype *p, const int dim) const
   {
      MFEM_ASSERT(1 <= dim && dim <= 3, "invalid dim: " << dim);
      p[0] = x;
      if (dim > 1)
   ...
   void Set(const fptype x1, const fptype x2, const fptype x3, const fptype w)
   { x = x1; y = x2; z = x3; weight = w; }

   void Set3w(const fptype *p) { x = p[0]; y = p[1]; z = p[2]; weight = p[3]; }

   void Set3(const fptype x1, const fptype x2, const fptype x3)
   { x = x1; y = x2; z = x3; }

   void Set3(const fptype *p) { x = p[0]; y = p[1]; z = p[2]; }

   void Set2w(const fptype x1, const fptype x2, const fptype w)
   { x = x1; y = x2; weight = w; }

   void Set2w(const fptype *p) { x = p[0]; y = p[1]; weight = p[2]; }

   void Set2(const fptype x1, const fptype x2) { x = x1; y = x2; }

   void Set2(const fptype *p) { x = p[0]; y = p[1]; }

   void Set1w(const fptype x1, const fptype w) { x = x1; weight = w; }

   void Set1w(const fptype *p) { x = p[0]; weight = p[1]; }
};
...
private:
   ...
   void AddTriPoints3(const int off, const fptype a, const fptype weight)
   { AddTriPoints3(off, a, 1. - 2.*a, weight); }

   void AddTriPoints3b(const int off, const fptype b, const fptype weight)
   { AddTriPoints3(off, (1. - b)/2., b, weight); }

   void AddTriPoints3R(const int off, const fptype a, const fptype b,
                       const fptype c, const fptype weight)
   {
   ...
   void AddTriPoints6(const int off, const fptype a, const fptype b,
                      const fptype c, const fptype weight)
   {
      IntPoint(off + 0).Set2w(a, b, weight);
   ...
   void AddTetPoints3(const int off, const fptype a, const fptype b,
                      const fptype weight)
   {
   ...
   void AddTetPoints6(const int off, const fptype a, const fptype b,
                      const fptype c, const fptype weight)
   {
   ...
   void AddTetMidPoint(const int off, const fptype weight)
   { IntPoint(off).Set(0.25, 0.25, 0.25, weight); }

   // given a, add the permutations of (a,a,a,b), where 3*a + b = 1
   void AddTetPoints4(const int off, const fptype a, const fptype weight)
   {
   ...
   void AddTetPoints4b(const int off, const fptype b, const fptype weight)
   {
      const fptype a = (1. - b)/3.;
      IntPoint(off).Set(a, a, a, weight);
      AddTetPoints3(off + 1, a, b, weight);
   }

   // add the permutations of (a,a,b,b), 2*(a + b) = 1
   void AddTetPoints6(const int off, const fptype a, const fptype weight)
   {
      const fptype b = 0.5 - a;
      AddTetPoints3(off,     a, b, weight);
      AddTetPoints3(off + 3, b, a, weight);
   }

   // given (a,b) or (a,c), add the permutations of (a,a,b,c), 2*a + b + c = 1
   void AddTetPoints12(const int off, const fptype a, const fptype bc,
                       const fptype weight)
   {
      const fptype cb = 1. - 2*a - bc;
   ...
   void AddTetPoints12bc(const int off, const fptype b, const fptype c,
                         const fptype weight)
   {
      const fptype a = (1. - b - c)/2.;
      AddTetPoints3(off,     a, b, weight);
...
public:
   ...
   const Array<fptype> &GetWeights() const;
...
public:
   ...
   static void GivePolyPoints(const int np, fptype *pts, const int type);
