...
static MFEM_HOST_DEVICE inline
void EvalLinearizedP_002(const double *Jpt, const double *HIden, double *P)
{
   for (int i = 0; i < 4; i++) {
       P[i] = 0.0;
       for (int j = 0; j < 4; j++) {
           P[i] += Jpt[j]*HIden[j + i*4];
       }
   }
}

MFEM_REGISTER_TMOP_KERNELS(void, AddMultPALinear_Kernel_2D,
                           const double metric_normal,
                           const double metric_param,
                           const int mid,
                           const int NE,
   ...
   MFEM_FORALL_2D(e, NE, Q1D, Q1D, NBZ,
   {
      ...
      MFEM_FOREACH_THREAD(qy,y,Q1D)
      {
         MFEM_FOREACH_THREAD(qx,x,Q1D)
         {
            ...
            Jpt[3] -= 1.0;
            double P[4];
            if (mid ==  2) { EvalLinearizedP_002(Jpt, HIden2, P); }
            for (int i = 0; i < 4; i++) { P[i] *= weight; }
...
void TMOP_Integrator::AddMultPA_2D(const Vector &X, Vector &Y) const
{
   ...
   if (PA.mulinear) {
       MFEM_LAUNCH_TMOP_KERNEL(AddMultPALinear_Kernel_2D,id,mn,mp,M,N,J,W,B,G,X,Y, HIden2);
   }
   else {
