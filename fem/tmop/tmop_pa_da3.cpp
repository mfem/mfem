                           ...
                           const int q1d)
{
   ...
   const fptype infinity = std::numeric_limits<fptype>::infinity();
   MFEM_VERIFY(sizeidx == 0,"");
   MFEM_VERIFY(MFEM_CUDA_BLOCKS==256,"");

   const fptype *nc_red = nc_reduce.Read();
   ...
   mfem::forall_3D(NE, Q1D, Q1D, Q1D, [=] MFEM_HOST_DEVICE (int e)
   {
      ...
      fptype min;
      MFEM_SHARED fptype min_size[MFEM_CUDA_BLOCKS];
      DeviceTensor<3,fptype> M((fptype*)(min_size),D1D,D1D,D1D);
      const DeviceTensor<3,const fptype> D((fptype*)(DDD+sizeidx),D1D,D1D,D1D);
                                               ...
                                               DenseTensor &Jtr) const
{
   ...
   const fptype input_min_size = lim_min_size;
