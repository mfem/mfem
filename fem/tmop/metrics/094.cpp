...
MFEM_REGISTER_KERNELS(Setup094, kernel_t, (int, int));

template <int D, int Q>
kernel_t Setup094::Kernel()
{
   return TMOPPASetupGrad2D::Mult<metric_t, D, Q>;
}

kernel_t Setup094::Fallback(int, int)
{
   return TMOPPASetupGrad2D::Mult<metric_t>;
}

template <>
void TMOPAssembleGradPA<94>(TMOPPASetupGrad2D &ker)
{
   const static auto specialized_kernels = [] { return TMOPAdd<Setup094>(); }();
   Setup094::Run(ker.Ndof(), ker.Nqpt(), ker);
}

void TMOPAddMultPA_094(TMOPAddMultPA2D &ker)
{
