...
MFEM_REGISTER_TMOP_KERNELS(void, AssembleDiagonalLinearPA_Kernel_3D,
                           const int NE,
                           const Array<double> &b,
                           const Array<double> &g,
                           const DenseTensor &j,
                           ...
                           const int q1d)
{
   ...
   const auto G = Reshape(g.Read(), Q1D, D1D);
   const auto J = Reshape(j.Read(), DIM, DIM, Q1D, Q1D, Q1D, NE);
   const auto HIden2 = Reshape(hiden2.Read(), DIM, DIM, DIM, DIM, 1, 1, 1, 1);
                  ...
                  MFEM_FOREACH_THREAD(qy,y,Q1D)
                  {
                     MFEM_FOREACH_THREAD(dz,z,D1D)
                     {
                        ...
                        for (int qz = 0; qz < Q1D; ++qz)
                        {
                           ...
                           const double R = j==2 ? Gz : Bz;
                           const double Jij = Jrt(i,i) * Jrt(j,j);
                           const double h = HIden2(v,i,v,j,0,0,0,0);
                           QQD(qx,qy,dz) += L * Jij * h * R;
                        }
                     }
                  }
...
void TMOP_Integrator::AssembleDiagonalPA_3D(Vector &D) const
{
   ...
   const Vector &H = PA.H;
   const Vector &HIden2 = PA.HIden2;

   if (PA.mulinear) {
       MFEM_LAUNCH_TMOP_KERNEL(AssembleDiagonalLinearPA_Kernel_3D,id,N,B,G,J,HIden2,D);
   }
   else {
