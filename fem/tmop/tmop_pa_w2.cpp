...
fptype EvalW_001(const fptype *Jpt)
{
...
static MFEM_HOST_DEVICE inline
fptype EvalW_002(const fptype *Jpt)
{
   kernels::InvariantsEvaluator2D ie(Args().J(Jpt));
...
static MFEM_HOST_DEVICE inline
fptype EvalW_007(const fptype *Jpt)
{
   kernels::InvariantsEvaluator2D ie(Args().J(Jpt));
...
static MFEM_HOST_DEVICE inline
fptype EvalW_056(const fptype *Jpt)
{
   kernels::InvariantsEvaluator2D ie(Args().J(Jpt));
...
static MFEM_HOST_DEVICE inline
fptype EvalW_077(const fptype *Jpt)
{
   kernels::InvariantsEvaluator2D ie(Args().J(Jpt));
   const fptype I2b = ie.Get_I2b();
...
static MFEM_HOST_DEVICE inline
fptype EvalW_080(const fptype *Jpt, const fptype *w)
{
   return w[0] * EvalW_002(Jpt) + w[1] * EvalW_077(Jpt);
}

static MFEM_HOST_DEVICE inline
fptype EvalW_094(const fptype *Jpt, const fptype *w)
{
   return w[0] * EvalW_002(Jpt) + w[1] * EvalW_056(Jpt);
}

MFEM_REGISTER_TMOP_KERNELS(fptype, EnergyPA_2D,
   ...
   mfem::forall_2D_batch(NE, Q1D, Q1D, NBZ, [=] MFEM_HOST_DEVICE (int e)
   {
      ...
      MFEM_FOREACH_THREAD(qy,y,Q1D)
      {
         MFEM_FOREACH_THREAD(qx,x,Q1D)
         {
            ...
            const fptype weight = metric_normal * m_coef * W(qx,qy) * detJtr;
...
fptype TMOP_Integrator::GetLocalStateEnergyPA_2D(const Vector &X) const
{
   const int N = PA.ne;
