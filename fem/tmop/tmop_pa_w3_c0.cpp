...
void TMOP_EnergyPA_C0_3D(
    const double lim_normal, const DeviceTensor<4, const double> &LD,
    const bool const_c0, const DeviceTensor<4, const double> &C0, const int NE,
    const DeviceTensor<6, const double> &J, const ConstDeviceCube &W,
    const ConstDeviceMatrix &b, const ConstDeviceMatrix &bld,
    const DeviceTensor<5, const double> &X0,
    const DeviceTensor<5, const double> &X1, DeviceTensor<4> &E,
...
double TMOP_Integrator::GetLocalStateEnergyPA_C0_3D(const Vector &x) const {
  ...
  if (d == 2 && q == 2) {
    ker = TMOP_EnergyPA_C0_3D<2, 2>;
  }
  if (d == 2 && q == 3) {
    ker = TMOP_EnergyPA_C0_3D<2, 3>;
  }
  if (d == 2 && q == 4) {
    ker = TMOP_EnergyPA_C0_3D<2, 4>;
  }
  if (d == 2 && q == 5) {
    ker = TMOP_EnergyPA_C0_3D<2, 5>;
  }
  if (d == 2 && q == 6) {
    ker = TMOP_EnergyPA_C0_3D<2, 6>;
  }

  if (d == 3 && q == 3) {
    ker = TMOP_EnergyPA_C0_3D<3, 3>;
  }
  if (d == 3 && q == 4) {
    ker = TMOP_EnergyPA_C0_3D<3, 4>;
  }
  if (d == 3 && q == 5) {
    ker = TMOP_EnergyPA_C0_3D<3, 5>;
  }
  if (d == 3 && q == 6) {
    ker = TMOP_EnergyPA_C0_3D<3, 6>;
  }

  if (d == 4 && q == 4) {
    ker = TMOP_EnergyPA_C0_3D<4, 4>;
  }
  if (d == 4 && q == 5) {
    ker = TMOP_EnergyPA_C0_3D<4, 5>;
  }
  if (d == 4 && q == 6) {
    ker = TMOP_EnergyPA_C0_3D<4, 6>;
  }

  if (d == 5 && q == 5) {
    ker = TMOP_EnergyPA_C0_3D<5, 5>;
  }
  if (d == 5 && q == 6) {
    ker = TMOP_EnergyPA_C0_3D<5, 6>;
  }
