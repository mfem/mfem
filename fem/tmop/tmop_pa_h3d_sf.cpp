...
MFEM_REGISTER_TMOP_KERNELS(void, AssembleDiagonalPA_Kernel_SF_3D,
                           const int NE,
         ...
         MFEM_FOREACH_THREAD(qx,x,Q1D)
         {
            MFEM_FOREACH_THREAD(qy,y,Q1D)
            {
               MFEM_FOREACH_THREAD(dz,z,D1D)
               {
                  ...
                  {
                     QQD(qx,qy,dz) += H0(v,v,qx,qy,qz,e);
                  }
               }
            }
         }
...
void TMOP_Integrator::AssembleDiagonalPA_SF_3D(Vector &D) const
{
   const int N = PA.ne;
   ...
   MFEM_LAUNCH_TMOP_KERNEL(AssembleDiagonalPA_Kernel_SF_3D,id,N,B,H0,D);
}
