...
static MFEM_HOST_DEVICE inline void EvalP_302(const real_t *J, real_t *P)
{
   real_t B[9];
   real_t dI1b[9], dI2[9], dI2b[9], dI3b[9];
   kernels::InvariantsEvaluator3D ie(
      Args().J(J).B(B).dI1b(dI1b).dI2(dI2).dI2b(dI2b).dI3b(dI3b));
...
// P_303 = dI1b/3
static MFEM_HOST_DEVICE inline void EvalP_303(const real_t *J, real_t *P)
{
   real_t B[9];
...
// P_315 = 2*(I3b - 1)*dI3b
static MFEM_HOST_DEVICE inline void EvalP_315(const real_t *J, real_t *P)
{
   real_t dI3b[9];
...
static MFEM_HOST_DEVICE inline void EvalP_318(const real_t *J, real_t *P)
{
   real_t dI3b[9];
   kernels::InvariantsEvaluator3D ie(Args().J(J).dI3b(dI3b));
...
static MFEM_HOST_DEVICE inline void EvalP_321(const real_t *J, real_t *P)
{
   real_t B[9];
   real_t dI1[9], dI2[9], dI3b[9];
   kernels::InvariantsEvaluator3D ie(
      Args().J(J).B(B).dI1(dI1).dI2(dI2).dI3b(dI3b));
   real_t sign_detJ;
...
static MFEM_HOST_DEVICE inline void EvalP_332(const real_t *J, const real_t *w,
                                              real_t *P)
{
   real_t B[9];
   real_t dI1b[9], dI2[9], dI2b[9], dI3b[9];
...
static MFEM_HOST_DEVICE inline void EvalP_338(const real_t *J, const real_t *w,
                                              real_t *P)
{
   real_t B[9];
   real_t dI1b[9], dI2[9], dI2b[9], dI3b[9];
...
MFEM_REGISTER_TMOP_KERNELS(void, AddMultPA_Kernel_3D,
                           const real_t metric_normal, const Vector &mc_,
                           const Array<real_t> &metric_param, const int mid,
                           const int NE, const DenseTensor &j_,
                           const Array<real_t> &w_, const Array<real_t> &b_,
                           const Array<real_t> &g_, const Vector &x_,
                           Vector &y_, const int d1d, const int q1d)
{
   ...
   const auto MC = const_m0 ? Reshape(mc_.Read(), 1, 1, 1, 1)
      ...
      MFEM_FOREACH_THREAD(qz, z, Q1D)
      {
         MFEM_FOREACH_THREAD(qy, y, Q1D)
         {
            MFEM_FOREACH_THREAD(qx, x, Q1D)
            {
               ...
               if (mid == 332)
               {
                  EvalP_332(Jpt, metric_data, P);
               }
               if (mid == 338)
               {
                  EvalP_338(Jpt, metric_data, P);
               }
               for (int i = 0; i < 9; i++)
               {
                  P[i] *= weight;
...
void TMOP_Integrator::AddMultPA_3D(const Vector &X, Vector &Y) const
{
   ...
   MFEM_LAUNCH_TMOP_KERNEL(AddMultPA_Kernel_3D, id, mn, MC, mp, M, N, J, W, B, G,
                           X, Y);
}
