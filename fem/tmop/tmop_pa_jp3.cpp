...
double TMOPNewtonSolver::MinDetJpr_3D(const FiniteElementSpace *fes,
  ...
  if (d == 2 && q == 2) {
    ker = TMOP_MinDetJpr_3D<2, 2>;
  }
  if (d == 2 && q == 3) {
    ker = TMOP_MinDetJpr_3D<2, 3>;
  }
  if (d == 2 && q == 4) {
    ker = TMOP_MinDetJpr_3D<2, 4>;
  }
  if (d == 2 && q == 5) {
    ker = TMOP_MinDetJpr_3D<2, 5>;
  }
  if (d == 2 && q == 6) {
    ker = TMOP_MinDetJpr_3D<2, 6>;
  }

  if (d == 3 && q == 3) {
    ker = TMOP_MinDetJpr_3D<3, 3>;
  }
  if (d == 3 && q == 4) {
    ker = TMOP_MinDetJpr_3D<3, 4>;
  }
  if (d == 3 && q == 5) {
    ker = TMOP_MinDetJpr_3D<3, 5>;
  }
  if (d == 3 && q == 6) {
    ker = TMOP_MinDetJpr_3D<3, 6>;
  }

  if (d == 4 && q == 4) {
    ker = TMOP_MinDetJpr_3D<4, 4>;
  }
  if (d == 4 && q == 5) {
    ker = TMOP_MinDetJpr_3D<4, 5>;
  }
  if (d == 4 && q == 6) {
    ker = TMOP_MinDetJpr_3D<4, 6>;
  }

  if (d == 5 && q == 5) {
    ker = TMOP_MinDetJpr_3D<5, 5>;
  }
  if (d == 5 && q == 6) {
    ker = TMOP_MinDetJpr_3D<5, 6>;
  }
