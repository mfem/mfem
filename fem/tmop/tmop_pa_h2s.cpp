...
void EvalH_001(const int e, const int qx, const int qy,
               const fptype weight, const fptype *Jpt,
               DeviceTensor<7,fptype> H)
{
   constexpr int DIM = 2;
   fptype ddI1[4];
...
{
   constexpr int DIM = 2;
   fptype ddI1[4], ddI1b[4], dI2b[4];
   kernels::InvariantsEvaluator2D ie(Args()
                                     .J(Jpt)
                                     .ddI1(ddI1)
                                     .ddI1b(ddI1b)
                                     .dI2b(dI2b));
   const fptype w = 0.5 * weight;
   for (int i = 0; i < DIM; i++)
   {
      for (int j = 0; j < DIM; j++)
      {
...
{
   constexpr int DIM = 2;
   fptype ddI1[4], ddI2[4], dI1[4], dI2[4], dI2b[4];
   kernels::InvariantsEvaluator2D ie(Args()
                                     .J(Jpt)
                                     .ddI1(ddI1)
                                     .ddI2(ddI2)
                                     ...
                                     .dI2(dI2)
                                     .dI2b(dI2b));
   const fptype c1 = 1./ie.Get_I2();
   const fptype c2 = weight*c1*c1;
   const fptype c3 = ie.Get_I1()*c2;
...
{
   constexpr int DIM = 2;
   fptype dI2[4], dI2b[4], ddI2[4];
   kernels::InvariantsEvaluator2D ie(Args()
                                     .J(Jpt)
                                     .dI2(dI2)
                                     .dI2b(dI2b)
                                     .ddI2(ddI2));
   const fptype I2 = ie.Get_I2(), I2inv_sq = 1.0 / (I2 * I2);
   ConstDeviceMatrix di2(ie.Get_dI2(),DIM,DIM);
   for (int i = 0; i < DIM; i++)
   {
      for (int j = 0; j < DIM; j++)
...
void EvalH_080(const int e, const int qx, const int qy,
               const fptype weight, const fptype *w, const fptype *Jpt,
               DeviceTensor<7,fptype> H)
{
   constexpr int DIM = 2;
   fptype ddI1[4], ddI1b[4], dI2[4], dI2b[4], ddI2[4];
   kernels::InvariantsEvaluator2D ie(Args()
                                     .J(Jpt)
                                     .dI2(dI2)
                                     .ddI1(ddI1)
...
{
   constexpr int DIM = 2;
   fptype ddI1[4], ddI1b[4], dI2b[4], ddI2b[4];
   kernels::InvariantsEvaluator2D ie(Args().J(Jpt)
                                     .ddI1(ddI1)
                                     .ddI1b(ddI1b)
                                     .dI2b(dI2b)
                           ...
                           const int q1d)
{
   ...
   mfem::forall_2D_batch(NE, Q1D, Q1D, NBZ, [=] MFEM_HOST_DEVICE (int e)
   {
      ...
      MFEM_SHARED fptype s_X[2][NBZ][MD1*MD1];
      MFEM_SHARED fptype s_DQ[4][NBZ][MD1*MQ1];
      MFEM_SHARED fptype s_QQ[4][NBZ][MQ1*MQ1];
