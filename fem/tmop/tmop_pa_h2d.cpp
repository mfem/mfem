...
MFEM_REGISTER_TMOP_KERNELS(void, AssembleDiagonalLinearPA_Kernel_2D,
                           const int NE,
                           const Array<double> &b,
                           const Array<double> &g,
                           const DenseTensor &j,
                           ...
                           const int q1d)
{
   ...
   const auto G = Reshape(g.Read(), Q1D, D1D);
   const auto J = Reshape(j.Read(), DIM, DIM, Q1D, Q1D, NE);
   const auto HIden2 = Reshape(hiden2.Read(), DIM, DIM, DIM, DIM, 1, 1, 1);
               ...
               for (int qy = 0; qy < Q1D; ++qy)
               {
                  ...
                  for (int i = 0; i < DIM; i++)
                  {
                     for (int j = 0; j < DIM; j++)
                     {
                        const double Jij = Jrt(i,i) * Jrt(j,j);
                        const double alpha = Jij * BG(i,j);
                        QD(i,j,qx,dy) += alpha * HIden2(v,i,v,j, 0, 0, 0);
                     }
                  }
               }
            }
...
void TMOP_Integrator::AssembleDiagonalPA_2D(Vector &D) const
{
   ...
   if (PA.mulinear) {
       MFEM_LAUNCH_TMOP_KERNEL(AssembleDiagonalLinearPA_Kernel_2D,id,N,B,G,J,HIden2,D);
   }
   else {
