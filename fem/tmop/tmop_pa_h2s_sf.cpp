...
namespace mfem
{
MFEM_REGISTER_TMOP_KERNELS(void, SetupGradPA_SF_2D,
                           ...
                           const int q1d)
{
   ...
   const bool const_c0 = C_.Size() == 1;
   const auto C0 = const_c0 ?
                   Reshape(C_.Read(), 1, 1, 1) :
                   Reshape(C_.Read(), Q1D, Q1D, NE);
   auto MQ = Reshape(MQ_.Read(), Q1D, Q1D, NE);
   auto VQ = Reshape(VQ_.Read(), Q1D, Q1D, NE);
   auto GQ = Reshape(GQ_.Read(), Q1D, Q1D, DIM, NE);
   auto HQ = Reshape(HQ_.Read(), Q1D, Q1D, DIM, DIM, NE);

   auto H0 = Reshape(h0_.Write(), Q1D, Q1D, DIM, DIM, NE);
...
void TMOP_Integrator::AssembleGradPA_SF_2D(const Vector &X) const
{
   MFEM_CONTRACT_VAR(X);
   const int N = PA.ne;
   ...
   Derivatives2D<L,P,0,0,0,0,MD,MQ>(N,B,G,J,gradq,hessq,dim,D1D,Q1D);

   MFEM_LAUNCH_TMOP_KERNEL(SetupGradPA_SF_2D,id,sn,SFM,SFG,gradq,hessq,C0SF,N,Hsf);
}
