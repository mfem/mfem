...
MFEM_REGISTER_TMOP_KERNELS(void, AddMultGradPALinear_Kernel_2D,
                           const int NE,
                           const Array<double> &b_,
                           const Array<double> &g_,
                           const DenseTensor &j_,
                           ...
                           const int q1d)
{
   ...
   auto HIden2 = Reshape(hiden2_.ReadWrite(), DIM, DIM, DIM, DIM, 1, 1, 1);
            ...
            for (int i = 0; i < DIM; i++)
            {
               for (int j = 0; j < DIM; j++)
               {
                  ...
                  for (int r = 0; r < DIM; r++)
                  {
                     for (int c = 0; c < DIM; c++)
                     {
                        M(i,j) += 1.0 * weight * HIden2(r,c,i,j,0,0,0) * J(r,c);
                     }
                  }
               }
            }
...
void TMOP_Integrator::AddMultGradPA_2D(const Vector &R, Vector &C) const
{
   ...
   if (PA.mulinear) {
       MFEM_LAUNCH_TMOP_KERNEL(AddMultGradPALinear_Kernel_2D,id,N,B,G,J,W,R,C,HIden2);
   }
   else {
