...
//               create derived classes from HyperbolicFormIntegrator with
//               overloaded ComputeFlux. One can optionally overload
//               ComputeFluxDotN to avoid creating dense matrix when computing
//               normal flux.
//
...
//               condition. Also, resetMaxCharSpeed should be called after each
//               Mult.
//
...
//                  |- HyperbolicFormIntegrator: (F(u,x), grad v) and <F̂(u, x,
//                  n), [[v]]) |  |- RiemannSolver: (F±, u±) ↦ F̂(u,x,n)
//                  |
 ...
 * @param avgs averaged state, (state1*sqrt(den1) + state2*sqrt(den2)) /
 * (sqrt(den1) + sqrt(den2))
...
/**
 * @brief Prolongate varying-order GridFunction to equal-order GridFunction by
 * interpolation. Usually used for visualization purpose.
 *
...
GridFunction *ProlongToMaxOrder(const GridFunction *x,
  ...
  FiniteElementSpace *spaceInt =
      new FiniteElementSpace(mesh, fecInt, fespace->GetVDim());
...
class HyperbolicFormIntegrator : public NonlinearFormIntegrator {
 protected:
  ...
  // The maximum characterstic speed, updated during element/face vector
  // assembly
 ...
 public:
   ...
   * @a flux: vdim x vdim x sdim and @a eigs: vdim x sdim should be provided
   * with correct size
   *
   * @param state
   * @param Tr
  ...
  virtual void ComputeFluxJacobian(const Vector &state,
                                   ...
                                   DenseTensor &Jacobian, DenseMatrix &eigs) {
  ...
  const IntegrationRule &GetRule(const FiniteElement &trial_fe,
    ...
    int order;
    order =
        std::max(trial_fe.GetOrder(), test_fe.GetOrder()) * 2 + IntOrderOffset;
 ...
 public:
  ...
  virtual void ComputeFluxJacobian(const Vector &state,
    ...
    const double pressure = (specific_heat_ratio - 1.0) *
                            (energy - 0.5 * (momentum * momentum) / density);
    // ∂ p / ∂ ρ = ½(γ -1)|u|^2
    const double dpdrho = -0.5 * (specific_heat_ratio - 1) *
                          (momentum * momentum) / (density * density);
    // ∂ p / ∂ (ρ u) = (γ - 1)u
    Vector dpdmom(dim);
    dpdmom = momentum;
    ...
    for (int i = 0; i < dim; i++) {
      ...
      // dF/dρ
      for (int j = 0; j < dim; j++) {
        current_col[1 + j] = -(momentum[i] / (density * density)) * momentum[j];
      }
      ...
      for (int j = 0; j < dim; j++) {
        Jacobian(j + 1, i + 1, i) +=
            momentum[j] / density;  // (ρu * e_i^T)/ρ, column
        Jacobian(j + 1, j + 1, i) +=
            momentum[i] / density;               // (ρu_i / ρ) I, diagonal
        Jacobian(i + 1, j + 1, i) += dpdmom[j];  // e_i * dp/d(ρu)^T, row
        Jacobian(dim + 1, j + 1,
                 i) =
            momentum[i] / density * dpdmom[j];  // ((ρu)⋅e_i)*dp/d(ρu)^T, entry
      }
      Jacobian(dim + 1, i + 1, i) +=
          (energy + pressure) / density;  // (E + p)/ρ e_i^T

      // dF/dE
      current_col += nvars * (1 + dim);  // move it to current column
      current_col[i + 1] = dpdE;         // dp/dE e_i
      current_col[dim + 1] =
          momentum[i] / density * (energy + dpdE);  // (ρu)/ρ * (E  + dp/dE)
    }
  }
...
class EulerDirichletBC : public EulerFormIntegrator /* Ollie Zhang */
{
 ...
 public:
  EulerDirichletBC(RiemannSolver *rsolver_, const int dim,
                   ...
                   const int IntOrderOffset_)
      : EulerFormIntegrator(rsolver_, dim, specific_heat_ratio_,
                            IntOrderOffset_),
  ...
  virtual void AssembleFaceVector(const FiniteElement &el1,
    ...
    //       Note that we have to replace state2 by g and there is NO second
    //       neighboring element.

    const int dof1 = el1.GetDof();  // creating a vairable dof1 to get the DOF
                                    // of interior element
    // const int dof2 = el2.GetDof();         //creating a variable dof1 to get
    // the DOF of exterior element
    ...
    Vector state2(
        num_equations);  // state value at an integration point - second elem
    Vector fluxN1(num_equations);  // flux dot n value at an integration point -
                                   // first elem
    Vector fluxN2(num_equations);  // flux dot n value at an integration point -
                                   // second elem
    Vector fluxN(num_equations);   // hat(F)(u,x)
#else
    shape1.SetSize(dof1);
    ...
    const DenseMatrix elfun1_mat(elfun.GetData(), dof1, num_equations);
    // const DenseMatrix elfun2_mat(elfun.GetData() + dof1 * num_equations,
    // dof2,
    //                              num_equations);
    ...
    for (int i = 0; i < ir->GetNPoints(); i++) {
      ...
      // Interpolate e
      elfun1_mat.MultTranspose(shape1, state1);
      dirichletData.Eval(state2, Tr.GetElement1Transformation(),
      ...
      rsolver->Eval(state1, state2, fluxN1, fluxN2, mcs, nor,
                    fluxN);  // get Fhat for the reimann solver
...
class AdvectionFormIntegrator : public HyperbolicFormIntegrator {
 ...
 public:
  ...
  AdvectionFormIntegrator(RiemannSolver *rsolver_, const int dim,
                          VectorCoefficient &b_, const int IntOrderOffset_ = 3)
      : HyperbolicFormIntegrator(rsolver_, dim, 1, IntOrderOffset_),
        b(b_),
        bval(dim) {}
  /**
...
class ShallowWaterFormIntegrator : public HyperbolicFormIntegrator {
 ...
 public:
  ...
  ShallowWaterFormIntegrator(RiemannSolver *rsolver_, const int dim,
      ...
      : HyperbolicFormIntegrator(rsolver_, dim, dim + 1, IntOrderOffset_),
        g(g_) {}
  /**
   * @brief Construct a new Shallow Water Element Form Integrator object with
   * given integral rule
