             ...
             || (eval_flags & NORMALS))
         {
            ...
            if (VDIM == 2 &&
                ((eval_flags & NORMALS)
                 || (eval_flags & DETERMINANTS)))
            {
               const fptype norm = sqrt(D[0]*D[0]+D[1]*D[1]);
   ...
   const int eval_flags)
{
   ...
   mfem::forall(NF, [=] MFEM_HOST_DEVICE (int f)
   {
      ...
      fptype r_F[max_ND1D][max_ND1D][max_VDIM];
      ...
      if (eval_flags & VALUES)
      {
         fptype Bu[max_NQ1D][max_ND1D][max_VDIM];
         for (int d2 = 0; d2 < ND1D; ++d2)
         {
            for (int q = 0; q < NQ1D; ++q)
            {
         ...
         fptype BBu[max_NQ1D][max_NQ1D][max_VDIM];
          ...
          || (eval_flags & NORMALS))
      {
         ...
         fptype Gu[max_NQ1D][max_ND1D][max_VDIM];
         fptype Bu[max_NQ1D][max_ND1D][max_VDIM];
         for (int d2 = 0; d2 < ND1D; ++d2)
         {
            for (int q = 0; q < NQ1D; ++q)
            {
               ...
               {
                  const fptype b = B(q,d1);
                  const fptype g = G(q,d1);
                  for (int c = 0; c < VDIM; c++)
                  {
                     const fptype u = r_F[d1][d2][c];
                     Gu[q][d2][c] += g*u;
                     Bu[q][d2][c] += b*u;
                  }
               }
         ...
         fptype BGu[max_NQ1D][max_NQ1D][max_VDIM];
         fptype GBu[max_NQ1D][max_NQ1D][max_VDIM];
         for (int q2 = 0; q2 < NQ1D; ++q2)
         {
            for (int q1 = 0; q1 < NQ1D; ++q1)
            {
            ...
            for (int q2 = 0; q2 < NQ1D; ++q2)
            {
               for (int q1 = 0; q1 < NQ1D; ++q1)
               {
                  ...
                  n[2] = s*( BGu[q2][q1][0]*GBu[q2][q1][1]-GBu[q2][q1][0]*
                             BGu[q2][q1][1] );
                  const fptype norm = sqrt(n[0]*n[0]+n[1]*n[1]+n[2]*n[2]);
   ...
   const int eval_flags)
{
   ...
   mfem::forall_3D(NF, NQ1D, NQ1D, VDIM, [=] MFEM_HOST_DEVICE (int f)
   {
      ...
      MFEM_SHARED fptype sm1[max_NQ1D*max_NQ1D*max_VDIM];
      MFEM_SHARED fptype sm2[max_NQ1D*max_ND1D*max_VDIM];

      auto s_F = (fptype(*)[max_ND1D][max_VDIM])sm1;
      ...
      if (eval_flags & VALUES)
      {
         auto Bu = (fptype (*)[max_ND1D][max_VDIM])sm2;
      ...
      {
         // We only compute the tangential derivatives
         auto Gu = (fptype (*)[max_ND1D][max_VDIM])sm2;
         MFEM_SHARED fptype Bu[max_NQ1D][max_ND1D][max_VDIM];
         MFEM_FOREACH_THREAD(d2,x,ND1D)
         {
            MFEM_FOREACH_THREAD(q1,y,NQ1D)
            {
         ...
         auto BGu = (fptype (*)[max_NQ1D][max_VDIM])sm1;
         MFEM_SHARED fptype GBu[max_NQ1D][max_NQ1D][max_VDIM];
         MFEM_FOREACH_THREAD(q2,x,NQ1D)
         {
            MFEM_FOREACH_THREAD(q1,y,NQ1D)
            {
