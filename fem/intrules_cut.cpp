...
namespace mfem
{
   ...
   void MomentFittingIntRules::ComputeSurfaceWeights1D(ElementTransformation &Tr)
   {
      ...
      if (LvlSet->Eval(Tr, ip0) * LvlSet->Eval(Tr, ip1) < 0.)
      {
         ...
         ip2.x = .5;
         while (LvlSet->Eval(Tr, ip2) > 1e-12 || LvlSet->Eval(Tr, ip2) < -1e-12)
         {
                                                      ...
                                                      const IntegrationRule *sir)
   {
      ...
      else if (LvlSet->Eval(Tr, ip0) <= -1e-12 || LvlSet->Eval(Tr, ip1) <= -1e-12)
      {
         for (int ip = 0; ip < ir.GetNPoints(); ip++)
         {
            IntegrationPoint &intp = ir.IntPoint(ip);
   ...
   void MomentFittingIntRules::ComputeSurfaceWeights2D(ElementTransformation &Tr)
   {
      ...
      for (int edge = 0; edge < me->GetNEdges(); edge++)
      {
         ...
         if (LvlSet->Eval(Trafo, ipA) < -1e-12 || LvlSet->Eval(Trafo, ipB) < -1e-12)
         {
            interior = false;
         }
         ...
         else if (LvlSet->Eval(Trafo, ipA) > 1e-15 && LvlSet->Eval(Trafo, ipB) <= 0.)
         {
            layout = Layout::intersected;
         }
         else if (LvlSet->Eval(Trafo, ipA) <= 0. && LvlSet->Eval(Trafo, ipB) > 1e-15)
         {
            layout = Layout::intersected;
         ...
         if (layout == Layout::intersected)
         {
            ...
            while (LvlSet->Eval(Trafo, ip) > 1e-12 || LvlSet->Eval(Trafo, ip) < -1e-12)
            {
         ...
         if (edge_int[edge] && !interior)
         {
            ...
            for (int ip = 0; ip < ir2->GetNPoints(); ip++)
            {
               ...
               for (int dof = 0; dof < nBasis; dof++)
               {
                  ...
                  RHS(dof) -= (grad * normal) * ir2->IntPoint(ip).weight * dist.Norml2() / edgelength(edge);
               }
            }
         }
      }
         ...
         if (edge_int[edge] && !interior)
         {
            ...
            for (int ip = 0; ip < ir2->GetNPoints(); ip++)
            {
               ...
               for (int dof = 0; dof < nBasisVolume; dof++)
               {
                  ...
                  RHS(dof) += (adiv * normal) * ir2->IntPoint(ip).weight * dist.Norml2() / edgelength(edge);
               }
            }
         }
      }
   ...
   void MomentFittingIntRules::ComputeSurfaceWeights3D(ElementTransformation &Tr)
   {
      ...
      for (int face = 0; face < me->GetNFaces(); face++)
      {
         ...
         if (LvlSet->Eval(Trafo, ipA) < -1e-12 || LvlSet->Eval(Trafo, ipB) < -1e-12 || LvlSet->Eval(Trafo, ipC) < -1e-12 || LvlSet->Eval(Trafo, ipD) < -1e-12)
         {
            interior = false;
         }
                                              ...
                                              DenseMatrix &shape)
   {
      ...
      for (int c = 1; c <= Order; c++)
      {
         ...
         {
            Vector a(2);
            a(0) = (real_t)(factorial[expo]) * pow(X(0), (real_t)(expo)) * pow(X(1), (real_t)(c - expo));
            a(1) = -1. * (real_t)(factorial[expo - 1]) * pow(X(0), (real_t)(expo - 1)) * pow(X(1), (real_t)(c - expo + 1));
   ...
   void MomentFittingIntRules::Basis2D(const IntegrationPoint &ip, Vector &shape)
   {
      ...
      for (int c = 0; c <= Order; c++)
      {
         ...
         {
            shape(count) = pow(X(0), (real_t)(expo)) * pow(X(1), (real_t)(c - expo));
            count++;
         }
      }
   }
                                         ...
                                         DenseMatrix &shape)
   {
      ...
      for (int c = 0; c <= Order; c++)
      {
         for (int expo = 0; expo <= c; expo++)
         {
            shape(count, 0) = .25 * pow(X(0), (real_t)(expo + 1)) * pow(X(1), (real_t)(c - expo)) / (real_t)(expo + 1);
            shape(count, 1) = .25 * pow(X(0), (real_t)(expo)) * pow(X(1), (real_t)(c - expo + 1)) / (real_t)(c - expo + 1);
            count++;
         }
      }
   }
   ...
   void MomentFittingIntRules::Basis3D(const IntegrationPoint &ip, Vector &shape)
   {
      ...
      for (int c = 0; c <= Order; c++)
         for (int expo = 0; expo <= c; expo++)
            ...
            {
               shape(count) = pow(X(0), (real_t)(expo)) * pow(X(1), (real_t)(expo2)) * pow(X(2), (real_t)(c - expo - expo2));
               count++;
            }
   }
                                         ...
                                         DenseMatrix &shape)
   {
      ...
      for (int c = 0; c <= Order; c++)
         for (int expo = 0; expo <= c; expo++)
            for (int expo2 = 0; expo2 <= c - expo; expo2++)
            {
               shape(count, 0) = pow(X(0), (real_t)(expo + 1)) * pow(X(1), (real_t)(expo2)) * pow(X(2), (real_t)(c - expo - expo2)) / (6. * (real_t)(expo + 1));
               shape(count, 1) = pow(X(0), (real_t)(expo)) * pow(X(1), (real_t)(expo2 + 1)) * pow(X(2), (real_t)(c - expo - expo2)) / (6. * (real_t)(expo2 + 1));
               ;
               shape(count, 2) = pow(X(0), (real_t)(expo)) * pow(X(1), (real_t)(expo2)) * pow(X(2), (real_t)(c - expo - expo2 + 1)) / (6. * (real_t)(c - expo + expo2 + 1));
               ;
               count++;
            }
   }
