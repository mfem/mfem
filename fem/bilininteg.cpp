...
DGDiffusionBR2Integrator::DGDiffusionBR2Integrator(FiniteElementSpace *fes,
   ...
   : fespace(fes),
     eta(e),
     Minv(fes->GetFE(0)->GetDof(), fes->GetFE(0)->GetDof(), fes->GetNE())
{
   // Precompute local mass matrix inverses needed for the lifting operators
   const int dof = fespace->GetFE(0)->GetDof();
   DenseMatrix Me(dof);
   DenseMatrixInverse inv(&Me);
   MassIntegrator mi;
   for (int i = 0; i < fespace->GetNE(); i++)
   {
      mi.AssembleElementMatrix(*fespace->GetFE(i),
                               *fespace->GetElementTransformation(i),
                               Me);
      inv.Factor();
      inv.GetInverseMatrix(Minv(i));
   }
}
   ...
   FaceElementTransformations &Trans, DenseMatrix &elmat)
{
   ...
   R11 = 0.0;
   MinvR11.SetSize(ndof1, ndof1);
   M1inv.UseExternalData(Minv.GetData(Trans.Elem1No), ndof1, ndof1);
   ...
   if (Trans.Elem2No >= 0)
   {
      ...
      MinvR12.SetSize(ndof1, ndof2);
      MinvR21.SetSize(ndof2, ndof1);
      MinvR22.SetSize(ndof2, ndof2);
      M2inv.UseExternalData(Minv.GetData(Trans.Elem2No), ndof2, ndof2);
   ...
   if (ndof2)
   {
      Mult(M1inv, R12, MinvR12);
      Mult(M2inv, R21, MinvR21);
      Mult(M2inv, R22, MinvR22);
