...
static void CeedPADiffusionAssemble(const FiniteElementSpace &fes, CeedDiffData& ceedData)
{
  Ceed ceed(config::GetCeed());
  mfem::Mesh *mesh = fes.GetMesh();
  const int order = fes.GetOrder(0);
  ...
  CeedQFunctionCreateInterior(ceed, 1, f_apply_diff,
                              __FILE__":f_apply_diff", &ceedData.apply_qfunc);
...
void DiffusionIntegrator::MultAssembled(Vector &x, Vector &y)
{
  if (config::UsingCeed())
  {
    CeedDiffData& ceedData = *static_cast<CeedDiffData*>(ceedDataPtr);
    ...
    CeedVectorGetArrayRead(ceedData.v, CEED_MEM_HOST, &array);
    CeedVectorRestoreArrayRead(ceedData.v, &array);
  } else {
...
static void CeedPAMassAssemble(const FiniteElementSpace &fes, CeedMassData& ceedData)
{
  Ceed ceed(config::GetCeed());
  mfem::Mesh *mesh = fes.GetMesh();
  const int order = fes.GetOrder(0);
  ...
  CeedQFunctionCreateInterior(ceed, 1, f_apply_mass,
                              __FILE__":f_apply_mass", &ceedData.apply_qfunc);
...
void MassIntegrator::Assemble(const FiniteElementSpace &fes)
{
    if (config::UsingCeed())
  {
    CeedMassData* ptr = new CeedMassData();
    ceedDataPtr = ptr;
...
void MassIntegrator::MultAssembled(Vector &x, Vector &y)
{
  if (config::UsingCeed())
  {
    CeedMassData& ceedData = *static_cast<CeedMassData*>(ceedDataPtr);
    CeedVectorSetArray(ceedData.u, CEED_MEM_HOST, CEED_USE_POINTER, x.GetData());
    CeedVectorSetArray(ceedData.v, CEED_MEM_HOST, CEED_USE_POINTER, y.GetData());
