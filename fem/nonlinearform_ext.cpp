...
void PANonlinearFormExtension::Update()
{
   ...
   elemR = fes.GetElementRestriction(edf);
   MFEM_VERIFY(elemR, "element restriction is required");
   ...
   PANonlinearFormExtension(nlf, edf_), eaGrad(*this)
{
   ...
   elem_vdofs = fes.GetFE(0)->GetDof() * fes.GetVDim();
...
void EANonlinearFormExtension::Update()
{
   PANonlinearFormExtension::Update();
   ne = fes.GetMesh()->GetNE();
   elem_vdofs = fes.GetFE(0)->GetDof() * fes.GetVDim();
   ea_data.SetSize(ne * elem_vdofs * elem_vdofs,
...
void FANonlinearFormExtension::RAP(OperatorHandle &A) const
{
   ...
   if ( auto pnlf = dynamic_cast<const ParNonlinearForm*>(nlf) )
   {
      ...
      MFEM_ASSERT(pfespace,
                  "parallel nonlinear form needs a parallel finite element space");
      mfem::ParallelRAP(*pfespace, *this->mat, A);
   }
   else
#endif
   {
      // ConformingAssemble deletes and replaces its input matrix when a
      // conforming prolongation is present. Assemble a copy so that mat remains
      // a reusable local-dof matrix and the returned handle owns its operator.
      std::vector<SparseMatrix *> mats_to_assemble =
      {
         new SparseMatrix(*this->mat)
      };
      mfem::ConformingAssemble(this->fes, mats_to_assemble);
      A.Reset(mats_to_assemble.front());
   }
}
   ...
   Operator *&A) const
{
   ...
   handleA.SetOperatorOwner(false);
}
...
void FANonlinearFormExtension::Update()
{
   EANonlinearFormExtension::Update();
   delete mat;
   mat = nullptr;
   faGrad.Update();
}
