...
KDTreeNodalTransfer::KDTreeNodalTransfer(GridFunction& dest_)
{
   dest=&dest_;
...
void KDTreeNodalTransfer::Transfer(Vector& coords, Vector& src,
         ...
         if (flag)
         {
            ...
            if (dist<err)
            {
               if (dest->FESpace()->GetOrdering()==Ordering::byNODES)
               {
                  ...
                  {
                     for (int di=0; di<vd; di++) {(*dest)[di*np+ind]=src[di*np+i];}
                  }
                  else
                  {
                     for (int di=0; di<vd; di++) {(*dest)[di*np+ind]=src[di+i*vd];}
                  }
               }
               else
               {
                  if (ordering==Ordering::byNODES)
                  {
                     for (int di=0; di<vd; di++) {(*dest)[di+ind*vd]=src[di*np+i];}
                  }
                  else
                  {
                     for (int di=0; di<vd; di++) {(*dest)[di+ind*vd]=src[di+i*vd];}
                  }
               }
            }
         }
