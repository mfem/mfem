...
fptype TMOP_Combo_QualityMetric::EvalWMatrixForm(const DenseMatrix &Jpt) const
{
   fptype metric = 0.;
...
fptype TMOP_Combo_QualityMetric::EvalW(const DenseMatrix &Jpt) const
{
   fptype metric = 0.;
   for (int i = 0; i < tmop_q_arr.Size(); i++)
   {
                       ...
                       const TargetConstructor &tc, Vector &weights) const
{
   ...
   const fptype pnm_sum = products_no_m.Sum();
                                                 ...
                                                 Vector &averages) const
{
   ...
   for (int e = 0; e < NE; e++)
   {
      ...
      for (int q = 0; q < nsp; q++)
      {
         ...
         const fptype w_detA = ip.weight * A.Det();
...
fptype TMOP_WorstCaseUntangleOptimizer_Metric::EvalW(const DenseMatrix &Jpt)
const
{
   fptype metric_tilde = EvalWBarrier(Jpt);
   fptype metric = metric_tilde;
   ...
   {
      fptype beta = max_muT+muT_ep;
      metric = metric_tilde/(beta-metric_tilde);
   }
   return metric;
}

fptype TMOP_WorstCaseUntangleOptimizer_Metric::EvalWBarrier(
...
fptype TMOP_Metric_001::EvalW(const DenseMatrix &Jpt) const
{
...
fptype TMOP_Metric_skew2D::EvalW(const DenseMatrix &Jpt) const
{
   ...
   fptype norm_prod = col1.Norml2() * col2.Norml2();
   const fptype cos_Jpr = (col1 * col2) / norm_prod,
                sin_Jpr = fabs(Jpr.Det()) / norm_prod;

   Jtr->GetColumn(0, col1);
   Jtr->GetColumn(1, col2);
   norm_prod = col1.Norml2() * col2.Norml2();
   const fptype cos_Jtr = (col1 * col2) / norm_prod,
                sin_Jtr = fabs(Jtr->Det()) / norm_prod;

   return 0.5 * (1.0 - cos_Jpr * cos_Jtr - sin_Jpr * sin_Jtr);
}

fptype TMOP_Metric_skew3D::EvalW(const DenseMatrix &Jpt) const
{
   MFEM_VERIFY(Jtr != NULL,
               "Requires a target Jacobian, use SetTargetJacobian().");
   ...
   fptype norm_c1 = col1.Norml2(),
          norm_c2 = col2.Norml2(),
          norm_c3 = col3.Norml2();
   fptype cos_Jpr_12 = (col1 * col2) / (norm_c1 * norm_c2),
          cos_Jpr_13 = (col1 * col3) / (norm_c1 * norm_c3),
          cos_Jpr_23 = (col2 * col3) / (norm_c2 * norm_c3);
   fptype sin_Jpr_12 = std::sqrt(1.0 - cos_Jpr_12 * cos_Jpr_12),
   ...
   norm_c2 = col2.Norml2(),
   norm_c3 = col3.Norml2();
   fptype cos_Jtr_12 = (col1 * col2) / (norm_c1 * norm_c2),
          cos_Jtr_13 = (col1 * col3) / (norm_c1 * norm_c3),
          cos_Jtr_23 = (col2 * col3) / (norm_c2 * norm_c3);
   fptype sin_Jtr_12 = std::sqrt(1.0 - cos_Jtr_12 * cos_Jtr_12),
...
fptype TMOP_Metric_aspratio2D::EvalW(const DenseMatrix &Jpt) const
{
   MFEM_VERIFY(Jtr != NULL,
               "Requires a target Jacobian, use SetTargetJacobian().");
   ...
   Jpr.GetColumn(0, col1);
   Jpr.GetColumn(1, col2);
   const fptype ratio_Jpr = col2.Norml2() / col1.Norml2();

   Jtr->GetColumn(0, col1);
   Jtr->GetColumn(1, col2);
   const fptype ratio_Jtr = col2.Norml2() / col1.Norml2();

   return 0.5 * (ratio_Jpr / ratio_Jtr + ratio_Jtr / ratio_Jpr) - 1.0;
}

fptype TMOP_Metric_aspratio3D::EvalW(const DenseMatrix &Jpt) const
{
   MFEM_VERIFY(Jtr != NULL,
               "Requires a target Jacobian, use SetTargetJacobian().");
   ...
   fptype norm_c1 = col1.Norml2(),
          norm_c2 = col2.Norml2(),
          norm_c3 = col3.Norml2();
   fptype ratio_Jpr_1 = norm_c1 / std::sqrt(norm_c2 * norm_c3),
   ...
   norm_c2 = col2.Norml2();
   norm_c3 = col3.Norml2();
   fptype ratio_Jtr_1 = norm_c1 / std::sqrt(norm_c2 * norm_c3),
...
fptype TMOP_Metric_002::EvalWMatrixForm(const DenseMatrix &Jpt) const
{
   return 0.5 * Jpt.FNorm2() / Jpt.Det() - 1.0;
}

fptype TMOP_Metric_002::EvalW(const DenseMatrix &Jpt) const
{
   ie.SetJacobian(Jpt.GetData());
...
fptype TMOP_Metric_004::EvalW(const DenseMatrix &Jpt) const
{
   ie.SetJacobian(Jpt.GetData());
...
fptype TMOP_Metric_007::EvalW(const DenseMatrix &Jpt) const
{
...
fptype TMOP_Metric_009::EvalW(const DenseMatrix &Jpt) const
{
...
fptype TMOP_Metric_022::EvalW(const DenseMatrix &Jpt) const
{
...
void TMOP_Metric_022::EvalP(const DenseMatrix &Jpt, DenseMatrix &P) const
{
   ...
   ie.SetJacobian(Jpt.GetData());
   const fptype c1 = 1.0/(ie.Get_I2b() - min_detT);
                                ...
                                DenseMatrix &A) const
{
   ...
   ie.SetDerivativeMatrix(DS.Height(), DS.GetData());
   const fptype c1 = 1.0/(ie.Get_I2b() - min_detT);
   const fptype c2 = weight*c1/2;
   const fptype c3 = c1*c2;
   const fptype c4 = (2*min_detT - ie.Get_I1())*c3; // weight*z
...
fptype TMOP_Metric_050::EvalWMatrixForm(const DenseMatrix &Jpt) const
{
...
fptype TMOP_Metric_050::EvalW(const DenseMatrix &Jpt) const
{
   // mu_50 = 0.5*|J^t J|^2/det(J)^2 - 1
...
fptype TMOP_Metric_055::EvalW(const DenseMatrix &Jpt) const
{
   // mu_55 = (det(J) - 1)^2 = (I2b - 1)^2
   ie.SetJacobian(Jpt.GetData());
   const fptype c1 = ie.Get_I2b() - 1.0;
   return c1*c1;
}
...
fptype TMOP_Metric_056::EvalWMatrixForm(const DenseMatrix &Jpt) const
{
   // mu_56 = 0.5 (det(J) + 1 / det(J)) - 1.
   const fptype d = Jpt.Det();
   return 0.5 * (d + 1.0 / d) - 1.0;
}

fptype TMOP_Metric_056::EvalW(const DenseMatrix &Jpt) const
{
...
fptype TMOP_Metric_058::EvalWMatrixForm(const DenseMatrix &Jpt) const
{
...
fptype TMOP_Metric_058::EvalW(const DenseMatrix &Jpt) const
{
   // mu_58 = I1b*(I1b - 2)
   ie.SetJacobian(Jpt.GetData());
   const fptype I1b = ie.Get_I1b();
...
fptype TMOP_Metric_077::EvalWMatrixForm(const DenseMatrix &Jpt) const
{
   // mu_77 = 0.5 (det(J)^2 + 1 / det(J)^2) - 1.
   const fptype d = Jpt.Det();
   return 0.5 * (d*d + 1.0/(d*d)) - 1.0;
}
fptype TMOP_Metric_077::EvalW(const DenseMatrix &Jpt) const
{
...
fptype TMOP_Metric_085::EvalW(const DenseMatrix &Jpt) const
{
   MFEM_VERIFY(Jtr != NULL,
               "Requires a target Jacobian, use SetTargetJacobian().");
...
// mu_98 = 1/(tau)|T-I|^2
fptype TMOP_Metric_098::EvalW(const DenseMatrix &Jpt) const
{
   MFEM_VERIFY(Jtr != NULL,
               "Requires a target Jacobian, use SetTargetJacobian().");
...
fptype TMOP_Metric_211::EvalW(const DenseMatrix &Jpt) const
{
...
fptype TMOP_Metric_252::EvalW(const DenseMatrix &Jpt) const
{
...
void TMOP_Metric_252::EvalP(const DenseMatrix &Jpt, DenseMatrix &P) const
{
   ...
   ie.SetJacobian(Jpt.GetData());
   const fptype I2b = ie.Get_I2b();
   const fptype c = (I2b - 1.0)/(I2b - tau0);
                                ...
                                DenseMatrix &A) const
{
   ...
   ie.SetDerivativeMatrix(DS.Height(), DS.GetData());
   const fptype I2b = ie.Get_I2b();
   const fptype c0 = 1.0/(I2b - tau0);
   const fptype c = c0*(I2b - 1.0);
   ie.Assemble_TProd(weight*c0*(1.0 - c)*(1.0 - c), ie.Get_dI2b(), A.GetData());
   ie.Assemble_ddI2b(weight*(c - 0.5*c*c), A.GetData());
}

fptype TMOP_Metric_301::EvalWMatrixForm(const DenseMatrix &Jpt) const
{
   // mu_301 = 1/3 |J| |J^-1| - 1.
   ie.SetJacobian(Jpt.GetData());
   DenseMatrix inv(3);
...
fptype TMOP_Metric_301::EvalW(const DenseMatrix &Jpt) const
{
...
void TMOP_Metric_301::EvalP(const DenseMatrix &Jpt, DenseMatrix &P) const
{
   ...
   ie.SetJacobian(Jpt.GetData());
   const fptype a = 1./(6.*std::sqrt(ie.Get_I1b()*ie.Get_I2b()));
                                ...
                                DenseMatrix &A) const
{
   ...
   const fptype I1b_I2b = ie.Get_I1b()*ie.Get_I2b();
   const fptype a = weight/(6*std::sqrt(I1b_I2b));
...
fptype TMOP_Metric_302::EvalWMatrixForm(const DenseMatrix &Jpt) const
{
...
fptype TMOP_Metric_302::EvalW(const DenseMatrix &Jpt) const
{
   // mu_2 = |J|^2 |J^{-1}|^2 / 9 - 1
...
fptype TMOP_Metric_303::EvalWMatrixForm(const DenseMatrix &Jpt) const
{
...
fptype TMOP_Metric_303::EvalW(const DenseMatrix &Jpt) const
{
...
fptype TMOP_Metric_304::EvalWMatrixForm(const DenseMatrix &Jpt) const
{
   // mu_304 = |J|^3 / 3^(3/2) / det(J) - 1
   const fptype fnorm = Jpt.FNorm();
   return fnorm * fnorm * fnorm / pow(3.0, 1.5) / Jpt.Det() - 1.0;
}

fptype TMOP_Metric_304::EvalW(const DenseMatrix &Jpt) const
{
   // mu_304 = (I1b/3)^3/2 - 1.
   ie.SetJacobian(Jpt.GetData());
...
fptype TMOP_Metric_311::EvalW(const DenseMatrix &Jpt) const
{
   // mu_311 = (det(J) - 1)^2 - det(J) + (det(J)^2 + eps)^{1/2}
   //        = (I3b - 1)^2 - I3b + sqrt(I3b^2 + eps)
   ie.SetJacobian(Jpt.GetData());
   const fptype I3b = ie.Get_I3b();
...
void TMOP_Metric_311::EvalP(const DenseMatrix &Jpt, DenseMatrix &P) const
{
   ie.SetJacobian(Jpt.GetData());
   const fptype I3b = ie.Get_I3b();
   const fptype c = 2*I3b-3+(I3b)/(std::pow((I3b*I3b+eps),0.5));
   P.Set(c, ie.Get_dI3b());
}
                                ...
                                DenseMatrix &A) const
{
   ...
   ie.SetDerivativeMatrix(DS.Height(), DS.GetData());
   const fptype I3b = ie.Get_I3b();
   const fptype c0 = I3b*I3b+eps;
   const fptype c1 = 2 + 1/(pow(c0,0.5)) - I3b*I3b/(pow(c0,1.5));
   const fptype c2 = 2*I3b - 3 + I3b/(pow(c0,0.5));
   ie.Assemble_TProd(weight*c1, ie.Get_dI3b(), A.GetData());
   ie.Assemble_ddI3b(c2*weight, A.GetData());
}

fptype TMOP_Metric_313::EvalW(const DenseMatrix &Jpt) const
{
   ie.SetJacobian(Jpt.GetData());

   const fptype I3b = ie.Get_I3b();
   fptype d = I3b - min_detT;
   if (d < 0.0 && min_detT == 0.0)
   {
      // The mesh has been untangled, but it's still possible to get negative
      // detJ in FD calculations, as they move the nodes around with some small
   ...
   const fptype c = std::pow(d, -2.0/3.0);
...
fptype TMOP_Metric_315::EvalW(const DenseMatrix &Jpt) const
{
   // mu_315 = mu_15_3D = (det(J) - 1)^2
   ie.SetJacobian(Jpt.GetData());
   const fptype c1 = ie.Get_I3b() - 1.0;
   return c1*c1;
}
...
fptype TMOP_Metric_316::EvalWMatrixForm(const DenseMatrix &Jpt) const
{
   // mu_316 = 0.5 (det(J) + 1/det(J)) - 1.
   return 0.5 * (Jpt.Det() + 1.0 / Jpt.Det()) - 1.0;
}

fptype TMOP_Metric_316::EvalW(const DenseMatrix &Jpt) const
{
...
fptype TMOP_Metric_318::EvalWMatrixForm(const DenseMatrix &Jpt) const
{
...
fptype TMOP_Metric_318::EvalW(const DenseMatrix &Jpt) const
{
...
fptype TMOP_Metric_321::EvalWMatrixForm(const DenseMatrix &Jpt) const
{
   // mu_321 = |J - J^-t|^2.
   ie.SetJacobian(Jpt.GetData());
   DenseMatrix invt(3);
...
fptype TMOP_Metric_321::EvalW(const DenseMatrix &Jpt) const
{
                                ...
                                DenseMatrix &A) const
{
   ...
   ie.SetJacobian(Jpt.GetData());
   ie.SetDerivativeMatrix(DS.Height(), DS.GetData());
   const fptype c0 = 1.0/ie.Get_I3b();
   const fptype c1 = weight*c0*c0;
   const fptype c2 = -2*c0*c1;
   const fptype c3 = c2*ie.Get_I2();
   ie.Assemble_ddI1(weight, A.GetData());
...
fptype TMOP_Metric_322::EvalWMatrixForm(const DenseMatrix &Jpt) const
{
...
fptype TMOP_Metric_322::EvalW(const DenseMatrix &Jpt) const
{
   // mu_322 = 1 / (6 det(J)) |J - adj(J)^t|^2
                                ...
                                const fptype weight, DenseMatrix &A) const
{
   ...
   ie.SetJacobian(Jpt.GetData());
   ie.SetDerivativeMatrix(DS.Height(), DS.GetData());
   const fptype p13 = weight * pow(ie.Get_I3b(),  1.0/3.0),
...
fptype TMOP_Metric_323::EvalWMatrixForm(const DenseMatrix &Jpt) const
{
   // mu_323 = |J|^3 - 3 sqrt(3) ln(det(J)) - 3 sqrt(3).
   fptype fnorm = Jpt.FNorm();
   return fnorm * fnorm * fnorm - 3.0 * sqrt(3.0) * (log(Jpt.Det()) + 1.0);
}

fptype TMOP_Metric_323::EvalW(const DenseMatrix &Jpt) const
{
...
fptype TMOP_Metric_352::EvalW(const DenseMatrix &Jpt) const
{
...
void TMOP_Metric_352::EvalP(const DenseMatrix &Jpt, DenseMatrix &P) const
{
   ...
   ie.SetJacobian(Jpt.GetData());
   const fptype I3b = ie.Get_I3b();
   const fptype c = (I3b - 1.0)/(I3b - tau0);
                                ...
                                DenseMatrix &A) const
{
   ...
   ie.SetDerivativeMatrix(DS.Height(), DS.GetData());
   const fptype I3b = ie.Get_I3b();
   const fptype c0 = 1.0/(I3b - tau0);
   const fptype c = c0*(I3b - 1.0);
...
fptype TMOP_Metric_360::EvalWMatrixForm(const DenseMatrix &Jpt) const
{
...
fptype TMOP_Metric_360::EvalW(const DenseMatrix &Jpt) const
{
   // mu_360 = (I1/3)^(3/2) - I3b.
   ie.SetJacobian(Jpt.GetData());
...
fptype TMOP_AMetric_011::EvalW(const DenseMatrix &Jpt) const
{
   MFEM_VERIFY(Jtr != NULL,
               "Requires a target Jacobian, use SetTargetJacobian().");
...
fptype TMOP_AMetric_014a::EvalW(const DenseMatrix &Jpt) const
{
   MFEM_VERIFY(Jtr != NULL,
               "Requires a target Jacobian, use SetTargetJacobian().");
   ...
   fptype sqalpha = pow(Jpr.Det(), 0.5),
          sqomega = pow(Jtr->Det(), 0.5);

   return 0.5*pow(sqalpha/sqomega - sqomega/sqalpha, 2.);
}

fptype TMOP_AMetric_036::EvalW(const DenseMatrix &Jpt) const
{
   MFEM_VERIFY(Jtr != NULL,
               "Requires a target Jacobian, use SetTargetJacobian().");
   ...
   fptype alpha = Jpr.Det(); // det(A)
   Jpr -= *Jtr; // A-W

   return (1./alpha)*(Jpr.FNorm2()); //(1/alpha)*(|A-W|^2)
}

fptype TMOP_AMetric_107a::EvalW(const DenseMatrix &Jpt) const
{
   MFEM_VERIFY(Jtr != NULL,
               "Requires a target Jacobian, use SetTargetJacobian().");
...
static inline void device_copy(fptype *d_dest, const fptype *d_src, int size)
{
         ...
         for (int q = 0; q < nqp; q++)
         {
            ...
            if (aspectratioidx != -1) // Set aspect ratio
            {
               if (dim == 2)
               {
                  ...
                  const fptype min_size = par_vals.Min();
                  MFEM_VERIFY(min_size > 0.0,
                              "Non-positive aspect-ratio propagated in the target definition.");

                  const fptype aspectratio = shape * par_vals;
                  D_rho = 0.;
               ...
               else
               {
                  ...
                  const fptype rho1 = shape * par_vals_c1;
                  const fptype rho2 = shape * par_vals_c2;
                  const fptype rho3 = shape * par_vals_c3;
                  D_rho = 0.;
            ...
            if (skewidx != -1) // Set skew
            {
               if (dim == 2)
               {
                  ...
                  const fptype skew = shape * par_vals;
               ...
               else
               {
                  ...
                  const fptype phi12  = shape * par_vals_c1;
                  const fptype phi13  = shape * par_vals_c2;
                  const fptype chi = shape * par_vals_c3;
            ...
            if (orientationidx != -1) // Set orientation
            {
               if (dim == 2)
               {
                  ...
                  const fptype theta = shape * par_vals;
               ...
               else
               {
                  ...
                  const fptype theta = shape * par_vals_c1;
                  const fptype psi   = shape * par_vals_c2;
                  const fptype beta  = shape * par_vals_c3;

                  fptype ct = cos(theta), st = sin(theta),
      ...
      case GIVEN_SHAPE_AND_SIZE:
      {
         ...
         for (int i = 0; i < ir.GetNPoints(); i++)
         {
            ...
            if (sizeidx != -1) // Set size
            {
               ...
               MFEM_VERIFY(min_size > 0.0,
                           "Non-positive size propagated in the target definition.");
               const fptype size = std::max(shape * par_vals, min_size);
               fptype dz_dsize = (1./dim)*pow(size, 1./dim - 1.);
            ...
            if (orientationidx != -1) // Set orientation
            {
               ...
               else
               {
                  ...
                  const fptype ct = cos(theta), st = sin(theta),
                               cp = cos(psi),   sp = sin(psi),
                               cb = cos(beta),  sb = sin(beta);
...
UpdateHessianTargetSpecification(const Vector &x,fptype dx,
...
fptype TMOP_Integrator::GetElementEnergy(const FiniteElement &el,
                                         ElementTransformation &T,
                                         const Vector &elfun)
{
   ...
   for (int i = 0; i < ir.GetNPoints(); i++)
   {
      ...
      fptype val = metric_normal * metric->EvalW(Jpt);
      ...
      if (adaptive_limiting)
      {
         const fptype diff = adapt_lim_gf_q(i) - adapt_lim_gf0_q(i);
...
fptype TMOP_Integrator::GetRefinementElementEnergy(const FiniteElement &el,
                                                   ElementTransformation &T,
                                                   const Vector &elfun,
                                                   const IntegrationRule &irule)
{
   ...
   for (int e = 0; e < NEsplit; e++)
   {
      ...
      for (int i = 0; i < ir.GetNPoints(); i++)
      {
         ...
         fptype val = metric_normal * h_metric->EvalW(Jpt);
         if (metric_coeff) { val *= metric_coeff->Eval(*Tpr, ip); }
...
fptype TMOP_Integrator::GetDerefinementElementEnergy(const FiniteElement &el,
                                                     ElementTransformation &T,
                                                     const Vector &elfun)
{
   int dof = el.GetDof(), dim = el.GetDim();
                                                 ...
                                                 Vector &elvect)
{
   ...
   for (int q = 0; q < nqp; q++)
   {
      ...
      fptype weight_m = weights(q) * metric_normal;
                                               ...
                                               DenseMatrix &elmat)
{
   ...
   for (int q = 0; q < nqp; q++)
   {
      ...
      if (lim_coeff)
      {
         ...
         for (int i = 0; i < dof; i++)
         {
            const fptype w_shape_i = weight_m * shape(i);
            for (int j = 0; j < dof; j++)
            {
               const fptype w = w_shape_i * shape(j);
                                               ...
                                               DenseMatrix &mat)
{
   ...
   for (int q = 0; q < nqp; q++)
   {
      ...
      const fptype w = weights(q) * lim_normal * adapt_lim_coeff->Eval(Tpr, ip);
                                             ...
                                             DenseMatrix &mat)
{
   ...
   for (int s = 0; s < dof_s; s++)
   {
      ...
      Tpr.SetIntPoint(&ip);
      fptype w = surf_fit_normal * surf_fit_coeff->Eval(Tpr, ip) *
                                              ...
                                              DenseMatrix &mat)
{
   ...
   for (int s = 0; s < dof_s; s++)
   {
      ...
      for (int idim = 0; idim < dim; idim++)
      {
         ...
         {
            fptype entry = w * ( surf_fit_grad_e(s, idim) *
...
fptype TMOP_Integrator::GetFDDerivative(const FiniteElement &el,
                                        ElementTransformation &T,
                                        Vector &elfun, const int dofidx,
                                        const int dir, const fptype e_fx,
                                        bool update_stored)
{
   int dof = el.GetDof();
   int idx = dir*dof+dofidx;
   elfun[idx]    += dx;
   fptype e_fxph  = GetElementEnergy(el, T, elfun);
   elfun[idx]    -= dx;
   fptype dfdx    = (e_fxph-e_fx)/dx;
                                              ...
                                              Vector &elvect)
{
   ...
   const fptype e_fx = GetElementEnergy(el, T, elfun);
      ...
      for (int j = 0; j < i+1; j++)
      {
         for (int k1 = 0; k1 < dim; k1++)
         {
            for (int k2 = 0; k2 < dim; k2++)
            {
               ...
               fptype e_fx   = ElemPertLoc(k2*dof+j);
               fptype e_fpxph = GetFDDerivative(el, T, elfunmod, i, k1, e_fx,
                                                false);
               elfunmod(k2*dof+j) -= dx;
               fptype e_fpx = ElemDerLoc(k1*dof+i);
...
void TMOP_Integrator::UpdateSurfaceFittingWeight(fptype factor)
{
...
fptype TMOP_Integrator::GetSurfaceFittingWeight()
{
   if (surf_fit_coeff)
   {
      auto cf = dynamic_cast<ConstantCoefficient *>(surf_fit_coeff);
                                    ...
                                    const FiniteElementSpace &fes)
{
   ...
   fptype detv_sum;
   fptype detv_avg_min = std::numeric_limits<float>::max();
   for (int i = 0; i < NE; i++)
   {
      ...
      fptype detv_avg = pow(detv_sum/nsp, 1./dim);
...
fptype TMOP_Integrator::ComputeMinDetT(const Vector &x,
                                       const FiniteElementSpace &fes)
{
   fptype min_detT = std::numeric_limits<fptype>::infinity();
...
fptype TMOP_Integrator::ComputeUntanglerMaxMuBarrier(const Vector &x,
                                                     const FiniteElementSpace &fes)
{
   fptype max_muT = -std::numeric_limits<fptype>::infinity();
   const int NE = fes.GetMesh()->GetNE();
   const int dim = fes.GetMesh()->Dimension();
   Array<int> xdofs;
   Jpr.SetSize(dim);
                                                     ...
                                                     const FiniteElementSpace &fes)
{
   ...
   fptype max_muT = ComputeUntanglerMaxMuBarrier(x, fes);
   fptype max_muT_all = max_muT;
#ifdef MFEM_USE_MPI
   if (pfes)
   {
...
fptype TMOPComboIntegrator::GetElementEnergy(const FiniteElement &el,
                                             ElementTransformation &T,
                                             const Vector &elfun)
{
   fptype energy= 0.0;
...
fptype TMOPComboIntegrator::GetRefinementElementEnergy(const FiniteElement &el,
                                                       ElementTransformation &T,
                                                       const Vector &elfun,
                                                       const IntegrationRule &irule)
{
...
fptype TMOPComboIntegrator::GetDerefinementElementEnergy(
...
fptype TMOPComboIntegrator::GetLocalStateEnergyPA(const Vector &xe) const
{
   fptype energy = 0.0;
   for (int i = 0; i < tmopi.Size(); i++)
   {
