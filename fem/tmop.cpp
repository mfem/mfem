...
template <typename type>
type mu313_ad(const std::vector<type> &T, const std::vector<type> &W,
              real_t tau0)
{
   auto detT = det_3D(T);
   return (1.0/3.0)*fnorm2_3D(T)*pow(abs(detT-tau0), -2.0/3.0);
};

// mu_303 = |J|^2 / 3 / det(J)^(2/3) - 1 = I1b/3 - 1
template <typename type>
type mu303_ad(const std::vector<type> &T, const std::vector<type> &W)
{
   auto fnorm2 = fnorm2_3D(T);
   auto det = det_3D(T);
   return fnorm2 / 3.0 / pow(det, 2.0/3.0) - 1.0;
};
...
void ADGradB(std::function<AD1Type(std::vector<AD1Type>&,
                                   std::vector<AD1Type>&,
                                   real_t&)>mu_ad,
             DenseMatrix &dmu, //output
             const DenseMatrix &X, // parameter 1
             ...
             const bool dX = true /*derivative with respect to X*/)
{
   ...
   if (dX)
   {
      ...
      {
         adX[i] = AD1Type{X.GetData()[i], 1.0};
         AD1Type rez = mu_ad(adX, adY, min_detT);
         dmu.GetData()[i] = rez.gradient;
         adX[i] = AD1Type{X.GetData()[i], 0.0};
      }
   }
...
// Given mu(X,Y), compute d2mu/dX2, where Y is an optional parameter.
void ADHessianB(std::function<AD2Type(std::vector<AD2Type>&,
                                      std::vector<AD2Type>&,
                                      real_t&)> mu_ad,
                DenseTensor &d2mu_dX2,
                const DenseMatrix &X,
                ...
                real_t min_detT=0.0)
{
   ...
   for (int ii = 0; ii < matsize; ii++)
   {
      ...
      {
         aduu[jj].gradient = AD1Type{1.0, 0.0};
         AD2Type rez = mu_ad(aduu, adY, min_detT);
         d2mu_dX2(ii).GetData()[jj] = rez.gradient.gradient;
         d2mu_dX2(jj).GetData()[ii] = rez.gradient.gradient;
         aduu[jj].gradient = AD1Type{0.0, 0.0};
      }
...
const
{
   ...
   {
      std::cout << "max_muT = " << max_muT << ", muT_ep = " << muT_ep << std::endl;
                                                   ...
                                                   DenseMatrix &P) const
{
   ...
   if (tmop_metric.Id() == 4 || tmop_metric.Id() == 14 ||
       tmop_metric.Id() == 66 || tmop_metric.Id() == 303)
   {
...
template <typename type>
type TMOP_Metric_303::EvalW_AD_impl(const std::vector<type> &T,
                                    const std::vector<type> &W) const
{
   return mu303_ad(T, W);
}

AD1Type TMOP_Metric_303::EvalW_AD1(const std::vector<AD1Type> &T,
                                   const std::vector<AD1Type> &W) const
{
   return EvalW_AD_impl<AD1Type>(T,W);
}

AD2Type TMOP_Metric_303::EvalW_AD2(const std::vector<AD2Type> &T,
                                   const std::vector<AD2Type> &W) const
{
   return EvalW_AD_impl<AD2Type>(T,W);
}
...
real_t TMOP_Metric_313::EvalW(const DenseMatrix &Jpt) const
{
   ...
   std::vector<AD1Type> T(matsize), W(matsize);
   for (int i=0; i<matsize; i++) { T[i] = AD1Type{Jpt.GetData()[i], 0.0}; }
   return mu313_ad(T, W, min_detT).value;
}
...
void TMOP_Metric_313::EvalP(const DenseMatrix &Jpt, DenseMatrix &P) const
{
   ...
   std::vector<AD1Type> T(matsize), W(matsize);
   for (int i=0; i<matsize; i++) { T[i] = AD1Type{Jpt.GetData()[i], 0.0}; }
   // std::cout << "mu313 value is " << mu313_ad(T, W, min_detT).value << " " << min_detT << std::endl;
   ADGradB(mu313_ad<AD1Type>, P, Jpt, nullptr, min_detT);
   return;
}
                                ...
                                DenseMatrix &A) const
{
   ...
   const int dim = Jpt.Height();
   DenseTensor H(dim, dim, dim*dim); H = 0.0;
   ADHessianB(mu313_ad<AD2Type>, H, Jpt, nullptr, min_detT);
   this->DefaultAssembleH(H,DS,weight,A);
}
