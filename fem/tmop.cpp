...
void AutomaticTC::ComputeAllElementTargets(const FiniteElementSpace &fes,
                                           const IntegrationRule &ir,
                                           const Vector &xe,
                                           DenseTensor &Jtr) const
{
                              ...
                              const FiniteElementSpace &x_fes)
{
   ...
   if (auto_tc)
   {
       SetAutomaticTCParameters(x_new, x_fes);
   }
...
void TMOP_Integrator::ComputeMeanGeometricParameters(const Vector &xe,
                                                     const FiniteElementSpace &fes,
                                                     Vector &AvgParameters)
{
   ...
   for (int e = 0; e < NE; e++)
   {
      ...
      for (int q = 0; q < nqp; q++)
      {
         ...
         double detJ_q;
         Vector aspr_q(1), skew_q(1), ori_q(1);

         fes.GetMesh()->GetGeometricParametersFromJacobian(Jpr, detJ_q, aspr_q,
                                                         skew_q, ori_q);
...
void TMOP_Integrator::SetAutomaticTCParameters(const Vector &xe,
                                               const FiniteElementSpace &fes)
{
    MFEM_VERIFY(auto_tc, "Automatic TC not set\n");
...
//   auto_tc->SetTargetAspectRatio(AvgParameters(3)/AvgParameters(4));
   ...
   if (myrank == 0) {
       mfem::out << " " << AvgParametersGlob(0)/AvgParametersGlob(4) << " " <<
                           AvgParametersGlob(1)/AvgParametersGlob(4) << " " <<
                           AvgParametersGlob(2)/AvgParametersGlob(4) << " " <<
                    AvgParametersGlob(3)/AvgParametersGlob(4) << " k10tc\n";
   }
}
