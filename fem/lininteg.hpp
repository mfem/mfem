...
public:
   VectorBoundaryFluxLFIntegrator(Coefficient &f, fptype s = 1.0,
                                  const IntegrationRule *ir = NULL)
...
public:
   DGDirichletLFIntegrator(Coefficient &u, const fptype s, const fptype k)
      : uD(&u), Q(NULL), MQ(NULL), sigma(s), kappa(k) { }
   DGDirichletLFIntegrator(Coefficient &u, Coefficient &q,
                           const fptype s, const fptype k)
...
class WhiteGaussianNoiseDomainLFIntegrator : public LinearFormIntegrator
{
   ...
   std::normal_distribution<fptype> dist;
