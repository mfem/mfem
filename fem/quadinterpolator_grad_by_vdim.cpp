   ...
   const Vector &e_vec, Vector &q_der) const
{
   ...
   switch (id)
   {
      case 0x2134: return Grad2D<L,P,1,3,4,8>(NE,B,G,J,X,Y);
      case 0x2146: return Grad2D<L,P,1,4,6,4>(NE,B,G,J,X,Y);
      case 0x2158: return Grad2D<L,P,1,5,8,2>(NE,B,G,J,X,Y);

      case 0x2234: return Grad2D<L,P,2,3,4,8>(NE,B,G,J,X,Y);
      case 0x2246: return Grad2D<L,P,2,4,6,4>(NE,B,G,J,X,Y);
      case 0x2258: return Grad2D<L,P,2,5,8,2>(NE,B,G,J,X,Y);

      case 0x3134: return Grad3D<L,P,1,3,4>(NE,B,G,J,X,Y);
      case 0x3146: return Grad3D<L,P,1,4,6>(NE,B,G,J,X,Y);
      case 0x3158: return Grad3D<L,P,1,5,8>(NE,B,G,J,X,Y);

      case 0x3334: return Grad3D<L,P,3,3,4>(NE,B,G,J,X,Y);
      case 0x3346: return Grad3D<L,P,3,4,6>(NE,B,G,J,X,Y);
      case 0x3358: return Grad3D<L,P,3,5,8>(NE,B,G,J,X,Y);
      default:
      {
         constexpr int MD1 = 8;
         constexpr int MQ1 = 8;
         ...
         if (dim == 2) { Grad2D<L,P,0,0,0,0,MD1,MQ1>(NE,B,G,J,X,Y,vdim,D1D,Q1D); }
         if (dim == 3) { Grad3D<L,P,0,0,0,MD1,MQ1>(NE,B,G,J,X,Y,vdim,D1D,Q1D); }
         return;
      }
   }
