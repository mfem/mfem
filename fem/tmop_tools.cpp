                                              ...
                                              const Vector &b) const
{
   ...
   for (int i = 0; i < 12; i++)
   {
      ...
      // {
      //    double min_detJ_bdr = ComputeMinDetBdr(d_loc, *fes);
...
real_t TMOPNewtonSolver::ComputeMinDetBdr(const Vector &d_loc,
                                          const FiniteElementSpace &fes) const
{
   const AnalyticCompositeSurface *surf = nullptr;
   if (parametric)
   ...
   for (int be = 0; be < fes.GetNBE(); be++)
   {
      ...
      x_0.GetElementDofValues(be, posV);
      if (periodic)
      {
         auto n_el = dynamic_cast<const NodalFiniteElement *>(fes.GetFE(be));
         n_el->ReorderLexToNative(dim, posV);
      }
