...
void BatchedLOR_H1::Assemble3D()
{
   ...
   else
   {
      ...
      MFEM_FORALL_3D(iel_ho, nel_ho, 2, 2, 2,
      {
         ...
         // Compute geometric factors at quadrature point
         //MFEM_FOREACH_THREAD(kz,z,ORDER)
         //for (int kz=0; kz<ORDER; ++kz)
         {
            const int kz = 0;
            //if (MFEM_THREAD_ID(x) > 0) return;
            //if (MFEM_THREAD_ID(y) > 0) return;
            //if (MFEM_THREAD_ID(z) > 0) return;
            //MFEM_FOREACH_THREAD(ky,y,ORDER)
