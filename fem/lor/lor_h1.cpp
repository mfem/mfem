...
void BatchedLOR_H1::Assemble3D()
{
   ...
   else if (ORDER == 1) ////////////////////////////////////////////////////////
   {
      ...
      assert(ORDER == 1 && nd1d == ORDER + 1);
      MFEM_FORALL_3D(iel_ho, nel_ho, 8, 4, 1, // 841 => 6.61128/s MDof/s
      {
   ...
   else ////////////////////////////////////////////////////////////////////////
   {
      ...
      MFEM_FORALL_3D(iel_ho, nel_ho, 2, 2, 2,
      {
         ...
         // Compute geometric factors at quadrature point, ORDER == 1 here
         const int kz = 0, ky = 0, kx = 0;
         ...
         /// Filling shared Q
         MFEM_SHARED double Q_[(ddm2 + 1)*nv];
         DeviceTensor<4> Q(Q_, ddm2 + 1, 2, 2, 2);
         MFEM_FOREACH_THREAD(iqz,z,2)
         {
            ...
            MFEM_FOREACH_THREAD(iy,y,2)
            {
               MFEM_FOREACH_THREAD(ix,x,2)
               {
                  ...
                  for (int jj_loc=0; jj_loc<nv; ++jj_loc)
                  {
                     ...
                     if (jj_loc <= ii_loc)
                     {
                        AtomicAdd(V(jj_off, ix+kx, iy+ky, iz+kz, iel_ho), local_mat(iz,iy,ix, jj_loc));
                     }
                     else
                     {
                        AtomicAdd(V(jj_off, ix+kx, iy+ky, iz+kz, iel_ho), local_mat(jz,jy,jx, ii_loc));
                     }
                  }
               }
            }
...
#warning Explicit template instantiations
