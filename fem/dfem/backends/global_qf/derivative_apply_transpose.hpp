...
#include "../local_qf/kernels_lo.hpp"
#include "../local_qf/kernels_ho.hpp"
#include "../local_qf/util.hpp"

#include <array>

// Transposed cached Jacobian apply (Jᵀ·w) for the global (tensor_array)
// q-function backend.
//
// Unlike the local backend, the global backend's q-functions use runtime-shaped
// tensor_array parameters, which are incompatible with the register-based
// tensor-product driver used by LocalQFImpl::DerivativeApplyTranspose (that
// driver needs the q-function's compile-time tensor shapes). This
// implementation is q-function-shape agnostic: it contracts the same qp_cache
// produced by DerivativeSetup using the field-operator metadata only, then maps
// the result back to the trial dofs with the quadrature-data helpers. It is a
// faithful transpose (not the forward action), so it is correct for
// non-symmetric operators as well.
