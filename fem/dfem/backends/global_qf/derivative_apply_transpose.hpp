...
// Q-function-shape-agnostic cached Jacobian transpose apply (Jᵀ·w)
...
struct DerivativeApplyTranspose
{
      ...
      qp_cache(qp_cache)
   {
      ...
      create_fieldbases(this->inputs,  input_to_infd,  ctx.infds,  ctx.ir,
                        input_bases);
      create_fieldbases(this->outputs, output_to_outfd, ctx.outfds, ctx.ir,
                        output_bases);
      ...
      {
         dir_q_offsets[i + 1] = dir_q_offsets[i] +
                                get<i>(this->outputs).size_on_qp * nqp * ne;
      });
      ...
      {
         result_q_offsets[i + 1] = result_q_offsets[i] +
                                   get<i>(this->inputs).size_on_qp * nqp * ne;
      });
      ...
      constexpr auto activity = detail::make_activity_map<derivative_id>(inputs_t{});
      ...
      // Find which input slot corresponds to the derivative field
      deriv_input_slot = SIZE_MAX;
      constexpr_for<0, n_inputs>([&](auto i)
      {
         if (get<i>(this->inputs).GetFieldId() == derivative_id)
         {
            deriv_input_slot = i.value;
         }
      });
      MFEM_ASSERT(deriv_input_slot != SIZE_MAX,
                  "DerivativeApplyTranspose: derivative field not found among inputs");
   }
      ...
      std::vector<Vector *> &ye) const
   {
      ...
      // Re-zero the pre-allocated Q temporaries (memory is already allocated)
      dir_q_local    = 0.0;
      result_q_local = 0.0;
      ...
      // ------------------------------------------------------------------
      // 1. Bring test cotangent (direction_l on outputs) to quadrature points
      // ------------------------------------------------------------------
      pull_output_cotangents_to_q(direction_l, dir_q_local);

      // ------------------------------------------------------------------
      // 2. Contract qp_cache with test directions at quadrature points
      // ------------------------------------------------------------------
      const real_t *cache_ptr = qp_cache.Read();
      const int res_sz = residual_size_on_qp;

      // For each output, we have its direction in dir_q_local.GetBlock(o) at Q
      constexpr_for<0, n_outputs>([&](auto o)
      {
         ...
         constexpr_for<0, n_inputs>([&](auto s)
         {
            ...
            {
               for (int i = 0; i < tv_o; ++i) // test vdim
               {
                  for (int k = 0; k < to_o; ++k) // test op_dim
                  {
                     ...
                     for (int j = 0; j < trial_vdim; ++j)
                     {
                        for (int m = 0; m < total_trial_op_dim; ++m)
                        {
                           ...
                           res_s[(j * total_trial_op_dim + m) + size_s * gq] += c * w;
                        }
                     }
                  }
               }
      ...
      push_result_q_to_trial_e(result_q_local, ye);
   }
...
private:
   ...
   size_t deriv_input_slot = SIZE_MAX;
                                    ...
                                    BlockVector& dir_q) const
   {
      ...
      constexpr_for<0, n_outputs>([&](auto i)
      {
         ...
         dir_out_l_owned[i] = Vector(const_cast<real_t *>(direction_l->GetData()) +
                                     l_offset, l_size);
   ...
   /// Push the computed result at Q (for the derivative trial input) back into
   /// the caller's E-vector using the input FieldBasis transpose.
   void push_result_q_to_trial_e(BlockVector& result_q,
                                 std::vector<Vector*>& ye) const
   {
      result_q.SyncToBlocks();

      input_bases[deriv_input_slot].transpose(
         result_q.GetBlock(deriv_input_slot), *ye[deriv_input_slot]);
   }
};
