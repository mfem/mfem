...
void NURBSExtension::Generate2DElementDofTable()
{
   ...
   el_dof = new Table(NumOfActiveElems, (Order.Max() + 1)*(Order.Max() + 1));
...
void NURBSExtension::Generate2DBdrElementDofTable()
{
   ...
   bel_dof = new Table(NumOfActiveBdrElems, Order.Max() + 1);
