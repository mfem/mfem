...
void MixedMesh::GetElementJacobian(int i, DenseMatrix &J)
{
...
double MixedMesh::GetElementSize(int i, int type)
{
   DenseMatrix J(Dim);
...
double MixedMesh::GetElementSize(int i, const Vector &dir)
{
   DenseMatrix J(Dim);
   Vector d_hat(Dim);
   GetElementJacobian(i, J);
...
double MixedMesh::GetElementVolume(int i)
{
...
void MixedMesh::GetBoundingBox(Vector &min, Vector &max, int ref)
{
...
void MixedMesh::PrintCharacteristics(Vector *Vh, Vector *Vk, std::ostream &out)
{
   double h_min, h_max, kappa_min, kappa_max;

   out << "MixedMesh Characteristics:";
...
FiniteElement *MixedMesh::GetTransformationFEforElementType(int ElemType)
{
   switch (ElemType)
   {
...
// static method
int MixedMesh::CountJaggedArrayEntries(const ElemArray & m)
{
   int count = 0;

   ElemConstIterator it;
...
void MixedMesh::GetTypeAndIndex(const ElemArray & m,
                                int i, Element::Type & type, int & index) const
{
   ElemConstIterator it = m.begin();
...
const Array<Element*> &
MixedMesh::GetArrayAndOffset(const ElemArray & m,
                                         ...
                                         IsoparametricTransformation *ElTr)
{
   ...
   if (Nodes == NULL)
   {
      int       nv = this->GetElement(i)->GetNVertices();
      const int *v = this->GetElement(i)->GetVertices();
      int n = vertices.Size();
...
ElementTransformation *MixedMesh::GetElementTransformation(int i)
{
...
ElementTransformation *MixedMesh::GetBdrElementTransformation(int i)
{
...
ElementTransformation *MixedMesh::GetFaceTransformation(int FaceNo)
{
                                      ...
                                      IsoparametricTransformation *EdTr)
{
   ...
   if (Dim == 1)
   {
      mfem_error("MixedMesh::GetEdgeTransformation not defined in 1D \n");
   }
...
ElementTransformation *MixedMesh::GetEdgeTransformation(int EdgeNo)
{
...
void MixedMesh::GetLocalPtToSegTransformation(
...
void MixedMesh::GetLocalSegToTriTransformation(
   IsoparametricTransformation &Transf, int i)
{
   const int *tv, *so;
...
void MixedMesh::GetLocalSegToQuadTransformation(
   IsoparametricTransformation &Transf, int i)
{
   const int *qv, *so;
...
void MixedMesh::GetLocalTriToTetTransformation(
   IsoparametricTransformation &Transf, int i)
{
   DenseMatrix &locpm = Transf.GetPointMat();
...
void MixedMesh::GetLocalQuadToHexTransformation(
   IsoparametricTransformation &Transf, int i)
{
   DenseMatrix &locpm = Transf.GetPointMat();
...
void MixedMesh::GetLocalFaceTransformation(
...
bool MixedMesh::IsSlaveFace(const FaceInfo &fi) const
{
...
FaceElementTransformations *MixedMesh::GetBdrFaceTransformations(int BdrElemNo)
{
   FaceElementTransformations *tr;
   int fn;
   if (Dim == 3)
...
void MixedMesh::GetFaceElements(int Face, int *Elem1, int *Elem2)
{
   *Elem1 = faces_info[Face].Elem1No;
   *Elem2 = faces_info[Face].Elem2No;
}

void MixedMesh::GetFaceInfos(int Face, int *Inf1, int *Inf2)
{
   *Inf1 = faces_info[Face].Elem1Inf;
   *Inf2 = faces_info[Face].Elem2Inf;
}

int MixedMesh::GetFaceGeometryType(int Face) const
{
   return (Dim == 1) ? Geometry::POINT : this->GetFace(Face)->GetGeometryType();
}

int MixedMesh::GetFaceElementType(int Face) const
{
   return (Dim == 1) ? Element::POINT : this->GetFace(Face)->GetType();
}
...
void MixedMesh::InitTables()
{
   el_to_edge =
      el_to_face = el_to_el = bel_to_edge = face_edge = edge_vertex = NULL;
}

void MixedMesh::SetEmpty()
{
...
void MixedMesh::DestroyPointers()
{
...
void MixedMesh::AddVertex(const double *x)
{
...
void MixedMesh::AddBdrQuadAsTriangles(const int *vi, int attr)
{
...
#ifdef MFEM_USE_GECKO
void MixedMesh::GetGeckoElementReordering(Array<int> &ordering)
{
   Gecko::Graph graph;
...
void MixedMesh::MarkTriMeshForRefinement()
{
...
void MixedMesh::GetEdgeOrdering(DSTable &v_to_v, Array<int> &order)
{
...
void MixedMesh::MarkTetMeshForRefinement(DSTable &v_to_v)
{
...
void MixedMesh::FinalizeTopology()
{
...
void MixedMesh::Finalize(bool refine, bool fix_orientation)
{
   if (NURBSext || ncmesh)
   {
...
void MixedMesh::PrintElementWithoutAttr(const Element *el, std::ostream &out)
{
...
Element *MixedMesh::ReadElement(std::istream &input)
{
   int attr;
   Element *el;
...
void MixedMesh::PrintElement(const Element *el, std::ostream &out)
{
   out << el->GetAttribute() << ' ';
   PrintElementWithoutAttr(el, out);
}

void MixedMesh::SetMeshGen()
{
   meshgen = 0;
   for (int i = 0; i < NumOfElements; i++)
   {
...
/*
MixedMesh::MixedMesh(Mesh *mesh_array[], int num_pieces)
{
...
*/
MixedMesh::MixedMesh(Mesh *orig_mesh, int ref_factor, int ref_type)
{
...
void MixedMesh::KnotInsert(Array<KnotVector *> &kv)
{
   if (NURBSext == NULL)
   {
      mfem_error("MixedMesh::KnotInsert : Not a NURBS mesh!");
   }

   if (kv.Size() != NURBSext->GetNKV())
   {
      mfem_error("MixedMesh::KnotInsert : KnotVector array size mismatch!");
   }
...
void MixedMesh::NURBSUniformRefinement()
{
...
void MixedMesh::DegreeElevate(int t)
{
   if (NURBSext == NULL)
   {
      mfem_error("MixedMesh::DegreeElevate : Not a NURBS mesh!");
   }
   ...
   if (!nurbs_fec)
   {
      mfem_error("MixedMesh::DegreeElevate");
   }
...
void MixedMesh::GetNodes(GridFunction &nodes) const
{
...
void MixedMesh::SetNodalFESpace(FiniteElementSpace *nfes)
{
   GridFunction *nodes = new GridFunction(nfes);
   SetNodalGridFunction(nodes, true);
}

void MixedMesh::SetNodalGridFunction(GridFunction *nodes, bool make_owner)
{
   GetNodes(*nodes);
   NewNodes(*nodes, make_owner);
}

const FiniteElementSpace *MixedMesh::GetNodalFESpace() const
{
...
int MixedMesh::GetNumFaces() const
{
   switch (Dim)
   {
      case 1: return GetNV();
...
int MixedMesh::CheckElementOrientation(bool fix_it)
{
...
int MixedMesh::GetTriOrientation(const int *base, const int *test)
{
   // Static method.
   ...
   for (int j = 0; j < 3; j++)
      ...
      {
         mfem_error("MixedMesh::GetTriOrientation(...)");
      }
#endif
...
int MixedMesh::GetQuadOrientation(const int *base, const int *test)
{
   int i;
   ...
   for (int j = 0; j < 4; j++)
      if (test[aor[j]] != base[j])
      {
         cerr << "MixedMesh::GetQuadOrientation(...)" << endl;
         cerr << " base = [";
         for (int k = 0; k < 4; k++)
         {
            cerr << " " << base[k];
...
int MixedMesh::CheckBdrElementOrientation(bool fix_it)
{
   int i, wo = 0;
      ...
      for (i = 0; i < NumOfBdrElements; i++)
      {
         if (faces_info[be_to_face[i]].Elem2No < 0)
         {
            ...
            switch (GetElementType(el))
            {
               ...
               default:
                  MFEM_ABORT("Unrecognized 3D elememnt type "
                             << GetElementType(el));
            }
         }
      }
...
void MixedMesh::GetElementEdges(int i, Array<int> &edges, Array<int> &cor) const
{
   if (el_to_edge)
   {
   ...
   {
      mfem_error("MixedMesh::GetElementEdges(...) element to edge table "
                 "is not generated.");
   }
...
void MixedMesh::GetFaceEdges(int i, Array<int> &edges, Array<int> &o) const
{
   if (Dim == 2)
   {
      edges.SetSize(1);
...
void MixedMesh::GetEdgeVertices(int i, Array<int> &vert) const
{
...
Table *MixedMesh::GetFaceEdgeTable() const
{
   if (face_edge)
   {
      return face_edge;
...
#ifdef MFEM_DEBUG
   if (CountFaces() != NumOfFaces)
   {
      mfem_error("MixedMesh::GetFaceEdgeTable : faces were not generated!");
   }
#endif
...
Table *MixedMesh::GetEdgeVertexTable() const
{
   if (edge_vertex)
   {
      return edge_vertex;
...
Table *MixedMesh::GetVertexToElementTable()
{
   int i, j, nv, *v;
...
Table *MixedMesh::GetFaceToElementTable() const
{
...
void MixedMesh::GetElementFaces(int i, Array<int> &fcs, Array<int> &cor)
const
{
   int n, j;
   ...
   {
      mfem_error("MixedMesh::GetElementFaces(...) : el_to_face not generated.");
   }
   ...
   for (j = 0; j < n; j++)
      ...
      {
         mfem_error("MixedMesh::GetElementFaces(...) : 2");
      }
#else
      else
      {
...
void MixedMesh::GetBdrElementFace(int i, int *f, int *o) const
{
   const int *bv, *fv;

   *f = be_to_face[i];
   bv = GetBdrElement(i)->GetVertices();
   fv = GetFace(be_to_face[i])->GetVertices();
   ...
   switch (GetBdrElementType(i))
   {
      ...
      case Element::QUADRILATERAL:
         *o = GetQuadOrientation(fv, bv);
         break;
      default:
         mfem_error("MixedMesh::GetBdrElementFace(...) 2");
   }
}

Geometry::Type MixedMesh::GetFaceBaseGeometry(int i) const
{
   /*
    ...
    switch (GetElementType(0))
    {
       ...
       default:
          mfem_error("MixedMesh::GetFaceBaseGeometry(...) #1");
    }
    return (-1);
   */
   return GetFace(i)->GetGeometryType();
}

int MixedMesh::GetBdrElementEdgeIndex(int i) const
{
   switch (Dim)
   {
                                             ...
                                             int &info) const
{
   ...
   const int *fv = (Dim > 1) ? GetFace(fid)->GetVertices() : NULL;
   const int *bv = GetBdrElement(bdr_el)->GetVertices();
   int ori;
...
Element::Type MixedMesh::GetElementType(int i) const
{
   return this->GetElement(i)->GetType();
}

Element::Type MixedMesh::GetBdrElementType(int i) const
{
   return this->GetBdrElement(i)->GetType();
}

void MixedMesh::GetPointMatrix(int i, DenseMatrix &pointmat) const
{
   int k, j, nv;
   const int *v;

   v  = this->GetElement(i)->GetVertices();
   nv = this->GetElement(i)->GetNVertices();
...
void MixedMesh::GetBdrPointMatrix(int i, DenseMatrix &pointmat) const
{
   int k, j, nv;
   const int *v;

   v  = this->GetBdrElement(i)->GetVertices();
   nv = this->GetBdrElement(i)->GetNVertices();
...
double MixedMesh::GetLength(int i, int j) const
{
...
// static method
void MixedMesh::GetElementArrayEdgeTable(const ElemArray &elem_array,
                                         const DSTable &v_to_v,
                                         Table &el_to_edge)
{
   el_to_edge.MakeI(CountJaggedArrayEntries(elem_array));

   ElemConstIterator it;
   int el = 0;
   for (it=elem_array.begin(); it!=elem_array.end(); it++)
   {
      for (int i = 0; i < it->second.Size(); i++, el++)
      {
         el_to_edge.AddColumnsInRow(el, it->second[i]->GetNEdges());
      }
   }
   el_to_edge.MakeJ();
   el = 0;
   for (it=elem_array.begin(); it!=elem_array.end(); it++)
   {
      for (int i = 0; i < it->second.Size(); i++, el++)
      {
         const int *v = it->second[i]->GetVertices();
         const int ne = it->second[i]->GetNEdges();
         for (int j = 0; j < ne; j++)
         {
            const int *e = it->second[i]->GetEdgeVertices(j);
            el_to_edge.AddConnection(el, v_to_v(v[e[0]], v[e[1]]));
         }
      }
   }
   el_to_edge.ShiftUpI();
...
void MixedMesh::GetVertexToVertexTable(DSTable &v_to_v) const
{
   if (edge_vertex)
   {
...
int MixedMesh::GetElementToEdgeTable(Table & e_to_f, Array<int> &be_to_f)
{
   int i, NumberOfEdges;
...
const Table & MixedMesh::ElementToElementTable()
{
   if (el_to_el)
   {
      return *el_to_el;
...
const Table & MixedMesh::ElementToFaceTable() const
{
   if (el_to_face == NULL)
   {
      mfem_error("MixedMesh::ElementToFaceTable()");
   }
   return *el_to_face;
}

const Table & MixedMesh::ElementToEdgeTable() const
{
   if (el_to_edge == NULL)
   {
      mfem_error("MixedMesh::ElementToEdgeTable()");
   }
   return *el_to_edge;
}

void MixedMesh::AddPointFaceElement(int lf, int gf, int el)
{
...
void MixedMesh::AddSegmentFaceElement(int lf, int gf, int el, int v0, int v1)
{
   Element::Type type;
   int index;
   this->GetFaceTypeAndIndex(gf, type, index);
   ...
   {
      int *v = faces[type][index]->GetVertices();
      faces_info[gf].Elem2No  = el;
      if ( v[1] == v0 && v[0] == v1 )
      {
         faces_info[gf].Elem2Inf = 64 * lf + 1;
                                       ...
                                       int v0, int v1, int v2)
{
   ...
   else  //  this will be elem2
   {
      ...
      orientation = GetTriOrientation(faces[type][index]->GetVertices(), vv);
                                   ...
                                   int v0, int v1, int v2, int v3)
{
   ...
   else  //  this will be elem2
   {
      ...
      int oo = GetQuadOrientation(faces[type][index]->GetVertices(), vv);
      MFEM_ASSERT(oo % 2 != 0, "");
      faces_info[gf].Elem2No  = el;
...
void MixedMesh::GenerateNCFaceInfo()
{
...
STable3D *MixedMesh::GetFacesTable()
{
...
void MixedMesh::ReorientTetMesh()
{
   int *v;
...
int *MixedMesh::CartesianPartitioning(int nxyz[])
{
   int *partitioning;
...
int *MixedMesh::GeneratePartitioning(int nparts, int part_method)
{
   ...
   else
   {
      ...
      if (part_method == 0 || part_method == 3)
      {
         ...
         err = METIS_PartGraphRecursive(&n,
                                        ...
                                        partitioning);
         if (err != 1)
            mfem_error("MixedMesh::GeneratePartitioning: "
...
#ifdef MFEM_DEBUG
      cout << "MixedMesh::GeneratePartitioning(...): edgecut = "
           << edgecut << endl;
#endif
   }
   ...
   // Check for empty partitionings (a "feature" in METIS)
   {
      ...
      if (empty_parts)
      {
         cerr << "MixedMesh::GeneratePartitioning returned " << empty_parts
              << " empty parts!" << endl;
...
void MixedMesh::CheckPartitioning(int *partitioning)
{
   ...
   if (n_empty > 0)
   {
      cout << "MixedMesh::CheckPartitioning(...) :\n"
...
void MixedMesh::CheckDisplacements(const Vector &displacements, double &tmax)
{
   ...
   for (int i = 0; i < NumOfElements; i++)
   {
      ...
      switch (el->GetType())
      {
         ...
         default:
            mfem_error("MixedMesh::CheckDisplacements(...)");
      }
   }
}

void MixedMesh::MoveVertices(const Vector &displacements)
{
...
void MixedMesh::GetVertices(Vector &vert_coord) const
{
...
void MixedMesh::SetVertices(const Vector &vert_coord)
{
   for (int i = 0, nv = vertices.Size(); i < nv; i++)
      for (int j = 0; j < spaceDim; j++)
      {
...
void MixedMesh::GetNode(int i, double *coord)
{
   if (Nodes)
   {
      FiniteElementSpace *fes = Nodes->FESpace();
...
void MixedMesh::SetNode(int i, const double *coord)
{
   if (Nodes)
   {
      FiniteElementSpace *fes = Nodes->FESpace();
...
void MixedMesh::MoveNodes(const Vector &displacements)
{
   if (Nodes)
   {
      (*Nodes) += displacements;
...
void MixedMesh::GetNodes(Vector &node_coord) const
{
   if (Nodes)
   {
      node_coord = (*Nodes);
...
void MixedMesh::SetNodes(const Vector &node_coord)
{
   if (Nodes)
   {
      (*Nodes) = node_coord;
...
void MixedMesh::NewNodes(GridFunction &nodes, bool make_owner)
{
   if (own_nodes) { delete Nodes; }
   Nodes = &nodes;
...
void MixedMesh::SwapNodes(GridFunction *&nodes, int &own_nodes_)
{
...
void MixedMesh::AverageVertices(int * indexes, int n, int result)
{
   int j, k;
...
/*
void MixedMesh::QuadUniformRefinement()
{
 int i, j, *v, vv[2], attr;
 const int *e;
...
*/
/*
void MixedMesh::HexUniformRefinement()
{
 int i;
 int * v;
 const int *e, *f;
...
*/
/*
void MixedMesh::LocalRefinement(const Array<int> &marked_el, int type)
{
 int i, j, ind, nedges;
 Array<int> v;
...
*/
/*
void MixedMesh::NonconformingRefinement(const Array<Refinement> &refinements,
                                 int nc_limit)
{
...
*/
/*
void MixedMesh::DerefineMesh(const Array<int> &derefinements)
{
...
*/
/*
bool MixedMesh::NonconformingDerefinement(Array<double> &elem_error,
...
*/
/*
bool MixedMesh::DerefineByError(Array<double> &elem_error, double threshold,
                           int nc_limit, int op)
{
...
bool MixedMesh::DerefineByError(const Vector &elem_error, double threshold,
                           int nc_limit, int op)
{
...
void MixedMesh::InitFromNCMesh(const NCMesh &ncmesh)
{
...
MixedMesh::MixedMesh(const NCMesh &ncmesh)
{
   Init();
   InitTables();
...
*/
void MixedMesh::Swap(MixedMesh& other, bool non_geometry)
{
...
void MixedMesh::UniformRefinement()
{
   ...
   {
      mfem_error("MixedMesh::UniformRefinement()");
   }
}
...
// void MixedMesh::EnsureNCMesh(bool triangles_nonconforming)
// {
...
*/
/*
void MixedMesh::Bisection(int i, const DSTable &v_to_v, int *middle)
{
 int *vert;
...
*/
void MixedMesh::InitRefinementTransforms()
{
...
int MixedMesh::FindCoarseElement(int i)
{
   int coarse;
...
/*
const CoarseFineTransformations& MixedMesh::GetRefinementTransforms()
{
...
*/
void MixedMesh::PrintXG(std::ostream &out) const
{
   ...
   else  // ===== Dim != 2 =====
   {
      if (Nodes)
      {
         mfem_error("MixedMesh::PrintXG(...) : Curved mesh in 3D");
      }
...
void MixedMesh::Printer(std::ostream &out, std::string section_delimiter) const
{
   int i, j;
...
void MixedMesh::PrintTopo(std::ostream &out,const Array<int> &e_to_k) const
{
   int i;
   Array<int> vert;
...
void MixedMesh::PrintVTK(std::ostream &out)
{
   out <<
   ...
   else
   {
      ...
      if (order == -1)
      {
         cerr << "MixedMesh::PrintVTK : can not save '"
   ...
   for (int i = 0; i < NumOfElements; i++)
   {
      ...
      if (order == 1)
      {
         switch (GetElement(i)->GetGeometryType())
         {
            ...
            default: MFEM_ABORT("Unrecognized element type");
         }
      }
      else if (order == 2)
      {
...
void MixedMesh::PrintVTK(std::ostream &out, int ref, int field_data)
{
   int np, nc, size;
   RefinedGeometry *RefG;
   DenseMatrix pmat;
...
void MixedMesh::GetElementColoring(Array<int> &colors, int el0)
{
...
void MixedMesh::PrintSurfaces(const Table & Aface_face, std::ostream &out) const
{
   int i, j;

   if (NURBSext)
   {
      mfem_error("MixedMesh::PrintSurfaces"
   ...
   for (int iAF=0; iAF < Aface_face.Size(); ++iAF)
      ...
      {
         out << iAF+1 << ' ';
         PrintElementWithoutAttr(GetFace(*iface),out);
      }
...
void MixedMesh::ScaleSubdomains(double sf)
{
   int i,j,k;
   Array<int> vert;
   DenseMatrix pointmat;
...
void MixedMesh::ScaleElements(double sf)
{
   int i,j,k;
   Array<int> vert;
   DenseMatrix pointmat;
...
void MixedMesh::Transform(void (*f)(const Vector&, Vector&))
{
...
void MixedMesh::Transform(VectorCoefficient &deformation)
{
...
void MixedMesh::RemoveUnusedVertices()
{
...
/*
void MixedMesh::RemoveInternalBoundaries()
{
 if (NURBSext || ncmesh) { return; }
...
*/
void MixedMesh::FreeElements(ElemArray & m)
{
   ElemIterator it;
   for (it=m.begin(); it!=m.end(); it++)
   {
...
std::ostream &operator<<(std::ostream &out, const MixedMesh &mesh)
{
   mesh.Print(out);
   return out;
}
                     ...
                     const bool closed)
{
   ...
   if (closed)
   {
      mesh2d = new MixedMesh(2, nvt, mesh->GetNE()*ny, mesh->GetNBE()*ny);
   }
   else
