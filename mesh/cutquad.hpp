...
/// Data type cut quadrilateral element
class Cutquad : public Element
{
protected:
   int indices[4];

public:
//   typedef Geometry::Constants<Geometry::CUTSQUARE> geom_t;
   typedef Geometry::Constants<Geometry::SQUARE> geom_t;

   Cutquad() : Element(Geometry::CUTSQUARE) {}

   /// Constructs quadrilateral by specifying the indices and the attribute.
   Cutquad(const int *ind, int attr = 1);

   /// Constructs quadrilateral by specifying the indices and the attribute.
   Cutquad(int ind1, int ind2, int ind3, int ind4, int attr = 1);

   /// Return element's type
   Type GetType() const { return Element::CUTQUAD; }
   ...
   { return new  Cutquad(indices, attribute); }

   virtual ~ Cutquad() { }
};

//extern BiLinear2DFiniteElement  CutquadFE;
