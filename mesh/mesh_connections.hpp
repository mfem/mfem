class MeshConnections
{
   ...
   bool        Contains(EntityID parent, EntityID child);
   Array<int> &Neighbors(EntityID entity, int shared_dim);
   Array<int> &ChildrenInEntity(EntityID parent, int child_dim);
   Array<int> &ChildrenInEntities(EntityIDs parents, int child_dim);
   Array<int> &ParentsWithEntity(EntityID child, int parent_dim);
   Array<int> &ParentsTouchedByEntities(EntityIDs children, int parent_dim);
