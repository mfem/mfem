...
void ParMesh::PrintAsOneWithAttributes(std::ostream &os) const
{
   ...
   if (MyRank == 0)
   {
      ...
      for (i = 0; i < NumOfElements; i++)
      {
         // processor number + 1 as attribute and geometry type
         ///os << 1 << ' ' << elements[i]->GetGeometryType();
         os << elements[i]->GetAttribute() << ' ' << elements[i]->GetGeometryType();
         // vertices
         nv = elements[i]->GetNVertices();
         v  = elements[i]->GetVertices();
         for (j = 0; j < nv; j++)
   ...
   if (MyRank == 0)
   {
      ...
      for (p = 0; p < NRanks; p++)
      {
         if (p)
         {
            MPI_Recv(nv_ne, 3, MPI_INT, p, 446, MyComm, &status);
            ints.SetSize(ne);
            if (ne)
            {
               MPI_Recv(ints.GetData(), ne, MPI_INT, p, 447, MyComm, &status);
            ...
            if (nc)
            {
               MPI_Recv(&attr_ne[0], nc, MPI_INT, p, 448, MyComm, &status);
            }
         }
         else
         {
   ...
   else
   {
      ...
      ne = ints.Size();
      ///MPI_Send(nv_ne, 2, MPI_INT, 0, 446, MyComm);
      MPI_Send(nv_ne, 3, MPI_INT, 0, 446, MyComm);
      if (ne)
      {
         MPI_Send(ints.GetData(), ne, MPI_INT, 0, 447, MyComm);
      }
      if (nc)
      {
         MPI_Send(&attr_ne[0], nc, MPI_INT, 0, 448, MyComm);
      }
   }
