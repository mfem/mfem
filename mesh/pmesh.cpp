...
ParMesh::ParMesh(MPI_Comm comm, apf::Mesh2* apf_mesh)
   : gtopo(comm)
{
   int i, j;
   Array<int> vert;
   ...
   while ((ent = apf_mesh->iterate(itr)))
   {
       ...
       elements[j] = Mesh::ReadElement(ent, geom_type, verts, attr, v_num_loc);
       j++;
   }
  //End iterator
   apf_mesh->end(itr);  
   ...
   while ((ent = apf_mesh->iterate(itr)))
   {
      ...
      if (apf_mesh->getModelType(mdEnt) == BcDim)
      {
        ...
        boundary[bdr_ctr] = Mesh::ReadElement( ent, geom_type, verts, attr, v_num_loc);
        bdr_ctr++;
      }
   }
   apf_mesh->end(itr); 
   ...
   {
      GridFunction* auxNodes = new GridFunction(this, apf_mesh, v_num_loc,
              crd_shape->getOrder());
