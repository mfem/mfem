...
//
// Compile with: make igaopt3d
//
// Sample runs:  mpirun -np 4 igaopt3d
//
...
int main(int argc, char *argv[])
{
   ...
   int port_order = 1;
   Mesh port_mesh(10, 10, Element::QUADRILATERAL, false, 0.2, 0.1);
   ...
   for (int i = 0; i < nrelem; ++i)
   {
      ...
      for (int iv = 0; iv < nrvert; ++iv)
      {
         ...
         if ((coords[0] < 0.12) && (coords[0] > 0.08) && (coords[1] < 0.06) && (coords[1] > 0.04))
            {
               is_waveguide = true;
               break;
            }
...
//    }
   ...
   // Setup PML length
   Array2D<real_t> length(dim, 2); length(0,0) = 0.2;length(0,1) = 0.2;length(1,0) = 0.1;length(1,1) = 0.1;
   length(2,0) = 0;length(2,1) = 0;
   ...
   for (int i = 0; i < nelem; ++i)
   {
      ...
      for (int iv = 0; iv < nrvert; ++iv)
      {
         ...
         if ((coords[1] < 0.27) && (coords[1] > 0.23))// && (coords[2] < 0.06) && (coords[2] > 0.04))
         {
            is_waveguide = true;
            break;
         }
...
{  
   //for dim = 2
   if (x(1)<0.24 || x(1)>0.26 || x(2)>0.06 || x(2)<0.04 )
   {
      f = 0.0;
   }
   else
   {
      Vector phy_point(2);
      phy_point(0) = x(1)-0.24;
      phy_point(1) = x(2)-0.04;
      ...
      f[0] = eps * gf_value * coeff * exp(alpha);
   }
}
...
{  
   ...
   else
   {
      ...
      real_t alpha = -pow(n, 2) * r;
      //gf_value = pow(gf_value, 2);
      f[2] = eps * gf_value * coeff * exp(alpha);
   }
}
