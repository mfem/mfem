...
//
// Compile with: make waveguide2Dmode_bend_out
//
// Sample runs:  mpirun -np 1 waveguide2Dmode_bend_out
//
...
int main(int argc, char *argv[])
{
   ...
   ofstream sol_r_ofs("bend_out_nurbs-sol_r.gf");
   ofstream sol_i_ofs("bend_out_nurbs-sol_i.gf");
   ...
   pd->SetPrefixPath("./ParaViewMode_bend_out");
...
void source_r(const Vector &x, Vector & f)
{
   ...
   if(xx < 4000 && xx > 0)
   {
      get_source_E(xx,zz,x[1], f[0], a, f[2], b, f[1], c);
   }
   else{
      f = 0.0;
   }
...
void source_i(const Vector &x, Vector & f)
{
   ...
   if(xx < 4000 && xx > 0)
   {
      get_source_E(xx,zz,x[1], a, f[0], b, f[2], c, f[1]);
   }
   else{
      f = 0.0;
   }
