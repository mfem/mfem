// Compile with: make mf_wg_sweep
// nvcc  -g -Xcompiler=-Wall -std=c++11 -x=cu --expt-extended-lambda -arch=sm_86 -ccbin mpicxx -I../.. -I../../../hypre/src/hypre/include -I../../../metis/include mf_wg_sweep.cpp -o mf_wg_sweep -L../.. -lmfem -L../../../hypre/src/hypre/lib -lHYPRE -lcusparse -lcurand -lcublas -L../../../metis/lib -lmetis -lcusparse -lrt -lnlopt -lm
// Sample runs:  mpirun -np 1 mf_wg_sweep
//
...
real_t cosphi(const Vector &x)
{
    return cos(q*(x(0)));
}

real_t sinphi(const Vector &x)
{
    return sin(q*(x(0)));
}
...
int main(int argc, char *argv[])
{
   ...
   for (int i = 0; i < nelem; ++i)
   {
      ...
      for (int iv = 0; iv < nrvert; ++iv)
      {
         ...
         if (((coords[1]<4400)&&(coords[1]>3600)&&(coords[2]<1200)&&(coords[2]>800)))
         {
            is_waveguide = true;
            break;
         }
   ...
   for (int i = 0; i < nelem; ++i)
   {
      ...
      for (int iv = 0; iv < nrvert; ++iv)
      {
         int vert_idx = vertices[iv];
         real_t *coords = pmesh->GetVertex(vert_idx);    
         if ((coords[0]<6000)&&(coords[0]>2000)&&(coords[2]<1200)&&(coords[2]>800)&&(coords[1]>3600)&&(coords[1]<4400))
         {
...
{  
   ...
   FunctionCoefficient cosqphi(cosphi);
   FunctionCoefficient sinqphi(sinphi);
   for(int i = 0;i<omega_num;i++)
   {
      ...
      if(i>=1)
      {
         ...
         ProductCoefficient k2epsilon_c1_real(k2epsilon_c1_rcf,cosqphi);
         ProductCoefficient k2epsilon_c1_imag(k2epsilon_c1_rcf,sinqphi);
         cp1[i-1]->AddDomainIntegrator(new NURBSHCurl_VectorMassIntegrator(k2epsilon_c1_real),new NURBSHCurl_VectorMassIntegrator(k2epsilon_c1_imag));
         cp1[i-1]->Assemble(0);
         ...
         ProductCoefficient k2epsilon_c2_real(k2epsilon_c2_rcf,cosqphi);
         ProductCoefficient k2epsilon_c2_imag(k2epsilon_c2_rcf,sinqphi);
         ProductCoefficient minus_k2epsilon_c2_imag(-1.0,k2epsilon_c2_imag);

         ProductCoefficient real_cp1();
         cp2[i-1]->AddDomainIntegrator(new NURBSHCurl_VectorMassIntegrator(k2epsilon_c2_real),new NURBSHCurl_VectorMassIntegrator(minus_k2epsilon_c2_imag));
         cp2[i-1]->Assemble(0);
...
{  
   ...
   SIMPInterpolationCoefficient SIMP_cf(&eps_cf,epsilon_min, epsilon_max, 1);
   ProductCoefficient eps_design(eps_initial,SIMP_cf);

   mfem::out<<"in simulation"<<std::endl;
   real_t fe = 2000;
   for(int freq_num = 0; freq_num < 1; freq_num++)
   {
    EMsolver_1.SetFrequency(1.0/(freq_num + fe));
    EMsolver_1.SetupFEM();
    ...
    mfem::out<<" frequency: "<<fe+freq_num<<std::endl;
    ParaViewDataCollection *pd = NULL;
    pd = new ParaViewDataCollection("nd_nurbs", pmesh);
    pd->SetPrefixPath("./mfq1wg");
