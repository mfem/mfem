...
//
// Compile with: make pndnurbs
//
// Sample runs:  mpirun -np 4 pndnurbs -m ../../data/square-nurbs.mesh
//               mpirun -np 4 pndnurbs -m ../../data/square-nurbs.mesh -o 2
//               mpirun -np 4 pndnurbs -m ../../data/cube-nurbs.mesh
//
...
int main(int argc, char *argv[])
{
   ...
   //int *partitioning = mesh->GeneratePartitioning(1, 1);
   ParMesh *pmesh = new ParMesh(MPI_COMM_WORLD, *mesh);
   mfem::out<<"main.cpp: test row 222"<<std::endl;
   delete mesh;
   ...
   HYPRE_BigInt num_ldof = fespace->GetTrueVSize();                                                                             
   mfem::out<<"ID "<<myid<<" "<<getpid()<<" Create NURBS fec and ext,finite element unknowns: "<<num_ldof <<std::endl;
   ...
   HYPRE_BigInt num_dof = fespace->GlobalTrueVSize();
   if(myid == 0)
   {
      cout << "main.cpp: test row 242 Number of finite element unknowns: "
        << num_dof << endl;
   }
   ...
   b.Assemble();
   mfem::out<<"main.cpp: test row 271"<<std::endl;
   ...
   VectorFunctionCoefficient E_Im(dim, E_bdr_data_Im);mfem::out<<"main.cpp: test row 280"<<std::endl;
   ...
   offsets.PartialSum();
   mfem::out<<"main.cpp: test row 375"<<std::endl;
   ...
   //pc_r.reset(new HypreDiagScale(*PCOpAh.As<HypreParMatrix>()));
